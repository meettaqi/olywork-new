...
Deliberately hits real upstreams on olywork's real platform keys, because the whole point is that the
...
def req(method: str, path: str, token: str | None = None, body=None, timeout=90):
    ...
    if token:
        r.add_header("X-Olywork-Token", token)
    try:
...
def platform_keys() -> list[str]:
    ...
    with open(os.path.join(here, ".env")) as fh:
        for line in fh:
            if line.startswith("OLYWORK_PLATFORM_KEY_") and "=" in line:
...
def main() -> int:
    ...
    email = f"e2e-{os.urandom(4).hex()}@olywork.local"
    ...
    if captured:
        ...
        check(False, "no provider failure could be produced", "every attempt was refused by olywork first")
