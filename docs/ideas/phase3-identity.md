...
- `/tmp/olywork-phase3-brief.md`
- `docs/REFACTOR-PLAN.md` sections 1.1, 1.2, 1.3, 1.6, 1.7, Stage 3, and section 5
- the current `src/olywork/api.py`, `src/olywork/routers/dependencies.py`, `src/olywork/session.py`,
  `src/olywork/mcp_oauth.py`, `src/olywork/bootstrap.py`, and import-linter configuration
...
Tests and runtime code import `olywork.session` and `olywork.mcp_oauth` directly. Unlike `olywork.api:app`, the
...
runtime owners import `olywork.domain.identity.session` and `olywork.domain.identity.mcp_oauth`. The section 5
...
src/olywork/domain/
  ...
  identity/
    ...
    session.py        exact body of current olywork/session.py
    mcp_oauth.py      exact body of current olywork/mcp_oauth.py
    access.py         Caller, resolution, roles, and auth dependencies

src/olywork/application/
  __init__.py
...
Python import and `src/olywork/routers/dependencies.py` is deleted. There is intentionally no compatibility
...
- `olywork.api._backfill_provider_extra_tools`
...
- optional `olywork.session` and `olywork.mcp_oauth` compatibility shims, pending decision 0.5

Import-linter in the same commit: add `Identity is a domain leaf`, with
`source_modules = ["olywork.domain.identity"]`, `as_packages = true`, and explicit forbidden sibling
...
`tests/test_auth.py::test_session_scopes_by_x_olywork_org` and
...
`api.py` remains the ordered route-table host and `olywork.api:app` compatibility entry point. During the
...
- `mock.patch` / `patch(...)`
- imports of `olywork.api`, `olywork.session`, `olywork.mcp_oauth`, and `routers.dependencies`
...
- connection tests import `olywork.api as A` and call `A._backfill_provider_extra_tools`; preserve or
...
handler makes a key change appear necessary, stop for review. The deployed `olywork.api:app` remains the all-role
