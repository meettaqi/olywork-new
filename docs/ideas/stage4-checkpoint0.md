...
- current `src/olywork/api.py` at 4,676 lines
- the shipped context fragments for API, proxy, money, auth/secrets, multi-tenancy, and landing sandbox
- `src/olywork/bootstrap.py`, `proxy.py`, `injectors.py`, `ledger.py`, `oauth.py`, `health.py`
...
- `olywork.billing` remains a temporary compatibility facade for existing imports and tests until a separately
...
| 694-711 | `_mark_olywork_own_errors` | app-wide HTTP handler owned by bootstrap; call-specific stamping is injected from `application.call` |
...
compatibility churn than value, use `bootstrap_http.py`; `olywork.bootstrap:create_app` must remain unchanged.

The app-wide exception handlers follow the same ownership rule. `_pool_saturated` and `_mark_olywork_own_errors`
...
data and a late-bound `blame: caller | olywork | upstream | org_connection`. The contract has six pins:

1. **Provider responses are data.** Every provider response, including 4xx and 5xx, is faithfully forwarded and
   is never represented as an exception. Typed failures exist only when olywork refuses the call or a provider does
...
3. **502 mechanisms are explicit.** Their stable kinds and compatibility mappings are defined together:
   ...
   | Kind | Blame | Ledger release reason | HTTP status | `X-Olywork-Error` during B commits |
   |---|---|---|---:|---|
   | `refresh_failed` | `org_connection` | `call_failed_502` | 502 | literal `1` |
   | `injection_failed` | `olywork` or `org_connection`, assigned by the single mapping point | `call_failed_502` | 502 | literal `1` |
   | `ssrf_refused` | `olywork` | `call_failed_502` | 502 | literal `1` |
...
4. **SSRF is its own kind.** It is a post-reserve olywork refusal, always releases the hold, never charges, and
...
6. **Compatibility headers remain literal.** During B commits `X-Olywork-Error` remains the literal `1`.
   Enriching it with a kind or adding `X-Olywork-Refused-By` changes a client-parsed API surface and requires a
...
`Retry-After`, `X-Olywork-Call-Id`, and `X-Olywork-Cost-Micro`. Domain packages do not import `HTTPException`.
...
- `olywork.proxy` may remain a direct re-export facade during compatibility, but no new runtime owner imports it.
...
`localrun.py` imports the leaf `infra.upstream.injectors` module directly, or the existing `olywork.injectors`
...
adjustments and a thin `olywork.ledger` compatibility facade. Update the import-linter money contract to target the
real owner.
...
those in context updates when code ships. Keep `olywork.oauth` compatibility exports because tests and lazy
...
5. `refactor(call): move the call HTTP surface`
   ...
   - Verify moved text/decorators, all four snapshots, `olywork.api` re-export identity, monkeypatch scan.
...
13. `refactor(call): make reservation application-owned`
   ...
   - Run pool-discipline and the full callmatrix on SQLite and Postgres. Retarget every moved `olywork.api`
...
17. `refactor(call): return framework-neutral upstream responses`
    ...
    - Run pool-discipline and the full callmatrix on SQLite and Postgres. Retarget the moved `olywork.api` relay
...
19. `refactor(call): make settlement application-owned`
    ...
    - Run pool-discipline and the full callmatrix on SQLite and Postgres. Retarget settlement monkeypatches from
      `olywork.api` or prove compatibility exports are the same objects.
...
15. **Compatibility surface.** Stop on any required route key/function-name change, `olywork.api:app` change,
...
- `tests/test_call_pool_discipline.py` currently patches collaborators through `olywork.api` at lines 65, 129,
...
- app-wide middleware and `_pool_saturated`/`_mark_olywork_own_errors` handlers under bootstrap, with call-specific
