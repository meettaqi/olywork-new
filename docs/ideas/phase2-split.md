...
- the current `src/olywork/api.py`, `src/olywork/bootstrap.py`, role manifests, and four surface snapshots
...
unchanged. `olywork.api:app` and the three role manifests stay unchanged.
...
- `GET /catalog/endpoints/{endpoint_id}/access`, handler `catalog_endpoint_access`, currently lines
  ...
  `_enforce_catalog_status`. Moving it without importing `olywork.api` would require either moving call
...
- `GET /providers.json` stays in `api.py`. It is the credential/provider registry consumed by
  `olywork upload`, not the endpoint Catalog described by `catalog_store`.
...
| 73 | `GET /.well-known/skills/olywork/SKILL.md` | `well_known_skill_md` |
...
- auth and identity: GitHub, Google, email OTP, CLI pairing/tokens, invite sign-in, olywork OAuth
...
these from `olywork.api` is forbidden, so the first commit extracts the existing dependency family, with
unchanged bodies, into `src/olywork/routers/dependencies.py`:
...
| `_utcnow_naive`, `_as_naive` | a small neutral `src/olywork/timeutil.py`, re-exported by `api.py` | admin reads and many remaining handlers share the exact timestamp convention |
...
- `routers/web.py` imports those helpers from its sibling `routers.catalog`, never from `olywork.api`.
...
The invariant is stronger than "no module-level import": no module anywhere under `olywork.routers` may
import `olywork.api`, directly or indirectly through an adapter that merely hides the edge.
...
type = "forbidden"
source_modules = ["olywork.routers"]
forbidden_modules = ["olywork.api"]
```

The exact `as_packages` setting will be verified against import-linter 2.13 before editing the config so
the contract covers every current and future module under `olywork.routers`. The contract must catch direct
