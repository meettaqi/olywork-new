...
配套设计文档：`docs/AIGC-PLAN-CN.md`。olywork 接入的单位是 **provider（模型厂商）**，不是单个模型：
...
| Midjourney | v7 (fast/turbo/upscale/variation) | ❌ **无官方 API** | - | ThankYouAI(经 youchuan 转售) | 只能靠非官方转售，olywork 不宜直listing |
...
youchuan）。对 olywork 的含义：**接一家聚合器 = 一次拿到整个长尾**。
