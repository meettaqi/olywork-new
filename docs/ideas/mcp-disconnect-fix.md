...
- `X-Olywork-Body-Encoding` 解码顺序和行为不变。
...
在当前分支用临时 SQLite 启动真实 `olywork.api:app`：

```bash
OLYWORK_DATABASE_URL=sqlite+aiosqlite:////tmp/olywork-mcp-disconnect/olywork.db \
OLYWORK_PUBLIC_URL=http://127.0.0.1:8000 \
OLYWORK_EMAIL_DEV_MODE=true \
uv run --frozen uvicorn olywork.api:app --host 127.0.0.1 --port 8000 --log-level debug
```
...
- `starlette.middleware.base.BaseHTTPMiddleware` 加
  `dispatch=olywork.api._security_headers` 改为新的 security 纯 ASGI class，kwargs 为空。
- `starlette.middleware.base.BaseHTTPMiddleware` 加
  `dispatch=olywork.api._legacy_host_redirect` 改为新的 legacy redirect 纯 ASGI class，kwargs 为空。
...
- `src/olywork/api.py`：两个纯 ASGI middleware class。
- `src/olywork/bootstrap.py`：以纯 ASGI class 注册并保持顺序。
