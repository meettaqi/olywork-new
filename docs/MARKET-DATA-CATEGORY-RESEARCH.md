...
Every existing olywork provider is **credit-priced**. Hunter sells 2,000 credits for $49, an endpoint
...
**A. Price them at $0 (`type: free`).** Once olywork pays the monthly fee, the marginal cost of one more
call really is zero, so this is *honest*. It also means olywork absorbs the subscription with no
...
never on olywork's. Costs nothing, and the category still has real value — but the headline feature
...
Ordered by how well they fit olywork, not by how good the data is.
...
non-commercial, which we must read carefully before serving it on olywork's key.
...
low-cost candidate if we do pick one to serve on olywork's key.
...
## Subscriptions — CONFIRMED from the dashboards (2026-08-15, Olywork Wise card)
...
3. **Whether the free tier's terms permit serving it through olywork** — Finnhub's free tier is
   explicitly non-commercial, and that matters if olywork's key is the one being used.
...
3. Whether to pay for any subscription at all right now, or ship the category **own-key only** and
   add olywork-key serving later once there is demand to justify the spend.
