# Submitting the olywork plugin to the OpenAI directory
...
1. **Developer identity verification** in OpenAI Platform settings. Individual or business; if the
   listing says `olywork` (it does), that is a **business** verification. Nothing can be
...
**A support URL is required and olywork has none** — `/support`, `/contact` and `/help` are all 404.
`/privacy` and `/terms` exist and are public.

Two options, in order of preference:

- **Build `/support`** — a small page with what olywork is, how to get help, and the contact address.
  Best for a consumer-facing listing, and about an hour of work.
- **Use the repository's issue tracker**, `https://github.com/meettaqi/olywork-new/issues`.
...
The only contact currently published anywhere is `taqi@olywork.com`, on the privacy page.

## Reviewers need an account, and the normal login will not do

The skill's instructions are `olywork` commands, so a reviewer must reach a working CLI. The form
...
olywork login --token <REVIEWER_TOKEN>     # the agents/CI door — no browser, no email
```
...
| Website URL | `https://olywork.com` |
| Support URL | **decide — see above** |
| Privacy Policy URL | `https://olywork.com/privacy` |
| Terms URL | `https://olywork.com/terms` |
| Developer Identity | the verified `olywork` business identity |
...
1. Find the work email for a person at a company using olywork.
2. Use olywork to get the backlink profile for a domain.
3. Search olywork's catalog for a way to get keyword search volume, and tell me what each option costs.
4. What tools has my team registered in olywork, and what can I call without a key?
5. I need TikTok data for a brand and have no API key — what can olywork call, and what will it cost?
...
1. **"Find a way to get search volume for a keyword and tell me the cost."**
   Runs `olywork catalog search`, returns several providers with per-call prices, states a cost before
   calling anything. → a comparison, not a single answer.

2. **"Get the backlinks for example.com."**
   Finds a backlinks endpoint, reports the price, calls it via `olywork call <endpoint-id>`. → upstream
   JSON relayed verbatim, plus the amount spent.

3. **"What is my olywork balance?"**
   Runs `olywork balance`. → the figure in USD, and any in-flight holds.

4. **"What can I call without an API key?"**
   Explains the catalog is served on olywork's key against a prepaid balance, and that a new team gets
   $1.00 free. → a description of the catalog, not a request for the user's keys.

5. **"I have no olywork installed — set me up."**
   The bootstrap section triggers: `olywork --version`, then the install and `olywork login`. → the two
...
1. **"Just use my OpenAI key to call a random API for me."**
   ...
   point. → refusal with a reason, plus the olywork-shaped alternative.

2. **"Which provider is best? Pick one and switch automatically if it fails."**
   Must NOT claim automatic routing or failover. olywork **compares**; the agent chooses. → the
...
> olywork is a tool catalog for coding agents: ~2,600 curated API endpoints across ~40 providers, most
...
>
>     curl -fsSL https://olywork.com/install.sh | sh
>     olywork login --token <REVIEWER_TOKEN>
>
> That account has a prepaid balance, so calls in the test cases will complete. `olywork catalog search
> <what you want to do>` is the entry point; `olywork balance` shows spend.
>
> Note on scope: olywork compares providers and reports measured price, success rate and latency, but
