...
- **OAuth** (`auth_kind="oauth"`) — olywork holds its own registered app; **heavy** (needs a dev-app registration
...
6. **LIVE bogus-key test — the load-bearing step.** Start the server and `POST /connections/token` with a
   *garbage* key against the **real** API:
   ...
   org and delete it after (test users are `e2e-…@olywork.local`).
...
1. olywork must register its **own** dev app on the network → `client_id`/`client_secret` → add the two settings
