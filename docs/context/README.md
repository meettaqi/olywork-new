...
|---|---|---|
| [Tools Registry — charter (what it is, why, the proxy model)](foundation/charter.md) | foundational | 2026-06-30-taqi-tools-registry.md, README.md |
...
| [Local proxy — catch a program's own outgoing calls (`olywork <command>`)](architecture/local-proxy.md) | shipped | localproxy.py, server.js |
| [Local CLI runs — run a vendor CLI as a dedicated user with a server-held credential (`olywork run`)](architecture/local-run.md) | shipped | localrun.py, egress.py, fsjail.py |
| [MCP — the front door for assistants, and olywork as an OAuth authorization server](architecture/mcp-oauth.md) | shipped | auth.py, mcp.py, health.py, mcp_oauth.py, … |
...
| [The CLI (olywork) + skill scaffolding](interface/cli.md) | shipped | cli.py, convert.py, agents.py |
...
| [Shell mode (olywork shell) — transparent CLI interception](interface/shell.md) | shipped | shell.py, cli.py |
...
| Fragment | Status | Covers |
|---|---|---|
| [Provider capacity — knowing what olywork's own vendor accounts have left](ops/capacity.md) | shipped (steps B–F built; production rollout = the shadow week, then OLYWORK_OVERFLOW_MODE=on) | __init__.py, collectors.py, policy.py, sweep.py, … |
...
| Fragment | Status | Covers |
|---|---|---|
| [Glossary](reference/glossary.md) | reference | 2026-06-30-taqi-tools-registry.md |
