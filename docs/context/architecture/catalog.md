...
status: shipped
sources:
  - src/olywork/catalog/contracts.yaml
  - src/olywork/catalog/adapters.yaml
  - src/olywork/catalog/examples/findymail.search.business-profile.json
  - src/olywork/domain/catalog/routing/__init__.py
  - src/olywork/domain/catalog/routing/contracts.py
  - src/olywork/domain/catalog/routing/paths.py
  - src/olywork/domain/catalog/routing/plan.py
  - src/olywork/domain/catalog/routing/synthetic.py
  ...
  - src/olywork/catalog/aliases.yaml
  - src/olywork/catalog/fx.yaml
  - src/olywork/catalog/aviato.yaml
  - src/olywork/catalog/crustdata.yaml
  - src/olywork/catalog/examples/aviato.companies.acquisitions.json
  - src/olywork/catalog/examples/aviato.companies.employees.json
  - src/olywork/catalog/examples/aviato.companies.enrich.bulk.json
  - src/olywork/catalog/examples/aviato.companies.enrich.json
  - src/olywork/catalog/examples/aviato.companies.founders.json
  - src/olywork/catalog/examples/aviato.companies.funding_rounds.json
  - src/olywork/catalog/examples/aviato.companies.investments.json
  - src/olywork/catalog/examples/aviato.companies.outbound_investments.json
  - src/olywork/catalog/examples/aviato.companies.search.json
  - src/olywork/catalog/examples/aviato.linkedin.company.posts.json
  - src/olywork/catalog/examples/aviato.linkedin.post.comments.json
  - src/olywork/catalog/examples/aviato.linkedin.post.reactions.json
  - src/olywork/catalog/examples/aviato.linkedin.post.reposts.json
  - src/olywork/catalog/examples/aviato.linkedin.user.posts.json
  - src/olywork/catalog/examples/aviato.people.contact.get.json
  - src/olywork/catalog/examples/aviato.people.email.find.json
  - src/olywork/catalog/examples/aviato.people.enrich.bulk.json
  - src/olywork/catalog/examples/aviato.people.enrich.json
  - src/olywork/catalog/examples/aviato.people.phone.find.json
  - src/olywork/catalog/examples/aviato.people.search.json
  - src/olywork/catalog/examples/aviato.people.search.simple.json
  - src/olywork/catalog/examples/crustdata.companies.autocomplete.json
  - src/olywork/catalog/examples/crustdata.companies.enrich.json
  - src/olywork/catalog/examples/crustdata.companies.identify.json
  - src/olywork/catalog/examples/crustdata.companies.jobs.search.json
  - src/olywork/catalog/examples/crustdata.companies.search.json
  - src/olywork/catalog/examples/crustdata.people.autocomplete.json
  - src/olywork/catalog/examples/crustdata.people.enrich.json
  - src/olywork/catalog/examples/crustdata.people.search.json
  - src/olywork/catalog/google-search-console.yaml
  - src/olywork/catalog/google-search-console.extended.yaml
  - src/olywork/catalog/google-tag-manager.yaml
  - src/olywork/catalog/google-tag-manager.extended.yaml
  - src/olywork/catalog/instagram.yaml
  - src/olywork/catalog/instagram.extended.yaml
  - src/olywork/catalog/justoneapi.extended.yaml
  - src/olywork/catalog/minimax.yaml
  - src/olywork/catalog/apify.yaml
  - src/olywork/catalog/brightdata.yaml
  - src/olywork/catalog/companyenrich.yaml
  - src/olywork/catalog/oceanio.yaml
  - src/olywork/catalog/akta.extended.yaml
  - src/olywork/catalog/dataforseo.extended.yaml
  - src/olywork/catalog/tikhub.extended.yaml
  - src/olywork/catalog/examples/minimax.video-gen.result.retrieve.json
  - src/olywork/catalog/examples/minimax.video-gen.from_image.json
  - src/olywork/catalog/examples/minimax.video-gen.task.status.json
  - src/olywork/catalog/openrouter.yaml
  - src/olywork/catalog/openrouter.extended.yaml
  - src/olywork/catalog/examples/openrouter.x.alibaba-wan-3-0.json
  - src/olywork/catalog/replicate.yaml
  - src/olywork/catalog/replicate.extended.yaml
  - src/olywork/catalog/examples/replicate.image-gen.flux-schnell.json
  - src/olywork/domain/catalog/__init__.py
  - src/olywork/domain/catalog/store.py
  - src/olywork/domain/money/settlement.py
  - src/olywork/domain/catalog/stats.py
  - src/olywork/infra/catalog_observations.py
  - src/olywork/routers/catalog.py
...
Crustdata and Aviato support both BYOK and olywork's platform-key tier. Their catalog costs stay in the
vendors' native credits; `fx.yaml` converts the actual replacement rates olywork pays ($0.30 per
...
for the final charge. This protects olywork from a documented maximum without overcharging the caller.
...
src/olywork/routers/catalog.py    # open Catalog JSON routes, attached in legacy registration order
```
...
the catalog published $0 and the balance moved $0.10. Where the upstream bills *olywork* (an
...
because olywork pays the difference. Watch for **conditional** rates in particular: X's $0.001 "owned
read" applies only when the caller owns the developer app, which on a registry connect is olywork —
...
endpoints:
  - id: tikhub.tiktok.user.profile   # unique; convention: <provider>.<capability>
    ...
    input:                           # split by location — mirrors olywork's binding model
...
`--await` that polled the absolute URL through `/call/https://…` was refused for a team on olywork's
...
price surface - the wall, `olywork catalog search`, the dashboard, `/access` - shows `$low-$high` for a
...
router serializes the effective descriptor into `X-Olywork-Async` before the response stream starts;
...
kinds; `requires` binds a path/query parameter to one of those kinds. On olywork's shared key, a 2xx
...
DataForSEO on-page/SERP task consumers remain callable with BYOK but never receive olywork's shared key.
...
- Two optional fields exist only in this tier, both added for the first-party OAuth providers:
  ...
  - `scope_gap: <one line>` — the credential olywork's OAuth app obtains CANNOT call this, and this is
...
`olywork catalog search`, `olywork catalog get`) lead with `usd` because a column is only comparable in
...
have an answer before olywork will spend its OWN money on an endpoint (see "platform-eligible" below),
...
A `credit_rates_usd` entry may carry **`kind: olywork_shared_plan`**: a rate OLYWORK SET for a flat-fee
provider (a subscription with a rate limit or unlimited calls), where no per-call vendor price can
exist. The credit is then "one call on olywork's shared plan" and the machinery is unchanged — the
honesty lives in the entry: the basis must start with "olywork shared-plan rate", name the vendor fee,
...
A second olywork-set kind, **`kind: olywork_trial`**, prices a provider at exactly **$0** with a
`trial_calls_per_team_day` allowance as data beside the zero: a capped taste served on olywork's own
...
#### Platform-eligible — when olywork may spend its OWN key on a call
...
5. **Cost.** Record the provider's price model per endpoint from their pricing page — with its
   ...
   re-checkable, which is what lets olywork serve the route on its own key.
...
7. **Verify + capture.** Run `scripts/catalog_verify.py <service>.yaml` with the credential in the
   `OLYWORK_CATALOG_CRED` env var. It calls every endpoint's `test_request`, checks `expect`, writes
...
cached under `~/.cache/olywork-catalog-ingest` (override `OLYWORK_INGEST_CACHE`); `--refresh` re-fetches.
...
- **The published spec outranks the probe** (`resolve_method`). A wrong method is not a cosmetic
  error: olywork *enforces* the recorded verb, so the endpoint becomes uncallable from both sides at
...
`~/.cache/olywork-catalog-ingest` is what an unqualified `catalog_ingest.py <provider>` reads, so a
...
not an alias chain or a route olywork will still spend against.

### `platform_blocked:` — works upstream, but not on olywork's plan

A third state sits between "offer" and "tombstone": the route works and the price is real, but
olywork's own subscription cannot serve it — Akta answers every alternative-data call (jobs, posts,
...
where olywork owns the OAuth app are the opposite question — *what can this one connected account
...
- **No test_request anywhere in this wave.** Every route needs a property id, a customer id or a
  ...
  replay against a live connection (`--via-olywork`), not by a generated blind call.
...
```
OLYWORK_CATALOG_CRED='<secret>' uv run python scripts/catalog_verify_extended.py tikhub --dry-run
OLYWORK_CATALOG_CRED='<secret>' uv run python scripts/catalog_verify_extended.py tikhub --budget 1.80
uv run python scripts/catalog_validate.py            # must exit 0
```
...
**The agent makes it, not olywork** — see `docs/CAPABILITY-CHOICE-PLAN.md` for the measurement behind
that. Two reasons, and the second is the load-bearing one. Providers of the same capability take
*different requests* (only 5 of 171 match exactly), so a router would need a canonical schema olywork
...
a name, and fail. Routing would also have been the first feature to break the founding rule that olywork
relays rather than models.

What olywork owes instead is the half only olywork can supply, because only olywork sees every call from every
...
- **A olywork refusal is not evidence about the endpoint.** Rows with `refused_by` set (a paywall 402,
...
`olywork catalog search` expanded Hailuo/Seedance/t2v to 521 endpoints and Flux to 172, including
...
to olywork's own key — so it cannot be side-stepped to spend our money (a URL-passthrough call resolves
...
- **A matched child brings its routed parent.** `find leads` matched `leadsforge.*` on the
  provider's NAME; the row an agent should see first for that job — `olywork.people.search`, where
  olywork chooses among every provider — contained no word of the query. `search` now adds
  `olywork.<capability>` at the best child's score whenever a child matched. The token-filter tests
...
- **A group shows its best `MAX_ROUTED_CHILDREN` (5) children.** One capability's 24 providers had
  ...
  `+ N more providers — olywork catalog get <parent>`, MCP says so in `routed`. To keep the page full
  after collapsing, search ranks a band of 4× the page (≤ 100) and cuts to `limit` AFTER grouping.

## Routing — first-party routed endpoints (`olywork.<capability>`)

The one place olywork **models** an upstream API, and the explicit opt-in where the caller asks olywork
...
- **The generated row** — `routing/synthetic.py`: every capability with ≥ 2 verified children gets
  `olywork.<capability>` (`provider: olywork`, `kind: routed`, `POST /<capability>`, `input` = the
...
- **Ranking** — `routing/plan.py`: own keys (tier 2) first at cost 0; then
  ...
  path. `X-Olywork-Route-Prefer` / `-Exclude` override; exhausted providers (capacity view)
...
- **Execution** — `application/call/route.py`, entered from `service._execute_call` when the
  ...
  next candidate, at most two extra, only for idempotent contracts. A olywork-side
  ...
  indicates olywork's provider credential, not the routed caller's request. Balance and spend-cap
  ...
  free); `X-Olywork-Route-Waterfall: 0` stops at the first miss. Every attempt is settled at its real
  price and `X-Olywork-Route-Max-Cost` (default $1) bounds the sum before each reserve (a candidate
  that would breach it is `skipped`). Response: `{output, raw, _olywork: {served_by, provider, tier,
  outcome, tried[], charged_micro}}`, `X-Olywork-Served-By`, `X-Olywork-Providers-Tried`,
  `X-Olywork-Route-Outcome`, `X-Olywork-Cost-Micro` = the sum, one `X-Olywork-Call-Id`. The parent owns
...
- **min_results, and why it is bounded (2026-08-29)**: `X-Olywork-Route-Min-Results: N` records a hit
  ...
  Pair it with `X-Olywork-Route-Max-Cost` on any capability where thin answers are normal.
...
- **The answer says what it ignored (2026-08-29)**: `ignored_filters` was on `_olywork.tried[]` only,
  which no caller reads. It is now also on `_olywork` itself for the child that served and on an
  `X-Olywork-Ignored-Filters` response header, so an agent can post-filter, or say why the rows are
  wrong, without walking the attempt list. **Opt-in refusal (2026-09-04)**: `X-Olywork-Route-Strict-Filters: 1`
...
- **Routed DISCOVERY is a runtime switch (2026-08-29)**: `OLYWORK_ROUTED_DISCOVERY=off` (default `on`)
  stops search leading with `olywork.<capability>` and stops a routed parent riding in when a child
  ...
  collapse. Same dashboard-flip shape as `platform_providers` and `OLYWORK_OVERFLOW_MODE` — no
...
- **What is not routed on purpose**: `*.bulk` endpoints (a routed call is one subject, one answer),
  ...
  olywork's platform key and serving live calls, was listed as unavailable). It now names the shape and
...
- **Coverage (2026-08-28)**: 74 routed capabilities = 80.9% of 30-day platform calls (88% was the
  ...
  rows still open — is `docs/CAPABILITY-EXPANSION.md` (git-excluded, Taqi's working doc).
- **Not built** (plan R4): "prefer routed" in the agent files after a shadow week; a proper
  ...
  `fmt` today; crustdata/diffbot/coresignal/apollo do not); own-key-dry → olywork-key fallback.
...
`OLYWORK_CATALOG_CRED` from the environment only. Captured examples are truncated (arrays → 2 items,
