...
- the presented `X-Olywork-Token` equals the env `admin_token` (`get_settings().admin_token`, from
  `OLYWORK_ADMIN_TOKEN`), compared with `hmac.compare_digest` → principal `"env-admin"`; OR
...
- **Reconciliation (Phase 5):** `admin_reconcile_drift|spend|repeats` (`?since_days=30`) — cross-org
  ...
  over existing rows; no scheduler. Logic in `src/olywork/reconcile.py`;
...
`python -m olywork upgrade` release phase, never on restart.

## CLI (`interface/cli.md`)
`olywork admin login --token` (saves the env key), `admin stats|orgs|org <id>|users|tools|calls|health`,
