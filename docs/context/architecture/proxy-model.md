...
sources:
  ...
  - src/olywork/infra/upstream/ssrf.py
  - src/olywork/api.py
  ...
  - src/olywork/application/asynctasks.py
  - src/olywork/client_identity.py
  - src/olywork/call_surface.py
  - src/olywork/sandbox_identity.py
...
2. **olywork's control/infra + edge forwarding headers** - `_CONTROL` (`x-olywork-token`, `x-olywork-org`,
   `ngrok-skip-browser-warning`, `x-forwarded-*`, `x-real-ip`, `forwarded`, `via`), dropped via
   `_DROP_REQUEST = _HOP_BY_HOP | _CONTROL`, so none leaks upstream. `_scrub_olywork_cookies` also strips
   olywork's own cookies (`olywork_session`, `olywork_oauth_state`) from the Cookie header - the dashboard's
...
> **What olywork keeps from a call.** Successes retain no content: the relay forwards bytes and the audit
...
> of what the caller sent and what the provider (or olywork-side 502) answered. Without it a failure is a
...
- body streamed via `content=request.stream()` (stream, never buffer). Exception: a caller may
  base64/gzip-encode the body with `X-Olywork-Body-Encoding` to slip SQL/HTML past a hosting-edge WAF;
...
**Platform bindings - injecting olywork's OWN credential.** A binding with a `platform_setting` key (instead
of a `secret_id`) injects one of olywork's own credentials read from `get_settings()` - the Google Ads
...
that needs a second credential olywork holds centrally, and by tier-4 catalog calls. Tier 4 also copies
...
uploader (`adsconv.py`) also spends olywork's own platform connection, but it is not a caller-issued
...
marked as callable on olywork's key or requiring an own credential. Marked catalog rows are excluded.
...
Olywork refusals on both call surfaces carry `X-Olywork-Error: 1`. Mechanism-keyed application
failures map to `caller | olywork | upstream | org_connection` blame; the HTTP adapter preserves
...
**Relay security + faithfulness (bug-hunt):** the response side strips a `Set-Cookie` for olywork's own
cookie names (an upstream must not overwrite `olywork_session`/`olywork_oauth_state` - fixation) and adds
`X-Content-Type-Options: nosniff` + `Content-Security-Policy: sandbox` (a browser navigating to `/call/…`
must not execute upstream HTML/JS under olywork's authenticated origin). It keeps `Content-Length` on a
...
A catalog row with `kind: routed` (`olywork.<capability>`, generated - `architecture/catalog.md`
...
unchanged. The parent only assembles `{output, raw, _olywork}` and owns the idempotency label.

## Platform capacity: refuse before reserve (plan step D)

Tier 4 spends olywork's own vendor account, and that account can be empty. `_resolve_marketplace_call`
...
`CallFailure("provider_capacity", 503, blame="olywork")` - **before any hold exists** - whose body carries
`resets_at` when known and the same-capability alternatives from `_capability_alternatives`. olywork still
does not choose for the caller (charter): it names the options. The audit row is `refused_by="capacity"`,
`X-Olywork-Error: 1`, cost 0. A stale, empty or "ok" view never refuses; only a confirmed signal does.
...
## Burst smoothing on olywork's own keys (plan step D′)
...
Both are visible: `X-Olywork-Smoothed: wait=<ms>` and/or `retry=1` on the response (metered exit only).
...
**Overflow = the same vendor endpoint, another account of ours.** When a tier-4 call fails on olywork's
own key for a olywork-side reason - a balance/quota signature, a burst-429 smoothing could not absorb -
...
4. `parse` → vendor status + body + the real in-band cost. `_platform_settle(child,
   observed_override=cost, overflow_spend=(aggregator, cost − olywork's direct price))` charges **exactly
...
5. The vendor's body goes back as the answer, `X-Olywork-Served-Via: overflow:<name>`, `X-Olywork-Cost-Micro`
   the child's charge, `X-Olywork-Call-Id` the parent's. Two audit rows share the `call_ref`: the primary
...
**Shadow mode** (`OLYWORK_OVERFLOW_MODE=shadow`): the aggregator is called, status / shape / cost logged
and the probe's cost recorded in `OverflowSpend` (olywork pays, budget-bounded) - the caller still gets
...
not enabled, or a team that opted out (`Org.platform_overflow_disabled`, `olywork org overflow off`) -
checked before any aggregator is contacted, on both entry points.

## Control-header isolation

`proxy._is_dropped_request_header` strips every request header beginning with `x-olywork-`,
...
an alert. `routers/call._attach_async_descriptor` adds `X-Olywork-Async` (the effective descriptor) to
