...
sources:
  ...
  - src/olywork/crypto.py
  - src/olywork/oauth.py
  ...
  - src/olywork/infra/oauth_exchange.py
  - src/olywork/infra/oauth_refresh.py
  - src/olywork/oauth_providers.py
  - src/olywork/health.py
  - src/olywork/application/connect.py
  - src/olywork/routers/connections.py
  - src/olywork/routers/resources.py
  - src/olywork/domain/tools/__init__.py
  - src/olywork/domain/tools/bindings.py
  - src/olywork/domain/tools/bundles.py
...
`OLYWORK_SECRET_KEY`, falling back to an ephemeral `_EPHEMERAL` key if unset (so secrets don't survive a
...
`olywork.oauth` re-exports the refresh family for compatibility with connect, health, call, and lazy local-run
...
refreshable credential is **always** `fresh` (olywork mints on demand, so the user is never nagged), only an
...
**olywork itself holds the approved app** (Google Search Console/Analytics/Business Profile/Tag Manager/Ads, YouTube,
...
platforms is the *approval* (a Google Ads developer token, Meta App Review), not the OAuth dance — olywork
has already cleared it. olywork's own client id/secret load from `Settings` (named by
...
`OLYWORK_OAUTH_REVIEW_PENDING` is the one operational source for incomplete provider reviews. Generic
...
and resource picker, with the selected `accounts/{id}` path stamped into the tool example. olywork
...
module symbols:
- `get(service)` — look up one provider. `credentials(provider)` — olywork's own id/secret (raises if this
  deployment hasn't set them). `is_configured(provider)` — whether this deployment can offer it (a
  `token`-kind provider needs nothing from olywork, so always offerable).
...
- `consent_notice` — one line the dashboard shows **before** the consent popup opens, for a provider
  whose consent screen names something the user has not seen on olywork. Only the Meta family carries one:
  the shared Meta app is registered as **Crewlet**, a sibling product of the same company (Olywork
  Dev Inc), and Facebook renders only that bare app name with no parent business, so without the notice
  a olywork user is asked to authorize a stranger. It rides `listing()` like any other provider field, so
...
- `platform_billed` + `billed_read_usd`/`billed_write_usd`/`billed_write_link_usd` — providers whose
  ...
  registry X connect spends olywork's credits and the proxy meters those calls against the org's balance
  (`api.py` `_billed_marketplace` → the tier-4 reserve→settle path; [money](money.md)). Gated on the
  deploy opting in via `OLYWORK_OAUTH_BILLED_PROVIDERS` (`config.oauth_billed_set` — empty means free,
...
- `auth_kind` = `"oauth"` (olywork's app), `"token"` (a user-pasted Bearer token: Slack plus the
  ...
  **Advertising** the ad-intel keys SpyFu, Apify, Meta Ad Library, SerpApi — alongside the OAuth ad
  ...
  olywork, so `is_configured` is always true for it. The pasted credential rides in a header
  ...
  second credential or olywork holds it) drives auto-building a callable tool on a successful connect;
  `needs_extra_credential` covers a second header the primary slot can't carry: Google Ads'
  `developer-token` (olywork-held, via `extra_credential_setting`) and Tomba's per-user `X-Tomba-Secret`
  ...
  **Split-host vendors get one extra Tool per host** (`extra_tools`): GA4 runs reports on
  ...
  path on the data host → Google 404; admin host → olywork "no registered tool"; 13 calls/7 orgs observed
  ...
  The default `python -m olywork` serve command runs
  that phase before Uvicorn; raw ASGI deployments run `python -m olywork upgrade` once per release. The
...
member: on Linux the CLI runs as a dedicated `olywork-run` user, so the credential lives under that user
...
shared-key tool they may run but not read) requires the **runner proof** (`X-Olywork-Run-Proof` ==
`OLYWORK_RUN_PROOF`, held only by the root-installed `olywork-run` runner) — so a direct member call can't read
