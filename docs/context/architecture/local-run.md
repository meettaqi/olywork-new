---
title: Local CLI runs — run a vendor CLI as a dedicated user with a server-held credential (`olywork run`)
status: shipped
sources:
  - src/olywork/localrun.py
  - src/olywork/egress.py
  - src/olywork/fsjail.py
related:
  - interface/api.md
  - interface/cli.md
...
# Local CLI runs (`olywork run --local`)

A member runs a vendor CLI (`stripe`, `gh`, `flyctl`, `gcloud`, …) on their own machine while the
credential stays in olywork custody. The security goal: **no other program of the same user may read the
credential** while the CLI runs. Design + the full decision trail: `docs/CLI-RUN-PLAN.md`.

> `olywork run` has a second tier — **`--server`** (Tier 0): the CLI runs on the registry server instead of
...
can read it). So `olywork run` runs the CLI as a dedicated **`olywork-run`** system user — the MongoDB pattern.

## The flow
1. The member's `olywork run <tool> -- <args>` (thin client) hands off to the `olywork-run` user via
   `sudo -u olywork-run <runner>`, passing the member's OWN olywork token through the environment
   (`OLYWORK_RUN_TOKEN/BASE/ORG`, preserved by the install-time sudoers rule). `sudo` connects the terminal,
   so input, output, signals, and exit code flow through naturally.
2. The **runner** (running as `olywork-run`, `cmd_run_helper` → the hidden `__run-helper`) calls
   `POST /tools/{name}/grant` with the member's token, receives the credential, runs the CLI with it, and
   reports the outcome. **The vendor credential only ever exists under `olywork-run`** — the member never
   holds it. Install (`olywork setup-local-run`, once, as admin) creates the user, installs the runner, and
   adds a narrow sudoers rule allowing the member to run ONLY that runner as `olywork-run` (never a shell).
3. Without that setup, `olywork run` falls back to running as the member, **best-effort**, with a clear
...
`olywork-run` user via `useradd` on Linux and via **`dscl`** on macOS (a hidden service account with a free
system uid from `_pick_macos_service_uid`), and `_run_local` hands off via `sudo -u olywork-run` on **both**. A
...
setup-time check warns if `olywork` itself lives inside the member's `0700` home — `olywork-run` can't traverse in
to exec it, so olywork must be installed at a **system-accessible path** (the isolation working against its own
launcher).

The trade-off (inherent to a real boundary): the CLI runs as `olywork-run`, so it cannot read the member's
...
or command-line flag) under `olywork-run`. An **oauth** secret is refreshed first (`oauth.ensure_fresh`) and
...
before olywork's first refresh and `access_token` after is handled by falling back to the sibling key).
Values are `.strip()`ed.
...
runner from a direct member call by the `X-Olywork-Run-Proof` header (the value only olywork-run can read, from
`/etc/olywork-run/proof`, installed by `setup-local-run --run-proof` and exported as `OLYWORK_RUN_PROOF`). A
...
**`auth_mechanism` splits the tiers.** `env`/`argv` are server-injectable (olywork holds the key); `config_file`
...
dashboard won't offer server-run for it. This field also drives `olywork upload clis` routing (see
...
member code as `olywork-run`: `gh extension`/`alias`/`auth token`/`--show-token`, `flyctl|turso auth token`,
...
stdout) nor deny (unknown features) fully stops. `src/olywork/egress.py` closes it by restricting the
`olywork-run` uid's **outbound network** to a fixed allow-list. Pure builders: `collect_hosts(registry, catalog)`
(the registry — so the runner can reach `/grant` — plus every catalog `base_url` host), `resolve_hosts`
(→ current IPv4/IPv6), `pf_ruleset` (macOS `pf` per-user `user olywork-run` rules — verified: macOS pf honors the
...
resolves + loads them now and persists them: macOS via a `LaunchDaemon` (`dev.olywork.egress`) that re-runs a
root-owned loader (`/usr/local/bin/olywork-egress-load`, which reloads Apple's ruleset + `/etc/olywork-run/egress.pf`
at boot); Linux writes `/etc/olywork-run/egress.nft`. Setup flags: `--no-egress` (opt out), `--registry` (the
...
harmlessly). The remedy is to **re-resolve**: `sudo olywork setup-local-run --refresh-egress` re-runs
...
carry each provider's known IP ranges (stable, no per-machine DNS), or olywork should ship an auto-refresh timer.
This churn is inherent to Option 1 (static IPs); it's the trade-off accepted against Option 2's per-run rules.

### Filesystem jail (`olywork run --fs-jail`, opt-in)
...
olywork-run-owned, pointed at as `HOME`/`TMPDIR`), removed after the run. `src/olywork/fsjail.py`: `macos_profile`
builds a seatbelt profile (`allow default`, then `deny file-write*` except the scratch subpath + `/dev/`);
`wrap_macos` prefixes `sandbox-exec -f <profile>`. `_run_helper` reads `OLYWORK_RUN_FSJAIL` (forwarded by
...
## Trust posture
The credential lives under `olywork-run`, unreadable by the member's uid (only root can read any process).
