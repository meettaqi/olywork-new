...
EOF so the deployed `olywork.api:app` import path remains the default `all` role.
...
`python -m olywork upgrade` release phase owns content-driven backfills; the default `python -m olywork`
...
`OLYWORK_CLAUDE_CONNECTOR_ENABLED=true` adds `/mcp/v2` and starts its lifespan. When the flag is false
