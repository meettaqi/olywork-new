...
sources:
  ...
  - src/olywork/application/billing.py
  ...
  - src/olywork/infra/__init__.py
  - src/olywork/infra/stripe.py
  - src/olywork/reconcile.py
...
A catalogued endpoint can be served on **olywork's own key** - no provider signup for the caller - which
means olywork pays the provider and bills the team. That needs a balance, a way to top it up, and a way
to prove afterwards that the numbers were real. Three modules, one job each:

Two wallets of olywork's spend through this machinery, and only these two: **tier-4 platform keys**
(`OLYWORK_PLATFORM_KEY_*`) and **oauth-billed apps** - providers like X whose upstream bills the app
owner per use, so even a call on the org's *own* connection spends olywork's prepaid credits
...
which side failed. A provider that answered 5xx releases as `provider_failed_<status>`; a call olywork
...
The response still reports `X-Olywork-Cost-Micro: 0`, which is what the call ends up costing - but the
...
still apply. Its response reports `X-Olywork-Cost-Micro: 0`; the original submission's hold remains
...
view formats it as `olywork call … -p name=value`), plus `ttl_note`. The CLI's `--await` calls the
same function; there is one reading of the descriptor, not a mirror. `/calls`,
`/calls/{ref}`, `olywork audit` and the dashboard Activity page all render from this one view.
...
now per-org) and `POST /billing/topup` with no amount uses it - which is what `olywork topup` sends.
...
`olywork balance` after a call. Refusing the number there while shipping it in an error was incoherent.
(Reported by Taqi, 2026-08-07.)
...
## Shared-plan pricing: flat-fee providers, and the rate olywork sets
...
| unlimited | a olywork-set rate with the break-even printed |

The honesty rule that makes the last two rungs defensible: **we never claim these are vendor
prices.** What olywork sells there is its own service - subscription custody, the key, a share of the
rate limit - at a published rate whose fee and break-even are printed beside it (fx.yaml
`kind: olywork_shared_plan`; `check_fx` makes the marker impossible to carry dishonestly). The price is
also congestion control: at $0, one looping agent exhausts a shared rate limit for every team at
once.

Mechanically a shared-plan provider is just a credit provider whose credit is "one call on olywork's
...
- **A 429 is never billable**, under any cost type. Capacity refusing a request is not the caller's
  bad input, and on a shared key it is olywork's own saturation. This also fixed a pre-existing wrong:
...
- **A relayed 405 is never billable**, under any cost type. A catalog caller cannot choose a stale
  ...
  can reject is therefore olywork's recorded method, so settling a `per_call` hold would charge the
  team for catalog metadata olywork owns. `_NOT_THE_CALLERS_FAULT` makes that path release the hold.
...
- **`shared_plan_recovery`** (`GET /admin/reconcile/shared-plans`): fee versus collected per
  olywork-set rate, with `suggested_usd = fee ÷ measured calls` and an action at ±50% thresholds. It
...
- **`price_drift` never sees these providers** - drift compares our estimate against the provider's
  ...
  policing a price olywork itself set.

### Trial pools: the $0 rung

A third olywork-set rate, `kind: olywork_trial` (fx.yaml): a provider served on olywork's own FREE-tier key
...
- Replays return `X-Olywork-Idempotent-Replay: true` and the original `X-Olywork-Cost-Micro`;
...
## Tag-based billing - a builder reselling olywork to their own users

A builder embeds olywork in their product and bills their own users. olywork's job is exactly three things:
**attribution**, **enforcement**, **export**. olywork never bills their end user, never holds their card
...
`olywork org agent-new --pin customer=cust_A`). The pin **beats the header**: naming a different value is
