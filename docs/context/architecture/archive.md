...
Async settlement archives terminal provider JSON under `olywork://asynctasks/<call_id>` using the
...
**Build state: COMPLETE (PR 6 of 6 — the panel shipped after the original five).** All five slices exist behind `OLYWORK_ARCHIVE_MODE`:
...
and the key value from settings. The refresh spend is olywork's own, attached to no org and absent from the ledger — the
...
the response carries `X-Olywork-Cache: hit`, `X-Olywork-Fetched-At`, `X-Olywork-Age`, and the audit row
...
read-after-write escape — the archive never guesses cross-endpoint effects); `X-Olywork-Max-Age`
...
`X-Olywork-Cache`-style serve headers do not exist yet. `archive.record()` is fire-and-forget with
...
`OLYWORK_ARCHIVE_MODE` (config `archive_mode`, default `off`) → `archive.mode()`:
...
`GET /admin/archive/panel` serves `src/olywork/web/archive-panel.html` — a data-free page SHELL,
