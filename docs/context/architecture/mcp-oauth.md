---
title: MCP — the front door for assistants, and olywork as an OAuth authorization server
status: shipped
sources:
  - src/olywork/application/auth.py
  - src/olywork/mcp.py
  - src/olywork/domain/identity/health.py
  - src/olywork/domain/identity/mcp_oauth.py
  - src/olywork/domain/identity/session.py
  - src/olywork/routers/auth.py
  - src/olywork/web/claude-connector.html
  - src/olywork/web/connect-demo.html
...
Everything else in olywork is reached by a CLI or an HTTP call. These are the doors an **assistant**
...
endpoints, team-owned tools, and imported skills. Olywork cannot reliably classify the side effects of
...
The two version labels describe different things. `/mcp/v2/` is Olywork's second MCP transport
...
`OLYWORK_CLAUDE_CONNECTOR_ENABLED` defaults to `false`. When false, V2 is not mounted, its lifespan does
...
`catalog_search` and `catalog_get`; they do not change `tools/list`. olywork also publishes no tool,
...
you notice olywork does not model the upstream: it relays to somebody else's API and cannot know whether
that endpoint charges, writes or deletes. Claiming otherwise would be a guess presented as a fact.
`catalog_request` is the one other non-read: a write, but a harmless one (a row on olywork itself,
...
Tools reach the rest of olywork through `httpx.ASGITransport` against our own app — a real request
...
drifting quietly. The in-process client stamps `X-Olywork-Client: mcp` for the team MCP and
`X-Olywork-Client: claude-connector` for V2 (attribution, never a gate), so each MCP surface is
...
explicit slots exist for the shapes that role can't express, mirroring `olywork call`'s flags —
...
- `headers` — extra upstream headers (`login-customer-id` is the canonical need). olywork's own
...
- Multipart file upload stays CLI-only (`olywork call --upload`) — MCP callers hold no files.
- `call` resolves the TEAM the way `balance` does (`_resolve_org`, before anything is spent): a
  multi-team identity token used to bounce off /call's raw `choose an org (send X-Olywork-Org)`
...
olywork replays the stored response, does not reach the provider, and charges nothing, with
...
rejected: two identical searches an hour apart are new work, olywork cannot tell that from a retry, and
...
open so a client could browse before authenticating. But every olywork tool needs auth, so there is
...
for MCP 2026-07-28 `subscriptions/listen`, which exposed the bug before olywork stopped advertising
...
# olywork as an authorization server

Elsewhere olywork speaks OAuth as a **client** (`oauth.py` signs in with GitHub, connects a provider
...
MCP server would work here — they consented to that server, not to olywork, and we would spend their
balance on it.
...
V2 advertises `olywork:directory` as a fallback resource marker. Hosted Claude can preserve the V2
...
(2026-08-17). `balance` reported the slug `olywork-7`; `olywork org ls` on their machine listed
`olywork` and `ai-taqi` and nothing else, because the CLI was signed in as a *different account*
...
- **`balance` and `my_tools` label the grant**: `team_name` (a slug alone cannot be sanity-checked)
  ...
  *how to move it* half of the hint is added only for an actual OAuth caller: a header token carries
  its own team, and `olywork mcp grants` would list nothing for it.
- **The team can be moved without re-consenting.** It lives on the refresh family's `OAuthGrant`
  ...
  `POST /oauth/grants/{family}/team` (`olywork mcp grants`, `olywork mcp use-team`) is a row update the
  ...
  - **"Not your team" and "no such team" answer identically** (404). Told apart, the route reports
    whether an arbitrary slug exists on olywork, to any signed-in account.
...
token exchange, tool calls. It uses only public endpoints; being served from olywork's domain gives it
nothing. `OLYWORK_CONNECT_DEMO_ENABLED` gates both the page and its callback and defaults to `false`, so
...
- **No routing or failover.** olywork publishes facts and calls what it is told; the agent chooses.
...
the installer uses: a **team-pinned token as an `Authorization: Bearer` header**. `olywork mcp install`
(`mcp_install.py`, sibling of `olywork skill bootstrap`) registers the server into every supported agent
...
`olywork login --token` runs. Learned the hard way: without it, a garbage token fans out silently into
...
Why a header works even though olywork advertises OAuth: a client only falls back to OAuth discovery on
a **401**, and olywork returns **200** for a valid header — verified against Claude Code, which
...
`olywork mcp install` — in one paste.

## Caller tags over MCP

Catalog-call tools also expose an optional `authorization_method`. MCP maps that explicit argument
to olywork's internal `X-Olywork-Authorization-Method` routing header; caller-supplied headers cannot
override it, and the header is not relayed to the provider.

`X-Olywork-Meta` (see [money](money.md)) is read off the MCP **transport** in `mcp.call()` and forwarded
...
billing figure you cannot reconcile is worse than no figure. `x-olywork-meta` is therefore also in the
