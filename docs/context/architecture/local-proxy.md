---
title: Local proxy — catch a program's own outgoing calls (`olywork <command>`)
status: shipped
sources:
  - src/olywork/localproxy.py
...
Every other way into olywork requires the caller to **know about olywork** — `olywork call`, `olywork run`, the
`/call/` URL. The moment a program writes its own request to `api.stripe.com`, olywork is invisible to it
...
```
  a program that has never heard of olywork
        │  HTTPS_PROXY=http://olywork:<session-token>@127.0.0.1:<port>
        ▼
  local proxy  (src/olywork/localproxy.py — this fragment)
        │
        ...
        └─ host IS registered   ──►  terminate TLS with a leaf we sign
                                     ...
                                     re-address it, keeping method/path/query/body
                                          │
                                          ▼
                       olywork server   POST /call/https://api.stripe.com/v1/charges
                                     + X-Olywork-Token / X-Olywork-Org / X-Olywork-Client
...
laptop** (`crypto.rs`, `connect.rs`). That is the exact situation olywork exists to prevent. A future
"inject locally under `olywork run`" mode is deliberately out of scope — deciding it later costs nothing,
...
|---|---|---|
| **`olywork <command>`** (`cmd_with`) | that one process and its children | the normal case — `olywork claude`, `olywork node app.js` |
| **`olywork shell start --proxy`** | one subshell | you want a session where the team's CLIs *and* raw calls both work |
| **`olywork serve`** | any terminal that opts in with `eval "$(olywork serve env)"` | you want it running across terminals |

**`olywork <command>` is the headline.** olywork is the PARENT of what it launches, so the environment reaches
that process only: `olywork claude` uses team access, plain `claude` is untouched and uses the member's own
keys. Nothing is written to any config file, so there is nothing to undo. `main()` falls through to it
via `_looks_like_a_program()` when the first word (after `with`'s own flags) is **not** a olywork subcommand
**and** exists on `PATH`. Both conditions matter: without the first, a stray `call` binary would shadow
`olywork call`; without the second, `olywork toool ls` would become an exec attempt instead of an ordinary
...
owns the terminal, and the child's exit code becomes olywork's.

**Deleted on purpose: the global hook.** A `olywork serve hook` that wrote `BASH_ENV` into
`~/.claude/settings.json` (as oneCLI's Claude plugin does) was built and removed the same day. It
captured every session of that agent on the machine, forever, whether or not the member wanted olywork that
...
| `authorized` | `Proxy-Authorization: Basic olywork:<token>`, `compare_digest` |
| `_blind_tunnel` | CONNECT for a non-registered host — copy bytes, decrypt nothing |
| `_intercept` | CONNECT for a registered host — TLS terminate, then relay each request to olywork |
| `_forward_plain` | absolute-form `http://` (we set `HTTP_PROXY` too, so http callers must not break) |
| `_olywork_url` | `https://host/path` → `{base}/call/https://host/path` (the URL-passthrough shape) |
| `_forward_headers` | drop transport + caller `x-olywork-*`; add our token/org/client |
| `_send_to_olywork` | the outbound request, including the edge-WAF base64 retry |
| `_write_response` / `_body_chunks` | stream the answer back, re-deriving the framing |
| `_explain_olywork_error` / `olywork_error_message` | turn olywork's own refusal into an instruction |
...
| `write_state` / `running` / `pid_alive` | the `olywork serve` daemon's `proxy.json` |
| `olywork_client` | the proxy's own httpx client — **`trust_env=False`** |
...
3. `CONNECT` → `split_hostport`. `_is_self` refuses our own address (a loop that would eat a connection
   per attempt).
4. `cfg.intercepts(host)` decides. It requires the host to be allow-listed **and** a CA **and** a olywork
...
6. Per request on that connection (keep-alive is honoured): parse head, read body
   ...
   build the olywork URL and headers, `_send_to_olywork`, then `_explain_olywork_error` if the answer carries
   `X-Olywork-Error`, else `_write_response`.
7. Response framing is **re-derived, not copied**: `Content-Length` when olywork gave one, chunked when it
   did not, because our hop must be self-consistent even if the hop into olywork was framed differently.
...
## Errors a caller can act on (`olywork_error_message`, `_explain_olywork_error`)
A raw 404 from olywork reads as "the vendor has no such endpoint", and an agent "fixes" it by rewriting a
perfectly good URL. olywork's own refusals are therefore replaced with a message naming olywork and the next
action (register the tool / ask an admin / `olywork login` / the cap is used up).

This fires **only** on `X-Olywork-Error`, the header `api.py`'s `_mark_olywork_own_errors` puts on olywork's own
`/call/` refusals — otherwise a genuine vendor 404 would be rewritten too. Against an older registry that
does not send it, replies pass through untouched. olywork being unreachable answers `502` naming olywork and
...
| 6 | `trust_env=False` on `olywork_client` | httpx reads `HTTPS_PROXY` from its own environment — the first call would loop back into us |
| 7 | Log host and status only (`_log`) | a body, a header or the token in a log file |

## The daemon's state (`olywork serve` only)
A service must be findable by other terminals, so `write_state` records port, pid, token, registry and
captured hosts in `~/.olywork/proxy/proxy.json` at mode **0600**, created before any bytes go in. That file
...
The detached child is launched as `sys.executable -c "from olywork.cli import main; main()" serve start
--foreground`, never the `olywork` on `PATH`: a Homebrew copy one version behind would be started instead
and would not have the command at all. Output goes to `~/.olywork/proxy/serve.log`, and the parent prints
...
generated for a tunnelled host (invariant 3); the caller cannot spoof `x-olywork-*`; `trust_env` is off;
a vendor's own error is never rewritten; and the end-to-end
`test_the_agent_calls_the_vendor_and_olywork_injects_the_key`, which drives a plain `httpx.Client` through
...
intercepted call adds a hop to olywork and back and **fails if olywork is down** — the price of the key never
landing. Large uploads flow through us. Some vendor CLIs still need `olywork run`: `gh` refuses before it
