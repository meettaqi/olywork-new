...
- **Machine identities** — a `User` on an **unroutable domain**, which is what makes it a machine
  ...
  (`AGENT_DOMAIN` = `agents.olywork.local`). Both are minted by an admin and act ONLY by their token:
...
- **`require_member`** resolves `X-Olywork-Token` → a `Membership` → a `Caller` (`membership, user, org`,
...
- **Observed agents (`GET /orgs/{id}/agents/observed`, admin+).** The OTHER half of the agents story:
  ...
  (`CLAUDECODE` → `claude-code`, `CODEX_*`, `CURSOR_*`, …; `OLYWORK_CLIENT` overrides) and sends
  `X-Olywork-Client`; `_client_of` normalizes it (slug ≤32, versions stripped, unknown-but-well-formed
...
- **Registration is shared across doors:** `application.signup.find_or_create_user(db, email)` finds a user or creates them
  ...
  mandatory welcome, or `olywork org create`); their identity token is user-scoped so it works before any
...
- **Slug vs id.** `_resolve_org` resolves `X-Olywork-Org` by slug first (an all-digit slug like `2024` is
...
A builder reselling olywork tags each call with their own ids (`X-Olywork-Meta: customer=cust_8123,
...
send any value, exactly like `X-Olywork-Client`. That is acceptable because every budget and every report
