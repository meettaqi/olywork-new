...
sources:
  ...
  - src/olywork/alembic/versions/0010_oauth_authorization_method.py
...
`OLYWORK_OAUTH_REVIEW_PENDING` controls the rollout without a code edit. The generic registry rules give
these three states:
...
Instagram account. Olywork derives and stores the Page token inside the encrypted OAuth blob. The
...
`authorization_method` argument; and direct API calls use `X-Olywork-Authorization-Method`. Selection
...
olywork and never relayed upstream. Shared clients accept method ids and display labels from registry
...
3. Set the production redirect URI to `https://olywork.com/oauth/callback`. For local testing, set
...
4. Copy the separate Instagram App ID and Instagram App Secret into local `.env` as
   `OLYWORK_INSTAGRAM_CLIENT_ID` and `OLYWORK_INSTAGRAM_CLIENT_SECRET`. Do not put these values in chat or
   source control.
...
6. Keep the existing Meta app credentials in `OLYWORK_META_CLIENT_ID` and
   `OLYWORK_META_CLIENT_SECRET` for `page-tools`. Confirm its Page and Instagram permissions in App
...
`OLYWORK_OAUTH_REVIEW_PENDING` only after Meta marks the related access as approved.
