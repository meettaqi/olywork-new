...
sources:
  ...
  - src/olywork/application/onboard/demo.py
  - src/olywork/cli.py
...
A brand-new user's fastest path to *believing* olywork ("call a real API with no key on your machine")
...
## The one brain - `src/olywork/application/onboard/demo.py`
...
- **Fake teammates** (`TEAMMATES`): Ada·admin, Ben·member, Cora·viewer — roster-only `User` rows
  with `demo=True` on the unusable domain **`demo.olywork.local`** (`DEMO_DOMAIN`). Reused across demo
  orgs (email is unique); they get a Membership but **no personal org** and **cannot log in** (see
  the OTP guard below).
- **A working tool** (`echo` → `postman-echo.com`) + its `echo-key` secret, so **Try-it / `olywork call`
...
That last branch is not hypothetical: with `OLYWORK_PLATFORM_PROVIDERS` unset (as in production today)
...
- **Demo** (`_run_demo`) — a purely **illustrative** walkthrough: **no team is created, nothing is uploaded**
  ...
  URL** (`olywork call https://api.resend.com/domains`) so it's unmistakably a real API but **executes via the
...
teammate `demo=True` and accepts a pending invite; `GUIDED_TEAMMATE` = Alex Rivera, `alex@demo.olywork.local`,
...
- `GET /auth/me` returns `onboarded`; `GET /orgs` rows carry `demo`. `create_invite` **skips the Resend
  email** for `@demo.olywork.local` invitees.
- **Guards:** `auth_email_start` refuses any `@demo.olywork.local` email (400) — fake teammates are never
...
- **Schema:** `User.onboarded` / `User.demo` / `Org.demo` (see [data-model](../architecture/data-model.md);
  schema changes are Alembic revisions under `src/olywork/alembic/`).

## CLI face (`olywork onboard`)
...
`X-Olywork-Org`; a per-org agent token is org-bound), plays `_splash` (skipped for scripted `--path`/`--yes`
...
After a first **human** `olywork login`, `_maybe_offer_onboarding` prompts `[Y/n]` then `_pick_path` +
...
carries a `plugin` URL (Grok Bot → the x.ai plugin page) an **"1. Install the olywork plugin"** link comes
...
(`/app?ref=grokbot` from `/grokbot`'s "Setup olywork") **preselects that agent**: the `ref` param is stashed in
`localStorage` (`olywork-ref`) because sign-in reloads the page, and `_welcomeAgentFromRef` consumes it when the
welcome opens (unknown refs are ignored). The picked agent itself persists as `olywork-agent` (a `welcome.agent`
