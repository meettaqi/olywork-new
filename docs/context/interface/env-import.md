...
status: in-progress
sources:
  - src/olywork/providers.py
  - src/olywork/skills.py
related:
  - interface/cli.md
  - interface/skill.md
...
# Import (`olywork upload`; read-only preview = `olywork scan`)

Instead of adding tools one by one, `olywork upload` scans a directory and registers what it finds.
Bare `olywork upload` does **both** sides of the dir; `olywork upload env` / `olywork upload skills` restrict it
...
app prefix (`OLYWORK_RESEND_API_KEY`) still resolves via its `RESEND` component. `_match_provider` prefers
...
isn't enough), and `build_binding` maps each shape to a olywork binding:
...
An entry may also carry a **`cli` block** — the run profile for `olywork run`
...
field the CLI auto-importer (below) routes on: `env`/`argv` are **server-injectable** (olywork holds + injects
...
## CLI auto-import (`olywork scan clis` / `olywork upload clis`) — scan the machine, register, report
The machine that runs `olywork scan`/`olywork upload` is the only place that knows which CLIs are **installed**
and logged in. (`import` was renamed: `olywork scan` = read-only preview, `olywork upload` = register; `olywork
...
`POST /tools {name,base_url,bindings:[{…,secret_id}],health_check}`. That probe means `olywork health --run`
...
> registered. The binding is the standard olywork shape — see [auth-secrets](../architecture/auth-secrets.md)
...
`~/.olywork/providers-cache.json`, and falls back to the cache then the **bundled `CATALOG`** when offline
(so `olywork upload` always works). `scan_env(path, catalog=…)` takes the resolved catalog; `--dry-run`
...
## LLM fallback for unknowns (`olywork upload --llm`)
`unknown_secret` vars (a credential with no catalog match) can be resolved by an LLM: `--llm` (with
`--llm-token` or `OLYWORK_LLM_TOKEN`) sends the **names only** to an **OpenAI-compatible** endpoint
...
## Skill directories (`olywork upload skills`, `skills.py`)
`scan_skills(dir, catalog, env_names)` classifies every skill subdir into a `SkillDetection`:
- **contract** — already has a `olywork.json` (`convert.load_contract`); used verbatim (google-ads, gsc).
  ...
  exact path wins, else the leading secret-dir segment is swapped — so a olywork.json written against
...
- **generated** — an API-tool skill with no contract: base_url from a **catalog match on the skill name**
  ...
  skipped. `olywork upload` also merges `os.environ` into `env_names`, so a credential already exported in
...
- **catalog CLI recipe** — a secret-less skill (SKILL.md only) whose NAME matches a catalog `cli` entry
  ...
  `getpass`, for any still missing). Registered, it runs via `olywork run --local <name>` (not the proxy). A
  hand-written `olywork.json` in the folder still wins over this (the contract path is checked first).
...
`{relpath: content}` so the whole skill travels, minus `.secret*`/`SKILL.md`/`olywork.json`/junk/binaries);
`write_contract` persists the generated `olywork.json` back into the skill
...
hand-written `olywork.json`. (The base_url may still guess wrong — a correct upstream host + any bespoke
placement remain reasons to commit a `olywork.json`, which overrides the generator.) `_flag_header_collisions`
...
- **Skill detection** keys on `SKILL.md` (a README-only dir isn't a phantom skill), skips dot-dirs,
  ...
  A `olywork.json` contract's `health`/`examples` carry into the tool, and its readiness is validated
...
tool name **before** POSTing the secret, so a clash never leaves an orphan secret. `olywork skill install`
