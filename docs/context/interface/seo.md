...
sources:
  ...
  - src/olywork/agent_pages.py
  - src/olywork/web/robots.txt
  - src/olywork/web/catalog.css
  - src/olywork/web/usecase.css
  - src/olywork/web/index.html
  - src/olywork/web/landing.html
  - src/olywork/web/people-search.html
  - src/olywork/web/grokbot.html
  - src/olywork/web/fable-gtm.html
  - src/olywork/web/llms.txt
  - scripts/indexnow_submit.py
  - src/olywork/web/support.html
...
| `/sitemap.xml` | **Generated**, not bundled — 80 of its URLs come from the catalog. Static pages take `lastmod` from their file's mtime, shelves from the newest mtime under `src/olywork/catalog/`. |
...
instead of a separate legend, each `.bot-card` splits a layered Grok-style dark-teal vignette stage from olywork's light copy surface, and the six
...
carries inline olywork and Grok Bot marks in its "by olywork" byline and action.
...
attribution. The failure is silent and total: no `adtrack.js` means no `olywork_ad` cookie, so
...
attribution — `olywork_ad` and `/adtrack.js` are already documented in that policy, so shipping those
...
**`{BASE}`, never a hardcoded `olywork.com`.** Every page is also served by self-hosted registries. A
...
`assets/brand/og-card.html` is the **source**; `src/olywork/web/media/og.png` is the render. Open the
...
draft showed Ahrefs, which olywork does not carry. LinkedIn's mark is inlined because its Google s2
...
at the hub, the agent pages' breadcrumb carries it as the parent (olywork.com / Agents / <name>), and
...
**Two halves, one rule each.** The hand-written half lives in `src/olywork/agent_pages.py` — a module
...
**Hosted only.** The copy describes olywork.com's own listings — the ChatGPT Connectors entry, the $1.00
...
works" cards, an optional screenshot. Then "Why go through olywork.com" (`WHY_OLYWORK`, six cards).
...
per-vendor success rate, median latency and sample size (publication approved by Taqi 2026-08-21)
...
comparison table can answer this"). The `.agents/skills/olywork-page` skill runs that research with
...
(2026-08-25). Finnhub, Tiingo and Twelve Data are served at $0 on olywork.com's own free-tier keys with
...
olywork.com's key, then your own key"). Separately, the `short` form assumed its one provider was an
...
first-party routed endpoints (`olywork.<capability>`, `kind: routed`) were leaking into `_uc_providers`
as a fake "olywork" provider row — and, found while fixing that, into every other public projection of
the catalog: the agent-page menu counted "olywork" as an extra provider on every routed job, the
use-case hub's card metas did the same, `_provider_rows` published a self-referential `/tools/olywork`
...
plus routed), `/tools/olywork` 404s (the all-rows-hidden fallback in `tools_provider` no longer
resurrects routed rows), and the advertised tool count dropped accordingly (2,745 → 2,669). The
public name is olywork.com; a bare "olywork" vendor row must never appear on any page —
`test_routed_rows_never_surface_a_provider_named_olywork` pins it.

**Pages with a free own-account row use a filtered WHY_OLYWORK** (2026-08-31). `WHY_OLYWORK` contains
...
first cut missed the YouTube pages). Those pages render `WHY_OLYWORK_OWN_KEY` instead, keeping only the
...
"olywork.com" (never bare "olywork") as the root. FAQ pairs are extracted from the "Before you sign up"
...
legacy-hub route serves no `.md`, so every one 404ed. The brand anchor reads `olywork.com`. And the
...
of a real run (`id="run"`), `WHY_OLYWORK`, the failure modes, the FAQ, and four related cards.
...
shipped in the package at `src/olywork/workflow_runs/<slug>.csv` and served at
...
- `/tools/<provider>` titles lead with it: `{Provider} API pricing: from $0.00245/result, no signup | olywork.com`
  (the price label carries its own billing unit, so the copy never says "per call" beside it;
  falls back to `{Provider} API pricing: from $X | olywork.com` past 65 characters; own-account
  providers get `{Provider}: connect your own account | olywork.com`). **Title and H1 now match**:
  ...
  canonical `set up olywork — {base}/llms.txt` (the em-dash is the documented exception, and a
