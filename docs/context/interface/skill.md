...
status: shipped
sources:
  - src/olywork/web/skill.md
  - src/olywork/routers/web.py
  - src/olywork/mcp_install.py
  - scripts/build_plugin.py
  - .claude-plugin/plugin.json
  - .claude-plugin/marketplace.json
  - plugin/.codex-plugin/plugin.json
  - plugins/olywork/.cursor-plugin/plugin.json
  - package.json
...
`src/olywork/web/skill.md` is the **product** skill that ships to consumers — the agent's whole interface to the
registry (distinct from `.claude/skills/tools-registry-context/`, which maintains *these* design docs).
Its frontmatter `name: olywork` + `description` make it loadable by a coding agent.
...
- **consumer** — discover + call tools with no credentials locally. Teaches the agent-native
  **URL-passthrough** first: take the real upstream URL and prefix it with `{BASE}/call/`
  + the `X-Olywork-Token` header; `olywork call <tool> <path>` is the CLI shorthand. Its
  "generate a video or an image" task teaches the async shape: `--await` for the CLI, the
  `X-Olywork-Async` descriptor and lazy 30-60 s polling for MCP agents, the shell-timeout warning
  (video takes 1-5 minutes), reserve-then-settle money with refunds on failure, and expiring
  result URLs that olywork never stores.
- **creator** — turn a local skill into a shared tool: `olywork secret add`, `olywork tool add` (single-key or
  `--bind` multi-credential), the `olywork skill scaffold → push` bundle flow, and `olywork oauth connect` for
  browser-consent tokens. Documents the two OAuth modes (auto-refresh vs manual) and the four auth shapes.
- **admin** — inventory + monitor: `olywork tool/secret/skill ls`, `olywork calls`, and `olywork health [--run]`
...
`~/.claude/skills/olywork/SKILL.md` right after installing the CLI — so `curl {BASE}/install.sh | sh`
gives a machine both the `olywork` command AND the skill that teaches an agent to use it. It restates the
...
|---|---|---|
| the installer | `install.sh` → `olywork skill bootstrap` → every detected agent's skills dir | people who ran the curl one-liner |
| Claude Code plugin | `.claude-plugin/` + generated `skills/olywork/SKILL.md` (repo root) | `/plugin marketplace add olywork/olywork` |
| Codex/ChatGPT plugin | `plugin/.codex-plugin/` + generated `plugin/skills/olywork/SKILL.md` | the directory ChatGPT and Codex share |
| Cursor plugin | `.cursor-plugin/marketplace.json` + generated `plugins/olywork/skills/olywork/SKILL.md` | the Cursor marketplace (plugin root is never the repo root) |
| DeepSeek Harness bundle | root `package.json` (`dsh.bundle`) + `dsh/cordis.patch.yml` + generated `dsh/skills/olywork/SKILL.md` | `dsh plugin --profile <name> add github:olywork/olywork` |
| MiniMax plugin | `plugins/minimax/.minimax-plugin/plugin.json` + generated `plugins/minimax/skills/olywork/SKILL.md`; `scripts/minimax_plugin.py` pre-runs their validator and builds the ZIP | the MiniMax Plugin Marketplace (MiniMax Code + MiniMax Agent), submitted by form as GitHub subdir `plugins/minimax`; skills-only because the package may hold no credential and the bootstrap omits `olywork mcp install`, which cannot write a MiniMax config. See [docs/MINIMAX-PLUGIN.md](../../MINIMAX-PLUGIN.md) |
| the domain itself | `GET /.well-known/skills/index.json` + `/.well-known/skills/olywork/SKILL.md` | anything speaking the agentskills.io convention (Hermes reads this directly) |
...
does the opposite, walking the agent through `install.sh` → `olywork login` → `olywork mcp install` so the
first run ends with the CLI *and* the tools. Skills-only is a property of the manifest, not of the
end state; the order in that bootstrap is load-bearing, because `olywork mcp install` exits without
...
profile. That layer carries a olywork MCP row whose `disabled` expression is evaluated at boot, so it
stays off until `OLYWORK_TOKEN` is in the environment — the same "no always-on tools that 401" stance as
the Claude manifest, but expressible as a row rather than an omission. Its bootstrap is its own for
two reasons the others do not have: the tools are namespaced (`mcp__olywork__call`, not `call`), and
`olywork mcp install` cannot help here (it writes Claude Code / Cursor / opencode configs, never a dsh
...
olywork; `integrate.md` is pasted into a builder's own repo and pointed at their coding agent so they can
**embed** olywork and bill their own customers for it.
...
- **Branch on `X-Olywork-Error`, not the status code** — a provider's own 4xx is relayed verbatim.
