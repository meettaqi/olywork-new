...
sources:
  ...
  - src/olywork/web/index.html
  - src/olywork/web/vendor/README.md
  - src/olywork/web/vendor/vue-3.5.41.global.prod.js
  - src/olywork/web/tutorial.js
  - src/olywork/web/tutorial.html
  - src/olywork/web/tour/tour.js
  - src/olywork/web/tour/index.html
  - src/olywork/api.py
...
grants. Registry review metadata and `OLYWORK_OAUTH_REVIEW_PENDING` define the effective experience.
...
A single-file Vue 3 dashboard in `src/olywork/web/index.html`, served **same-origin** by the API
...
`src/olywork/web/vendor/` by an `_ImmutableStatic` mount in `bootstrap.py`, never from unpkg. It used to come
from `unpkg.com/vue@3`, and a visitor whose network could not reach unpkg got a **blank signed-in
dashboard with no error** ([#137](https://github.com/meettaqi/olywork-new/issues/137): mainland-China
...
- **Pin the version in the filename** (`vue-3.5.41.global.prod.js`) and verify new bytes against a
  second CDN before committing them — see `src/olywork/web/vendor/README.md`. A floating `vue@3` tag is
...
`resources.html`, `tutorial.html`) sets the first-touch `olywork_utm` cookie and initialises PostHog with
...
- Either way the dashboard authenticates with the **cookie** (`credentials:'include'`) + picks the org
  with **`X-Olywork-Org`**, detected via `/auth/me` on load. Cookie `Secure` only on HTTPS (`_is_https`).
  ...
  where no `X-Olywork-Org` can travel, it still resolves to that team (fixing the "several teams, none
  ...
  The per-tool snippets embed it (+ `X-Olywork-Org`, now redundant but harmless), and **"Copy API token"**
...
- **Token fallback (agents/CLI):** paste one `X-Olywork-Token` **per org** into `localStorage` (`olywork-dash`).
...
The session cookie is HMAC-signed with `OLYWORK_SESSION_SECRET`/`OLYWORK_SECRET_KEY`, falling back to a
...
`X-Olywork-Org`), and `require_superadmin`
...
- **Tools** — `GET /tools` + `GET /bundles` + `GET /health`, rendered as a **segmented tabular home**
  ...
  **Recipes** = bundles with no tool. Per-tool **Copy** (syntax-highlighted snippet builder — cURL / CLI /
  Claude Code / Python / Node, cURL default; embeds the real token via a `OLYWORK_API_TOKEN` var + shows it
  ...
  **Members · Projects · Policy · Billing · Team settings**. (The former **My teams** tab is gone — it
  ...
  - **Billing** (admin+) — balance, a **Top up** button that opens the top-up modal, the auto-top-up
    ...
    *Receipt*, else an em dash. Amounts come from our own ledger and the links from Stripe, so when
    ...
    bonus stacks via `refPresetBonus`), a summary box (credit + bonus, total due = the amount — olywork
...
- **Agents live INSIDE Team → Members now** — the separate Agents page and its sidebar entry are
  ...
  **＋ Add agent** (mint form: name / role / cap + project picker `agentProjSel`, all-checked =
  ...
  button (`showAgentSetup`) that reopens those snippets for an existing agent using `$OLYWORK_TOKEN` as a
...
- **Getting started** (`view==='start'`, the FIRST nav item, above Catalog; the sidebar's team half is
  labeled **"Your vault"** — Taqi's explicit naming call, overriding the no-"vault" vocabulary rule) —
  ...
  **① Set up your \<agent\>** — an agent dropdown (`startAgentOpen`, shares `welcome.agent` +
  ...
  remembered in `localStorage` `olywork-agent`) above an **"install the olywork plugin"** link for agents whose
  entry carries a `plugin` URL (Grok Bot) and the per-agent setup line (`welcomeSetupCmd` = `buildAgentPrompt` — `set up olywork — <proxy>/llms.txt with token <T>, team <slug>`; the long multi-step agent prompt is retired, llms.txt itself now carries the setup flow, the do-not-stop authorization framing and the star ask, and its money rules no longer demand per-call price confirmation) and, combined into the
...
- **Activity** — one time-sorted feed (`activityRows`) merging `GET /calls` (proxy calls) + `GET /runs`
  ...
  routes. olywork never proxies or stores the media itself.
...
- **First-run onboarding** — a brand-new user has **zero teams** (no auto personal org), so `maybeOnboard`
  ...
  agent (Grok Bot) an "Install the olywork plugin" link precedes it as step 1 and the line becomes step 2 —
...
- **Tutorial (`view==='help'`; no nav entry since 2026-08-12)** — the full interactive walkthrough, rendered natively (Vue) from the shared
  `window.OLYWORK_TUTORIAL` data (`tutGo`/`tutHL`/`tutCopy`, syntax-highlighted command + output blocks,
...
mount `_LOGO_DIR` (`src/olywork/web/logos/`). `connCount` labels how many accounts are already connected.
...
Facebook's consent screen shows a name the user never saw on olywork. The template is `v-if`'d on the field,
...
- **Footer** — the endpoint count on the left, and on the right the starting price from the row's
  ...
  *olywork's app* per use, so a connected X account changes who made the call and not who is billed. The
...
`table-layout:fixed`, so a nowrap path or `olywork call` line scrolls **inside** its cell instead of widening
...
where the upstream bills olywork's app per call and connecting changes nothing about the price. Reading the
flag off the server rather than naming X here means the display follows `OLYWORK_OAUTH_BILLED_PROVIDERS`:
...
paste-ready **`olywork call`** line the row carries as `call_template` with a Copy button, the docs link and
...
(the parameters, the provider facts, and the `olywork call` line) and **Example response** (the captured
...
For a **key/token** provider, **▶ Try it** (`openEpTry`) is the ink-fill **primary** — trying on olywork's
...
banner, which now carries a real **Connect** / **Bring your own key** button instead of prose alone. For an **OAuth** provider olywork *can't* serve on
...
the setup line everywhere else stays clean) plus a ready "Use olywork to call `<id>` — `<summary>`" prompt
(`epTryAgentUse`); **CLI** — install/login, `olywork catalog get <id>`, and the filled `olywork call <id>
...
(`epTryCurl`, adding `X-Olywork-Org` in session mode since the minted token is an identity token), plus
...
Connected state, the `olywork call` line and the provider facts, the cross-currency cheapest rules, the credit-priced "see provider" fallback, the scope
...
standalone `/tutorial`, the connect/setup instruction panes, the ledger's `olywork call` line and captured
...
`localStorage['olywork-next']` across an OAuth sign-in hop (the callback always lands on `/app`, which would
otherwise drop the path).

## The tutorial (one source, two renderers)
`src/olywork/web/tutorial.js` is the **single source of truth**: `window.OLYWORK_TUTORIAL` (`concepts`, `roles`,
`personas`, `steps[]` = `{part,who,title,explain,cmd,out,notice}`, plus the two focused arrays
`importShell[]` and `access[]`, same step shape) + a self-contained `olyworkHL(text,lang)` shell/json
...
render) and the **standalone** `src/olywork/web/tutorial.html` (vanilla render, served at `/tutorial`;
...
stepper (`tourGo` via `tourI`, `personaTour`, per-Part `tourMatColor` mats) from **`window.OLYWORK_TOUR`**
(`src/olywork/web/tour/tour.js`, one source shared with the standalone page). WebP images live at
`src/olywork/web/tour/img/` and are served via a `StaticFiles(html=True)` mount at **`/dashboard-tour/`**
...
shipped. Packaging: `src/olywork/web` lives inside the `olywork` package, so the wheel's `packages`
...
the balance chip (where someone is already thinking about what olywork costs them).
...
whether their reward has landed is the one who makes it land. There is no scheduler in olywork, so this
