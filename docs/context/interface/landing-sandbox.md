...
status: shipped
sources:
  - src/olywork/sandbox.py
  - src/olywork/sandbox_identity.py
  - src/olywork/application/onboard/pubfeed.py
  - src/olywork/application/onboard/sandbox.py
  - src/olywork/application/onboard/__init__.py
  - src/olywork/domain/governance/sandbox.py
  - src/olywork/api.py
  - src/olywork/routers/onboard.py
  - src/olywork/routers/web.py
  - src/olywork/web/index.html
  - src/olywork/web/install.sh
related:
...
> tour, stores `localStorage['olywork-sbx']`, or calls `POST /demo/sandbox`. A use-case CTA arriving at
...
`mint(db)` creates a login-free team: a `visitor-<hex>@sandbox.olywork.local` `User` (can never sign in),
...
`is_sandbox_user(user)` is the companion check on the **visitor** (email ends in `@sandbox.olywork.local`,
...
compute exactly what olywork would send upstream (the injected header/query), then returns a **labelled
...
There is a single deliberate exception, gated on env `OLYWORK_DEMO_STRIPE_KEY` (`settings.demo_stripe_key`,
...
it returns **404 when `OLYWORK_DEMO_STRIPE_WEBHOOK_SECRET` (`settings.demo_stripe_webhook_secret`) is unset**, so
...
skill manifest (olywork.json + SKILL.md, secret values redacted to placeholders).
...
files a skill folder is — `SKILL.md` (agent recipe: call the olywork proxy, key injected server-side),
`olywork.json` (wiring: base_url + which secret by name), `.secret` (empty; value stays in the vault).
...
- `GET /skills/{name}/install.sh?token=` (`skill_install`) — `install_script(name, base, token)` emits a
  ...
  calls `{BASE}/call/…` with the caller's olywork token — the API key never lands on the machine. The
...
## CLI installer
`GET /install.sh` (`install_sh`) serves `src/olywork/web/install.sh`, `{BASE}`-templated like `llms.txt`
...
`pip3 install --user --upgrade`, then runs `olywork config --base-url {BASE}`. The uv/pipx paths pin the
...
**tools-registry skill** into every detected agent via `olywork skill bootstrap` (Claude Code, Cursor, Codex,
Gemini, Copilot, OpenCode, Windsurf …), falling back on older CLIs to a Claude-only drop that curls
`{BASE}/skill.md` into `~/.claude/skills/olywork`. Because the package is public on PyPI,
`curl … /install.sh | sh` now works for anyone with no repo/git access needed. The **Getting started**
dashboard view (`view==='start'`) surfaces this install command + `olywork login`/`onboard`/`add`/`call` and
