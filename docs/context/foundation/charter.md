...
status: foundational
sources:
  - external:meetings/2026-06-30-taqi-tools-registry.md
  - README.md
related:
...
olywork carries ~2,600 catalogued endpoints across ~40 providers (SEO, backlinks, social, enrichment,
...
| Which wins | your own credential always beats olywork's, automatically |
...
| Not built | automatic routing or failover **between providers** of one capability. `olywork catalog search` compares them; **choosing is the caller's**. What olywork *may* do (when a deployment turns it on, [capacity](../ops/capacity.md)): if its **own** account for a provider is out, serve the **same endpoint** through a relay — same request, same response shape, the relay's real price, disclosed via `X-Olywork-Served-Via`; a team can opt out. **First-party routed endpoints** (`olywork.<capability>`, generated from verified adapters) are the explicit opt-in where the caller asks olywork to choose among providers; each always names the child that served and returns its raw body ([catalog](../architecture/catalog.md) § Routing) |

Money and metering: [money](../architecture/money.md). The endpoint data and its pricing rules:
[catalog](../architecture/catalog.md).

> **Status:** shipped and live on `olywork.com` (Render, multi-tenant; `olywork.olywork.com` is the legacy alias). This fragment records
...
> not how olywork is presented to anyone.
...
## What a "tool" is
A **skill + its auth**. Auth shapes to support (from the real `olywork-agi/.claude/skills/`): plain
...
- **Standalone for now**, separate from Olywork. May **later merge with Loopni** (loops + context +
...
Auto-convert **every skill in `olywork-agi/.claude/skills/`** into a tool and test each (PostHog, GSC,
