---
title: Provider capacity — knowing what olywork's own vendor accounts have left
status: shipped (steps B–F built; production rollout = the shadow week, then OLYWORK_OVERFLOW_MODE=on)
sources:
  - src/olywork/domain/capacity/__init__.py
  - src/olywork/domain/capacity/collectors.py
  - src/olywork/domain/capacity/policy.py
  - src/olywork/domain/capacity/sweep.py
  - src/olywork/domain/capacity/view.py
  - src/olywork/domain/capacity/routes.py
  - src/olywork/domain/capacity/signatures.py
  - src/olywork/domain/capacity/verify.py
  - src/olywork/domain/capacity/marks.py
  - tests/test_capacity_protect.py
  - src/olywork/infra/upstream/limiter.py
  - src/olywork/domain/capacity/overflow_spend.py
  - src/olywork/domain/capacity/routes_view.py
  ...
  - src/olywork/domain/capacity/overflow_seed.json
  - src/olywork/infra/upstream/aggregators/__init__.py
  - src/olywork/infra/upstream/aggregators/orthogonal.py
  - src/olywork/infra/upstream/aggregators/monid.py
  - src/olywork/infra/upstream/aggregators/catalogs.py
...
**Problem.** Tier 4 serves ~2,850 catalog endpoints on olywork's own vendor keys. When one of *our*
...
Scope: olywork-owned platform credentials only. Tiers 1/2 (a caller's own tool or key) are never
consulted or affected by anything here.

## Pieces (`src/olywork/domain/capacity/`)
...
## Where it runs — `olywork-worker`

`olywork-worker capacity sweep [--only a,b] [--json]` (`src/olywork/worker.py`, console script in the
`[server]` extra). It is deliberately **not** a `olywork` subcommand: the light CLI may not import the
...
is a **Render cron job** (`olywork-capacity-sweep` in `render.yaml`, hourly) that pulls its env from
...
`olywork.domain.capacity` may not import `olywork.api`, `olywork.routers`, `olywork.application`,
`olywork.bootstrap`, `olywork.audit`, FastAPI or Starlette (contract "Capacity domain does not depend on
...
**Overflow** = the *same* vendor endpoint served through a olywork-owned **aggregator** account
...
- **The seed** — `overflow_seed.json`: the 461 candidate pairs from the 2026-08-26 mapping run,
  ...
  price). Under the rules, `olywork-worker overflow sync` enables **113** of the 145 (pinned by test);
  ...
  `olywork-worker overflow verify`.
...
- **`verify.py`** + `olywork-worker overflow verify` — the weekly re-verify: one cheap call per
...
alternatives named, one probe a minute excepted; two balance/quota signatures in a row on olywork's
...
it: `OLYWORK_OVERFLOW_MODE` = `off` (default) | `shadow` | `on`; `OLYWORK_OVERFLOW_DAILY_BUDGET_USD` (20)
...
aggregator charged but olywork cannot charge to the caller. Known no-charge failures return the whole
...
The aggregator keys `OLYWORK_OVERFLOW_KEY_*` live in the web service env (the cron pulls them). The
...
(`credential_tier=platform-overflow`, `error_response="olywork overflow: shadow"`) — then switch to
...
`GET/PATCH /orgs/{id}/settings` carries `platform_overflow` (default true), `olywork org overflow
...
plugin), `README.md` and `USAGE.md` now say what olywork may do and how it discloses it.

**Rollout runbook (Taqi):** 1. set `OLYWORK_OVERFLOW_KEY_ORTHOGONAL` / `_MONID` (rotated keys) in the
Render web service; 2. `olywork-worker overflow sync` (113 routes enable from the seed; `--live` also
refreshes prices) and schedule `olywork-worker overflow verify` weekly (routes decay off after 7 days
without it); 3. `OLYWORK_OVERFLOW_MODE=shadow` for a week — watch the `overflow SHADOW` log lines and
the `platform-overflow` audit rows for shape mismatches and the daily `OverflowSpend`; 4.
`OLYWORK_OVERFLOW_MODE=on`. Keep the Orthogonal balance ≤ \$20 (ToS risk accepted 2026-08-26).
...
`OLYWORK_OVERFLOW_MODE` is `off` and olywork still relays a vendor's 402 unchanged (or answers the typed 503
