...
sources:
  - pyproject.toml
  - src/olywork/__main__.py
  - src/olywork/maintenance.py
  - src/olywork/alembic/env.py
  - src/olywork/worker.py
  - src/olywork/web/selfhost.sh
  - src/olywork/config.py
  - src/olywork/infra/db.py
  - src/olywork/email.py
  - src/olywork/audit.py
...
`python -m olywork upgrade` runs the explicit release phase. `maintenance._upgrade_schema()` runs
...
`tools-registry[server]==0.14.*`, run `python -m olywork upgrade` there once, then continue onward. A
...
The default `python -m olywork` serve path runs that same upgrade phase and then
`api._bootstrap_single_user()`, then disposes the async engine inside the pre-serve event loop before calling
`uvicorn.run("olywork.api:app", host="0.0.0.0", port=int($PORT or 18790))` (`--reload` optional). It honors
`$PORT` (Render/Heroku route + health-check that port). `python -m olywork keygen` prints a Fernet key for
`OLYWORK_SECRET_KEY` without importing the server maintenance stack. `olywork.api:app` is
`bootstrap.create_app(role="all")`.

The FastAPI lifespan calls read-only `verify_db()`. It refuses an unstamped database or a known
revision behind head, directing raw-ASGI operators to `python -m olywork upgrade`. An unknown-newer
...
`startCommand: python -m olywork` repeats the fast idempotent release phase for self-hosted parity.

Both pre-Uvicorn entry paths dispose the engine before their `asyncio.run()` loop closes: the default
serve path does so after single-user bootstrap, and `python -m olywork upgrade` does so after all release
...
- **Alembic is authoritative:** migration scripts ship inside `src/olywork/alembic/` in the wheel.
...
- **Fails loud on a missing key + real DB:** if `OLYWORK_SECRET_KEY` is empty and `database_url` isn't
...
- **`OLYWORK_DB_POOL_OVERRIDES`** (`"admin.pool_size=4,background.pool_size=16"`) patches `POOL_SPECS`
...
- **The pools are measured, not argued about: `db_pool_gauge`.** `bootstrap.pool_gauge` samples
  ...
  at capacity is one whose waiters are timing out (`api`: `503 olywork_saturated`; `background`: an
...
- **Other Postgres pool hygiene:** `pool_pre_ping=True`, `pool_recycle=300`, and `pool_timeout=5` on
  every pool. A request that gets no slot in 5 s is answered `503 {"olywork_saturated": true}` with
...
## Config (`config.py`)
`Settings` (pydantic-settings, env prefix `OLYWORK_`, reads `.env`), cached via `get_settings()`:
- `database_url` — default `sqlite+aiosqlite:///./olywork.db` (SQLite dev, Postgres on Render, same code).
...
- `public_url` — default `https://olywork.com`; the reference deployment is cut over in STAGES —
  ...
  `OLYWORK_PUBLIC_URL`. Used to build the OAuth callback URI.
...
- `topup_min_usd`, `topup_default_usd`, `topup_presets`, `topup_bonus_tiers`, `topup_default_cap_usd`
  ...
  to the dashboard.
- `admin_token` — the cross-tenant **super-admin** bearer (`OLYWORK_ADMIN_TOKEN`); empty disables the env
  path (only `is_superadmin` users reach `/admin`). Keep it long + secret. See
  [super-admin](../architecture/super-admin.md).
- **Registry OAuth-marketplace apps** — olywork's OWN approved OAuth clients, so a member can connect a
  ...
  `google_ads_developer_token` (olywork's token from OUR approved manager account, injected on every Ads
...
- **Ad conversion tracking** — `google_ads_customer_id` (the target Ads account) and
  `ads_conv_refresh_token` (olywork's OWN long-lived refresh token for the Data Manager uploader,
  ...
  customer's consent screen for a permission only olywork's own marketing uploader uses). Self-hosters
  and the test suite carry zero ad-conversion machinery by default. See
  [ads-conversions](../architecture/ads-conversions.md).
- **Landing live-wire (optional):** `demo_stripe_key` (`OLYWORK_DEMO_STRIPE_KEY`, a Stripe **sandbox
  ...
  (`OLYWORK_DEMO_STRIPE_WEBHOOK_SECRET`, `whsec_…`) signs the landing payments feed; empty ⇒ `POST
...
- **Frictionless local mode** (`single_user`, `single_user_token_file`): `curl {BASE}/selfhost.sh | sh`
  ...
  `you@local.olywork` owner + `personal` team and writes the token (0600) for the installer to hand to the
  ...
  it demands a **local sqlite** DB **and** a **loopback `public_url`**, so a stray `OLYWORK_SINGLE_USER=true`
...
- `github_client_id` / `github_client_secret` / `session_secret` — GitHub OAuth login for the dashboard
  (`OLYWORK_GITHUB_*`, `OLYWORK_SESSION_SECRET`); empty hides the GitHub button. Callback must be
...
- `email_dev_mode` — default **False** (returning the OTP in the response is an unauth account-takeover
  ...
  **only** on a trusted dev box (`OLYWORK_EMAIL_DEV_MODE=true`); real deploys must not. **Double guard:** the
  code is exposed only through `Settings.expose_dev_code`, which requires `email_dev_mode` **and** a
  **local sqlite** `database_url` — so even a stray `OLYWORK_EMAIL_DEV_MODE=true` on Postgres (a real deploy)
  can never leak a login code.
- `run_proof` (`OLYWORK_RUN_PROOF`) — the **isolated-runner proof** for `olywork run --local`. A local run whose
  grant would return a secret the caller does **not** own (a shared-key tool a member may run but not read)
  must present this value in the `X-Olywork-Run-Proof` header — a value held **only** by the root-installed
  `olywork-run` runner, never by the member. Empty = shared-key local runs are refused (runs against a
  secret the caller owns still work). To enable shared local runs, set it on the server **and** install it
  via `olywork setup-local-run --run-proof`. See [local-run](../architecture/local-run.md).
- `run_allowed_bins` (`OLYWORK_RUN_ALLOWED_BINS`) — the **command allow-list** for `olywork run --server`. The
...
- `run_rlimits` (`OLYWORK_RUN_RLIMITS`, default **true**) + `run_cpu_seconds` (`OLYWORK_RUN_CPU_SECONDS`,
  default 300) + `run_fsize_mb` (`OLYWORK_RUN_FSIZE_MB`, default 100) — the **resource-limit sandbox** for
  `olywork run --server` (`runner._rlimit_preexec`): every run's child gets a CPU-seconds cap, a max-file-size
...
- `proxy_ssrf_check` (`OLYWORK_PROXY_SSRF_CHECK`) — the **call-time SSRF guard** on the proxy: resolve the
...
- `claude_connector_enabled` (`OLYWORK_CLAUDE_CONNECTOR_ENABLED`) — enables the catalog-only Claude
  ...
  `/mcp/` connector.
- `connect_demo_enabled` (`OLYWORK_CONNECT_DEMO_ENABLED`) — enables the developer OAuth test page and
...
- `intercom_app_id` / `intercom_secret` (`OLYWORK_INTERCOM_APP_ID` / `OLYWORK_INTERCOM_SECRET`) — support
  chat via the **Intercom Messenger** (olywork's own workspace). Empty app_id = the widget is OFF
  everywhere — `/meta`
...
- `resend_api_key` / `email_from` — transactional email via **Resend** (`src/olywork/email.py`): the OTP
  ...
  Resend-verified domain — `olywork.com` is **verified** (DKIM + SPF records on the olywork.com zone;
  `olywork.olywork.com` stays verified as a fallback), so the default is `no-reply@olywork.com`. **On Render:** set
  `OLYWORK_RESEND_API_KEY`, optionally `OLYWORK_EMAIL_FROM`, and leave `OLYWORK_EMAIL_DEV_MODE` false.

## Web dashboard
`GET /` serves the single-file dashboard (`src/olywork/web/index.html`) same-origin; the whole `web/` dir
(incl. `tutorial.js` at `/tutorial.js` and `tutorial.html` at `/tutorial`) ships in the wheel because it
lives inside the `olywork` package (the `packages` inclusion covers non-.py assets). See
[dashboard](../interface/dashboard.md).

## Current hosting (shipped)
Deployed on **Render** at `https://olywork.com` (with `olywork.olywork.com` attached as the legacy
...
`preDeployCommand: python -m olywork upgrade`, `startCommand: python -m olywork`, and health check on `/meta`.
...
`OLYWORK_DEMO_STRIPE_KEY` + `OLYWORK_DEMO_STRIPE_WEBHOOK_SECRET`); `OLYWORK_PUBLIC_URL`,
`OLYWORK_EMAIL_DEV_MODE=false`, and `OLYWORK_EMAIL_FROM` are set inline. `asyncpg` is a dependency (Postgres
...
`OLYWORK_PLATFORM_DAILY_CAP_USD` (default **$100**) is the per-org, per-UTC-day ceiling on tier-4 spend
...
`OLYWORK_OAUTH_BILLED_PROVIDERS` — comma-separated providers whose upstream bill lands on OLYWORK's
...
`OLYWORK_ROUTED_DISCOVERY` (`on` by default, `off` to disable) decides whether DISCOVERY steers to
`olywork.<capability>` rows. Off, search and the platform browse view stop showing them and
...
flip, no redeploy, like `OLYWORK_PLATFORM_PROVIDERS` and `OLYWORK_OVERFLOW_MODE`.
...
Five more `OLYWORK_PLATFORM_KEY_*` env vars beside the originals, and the providers must ALSO be in
`OLYWORK_PLATFORM_PROVIDERS` (both halves, or tier 4 refuses):

- `OLYWORK_PLATFORM_KEY_COINGECKO` — PRO key; billed $0.00029/credit
- `OLYWORK_PLATFORM_KEY_MARKETSTACK` — billed $0.000999/call against a 10,000/mo vendor cap
- `OLYWORK_PLATFORM_KEY_FINNHUB`, `_TWELVEDATA`, `_TIINGO` — FREE-tier keys serving $0 trial pools,
  capped per team per day from fx.yaml (`olywork_trial`). These are free accounts: if one is
...
Seven more slots (`OLYWORK_PLATFORM_KEY_COMPANYENRICH`, `_OCEANIO`, `_PREDICTLEADS` — base64 of
...
account, setting the env var, and adding the service to `OLYWORK_PLATFORM_PROVIDERS`. Ocean.io stays
...
`OLYWORK_PLATFORM_KEY_INFLUENCERSCLUB` — the dashboard API key (a JWT), sent as `Authorization: Bearer`.
...
`OLYWORK_PLATFORM_PROVIDERS`. Mind the 60s gateway 504 on cold enrichment calls: under `per_success`
...
`OLYWORK_PLATFORM_KEY_CRUSTDATA` and `OLYWORK_PLATFORM_KEY_AVIATO` are funded pay-as-you-go keys. Add both
services to `OLYWORK_PLATFORM_PROVIDERS` to serve them on tier 4. Crustdata's platform binding also
...
`OLYWORK_PLATFORM_KEY_EXA` is Taqi's own Exa API key (dollar-metered, $20 signup credit + $10/month
free tier; top up on the Exa dashboard). Add `exa` to `OLYWORK_PLATFORM_PROVIDERS` to serve its nine
...
`olywork-worker` (console script, `[server]` extra) hosts the scheduled maintainer commands -
`capacity sweep` and `overflow verify --all` (see `ops/capacity.md`). `render.yaml` describes only the
first as a cron service (`olywork-capacity-sweep`, hourly), with the DB URL, Fernet key and every
`OLYWORK_PLATFORM_KEY_*` pulled from the web service via `fromService`.
...
> live vs 24 in the file; `OLYWORK_OVERFLOW_MODE` is `on` live and `off` in the file). Treat the file
...
> The overflow re-verify cron (`olywork-overflow-verify`, Mondays 06:00 UTC, dashboard-made, command
> `olywork-worker overflow verify --all` since 2026-09-02) is deliberately absent from the file for
> that reason.
...
"olywork-worker overflow verify --all"`, then the same with `overflow sync` - and read the
...
Aggregator keys
(`OLYWORK_OVERFLOW_KEY_ORTHOGONAL` / `_MONID`) are dashboard-managed on the web service and flow the same
way. `OLYWORK_OVERFLOW_MODE` (`off` default | `shadow` | `on`) and `OLYWORK_OVERFLOW_DAILY_BUDGET_USD` (20)
govern the overflow child cycle (`ops/capacity.md`); the keys serve nothing while the mode is `off`.

`olywork-worker asynctasks settle` is the second cron command. `olywork-asynctasks-settle` runs every two
...
## A `src/olywork/infra/db.py` change needs a Postgres-shaped deploy plan
