...
- Branch is `feat/ads-conversion-tracking`. **Commit with `git commit -- <paths>` (path-scoped)** — there is unrelated staged work in `src/olywork/api.py`, `docs/context/interface/api.md` and `src/olywork/web/landing.html` that must never enter a commit. A plain `git commit` takes the whole index and would sweep that work in.
...
| File | Responsibility |
|---|---|
| `src/olywork/adsconv.py` | **new** — FX helper, action constants, `queue()` outbox writer, uploader worker. All Ads-conversion logic lives here. |
| `src/olywork/models.py` | **modify** — four new `Org` columns, new `AdConversion` table. |
| `src/olywork/db.py` | **modify** — guarded `ALTER`s for the new `Org` columns. |
| `src/olywork/config.py` | **modify** — three settings that gate the feature off by default. |
| `src/olywork/api.py` | **modify** — gclid read at signup, two fire chokepoints, worker start in `lifespan`. |
| `src/olywork/billing.py` | **modify** — first top-up chokepoint inside `_credit()`. |
| `src/olywork/web/adtrack.js` | **new** — the ~10-line first-party capture snippet. |
...
Expected: FAIL with `ModuleNotFoundError: No module named 'olywork.adsconv'`

- [ ] **Step 3: Write minimal implementation**

```python
# src/olywork/adsconv.py
...
```bash
git commit -m "feat(ads): fixed-rate USD->AUD conversion and conversion action ids" -- src/olywork/adsconv.py tests/test_adsconv.py
```
...
**Files:**
- Modify: `src/olywork/models.py` (add to `class Org`, add new table after `class Secret`)
- Modify: `src/olywork/db.py` (guarded ALTERs alongside the existing `(A33)`/`(A34)` block)
- Test: `tests/test_adsconv.py`
...
from olywork.db import session_maker
from olywork.models import AdConversion, Org
...
> Fixture note: `clients` (`tests/conftest.py:144`) yields **one** `AsyncClient`, already authed as a registered user — it is not a tuple. Direct DB access goes through `session_maker` imported from `olywork.db`, which is the pattern `tests/test_billing.py` uses. Taking `clients` as a parameter is still required even when a test only touches the DB: the fixture is what calls `reset_db()`.

- [ ] **Step 3: Add the columns and the table**

In `src/olywork/models.py`, inside `class Org`, after the `balance_micro` field:
...
In `src/olywork/db.py`, immediately after the `(A34)` block:
...
```bash
git commit -m "feat(ads): Org attribution columns and the AdConversion outbox table" -- src/olywork/models.py src/olywork/db.py tests/test_adsconv.py
```
...
**Files:**
- Modify: `src/olywork/adsconv.py`
- Modify: `src/olywork/config.py`
- Test: `tests/test_adsconv.py`
...
Run: `uv run --frozen python -m pytest tests/test_adsconv.py -q`
Expected: FAIL with `AttributeError: module 'olywork.adsconv' has no attribute 'queue'`

- [ ] **Step 3: Add the settings**

In `src/olywork/config.py`, next to the existing `google_ads_*` settings (around line 233):
...
```python
    ...
    # Which team's google-ads OAuth connection the uploader authenticates as. olywork uploads to its
...
Append to `src/olywork/adsconv.py`:
...
```bash
git commit -m "feat(ads): durable outbox writer, gated off by default" -- src/olywork/adsconv.py src/olywork/config.py tests/test_adsconv.py
```
...
**Files:**
- Create: `src/olywork/web/adtrack.js`
- Modify: `src/olywork/api.py` (serve the file; read the cookie in `register_user()` at ~3814 and `create_org()` at ~3877)
- Test: `tests/test_adsconv.py`
...
async def test_signup_persists_the_gclid_cookie(clients):
    r = await clients.post(
        ...
        cookies={"olywork_ad": "CLICK_XYZ|p3"},
    )
...
(function () {
  try {
    ...
    document.cookie = 'olywork_ad=' + v + ';path=/;max-age=7776000;samesite=lax' +
...
In `src/olywork/api.py`, add a route next to the other static file routes (e.g. near the `/usecase.css` route at ~2323):
...
def _ad_attribution_from(request: Request) -> tuple[str, str]:
    ...
    raw = request.cookies.get("olywork_ad") or ""
    if not raw:
        return "", ""
...
In `src/olywork/web/index.html`, immediately before `</body>`:
...
```bash
git commit -m "feat(ads): first-party gclid capture, persisted to the org at signup" -- src/olywork/web/adtrack.js src/olywork/web/index.html src/olywork/api.py tests/test_adsconv.py
```
...
**Files:**
- Modify: `src/olywork/api.py` (`_grant_signup_promo()` at ~3748)
- Test: `tests/test_adsconv.py`
...
async def test_signup_queues_a_conversion_when_attributed(clients):
    r = await clients.post("/users", json={"email": "conv@example.com"},
                              cookies={"olywork_ad": "CLICK_SIGNUP|p1"})
    assert r.status_code == 200, r.text
    async with session_maker() as db:
...
In `_grant_signup_promo()` in `src/olywork/api.py`, inside the existing `try:` block after `await ledger.grant(db, org.id)`:
...
```bash
git commit -m "feat(ads): queue a signup conversion from the promo-grant chokepoint" -- src/olywork/api.py tests/test_adsconv.py
```
...
**Files:**
- Modify: `src/olywork/api.py` (the `/call/{rest:path}` handler at ~9098)
- Test: `tests/test_adsconv.py`
...
from olywork.db import reset_db, session_maker


def _h(token: str) -> dict:
    return {"X-Olywork-Token": token}
...
```bash
git commit -m "feat(ads): set first_call_at and queue its conversion on the call path" -- src/olywork/api.py tests/test_adsconv.py
```
...
**Files:**
- Modify: `src/olywork/billing.py` (`_credit()`)
...
In `_credit()` in `src/olywork/billing.py`, inside the existing `if fresh:` block:
...
```python
        ...
        # a payer, not revenue — value-based bidding needs volume olywork does not have yet.
        if org is not None:
...
```bash
git commit -m "feat(ads): queue the first-top-up conversion from the credit branch" -- src/olywork/billing.py tests/test_billing.py
```
...
**Files:**
- Modify: `src/olywork/adsconv.py`
- Modify: `src/olywork/api.py` (`lifespan` at ~140)
- Test: `tests/test_adsconv.py`
...
Run: `uv run --frozen python -m pytest tests/test_adsconv.py -q`
Expected: FAIL with `AttributeError: module 'olywork.adsconv' has no attribute 'build_payload'`
...
async def worker(session_factory, client) -> None:
    ...
    while True:
        try:
            ...
            logging.getLogger("olywork").warning("ads conversion drain failed: %s", exc)
...
`login-customer-id`. Follow the pattern at `src/olywork/health.py:205`, which already refreshes a
...
In `src/olywork/api.py`, inside `lifespan` after `app.state.http` is created:
...
```bash
git commit -m "feat(ads): background uploader draining the conversion outbox to Ads v22" -- src/olywork/adsconv.py src/olywork/api.py tests/test_adsconv.py
```
...
- Modify: `src/olywork/web/privacy.html`

- [ ] **Step 1: Run the drift check**

Run: `bash .agents/skills/tools-registry-context/scripts/drift.sh`
Read its output and map every changed source to the fragment that names it. `src/olywork/api.py`, `billing.py`, `models.py`, `db.py` and `config.py` all changed and are all named in existing fragments — those fragments need a line each about the new columns, the outbox and the worker.
...
`src/olywork/web/privacy.html` needs a sentence covering advertising measurement: a first-party cookie
...
```bash
git commit -m "docs(ads): conversion-tracking fragment, v22 pin fix, privacy line" -- docs/context/architecture/ads-conversions.md .agents/skills/google-ads/SKILL.md src/olywork/web/privacy.html
```
