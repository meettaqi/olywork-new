...
olywork's Codex/ChatGPT plugin is built around the **MCP connector**, and that listing sits in a review
queue. The skill does not need MCP: `src/olywork/web/skill.md` already teaches an agent to install the
...
The skill loads as `olywork:olywork`, and the install itself needs **no token and no configuration** —
...
```bash
curl -fsSL https://olywork.com/install.sh | sh   # 1. the CLI (skipped if olywork --version works)
olywork login                                   # 2. sign in — the token
olywork mcp install                             # 3. the tools, header-authed with that token
```

So the intended end state is CLI **and** skill **and** MCP tools — reached by the skill, not by the
manifest.

**The order is load-bearing.** `cmd_mcp_install` (`src/olywork/cli.py`) reads the token from config and
...
- **`install.sh` always runs `olywork skill bootstrap`** — there is no flag to suppress it — so step 1
  drops a *second* copy of this same skill into `~/.claude/skills/olywork/`. Harmless, but redundant
...
skills/olywork/SKILL.md              GENERATED — never edit by hand
...
`skills/olywork/` is at the top level rather than under `plugin/`. That one path is also what
...
**Never edit `skills/olywork/SKILL.md`.** Change `src/olywork/web/skill.md` and regenerate:
...
**Published 2026-08-14: [clawhub.ai/olyworkdev](https://clawhub.ai/olyworkdev) · `olywork@0.11.0`
· owner `@olyworkdev` · MIT-0.** Verified by installing it back out of the registry into a scratch
dir — the delivered `SKILL.md` is byte-identical to `skills/olywork/SKILL.md` in this repo.

The publisher org was created with
`clawhub publisher create olyworkdev --display-name "Olywork dev, Inc."`. Publishing under a
...
clawhub skill publish "$PWD/skills/olywork" \
  --slug olywork --name "olywork" --version 0.11.0 \
  --changelog "..." \
  --topics "api,seo,serp,backlinks,enrichment,scraping,agent-tools,web-search" \
  --source-repo olywork/olywork \
  --source-commit "$(git rev-parse HEAD)" \
  --source-ref main --source-path skills/olywork \
...
clawhub scan --slug olywork --version 0.11.0 --update    # took ~37 minutes; let it run
```
...
curl -s -H "Authorization: Bearer $(clawhub token)" \
  https://clawhub.ai/api/v1/skills/olywork/moderation
```
...
| public `GET /api/v1/skills/olywork` | 403, hidden | returns the skill |
...
- **Pass an ABSOLUTE path.** A relative `./skills/olywork` fails with `Path must be a folder`, because
  paths resolve against `--workdir` + `--dir` (which defaults to `skills`), so it looks for
  `skills/skills/olywork`. `"$PWD/skills/olywork"` is the reliable form.
...
- Publishing is not irreversible: `clawhub delete` soft-deletes a skill or withdraws one version,
  ...
  skill out of a `olyworkdev` org later.

Requirements the generated skill already satisfies: frontmatter `name` + `description` + semver
`version`, with `name` matching the parent directory (`olywork`). A security scan runs post-publish;
...
- **Moderation** (`/api/v1/skills/olywork/moderation`) is what hides or shows a skill. Ours is
...
- **`clawscan`** (`clawhub scan`) is an LLM review published as an **advisory on the listing**. It
  rated olywork `suspicious` — and that changed nothing about visibility.
...
`EA2` and `SQP-2` are the trade-offs olywork made knowingly. **`PE3` is a real description of a real
...
> `https://github.com/meettaqi/olywork-new/tree/main/skills/olywork`
...
**skills.sh / `npx skills` — pass `-s olywork`, always:**

```bash
npx skills add olywork/olywork -s olywork     # ✅ just the olywork skill
npx skills add olywork/olywork             # ❌ installs NINE skills
```

Verified on a scratch machine: the bare form scans the whole repo, finds `.agents/skills/*` as well
as `skills/olywork/`, and installs this repo's **internal development skills** (`dev-local`,
`tools-registry-context`, `write-provider-skill`, `add-oauth-provider`, `vendor-listing`, the three
Google-provider skills) into the user's agent alongside olywork. That is our layout leaking into a
product install. `skills.sh.json` does not prevent it — it only supplies category labels.

`-s olywork` scopes it to one skill and is what every install line we publish must carry. With it, that
...
## The fourth door: olywork.com itself

`GET /.well-known/skills/index.json` + `/.well-known/skills/olywork/SKILL.md` (in `src/olywork/api.py`)
...
itself rather than olywork.com.
...
- Install it for real from a scratch clone, because markup that reads correctly still breaks:
  ...
  ```bash
  git clone --depth 1 https://github.com/meettaqi/olywork-new /tmp/olywork-check
  claude plugin marketplace add /tmp/olywork-check
  claude plugin install olywork@olywork
  claude plugin details olywork@olywork     # then: uninstall + marketplace remove
  ```

  `details` is the real check. It must report **Skills (1) olywork** and **MCP servers (0)** — a zero
...
- `claude plugin tag` creates the `olywork--v{version}` release tag and validates that `plugin.json`
  and the enclosing marketplace entry agree — worth running before handing a tag to Anthropic.
- Confirm the copy still says olywork **compares** providers and the caller chooses. olywork does not route
