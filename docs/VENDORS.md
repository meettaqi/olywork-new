# List your API on olywork

olywork is the tool catalog for AI agents: ~2,600 endpoints across ~40 providers, discoverable by
...
API through olywork's proxy with the credential injected server-side.

**What listing gets you:**

- **Discovery** — every agent connected to a olywork deployment can find your endpoints when they
...
- **Zero-friction customers** — teams that connect their own key to your API call you directly;
  you keep the billing relationship. olywork never sits between you and your customer's account.
- **Metered platform traffic (optional)** — if your pricing is published and machine-checkable,
  olywork can serve your endpoints on *its own* key, metered per call from teams' prepaid balances.
...
| **Key rides in a header or query param** | olywork injects credentials as a header (preferred) or query param. Keys embedded in the URL path are not supported |
...
- **Per-success billing** (errors free) — makes verification and retries cheap, and means broken
  calls never bill olywork users.
...
[`/vendor-listing`](https://olywork.com/vendor-listing) and opens the PR for you.
...
### 1. Registry entry — `src/olywork/oauth_providers.py`

How a team connects a key, and how olywork verifies it is real:
...
…added to the `REGISTRY` tuple, plus a neutral lettermark at `src/olywork/web/logos/acmeseo.svg`
...
### 2. Core catalog file — `src/olywork/catalog/acmeseo.yaml`
...
OLYWORK_CATALOG_CRED='<key>' uv run python scripts/catalog_verify.py acmeseo.yaml
...
committed to `src/olywork/catalog/examples/` — that example is what convinces an agent choosing
between providers, so passing verification is the listing's main asset.

## Platform-eligible — getting paid calls from olywork's own key

Beyond discovery, olywork can serve your endpoints on its **own** key, charging the calling team's
...
Eligibility is necessary but not sufficient — olywork also has to hold a funded account with you
...
- **Success rates are public.** olywork aggregates observed success rate and latency per endpoint
