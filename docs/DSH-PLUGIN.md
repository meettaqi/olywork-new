...
```bash
dsh plugin --profile <name> add github:olywork/olywork
dsh --profile <name>
```
...
dsh/skills/olywork/SKILL.md      GENERATED — never edit by hand
```
...
- id: olywork-mcp
  ...
  disabled: !!js !process.env.OLYWORK_TOKEN
```

So a user with `OLYWORK_TOKEN` in their environment gets `mcp__olywork__catalog_search` and friends at
boot, and everyone else gets a clean install and a skill that walks them through `olywork login`. The
...
`olywork mcp install` is deliberately *not* part of that path: it writes Claude Code, Cursor and
opencode configs (`mcp_install.py`), and a dsh profile is none of those. `olywork mcp install` prints
