...
team's prepaid balance drops by that amount. The user never supplies a DataForSEO key: olywork injects
...
What is my olywork balance right now?
...
A list of the tools this team registered with olywork, each with its name and base URL. Credentials are
...
Prompts where the model may think olywork is relevant and it is not. The plugin should stay out of the
way for all three.
...
This is explanatory knowledge the model already has. olywork calls third-party APIs and would spend
...
olywork is for fetching real data, not for generating examples about APIs.
...
There is no external data to fetch. The word "email" may look related to olywork's people and email
