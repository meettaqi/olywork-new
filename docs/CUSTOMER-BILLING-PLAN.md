# Customer-level billing — builders reselling olywork to their own customers
...
Builders are integrating olywork into their own agent products. Their end-customers use the product;
the product calls olywork underneath. The builder needs to know and limit what each of THEIR customers
consumed, and to bill those customers themselves.

## The boundary decision everything follows from

**olywork meters and enforces; the builder monetizes.** olywork never bills a builder's end-customer, never
holds their card, never sets their price. What olywork owes the builder is exactly three things:
...
| Model | Who pays olywork | Status |
|---|---|---|
| **Customer-pays** | each end-customer, their own team + balance | **Shipped.** The builder's product connects through olywork's OAuth server (consent screen, team picker). Built for ChatGPT, deliberately client-agnostic |
| **Builder-pays** | the builder, one org, one balance | **This plan** |

Customer-pays suits developer-tool products whose users would want their own olywork anyway. Builder-pays
...
small tag bag, and `customer` is a reserved key inside it:
    ...
    X-Olywork-Token: <builder org token>
    X-Olywork-Meta: customer=cust_8123, feature=email-finder, env=prod
...
message, not a olywork error about someone else's balance. Never include the org's overall balance in
...
1. The bag: accept the `X-Olywork-Meta` header + the MCP `meta` arg, record into audit telemetry +
