...
olywork's fifth shop window: the **MiniMax Plugin Marketplace**, which serves both **MiniMax Code**
...
credential, and olywork's MCP is bearer-authed per user. An authenticated MCP would have to go through
...
`plugins/minimax/` is the **plugin root** (a sibling of the Cursor root `plugins/olywork/`; each store needs its own root because the manifests and generated bootstraps differ): `.minimax-plugin/plugin.json` sits directly inside it, which is
what MiniMax requires of a ZIP root or a GitHub subdirectory. Manifest `name` is `olywork`; the skill is
exposed at runtime as `olywork:olywork`.

**Never edit `plugins/minimax/skills/olywork/SKILL.md`.** Change `src/olywork/web/skill.md` and regenerate:
...
The Claude/Cursor bootstrap is three steps: install CLI → `olywork login` → `olywork mcp install`. The
MiniMax one has **two** and says explicitly not to run step 3. `olywork mcp install` writes configs for
...
| Region | **US** (and CN if wanted — separate dimension; olywork.com is reachable from CN but billing is Stripe) |
...
| Source | GitHub |
| Repo | `https://github.com/meettaqi/olywork-new` |
| Ref | `main` (or the release tag, e.g. `v0.12.1`) |
| Plugin subdirectory | `plugins/minimax` |
| Email | taqi@olywork.com |

ZIP alternative — built deterministically, no wrapper directory:

```bash
python3 scripts/minimax_plugin.py --zip olywork-minimax-$(python3 -c 'import tomllib;print(tomllib.load(open("pyproject.toml","rb"))["project"]["version"])').zip
```
...
**Updating:** `name` stays `olywork`, bump `version` (the release bump does this), provide proof of
...
1. **`curl … | sh` in the skill.** The package contains no install script, but the skill *tells the
   ...
   `docs/CLAUDE-PLUGIN.md`). If a reviewer objects, the fallback is `pip install olywork` /
   `uv tool install olywork` — the CLI is on PyPI — and a one-line swap in `MINIMAX_BOOTSTRAP`.
2. **Cloud target.** On MiniMax Agent the skill only works if the sandbox has a shell with network
   egress and can open a browser for `olywork login` (device-code login is what to point them at if
...
To get the five MCP tools into MiniMax natively, olywork would enter their **App/Connector** track:
...
error codes. MiniMax then assigns a `provider` and the plugin gains `olywork.app.json`
(`{"schemaVersion":1,"provider":"olywork"}`). olywork's MCP currently authenticates with a static bearer
