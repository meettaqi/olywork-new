...
This runbook is the owner-facing release gate for Olywork's catalog-only Claude connector. Do not
...
| Server URL | `https://olywork.com/mcp/v2/` |
| Authentication | OAuth DCR/CIMD with S256 PKCE |
| Documentation | `https://olywork.com/connectors/claude` |
| Privacy | `https://olywork.com/privacy` |
| Support | `https://olywork.com/support` |
...
Describe Olywork as a **third-party/community connector published by Olywork**. Do not call the
...
- [ ] Choose one reviewer-access option:
  - **Default:** a reviewer signs up with any verified email. A new Olywork team receives **$1.00 in
    free credit**, requires no payment method, and can immediately use platform-provided catalog
    endpoints.
  - **Extended testing:** the reviewer sends Olywork the exact verified Olywork email used for
    sign-in. Olywork sends an email-bound invitation to `claude-connector-review-org`, which has
...
- [ ] MCP Inspector connects to `https://olywork.com/mcp/v2/`, authenticates, lists exactly the six
...
- [ ] The existing team MCP contract at `https://olywork.com/mcp/` remains unchanged.
...
1. In Claude.ai, add `https://olywork.com/mcp/v2/` as a custom connector.
...
3. Ask Claude to search Olywork for backlink endpoints for `example.com`, compare multiple priced
