...
| Unlimited | no anchor, so a olywork-set price with the break-even printed next to it |
...
The vendor has no per-call price. What olywork sells on those rungs is its own service — holding the
...
provider whose credit is "one call on olywork's shared plan."**
...
$0.001, with a basis line reading like the PDL one does today:
    ...
    alphavantage: {usd: 0.001, kind: olywork_shared_plan,
      basis: "olywork shared-plan rate. Vendor sells flat $49.99/mo at 75 req/min (= 3.24M calls/mo
...
1. **A `kind: olywork_shared_plan` marker** on such fx.yaml entries, and the block documented. It is
   what separates "the vendor's rate card" from "olywork's rate card" everywhere downstream.
2. **The validator** learns the marker and requires a fee + break-even in the basis of any entry
   carrying it. A olywork-set price with no printed break-even is exactly the dishonesty this design
   exists to avoid.
...
1. fx.yaml `olywork_shared_plan` convention + validator rules + ONE pilot entry (apollo or a market-data
...
- **The marker not propagating.** If nothing downstream can tell a olywork rate from a vendor rate, some
  future surface will present our price as the vendor's. The validator rule and the basis wording are
  the guard; a test should assert every `olywork_shared_plan` entry names a fee and a break-even.
- **Double-charging a team on its own key.** Tier 2 is already unmetered, so a team bringing its own
  ...
  because this is the first time a provider has a olywork price AND an obvious own-key path.
