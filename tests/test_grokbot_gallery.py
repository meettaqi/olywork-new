...
PAGE = Path(__file__).parents[1] / "src" / "olywork" / "web" / "grokbot.html"
...
def test_grokbot_gallery_lists_the_six_olywork_bots_once_in_workflow_order() -> None:
    html = PAGE.read_text()
...
def test_existing_grokbot_setup_and_plugin_ctas_stay_intact() -> None:
    ...
    for cta_id in ("ctaNav", "ctaHero", "ctaFinal"):
        assert f'id="{cta_id}">Setup olywork</a>' in html
