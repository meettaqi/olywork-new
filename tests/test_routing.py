...
`olywork.people.email.find` picks a child and runs it through the ordinary call use case."""
...
from olywork.domain.catalog.routing.plan import Candidate, cost_at, rank
from olywork.infra.catalog_observations import CachedEndpointObservationReader
from olywork.models import CallRecord, Hold, LedgerEntry

from test_marketplace_call import _balance, platform_on  # noqa: F401

ROUTED = "olywork.people.email.find"
...
def test_every_shipped_adapter_round_trips_its_fixture():
    ...
    assert bad == {}, bad
    ep = cat.by_id[ROUTED]
    assert ep["kind"] == "routed" and ep["provider"] == "olywork" and len(ep["routed_children"]) >= 8
...
async def test_concurrent_routed_plans_share_one_cached_observation_refresh(
    ...
    from olywork.domain.catalog import stats
...
async def test_routed_call_runs_the_cheapest_child_and_returns_output_raw_and_provenance(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    assert d["_olywork"]["charged_micro"] == charged
    async with session_maker() as db:
        assert (await db.execute(select(Hold))).scalars().all() == []
        entries = (await db.execute(select(LedgerEntry))).scalars().all()
    assert {e.call_id for e in entries if e.kind == "settle"} == {r.headers["X-Olywork-Call-Id"] + ":r0"}
    await audit.drain()
...
async def test_error_on_the_first_child_falls_back_to_the_second(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    assert r.status_code == 200, r.text
    d = r.json()
    assert [t["outcome"] for t in d["_olywork"]["tried"]] == ["error", "hit"]
    assert d["_olywork"]["served_by"] == "findymail.search.name" and d["output"]["email"] == "patrick@stripe.com"
    assert r.headers["X-Olywork-Providers-Tried"] == "tomba,findymail"
...
async def test_child_capability_pin_refusal_falls_back_to_the_pinned_provider(
    ...
    response = await clients.post(
        ...
        headers={"X-Olywork-Route-Prefer": "tomba,hunter"},
    )

    assert response.status_code == 200, response.text
    doc = response.json()
    assert doc["_olywork"]["served_by"] == "hunter.people.email.find"
    assert [attempt["outcome"] for attempt in doc["_olywork"]["tried"]] == ["error", "hit"]
    assert doc["_olywork"]["tried"][0]["endpoint_id"] == "tomba.people.email.find"
...
async def test_platform_vendor_401_falls_back_to_the_next_provider(
    ...
    response = await clients.post(
        ...
        json={"full_name": "Patrick Collison", "domain": "stripe.com"},
        headers={
            "X-Olywork-Route-Prefer": "tomba,findymail",
            "X-Olywork-Route-Exclude": "hunter,leadmagic,leadsforge,aviato,fiber-ai",
        },
    )

    assert response.status_code == 200, response.text
    doc = response.json()
    assert doc["_olywork"]["served_by"] == "findymail.search.name"
    assert [attempt["outcome"] for attempt in doc["_olywork"]["tried"]] == ["error", "hit"]
...
async def test_routed_insufficient_balance_still_stops_before_fallback(
    ...
    response = await clients.post(
        f"/call/{ROUTED}",
        json={"full_name": "Patrick Collison", "domain": "stripe.com"},
        headers={"X-Olywork-Route-Prefer": "tomba,findymail"},
    )
...
async def test_waterfall_is_on_by_default_can_be_turned_off_and_respects_max_cost(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Nobody Here", "domain": "stripe.com"},
                           headers={"X-Olywork-Route-Waterfall": "0"})
    assert r.status_code == 200 and r.json()["_olywork"]["outcome"] == "miss" and r.json()["output"]["email"] is None
    assert r.headers["X-Olywork-Route-Outcome"] == "miss" and len(seen) == 1, "waterfall off: stop at the first miss"
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Nobody Here", "domain": "stripe.com"},
                           headers={"X-Olywork-Route-Max-Cost": "0.08"})
    assert r.status_code == 200, r.text
    tried = r.json()["_olywork"]["tried"]
    assert [t["outcome"] for t in tried] == ["miss", "miss", "hit"] and r.json()["_olywork"]["served_by"] == "hunter.people.email.find"
    assert [p for p, *_ in seen] == ["tomba", "findymail", "hunter"]
    assert r.json()["_olywork"]["charged_micro"] == 24_500, "misses on per-success providers are free; only the hit is billed"
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Nobody Here", "domain": "stripe.com"},
                           headers={"X-Olywork-Route-Max-Cost": "0.02"})
    assert r.status_code == 200 and r.json()["_olywork"]["outcome"] == "miss"
    ...
    assert all(t["outcome"] in ("miss", "skipped") for t in r.json()["_olywork"]["tried"]) and r.json()["_olywork"]["charged_micro"] == 0
...
async def test_max_cost_below_the_cheapest_refuses_before_any_call(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "P C", "domain": "stripe.com"}, headers={"X-Olywork-Route-Max-Cost": "0.001"})
...
async def test_caller_fault_on_a_child_stops_and_own_key_ranks_first(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    before = await _balance(clients)
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Patrick Collison", "domain": "stripe.com"})
    assert r.status_code == 200 and r.json()["_olywork"]["served_by"] == "hunter.people.email.find"
    assert r.json()["_olywork"]["tier"] == "credential" and await _balance(clients) == before, "own key: first, and free"
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Patrick Collison", "domain": "stripe.com"})
    assert r.status_code == 200 and r.json()["_olywork"]["served_by"] == "tomba.people.email.find", r.text
...
async def test_a_2xx_without_the_required_core_is_a_miss_not_a_hit(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Patrick Collison", "domain": "stripe.com"})
    assert r.status_code == 200, r.text
    outcomes = {t["endpoint_id"]: t["outcome"] for t in r.json()["_olywork"]["tried"]}
    assert outcomes["hunter.people.email.find"] == "miss" and r.json()["_olywork"]["served_by"] == "tomba.people.email.find"
...
async def test_idempotent_replay_of_a_routed_call_never_calls_a_provider_twice(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    assert r1.status_code == 200 and r2.status_code == 200 and r2.headers.get("X-Olywork-Idempotent-Replay") == "true"
...
async def test_idempotent_replay_preserves_a_routed_failure_after_partial_charge(
    ...
    routed = "olywork.people.email.verify"
    ...
    headers = {
        ...
        "X-Olywork-Route-Prefer": "tomba,leadmagic",
        "X-Olywork-Route-Exclude": "hunter",
    }
    before = await _balance(clients)
    ...
    assert r2.headers.get("X-Olywork-Idempotent-Replay") == "true"
    assert r1.headers["X-Olywork-Cost-Micro"] == r2.headers["X-Olywork-Cost-Micro"] == "8900"
...
def test_a_per_success_miss_settles_at_zero_when_the_adapter_can_tell():
    ...
    from olywork.application.call import settle as A
...
async def test_discovery_puts_the_routed_parent_first_and_its_children_under_it(clients: AsyncClient):
    ...
    from olywork.domain.catalog.store import group_routed
...
async def test_a_registered_tool_for_a_provider_ranks_first_and_is_free(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"full_name": "Patrick Collison", "domain": "stripe.com"})
    assert r.status_code == 200, r.text
    assert r.json()["_olywork"]["served_by"] == "hunter.people.email.find" and r.json()["_olywork"]["tier"] == "tool"
    assert await _balance(clients) == before and r.json()["_olywork"]["charged_micro"] == 0


async def test_mcp_search_shows_the_routed_parent_first_with_its_children(clients: AsyncClient):
    from olywork import mcp as M
    ...
    parent = ids.index(ROUTED)
    kids = [i for i, r in enumerate(out["results"]) if r["provider"] != "olywork" and r["endpoint_id"].split(".", 1)[1] in ("people.email.find", "people.email.find.linkedin", "search.name")]
    assert kids and parent < min(kids)
    assert out["results"][parent]["routed"].startswith("olywork picks among")
...
async def test_a_keyless_provider_is_dropped_at_planning_not_failed_at_call_time(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    from olywork.application.call.route import RouteOptions, build_plan
    cat = catalog_store.load()
    ep = cat.by_id["olywork.people.email.find"]
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_AVIATO", "")  # aviato stays eligible, but keyless
    get_settings.cache_clear()
    class _Org: id = 1
...
def test_a_contract_may_set_its_own_default_ceiling():
    from olywork.application.call.route import RouteOptions, DEFAULT_MAX_COST_MICRO
    cat = catalog_store.load()
    ...
    assert RouteOptions.from_headers(lambda k: "0.02" if k == "x-olywork-route-max-cost" else None, 500_000).max_cost_micro == 20_000
...
async def test_routed_call_and_access_name_the_providers_dropped_for_this_deployment(clients: AsyncClient, enrichment_on, monkeypatch):
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_AVIATO", "")
    get_settings.cache_clear()
    seen = []
    monkeypatch.setattr(call_service, "relay", _relay_by_provider(
    ...
    r = await clients.post(f"/call/{ROUTED}", json={"linkedin_url": "https://www.linkedin.com/in/x"}, headers={"X-Olywork-Route-Max-Cost": "0.03"})
    assert r.status_code == 200 and r.json()["_olywork"]["outcome"] == "miss"
    dropped = r.json()["_olywork"]["dropped"]
...
def test_rank_prefers_the_candidate_that_uses_more_of_the_identity():
    ...
    from olywork.domain.catalog.routing.plan import Candidate, rank
...
def test_a_provider_that_cannot_express_a_supplied_filter_ranks_last_among_equals():
    ...
    from olywork.domain.catalog.routing.plan import Candidate, ignored_filters, rank
...
async def test_the_geo_aware_child_wins_a_filtered_search_and_the_answer_says_what_was_dropped(
    ...
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_ICYPEAS", "PLATFORM-ICYPEAS-KEY")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "hunter,tomba,leadmagic,leadsforge,findymail,aviato,fiber-ai,icypeas")
    get_settings.cache_clear()
    seen = []
    monkeypatch.setattr(call_service, "relay", _relay_by_provider(
        {"icypeas": [(200, {"leads": [{"firstname": "Aleksei", "lastname": "Strizhak", "lastJobTitle": "Senior Backend Engineer",
         ...
         "*": [(200, {"persons": []})] * 12}, seen))
    r = await clients.post("/call/olywork.people.search",
                           json={"q": "backend engineer", "title": "Backend Engineer",
                                 "location": "London, United Kingdom", "country": "GB", "limit": 15})
    assert r.status_code == 200, r.text
    d = r.json()
    assert d["_olywork"]["served_by"] == "icypeas.people.search", \
    ...
    assert "ignored_filters" not in d["_olywork"] and "X-Olywork-Ignored-Filters" not in r.headers
    ...
    r2 = await clients.post("/call/olywork.people.search", json={"q": "backend engineer", "country": "GB", "limit": 15})
    assert r2.status_code == 200, r2.text
    d2 = r2.json()
    assert d2["_olywork"]["served_by"] == "aviato.people.search", "no geo-aware child answers a {q}-only brief here"
    assert d2["_olywork"]["ignored_filters"] == ["country"], "the caller sent it; aviato has no place for it"
    assert r2.headers["X-Olywork-Ignored-Filters"] == "country"
    assert d2["_olywork"]["tried"][-1]["ignored_filters"] == ["country"], "same set in all three places"


async def test_routed_discovery_is_a_runtime_switch(clients: AsyncClient, enrichment_on, monkeypatch):
    """`OLYWORK_ROUTED_DISCOVERY=off` stops search LEADING with `olywork.<capability>` — nothing else.
    ...
    assert any(i.startswith("olywork.") for i in ids_on), "steering on: the routed row is in the page"

    monkeypatch.setenv("OLYWORK_ROUTED_DISCOVERY", "off")
    get_settings.cache_clear()
    off = (await clients.get("/catalog/search", params=q)).json()
    ids_off = [r["id"] for r in off["results"]]
    assert not any(i.startswith("olywork.") for i in ids_off), \
    ...
    assert "olywork.people." not in flat, "browse view: no routed row while steering is off"
    ...
    r = await clients.get("/catalog/endpoints/olywork.people.email.find")
    ...
    assert call.status_code == 200 and call.json()["_olywork"]["served_by"] == "tomba.people.email.find"
    get_settings.cache_clear()
...
def test_keywords_are_a_filter_so_they_reach_every_provider_that_can_express_them():
    ...
    from olywork.domain.catalog.routing.plan import ignored_filters
...
async def test_a_thin_hit_does_not_end_the_waterfall_when_the_caller_set_min_results(
    ...
    """`X-Olywork-Route-Min-Results: 3` — one row is not an answer to "find me candidates". The
    ...
    body = {"q": "backend engineer", "title": "Backend Engineer",
            "location": "London, United Kingdom", "country": "GB", "limit": 15}
    r = await clients.post("/call/olywork.people.search", json=body, headers={"X-Olywork-Route-Min-Results": "3"})
    assert r.status_code == 200, r.text
    d = r.json()
    assert [t["outcome"] for t in d["_olywork"]["tried"]][0] == "weak", "1 row < 3 is not an answer"
    assert d["_olywork"]["served_by"] != "icypeas.people.search" and len(d["output"]["people"]) == 9

    # default (min_results 1): the same thin answer ends the search, as before
    seen2 = []
    monkeypatch.setattr(call_service, "relay", _relay_by_provider(
        {"icypeas": [(200, thin)], "*": [(200, full)] * 12}, seen2))
    r2 = await clients.post("/call/olywork.people.search", json=body)
    assert r2.json()["_olywork"]["served_by"] == "icypeas.people.search" and len(seen2) == 1
    ...
    r3 = await clients.post("/call/olywork.people.search", json=body, headers={"X-Olywork-Route-Min-Results": "5"})
...
async def test_the_weak_hit_fallback_is_bounded_like_the_error_fallback(
    ...
    r = await clients.post("/call/olywork.people.search", json=body,
                           headers={"X-Olywork-Route-Min-Results": "5"})   # nothing will ever clear 5
    assert r.status_code == 200, r.text
    d = r.json()
    attempts = [t for t in d["_olywork"]["tried"] if t["outcome"] == "weak"]
...
async def test_merge_unions_the_rows_the_caller_already_paid_for(
    ...
    """`X-Olywork-Route-Merge: 1` — a list answer is the one shape a union makes sense for, and the
    ...
    r = await clients.post("/call/olywork.people.search", json=body,
                           headers={"X-Olywork-Route-Min-Results": "5", "X-Olywork-Route-Merge": "1"})
    assert r.status_code == 200, r.text
    d = r.json()
    ...
    assert len(d["_olywork"]["merged_from"]) >= 2 and "X-Olywork-Merged-From" in r.headers
    assert d["_olywork"]["charged_micro"] == int(r.headers["X-Olywork-Cost-Micro"]), \
    ...
    r2 = await clients.post("/call/olywork.people.search", json=body,
                            headers={"X-Olywork-Route-Min-Results": "5"})
    assert "merged_from" not in r2.json()["_olywork"] and len(r2.json()["output"]["people"]) == 2
...
def test_a_per_success_endpoint_with_no_adapter_settles_on_the_providers_own_success_rule():
    ...
    published terms ("only a code-0 response is billed") — and olywork settled $0.0295 against the
    caller."""
    from test_marketplace_call import _mk
    from olywork.application.call import settle as A
    cat = catalog_store.load()
...
async def test_a_declared_miss_status_is_a_miss_not_a_caller_fault(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    assert r.status_code == 200, r.text
    assert r.headers["X-Olywork-Route-Outcome"] == "miss" and r.json()["_olywork"]["served_by"] is None
    outcomes = {t["endpoint_id"]: t["outcome"] for t in r.json()["_olywork"]["tried"]}
    ...
    r = await clients.post(f"/call/{routed}", json={"linkedin_url": "https://www.linkedin.com/in/nobody-here"},
                           headers={"X-Olywork-Route-Prefer": "aviato"})
    ...
    r = await clients.post(f"/call/{routed}", json={"linkedin_url": "https://www.linkedin.com/in/nobody-here"},
                           headers={"X-Olywork-Route-Prefer": "aviato", "X-Olywork-Route-Waterfall": "0"})
    assert r.status_code == 200 and r.headers["X-Olywork-Route-Outcome"] == "miss" and len(seen) == 1, r.text
...
async def test_lusha_is_the_last_rung_of_the_phone_waterfall_and_settles_on_its_own_bill(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    assert r.status_code == 200 and r.json()["_olywork"]["served_by"] == "lusha.people.phone.find", r.text
    ...
    before = await _balance(clients)
    r = await clients.post(f"/call/{routed}", json={"full_name": "Ana Perez", "domain": "acme.gt"})
    assert r.status_code == 200 and r.headers["X-Olywork-Route-Outcome"] == "miss" and r.json()["output"]["phone"] is None, r.text
...
async def test_strict_filters_refuses_a_looser_answer_instead_of_billing_it(clients: AsyncClient, enrichment_on, monkeypatch):
    ...
    r = await clients.post("/call/olywork.people.search", json={"full_name": "Carlos Lopez", "country": "GT", "limit": 3},
                           headers={"X-Olywork-Route-Strict-Filters": "1", "X-Olywork-Route-Exclude": "aviato"})
    ...
    assert "X-Olywork-Route-Strict-Filters" in d["message"] and "full_name" in d["message"]
    # without the header the same call goes out, is billed, and says what it ignored
    r = await clients.post("/call/olywork.people.search", json={"full_name": "Carlos Lopez", "country": "GT", "limit": 3},
                           headers={"X-Olywork-Route-Exclude": "aviato"})
    assert r.status_code == 200 and r.headers["X-Olywork-Ignored-Filters"] == "country" and r.json()["_olywork"]["ignored_filters"] == ["country"], r.text
    assert len(seen) == 1
    ...
    r = await clients.post("/call/olywork.people.search", json={"full_name": "Carlos Lopez", "country": "GT", "limit": 3},
                           headers={"X-Olywork-Route-Strict-Filters": "1"})
