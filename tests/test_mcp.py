...
an endpoint whose price nobody knows. `mcp.py` gets that by routing through olywork's own API rather
...
async def mcp_session(client: AsyncClient):
    ...
    The default `X-Olywork-Token` is dropped: MCP carries identity in `Authorization`, and leaving a
    ...
    from olywork import mcp as _mcp
    ...
    # `olywork.api.app` internally, so the enforcement under test is the production path.
    fresh = _mcp.build_mcp_app()
    host = FastAPI()
    host.mount("/mcp", fresh)
    client.headers.pop("X-Olywork-Token", None)
...
async def test_catalog_search_returns_priced_results(clients):
    ...
    token = (await clients.post("/users", json={"email": "searcher@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
...
async def test_no_key_needed_is_false_when_the_deploy_holds_no_key(clients):
    """`no_key_needed` must mean "THIS deploy will serve it on olywork's key", not "the row is priced".
    ...
    token = (await clients.post("/users", json={"email": "keyless@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
...
async def test_search_says_so_when_nothing_matches(clients):
    token = (await clients.post("/users", json={"email": "nomatch@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
    ...
    from olywork.models import SearchMiss
...
async def test_catalog_request_files_the_gap_with_attribution(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import ToolRequest

    token = (await clients.post("/users", json={"email": "wisher@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
    ...
    assert row.source == "mcp"
    assert row.user_email == "wisher@olywork.com"
...
async def test_a_real_token_reads_its_OWN_balance(clients):
    ...
    r = await clients.post("/users", json={"email": "mcpuser@olywork.com"})
    token = r.json()["token"]
    async with mcp_session(clients) as c:
...
async def test_one_team_cannot_read_another_teams_tools(clients):
    ...
    a = (await clients.post("/users", json={"email": "org-a@olywork.com"})).json()["token"]
    b = (await clients.post("/users", json={"email": "org-b@olywork.com"})).json()["token"]
    clients.headers["X-Olywork-Token"] = a
...
async def test_call_reaches_the_TEAMS_OWN_tools_too(clients):
    ...
    assert made.status_code == 200, made.text
    token = clients.headers.get("X-Olywork-Token")
...
async def test_tool_descriptions_do_not_promise_routing(clients):
    ...
    olywork COMPARES providers and the caller chooses. These descriptions are read by every model that
    installs the plugin, so a false claim here travels further than the website's did."""
    token = (await clients.post("/users", json={"email": "descs@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        await _rpc(c, "initialize", {"protocolVersion": "2025-06-18", "capabilities": {},
                                     "clientInfo": {"name": "t", "version": "1"}}, token)
        r = await _rpc(c, "tools/list", token=token)
...
async def test_an_unknown_host_is_refused(clients):
    ...
    token = (await clients.post("/users", json={"email": "host@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
...
async def test_the_deployments_own_host_is_allowed():
    """The list must contain the host olywork actually answers on, or production 421s every call."""
    from urllib.parse import urlsplit

    from olywork.config import get_settings
    from olywork.mcp import _allowed_hosts
...
async def test_the_price_is_visible_before_spending(clients):
    ...
    token = (await clients.post("/users", json={"email": "pricer@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        out = await _call_tool(c, "catalog_search", {"query": "backlinks", "limit": 5}, token=token)
...
async def test_every_tool_declares_what_it_can_do(clients):
    ...
    olywork does not model the upstream, so it cannot promise the call is safe, and saying otherwise
...
async def test_a_BROWSER_origin_is_accepted(clients):
    ...
    token = (await clients.post("/users", json={"email": "browser@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        r = await c.post("http://localhost/mcp/", json={
...
async def test_an_UNKNOWN_origin_is_still_refused(clients):
    ...
    token = (await clients.post("/users", json={"email": "origin@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        r = await c.post("http://localhost/mcp/", json={
...
async def test_structured_content_VALIDATES_against_the_advertised_schema(clients):
    ...
    token = (await clients.post("/users", json={"email": "strict@olywork.com"})).json()["token"]
...
async def test_large_mcp_responses_are_gzipped_AT_THE_ORIGIN(clients):
    ...
    token = (await clients.post("/users", json={"email": "gz@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        await _rpc(c, "initialize", {
            "protocolVersion": "2025-06-18", "capabilities": {},
...
async def test_mcp_responses_forbid_edge_TRANSFORMS(clients):
    ...
    token = (await clients.post("/users", json={"email": "edge@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
...
async def test_EVERY_tool_needs_a_credential_including_the_catalog(clients, tool, args):
    ...
    public on the WEBSITE, which the landing page and `olywork catalog search` both rely on."""
    async with mcp_session(clients) as c:
        r = await c.post("http://localhost/mcp/", json={
            "jsonrpc": "2.0", "id": 1, "method": "tools/call",
            "params": {"name": tool, "arguments": args}}, headers=MCP_HEADERS)
...
async def test_EAGER_auth_challenges_initialize_itself(clients):
    """Eager, not lazy. Every olywork tool needs auth, so there is nothing to browse anonymously — and
...
async def test_auth_body_replay_waits_for_the_real_disconnect_on_a_long_response() -> None:
    ...
    from olywork.mcp import RequireAuthForProtectedTools
...
async def test_subscription_listen_is_not_served(clients) -> None:
    """SDK 2.0 registers listen by default; olywork refuses it through the public middleware seam."""
    async with mcp_session(clients) as c:
...
async def test_call_accepts_an_ARRAY_body(clients):
    ...
    token = (await clients.post("/users", json={"email": "arraybody@olywork.com"})).json()["token"]
...
async def test_a_list_is_refused_for_a_GET_with_a_clear_reason(clients):
    ...
    token = (await clients.post("/users", json={"email": "listget@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        out = await _call_tool(c, "call", {
...
def test_a_relayed_402_carries_NO_link_out(clients):
    ...
    purchases", and only PHYSICAL goods can be supported. olywork sells prepaid API credit — a digital
    ...
    """
    from olywork.mcp import _without_purchase_pointers
...
def test_the_strip_does_not_depend_on_which_host_we_run_as(clients):
    ...
    for host in ("https://olywork.ngrok.app", "http://127.0.0.1:18790", "https://anything.example"):
...
async def test_a_402_THROUGH_THE_CALL_TOOL_carries_no_link(clients, monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_TIKHUB", "PLATKEY")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "tikhub")
    get_settings.cache_clear()

    token = (await clients.post("/users", json={"email": "broke402@olywork.com"})).json()["token"]
    prev = clients.headers.get("X-Olywork-Token")
    clients.headers["X-Olywork-Token"] = token
...
async def test_an_EXPIRED_access_token_gets_401_invalid_token(clients):
    ...
    dead = mcp_oauth.make_access_token(user_id=7, org_id=3, audience=mcp_oauth.mcp_resource_url(),
                                       scope="olywork:call", ttl=-60)  # born expired
    async with mcp_session(clients) as c:
        r = await c.post("http://localhost/mcp/", json={
            "jsonrpc": "2.0", "id": 1, "method": "tools/call",
            "params": {"name": "balance", "arguments": {}}},
...
async def test_a_wrong_audience_access_token_gets_401_invalid_token(clients):
    ...
    other = mcp_oauth.make_access_token(user_id=7, org_id=3,
                                        audience="https://evil.example/mcp/", scope="olywork:call")
    async with mcp_session(clients) as c:
        r = await c.post("http://localhost/mcp/", json={
            "jsonrpc": "2.0", "id": 1, "method": "tools/call",
            "params": {"name": "balance", "arguments": {}}},
...
async def test_call_passes_an_idempotency_key_through(clients):
    ...
    token = (await clients.post("/users", json={"email": "mcpidem@olywork.com"})).json()["token"]
    prev = clients.headers.get("X-Olywork-Token")
    clients.headers["X-Olywork-Token"] = token
    made = await clients.post("/tools", json={"name": "echo", "base_url": "http://upstream"})
    if prev:
...
# ---------------------------------------------------------------------------------------------
# `call` request-shape parity with the CLI (`olywork call`)
#
# Every flag on `olywork call` exists because a real endpoint needed it — --query alongside --data
...
async def _register_echo(clients) -> str:
    made = await clients.post("/tools", json={"name": "echo", "base_url": "http://upstream"})
    assert made.status_code == 200, made.text
    return clients.headers.get("X-Olywork-Token")
...
async def test_call_relays_extra_headers_but_never_olyworks_own(clients):
    """--header parity (login-customer-id is the canonical need). olywork's own auth/routing headers
    are filtered from the relay: the MCP bearer IS the identity and must not be maskable."""
    token = await _register_echo(clients)
    async with mcp_session(clients) as c:
        out = await _call_tool(c, "call", {
            "endpoint_id": "echo/ads",
            "headers": {"login-customer-id": "1234567890", "X-Olywork-Token": "evil",
    ...
    assert seen["headers"].get("x-olywork-token") != "evil"
...
async def test_call_resolves_the_team_for_an_identity_token(clients):
    """The same production bug `balance` had, on the spending path: an IDENTITY token (`olywork
    login` — what most people hold) belongs to a person who may be in several teams, and /call
    answers it with a raw "choose an org (send X-Olywork-Org)" 400 — a header hint an MCP caller
...
async def test_an_id_that_misses_by_one_segment_names_the_real_one(clients):
    ...
    token = (await clients.post("/users", json={"email": "nearmiss@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
...
async def test_catalog_query_arrays_use_the_endpoints_declared_wire_encoding(monkeypatch):
    ...
    """
    from olywork.domain.catalog import store as cs
    from olywork import mcp as _mcp
...
async def test_search_breaks_ties_on_what_olywork_has_MEASURED(clients):
...
async def test_balance_says_WHOSE_grant_and_which_team_by_name(clients):
    """A slug alone cannot be sanity-checked: `olywork-7` looks plausible to the agent and to
    ...
    token = (await clients.post("/users", json={"email": "whose@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
        out = await _call_tool(c, "balance", {}, token=token)
    assert out["team"] and out["team_name"]
    assert out["identity"] == "whose@olywork.com"
    # This one is a HEADER token, whose team is baked in — so it is labelled, but not sent to
    # `olywork mcp grants`, which would list nothing for it. The OAuth half is asserted in
...
async def test_the_tie_band_covers_the_WHOLE_equal_scoring_group(clients, monkeypatch):
    ...
    token = (await clients.post("/users", json={"email": "wide@olywork.com"})).json()["token"]
    async with mcp_session(clients) as c:
...
async def test_a_near_miss_never_suggests_a_DIFFERENT_provider(clients):
    ...
    path that spends money that is provider routing wearing a spellcheck's clothes. olywork compares
...
def test_a_header_token_is_not_told_to_run_a_command_that_lists_nothing():
    """`olywork mcp grants` only has an answer for an OAuth grant. A header token already carries its
    ...
    plain = asyncio.run(_mcp._whose_grant(_Dead(), "olywork", oauth=False))
    granted = asyncio.run(_mcp._whose_grant(_Dead(), "olywork", oauth=True))
    assert "use-team" not in (plain.get("hint") or "")
    assert "use-team" in granted["hint"]
    assert plain["team"] == "olywork", "and it still labels the team it does know"
...
async def test_the_SEARCH_TOOL_itself_ranks_on_evidence_not_just_the_helper(clients):
    ...
    from olywork.domain.catalog import stats as endpoint_stats
    from olywork.infra.db import session_maker
