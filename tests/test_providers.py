"""Provider catalog + env scanner (`olywork upload`, Phase 1 detect + Phase 2 plan).
...
def test_app_prefix_is_transparent(tmp_path):
    # A OLYWORK_/APP_ prefix must not hide the provider token.
    env = _write_env(tmp_path, "OLYWORK_RESEND_API_KEY=x\n")
    d = _by_var(prov.scan_env(env))
    assert d["OLYWORK_RESEND_API_KEY"].provider == "Resend"
...
def test_every_catalog_cli_profile_is_valid_and_typed():
    ...
    report-only)."""
    from olywork.localrun import AUTH_MECHANISMS, validate_cli_profile
...
def test_validate_cli_profile_accepts_and_rejects_the_new_fields():
    from olywork.localrun import validate_cli_profile
