...
from olywork.application.call.types import CallFailure
from olywork.bootstrap_handlers import _pool_saturated
from olywork.config import get_settings
from olywork.routers.call import _translate_call_failure
...
async def test_batch_shape_and_drain(enabled, posts):
    ...
    assert first["properties"]["$lib"] == "olywork-server"
...
def test_handler_ignores_analytics_logger_and_children(enabled):
    handler = analytics.install_fault_handler()
    logging.getLogger("olywork.analytics").error("self failure")
    logging.getLogger("olywork.analytics.sender").error("child failure")
