"""Skill-directory scanner + payload builder (`olywork upload skills`).
...
from olywork import skills as sk


def _skill(tmp_path, name, *, skillmd="# s\n", script=None, secret=None, olywork=None):
    d = tmp_path / name
    d.mkdir()
    ...
    if olywork:
        import json
        (d / "olywork.json").write_text(json.dumps(olywork))
    return d
...
def test_existing_contract_used_verbatim(tmp_path):
    _skill(tmp_path, "gsc", olywork={"name": "gsc", "base_url": "https://searchconsole.googleapis.com",
...
def test_write_contract_only_for_generated(tmp_path):
    ...
    assert path and (tmp_path / "render" / "olywork.json").exists()
...
def test_contract_health_and_examples_carried(tmp_path):
    _skill(tmp_path, "svc", olywork={"name": "svc", "base_url": "https://api.svc.com",
...
def test_contract_flags_absent_env_and_missing_file(tmp_path):
    _skill(tmp_path, "a", olywork={"name": "a", "base_url": "https://api.a.com",
