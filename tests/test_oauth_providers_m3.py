...
def all_apps(monkeypatch):
    for k in ("GOOGLE", "SLACK", "X", "TIKTOK"):
        monkeypatch.setenv(f"OLYWORK_{k}_CLIENT_ID", f"{k.lower()}-cid")
        monkeypatch.setenv(f"OLYWORK_{k}_CLIENT_SECRET", f"{k.lower()}-csec")
...
def test_slack_is_bring_your_own_bot():
    ...
    olywork app would sit between a team and their own messages — and couldn't be installed on their
...
async def test_unconfigured_providers_are_listed_but_flagged(clients: AsyncClient, monkeypatch):
    monkeypatch.setenv("OLYWORK_X_CLIENT_ID", "")
    monkeypatch.setenv("OLYWORK_X_CLIENT_SECRET", "")
    get_settings.cache_clear()
    try:
...
async def test_linkedin_does_not_get_googles_consent_params(clients: AsyncClient, monkeypatch):
    monkeypatch.setenv("OLYWORK_LINKEDIN_CLIENT_ID", "li-cid")
    monkeypatch.setenv("OLYWORK_LINKEDIN_CLIENT_SECRET", "li-csec")
    get_settings.cache_clear()
    try:
