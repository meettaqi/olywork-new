"""Phase A: olywork keeps OAuth tokens fresh. A stale oauth secret is refreshed (via its token_uri)
...
async def test_stale_token_is_refreshed_before_call(clients: AsyncClient):
    ...
    assert r.status_code == 200, r.text
    assert r.json()["auth"] == "Bearer REFRESHED"  # olywork refreshed it silently
...
async def test_manual_mode_token_without_refresh_fields_is_injected_as_is(clients: AsyncClient):
    ...
    # is injected verbatim — olywork never tries to refresh what it can't.
...
async def test_token_endpoint_io_holds_no_database_connection(
    ...
    from olywork.infra.db import _engine
...
async def test_exchange_code_persists_basic_auth_method_into_blob():
    ...
    p = PendingOAuth(
        ...
        client_id="cid", client_secret=crypto.encrypt("csec"), redirect_uri="https://olywork/cb",
