"""`olywork run` — grants for local CLI execution (docs/CLI-RUN-PLAN.md).
...
from olywork import cli as _cli
...
def _no_installed_runner(monkeypatch):
    """Unit tests must not depend on whether THIS machine has `olywork setup-local-run` installed. Force the
...
def test_check_deny_word_boundary_and_source():
    ...
    assert hit == (LIVE, "the olywork catalog defaults")
    hit = localrun.check_deny(prof, ["refunds", "create"])
    assert hit is not None and hit[1] == "this skill's olywork.json"
...
def test_catalog_cli_recipe_becomes_runnable_tool(tmp_path):
    ...
    from olywork import skills as sk, providers as prov
...
async def test_grant_local_tier_cli_injects_nothing(clients: AsyncClient):
    ...
    secret), not a 'profile has no inject entries' error — so `olywork run gh` just execs gh."""
...
async def test_grant_creator_deny_and_deny_defaults_opt_out(clients: AsyncClient):
    ...
    assert denied.status_code == 403 and "olywork.json" in denied.json()["detail"]
...
async def test_grant_no_profile_409_with_template(clients: AsyncClient):
    ...
    assert r.status_code == 409 and '"cli"' in r.json()["detail"]  # the olywork.json template
...
def _run_args(tool: str, *cli_args: str):
    from olywork import cli as cli_mod
...
def test_cmd_run_linux_hands_off_to_olywork_run_user(monkeypatch):
    # On Linux with local-run set up, the member never fetches the credential: it execs
    # `sudo -u olywork-run <runner>` and passes its own token through the environment.
    from olywork import cli as cli_mod
    ...
    assert seen["argv"][:4] == ["sudo", "-u", "olywork-run", "--"]
    assert cli_mod._RUNNER_PATH in seen["argv"] and "stripe" in seen["argv"]
    assert seen["env"]["OLYWORK_RUN_TOKEN"] == "MEMBER_TOK" and seen["env"]["OLYWORK_RUN_ORG"] == "acme"


def test_cmd_run_helper_uses_env_context(monkeypatch):
    # The olywork-run-side helper rebuilds its config from the env the member passed through sudo.
    from olywork import cli as cli_mod
    grant = _FakeResp(200, {"bin": "gh", "inject": [{"via": "env", "name": "GH_TOKEN", "value": "t"}],
    ...
    monkeypatch.setenv("OLYWORK_RUN_TOKEN", "T"); monkeypatch.setenv("OLYWORK_RUN_BASE", "http://x")
    monkeypatch.setenv("OLYWORK_RUN_ORG", "acme")
    with pytest.raises(SystemExit):
...
def test_cmd_run_refusal_prints_server_detail(monkeypatch, capsys):
    from olywork import cli as cli_mod
    fake = _FakeClient({"/grant": _FakeResp(403, {"detail": "denied by the olywork catalog defaults: pattern '--live'"})})
    monkeypatch.setattr(cli_mod, "_client", lambda cfg: fake)
    with pytest.raises(SystemExit) as e:
        cli_mod.cmd_run(_run_args("stripe", "get", "--live"), {"base_url": "http://x"})
    assert "denied by the olywork catalog defaults" in str(e.value.code)
...
def test_run_parser_remainder():
    ...
    # without the separator, args that don't collide with olywork flags also pass through
...
async def test_owner_only_binding_and_runner_proof(clients: AsyncClient, monkeypatch):
    ...
    assert r.status_code == 403 and "olywork-run runner" in r.text
    ...
    r = await clients.post("/tools/shared/grant", json={"argv": []}, headers={**hb, "X-Olywork-Run-Proof": "PROOF123"})
    assert r.status_code == 200, r.text
    # wrong proof → still refused
    r = await clients.post("/tools/shared/grant", json={"argv": []}, headers={**hb, "X-Olywork-Run-Proof": "nope"})
    assert r.status_code == 403
...
def test_redact_argv_masks_short_flag_values_and_jwts():
    from olywork.api import _redact_argv
...
def test_cmd_run_rejects_a_tier_flag_after_the_tool(monkeypatch):
    from olywork import cli as cli_mod
    # a tier flag typed AFTER the tool (no `--`) → wrong tier silently, so olywork errors
    monkeypatch.setattr(cli_mod.sys, "argv", ["olywork", "run", "stripe", "--server", "list"])
...
def test_cmd_run_allows_vendor_flags_after_the_separator(monkeypatch):
    ...
    monkeypatch.setattr(cli_mod.sys, "argv", ["olywork", "run", "db", "--", "fetch", "--timeout", "30"])
