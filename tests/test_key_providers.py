...
async def test_basic_provider_encodes_a_raw_login_password_once(clients: AsyncClient, monkeypatch):
    """DataForSEO/Moz take HTTP Basic. Pasting the raw `login:password`, olywork Base64s it once and the
...
async def test_basic_provider_accepts_a_ready_made_base64_blob(clients: AsyncClient, monkeypatch):
    ...
    that at least as often as the raw pair. olywork must NOT Base64 it a second time — the upstream has to
