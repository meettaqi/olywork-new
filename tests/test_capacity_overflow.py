...
served through a olywork-owned aggregator account when olywork's own account fails a tier-4 call.
...
from olywork import audit, ratestore
...
from olywork.domain.capacity.policy import LatestState
from olywork.domain.capacity.routes_view import view as routes_view
from olywork.domain.capacity.marks import LOCK_NS, Lock
from olywork.domain.capacity.sweep import STATE_NS
from olywork.domain.capacity.view import view as capacity_view
from olywork.models import Hold, LedgerEntry, OverflowRoute, OverflowSpend
...
def overflow_on(monkeypatch, platform_on):
    monkeypatch.setenv("OLYWORK_OVERFLOW_MODE", "on")
    monkeypatch.setenv("OLYWORK_OVERFLOW_KEY_ORTHOGONAL", "ORTH-KEY")
    monkeypatch.setenv("OLYWORK_OVERFLOW_KEY_MONID", "MONID-KEY")
...
async def test_off_by_default_the_vendor_402_is_relayed_unchanged(clients: AsyncClient, platform_on, monkeypatch):
    ...
    assert r.status_code == 402 and seen == [] and "X-Olywork-Served-Via" not in r.headers
...
async def test_findymail_shaped_402_on_tier4_runs_one_child_cycle(clients: AsyncClient, overflow_on, monkeypatch):
    ...
    assert r.headers["X-Olywork-Served-Via"] == "overflow:orthogonal"
    assert r.headers["X-Olywork-Cost-Micro"] == "3000", "the aggregator's real price, 0% markup"
    ...
    parent = r.headers["X-Olywork-Call-Id"]
...
async def test_overflow_budget_reconciles_the_estimate_to_the_actual_cost(
    ...
    assert response.headers["X-Olywork-Cost-Micro"] == "2500"
    spend = await _rows(OverflowSpend)
    assert len(spend) == 1
...
async def test_overflow_substitutes_consumed_path_params_before_calling_aggregator(
    ...
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_PREDICTLEADS", "TEST-PREDICTLEADS-KEY")
    monkeypatch.setenv(
        "OLYWORK_PLATFORM_PROVIDERS", "tikhub,scrapecreators,dataforseo,brightdata,predictleads",
    )
    get_settings.cache_clear()
    async with session_maker() as db:
...
async def test_aggregator_402_releases_the_child_marks_it_unhealthy_and_answers_a_typed_503(
    ...
    assert r.headers["X-Olywork-Error"] == "1" and r.headers.get("X-Olywork-Cost-Micro") in (None, "0")
    ...
    assert r2.status_code == 200 and r2.headers["X-Olywork-Served-Via"] == "overflow:monid"
    assert seen[-1].json["provider"] == "tikhub" and r2.headers["X-Olywork-Cost-Micro"] == "2000"
...
async def test_budget_crossing_skips_overflow(clients: AsyncClient, overflow_on, monkeypatch):
    await _route(price_micro=3_000)
    monkeypatch.setenv("OLYWORK_OVERFLOW_DAILY_BUDGET_USD", "0.004")
    get_settings.cache_clear()
    async with session_maker() as db:
        from olywork.domain.capacity.overflow_spend import add_in_transaction
...
async def test_shadow_mode_probes_records_spend_and_returns_the_vendor_error(clients: AsyncClient, overflow_on, monkeypatch):
    monkeypatch.setenv("OLYWORK_OVERFLOW_MODE", "shadow")
    get_settings.cache_clear()
    await _route(price_micro=3_000)
    monkeypatch.setattr(call_service, "relay", _fake_relay(402, b'{"detail":"Insufficient balance"}'))
    seen = []
    ...
    before = await _balance(clients)
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 402 and "X-Olywork-Served-Via" not in r.headers and r.headers["X-Olywork-Cost-Micro"] == "0"
...
async def test_cancellation_cleanup_releases_both_holds_exactly_once(clients: AsyncClient, overflow_on):
    ...
    from olywork.application.call.settle import _finish_cancelled_call
...
async def test_an_exhausted_account_with_a_route_skips_the_direct_attempt(clients: AsyncClient, overflow_on, monkeypatch):
    ...
    assert r.status_code == 200 and r.headers["X-Olywork-Served-Via"] == "overflow:orthogonal", r.text
    ...
    assert replay.status_code == 200 and replay.headers.get("X-Olywork-Idempotent-Replay") == "true"
...
async def test_org_opt_out_is_honoured_before_any_aggregator_is_contacted(clients: AsyncClient, overflow_on, monkeypatch):
    ...
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 200 and r.headers["X-Olywork-Served-Via"] == "overflow:orthogonal" and len(seen) == 1
...
async def test_child_with_no_reported_cost_settles_at_the_aggregator_reserve(
    ...
    response = await clients.get(f"/call/{EP}?aweme_id=7")
    assert response.status_code == 200
    assert response.headers["X-Olywork-Cost-Micro"] == "3000"
    assert before - await _balance(clients) == 3_000
...
async def test_apollo_out_of_credits_422_on_tier4_overflows_through_orthogonal(clients: AsyncClient, overflow_on, monkeypatch):
    ...
    assert r.status_code == 200, r.text
    assert r.json() == APOLLO_PERSON and r.headers["X-Olywork-Served-Via"] == "overflow:orthogonal"
    assert r.headers["X-Olywork-Cost-Micro"] == "10000" and before - await _balance(clients) == 10_000
...
async def test_apollo_out_of_credits_on_a_per_call_endpoint_is_never_billed_to_the_caller(clients: AsyncClient, platform_on, monkeypatch):
    ...
    account, so the hold is released, and the caller pays nothing for olywork's empty pool."""
    monkeypatch.setattr(call_service, "relay", _fake_relay(422, APOLLO_OUT_OF_CREDITS))
    before = await _balance(clients)
...
async def test_apollo_out_of_credits_on_a_per_call_endpoint_pays_the_aggregator_once(clients: AsyncClient, overflow_on, monkeypatch):
    ...
    before = await _balance(clients)
    r = await clients.post(f"/call/{APOLLO_SEARCH}", json={"q_organization_name": "x"})
    assert r.status_code == 200 and r.headers["X-Olywork-Served-Via"] == "overflow:orthogonal"
...
async def test_aggregator_relaying_the_vendors_own_out_of_credits_dialect_is_the_aggregators_dry_account(
    ...
    assert r2.status_code == 422 and again == [] and "X-Olywork-Served-Via" not in r2.headers
...
async def test_apollo_validation_422_is_the_callers_and_never_overflows(clients: AsyncClient, overflow_on, monkeypatch):
    ...
    assert "X-Olywork-Served-Via" not in r.headers
...
async def test_a_call_rescued_by_overflow_is_one_ok_event_not_a_refusal(clients: AsyncClient, overflow_on, monkeypatch, posthog_events):
    ...
    assert p["charged_micro"] == 3_000 and p["call_ref"] == r.headers["X-Olywork-Call-Id"]
    await audit.drain()
...
async def test_a_call_overflow_could_not_rescue_is_still_one_refusal_event(clients: AsyncClient, overflow_on, monkeypatch, posthog_events):
    ...
    (e,) = await posthog_events()
    p = e["properties"]
    assert p["status_code"] == 503 and p["outcome"] == "olywork_refused" and p["refused_by"] == "capacity"
