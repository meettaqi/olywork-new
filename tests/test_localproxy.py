"""The local proxy (`src/olywork/localproxy.py`) — P0 (skeleton, blind tunnel) and P1 (certificates).

Everything here runs against loopback sockets; nothing reaches the internet and nothing needs a olywork
...
from olywork import localproxy as lp
...
def _proxy_connect(port: int, token: str, target: str) -> socket.socket:
    ...
    cred = base64.b64encode(f"olywork:{token}".encode()).decode()
    s.sendall(
...
def test_unreachable_upstream_is_a_readable_502(proxy):
    """A olywork-side or network failure must not read like the vendor being down."""
    ...
    assert b"olywork proxy" in head + s.recv(4096)
    s.close()
...
def test_relative_form_request_is_refused(proxy):
    ...
    cred = base64.b64encode(f"olywork:{proxy.token}".encode()).decode()
    s = socket.create_connection(("127.0.0.1", proxy.port), timeout=5)
...
def test_authorized_accepts_only_the_exact_token():
    ok = {"proxy-authorization": "Basic " + base64.b64encode(b"olywork:abc").decode()}
...
def test_no_leaf_key_is_left_on_disk(ca):
    ...
    leftovers = [p.name for p in ca.dir.iterdir() if p.name.startswith("olywork-leaf-")]
    assert leftovers == []
...
def test_proxy_env_carries_the_flags_that_matter(ca):
    env = lp.proxy_env(18791, "tok en/+", ca.bundle_path, "olywork.com")
    assert env["HTTPS_PROXY"] == "http://olywork:tok%20en%2F%2B@127.0.0.1:18791"
    ...
    assert "127.0.0.1" in env["NO_PROXY"] and "olywork.com" in env["NO_PROXY"]
...
def test_handle_env_matches_the_running_proxy(ca):
    ...
    try:
        env = handle.env("olywork.example")
...
def test_proxy_dir_follows_olywork_config(tmp_path, monkeypatch):
    """Tests and per-agent identities redirect the CLI with OLYWORK_CONFIG; the CA must follow, or a test
    run writes into the developer's real ~/.olywork."""
    monkeypatch.setenv("OLYWORK_CONFIG", str(tmp_path / "cfg" / "config.json"))
    assert lp.proxy_dir() == tmp_path / "cfg" / "proxy"
    monkeypatch.delenv("OLYWORK_CONFIG")
...
# ---- P2 · interception: the call goes to olywork, the key never comes here --------------------
class _FakeOlywork:
...
def _agent(handle, ca) -> httpx.Client:
    ...
    proxy, trusting our bundle. It has no idea olywork exists."""
    return httpx.Client(
        proxy=f"http://olywork:{handle.token}@127.0.0.1:{handle.port}",
        verify=_trusting(ca), timeout=10,
    )


def test_intercepted_call_is_re_addressed_to_olywork(intercepting, ca, olywork):
    """The heart of P2. The agent calls Stripe; the request lands on olywork's `/call/` passthrough
    carrying the member's olywork token — and no vendor key exists anywhere on this machine."""
    with _agent(intercepting, ca) as agent:
        r = agent.get("https://api.stripe.com/v1/charges?limit=3")
    assert r.status_code == 200 and r.json() == {"ok": True}

    sent = olywork.seen[0]
    assert str(sent.url) == "https://olywork.example/call/https://api.stripe.com/v1/charges?limit=3"
    assert sent.headers["x-olywork-token"] == "member-token-abc"
    assert sent.headers["x-olywork-org"] == "acme"
    assert sent.headers["x-olywork-client"] == "claude-code"


def test_method_body_and_duplicate_headers_survive(intercepting, ca, olywork):
    """Faithful relay: what the agent typed is what olywork receives."""
    with _agent(intercepting, ca) as agent:
        r = agent.request(
...
def test_the_caller_cannot_speak_as_someone_else(intercepting, ca, olywork):
    """An agent that sets its own `X-Olywork-Token` must not be able to spend another member's quota
    through our proxy. The proxy's identity is the only one that reaches olywork."""
    with _agent(intercepting, ca) as agent:
        agent.get("https://api.stripe.com/v1/charges",
                  headers={"X-Olywork-Token": "stolen", "X-Olywork-Org": "victim", "X-Olywork-Client": "liar"})
    sent = olywork.seen[0]
    assert sent.headers["x-olywork-token"] == "member-token-abc"
    assert sent.headers["x-olywork-org"] == "acme"
    assert sent.headers["x-olywork-client"] == "claude-code"
...
def test_only_allow_listed_hosts_are_decrypted(intercepting, ca, olywork):
    """Non-negotiable #3. A host that is not a registered tool is tunnelled: no certificate is signed
    for it, and olywork never hears about it."""
    echo = _EchoServer()
    try:
...
def test_a_response_streams_back_without_a_content_length(intercepting, ca, olywork):
    """olywork may answer with no length (a streamed upstream). We must re-frame it ourselves rather
    than copy framing that no longer applies to our hop."""
    async def _body():
        for part in (b"one ", b"two ", b"three"):
            yield part

    olywork.replies.append(httpx.Response(200, content=_body(), headers={"Content-Type": "text/plain"}))
    with _agent(intercepting, ca) as agent:
        r = agent.get("https://api.stripe.com/v1/charges")
    assert r.status_code == 200 and r.content == b"one two three"


def test_upstream_status_and_headers_come_back_verbatim(intercepting, ca, olywork):
    olywork.replies.append(httpx.Response(402, json={"error": "card_declined"},
...
def test_a_head_request_gets_no_body(intercepting, ca, olywork):
    olywork.replies.append(httpx.Response(200, headers={"Content-Length": "1234"}))
    with _agent(intercepting, ca) as agent:
        r = agent.head("https://api.stripe.com/v1/charges")
    assert r.status_code == 200 and r.content == b""


def test_two_calls_share_one_intercepted_connection(intercepting, ca, olywork):
    ...
    assert len(olywork.seen) == 2
    assert str(olywork.seen[1].url).endswith("/v1/customers")


def test_olywork_being_down_does_not_look_like_the_vendor_being_down(intercepting, ca, olywork):
    """An agent told '502 Bad Gateway' would blame Stripe and retry against a healthy service. The
    message has to name olywork, and say the call was not made."""
    olywork.replies.append(httpx.ConnectError("no route"))
    with _agent(intercepting, ca) as agent:
        r = agent.get("https://api.stripe.com/v1/charges")
...
def test_an_edge_waf_block_is_retried_base64(intercepting, ca, olywork):
    """Render's edge 403s a body that looks like SQL injection. The CLI already re-sends such a body
    base64-encoded under X-Olywork-Body-Encoding; an intercepted call has to do the same or a
    legitimate query fails for a reason no one can see."""
    olywork.replies.append(httpx.Response(403, html="<html>blocked</html>"))
    olywork.replies.append(httpx.Response(200, json={"ok": True}))
    with _agent(intercepting, ca) as agent:
        r = agent.post("https://api.stripe.com/v1/query", content=b"SELECT * FROM charges")
    assert r.status_code == 200
    assert len(olywork.seen) == 2
    assert olywork.seen[1].headers["x-olywork-body-encoding"] == "base64"
    assert base64.b64decode(olywork.seen[1].content) == b"SELECT * FROM charges"


def test_a_json_403_from_olywork_is_not_retried(intercepting, ca, olywork):
    """olywork's own refusals are JSON. Retrying one base64-encoded would double every denied call."""
    olywork.replies.append(httpx.Response(403, json={"detail": "you do not have access"}))
    with _agent(intercepting, ca) as agent:
        r = agent.post("https://api.stripe.com/v1/charges", content=b"x")
    assert r.status_code == 403 and len(olywork.seen) == 1


def test_a_pinned_client_fails_alone(intercepting, ca, olywork):
    ...
    with httpx.Client(proxy=f"http://olywork:{intercepting.token}@127.0.0.1:{intercepting.port}",
...
def test_interception_needs_a_credential(ca):
    """A proxy with no olywork identity must not decrypt anything: it would terminate TLS and then have
    ...
    full = lp.ProxyConfig(**base, base_url="https://x", olywork_token="tok")
    assert full.intercepts("api.stripe.com") and full.intercepts("API.STRIPE.COM")
    assert not full.intercepts("api.openai.com")
    assert not lp.ProxyConfig(token="t", base_url="https://x", olywork_token="tok",
                              hosts=frozenset({"api.stripe.com"})).intercepts("api.stripe.com")  # no CA


# ---- P2 · unit pieces ----------------------------------------------------------------------
def test_olywork_url_uses_the_passthrough_shape():
    assert lp._olywork_url("https://olywork.example", "api.stripe.com", "/v1/charges?a=1") == \
        "https://olywork.example/call/https://api.stripe.com/v1/charges?a=1"
    assert lp._olywork_url("https://olywork.example/", "api.stripe.com", "/") == \
        "https://olywork.example/call/https://api.stripe.com/"


def test_forward_headers_drops_transport_and_adds_control():
    cfg = lp.ProxyConfig(token="t", base_url="https://x", olywork_token="tok", org="acme")
...
def test_forward_headers_keeps_the_callers_encoding_choice():
    cfg = lp.ProxyConfig(token="t", base_url="https://x", olywork_token="tok")
...
def test_the_proxys_own_client_ignores_https_proxy(monkeypatch):
    """Non-negotiable #6. httpx reads HTTPS_PROXY from its own environment — inside a olywork shell that
...
async def test_the_agent_calls_the_vendor_and_olywork_injects_the_key(clients, ca):
    ...
    cfg = lp.ProxyConfig(
        token=lp.mint_token(), port=0, ca=ca,
        base_url="http://registry", olywork_token=clients.headers["X-Olywork-Token"],
    ...
    def _agent_call():
        # Deliberately a plain client with no idea olywork exists — only the two things the shell's
        # environment gives it: the proxy address and the trust bundle.
        with httpx.Client(proxy=f"http://olywork:{cfg.token}@127.0.0.1:{port}",
    ...
    # olywork's control headers are the proxy's, and the relay strips them before the vendor sees them.
    assert "x-olywork-token" not in echoed["headers"]
...
async def test_an_unregistered_host_is_a_readable_404(clients, ca):
    """P3 refines the wording; what matters already is that the answer comes from olywork and says
    ...
    cfg = lp.ProxyConfig(
        ...
        olywork_token=clients.headers["X-Olywork-Token"], hosts=frozenset({"api.nothing-here.com"}),
    )
    registry = AsyncClient(transport=ASGITransport(app=api.app), base_url="http://registry")
    server = await lp.serve(cfg, client=registry)
    port = lp.listening_port(server, 0)
...
async def test_the_allow_list_comes_from_the_members_own_tool_list(clients, ca):
    ...
    registry = AsyncClient(transport=ASGITransport(app=api.app), base_url="http://registry")
    try:
        hosts = await lp.fetch_hosts(registry, "http://registry", clients.headers["X-Olywork-Token"])
    finally:
        await registry.aclose()
...
async def test_refresh_picks_up_a_tool_registered_mid_session(olywork):
    """A tool added ten minutes into a shell session should start working without a restart."""
    cfg = lp.ProxyConfig(token="t", base_url="http://registry", olywork_token="tok")
    olywork.replies.append(httpx.Response(200, json=[{"host": "api.stripe.com"}, {"host": "API.NEW.COM"}]))
    client = olywork.client()
    try:
...
async def test_a_transient_failure_never_empties_a_working_allow_list(olywork):
    ...
    olywork.replies.append(httpx.Response(500))
    client = olywork.client()
    try:
        task = asyncio.create_task(lp.refresh_hosts_forever(cfg, client, interval=0))
...
def test_olywork_errors_say_who_refused_and_what_fixes_it():
    """A raw 404 reads as 'the vendor has no such endpoint', and an agent 'fixes' it by rewriting a
    perfectly good URL. Every message names olywork and the next action."""
    m404 = lp.olywork_error_message(404, "api.stripe.com", "no registered tool")
    assert "olywork" in m404 and "olywork tools add" in m404
    ...
    assert "ask an admin" in lp.olywork_error_message(403, "api.stripe.com", "").lower()
    assert "olywork login" in lp.olywork_error_message(401, "api.stripe.com", "")
    assert "daily call limit" in lp.olywork_error_message(429, "api.stripe.com", "")
    assert "more than one" in lp.olywork_error_message(409, "api.stripe.com", "")
    assert "418" in lp.olywork_error_message(418, "api.stripe.com", "teapot")


async def test_an_unregistered_host_is_explained_not_just_404d(clients, ca):
    """End to end: olywork marks its own refusal, the proxy turns it into an instruction."""
    from httpx import ASGITransport, AsyncClient
    ...
    cfg = lp.ProxyConfig(
        token=lp.mint_token(), port=0, ca=ca, base_url="http://registry",
        olywork_token=clients.headers["X-Olywork-Token"], hosts=frozenset({"api.unregistered.com"}),
    )
    registry = AsyncClient(transport=ASGITransport(app=api.app), base_url="http://registry")
    server = await lp.serve(cfg, client=registry)
    port = lp.listening_port(server, 0)
    ...
    assert resp.status_code == 404
    payload = resp.json()
    assert payload["source"] == "olywork"
    assert "no tool is registered for api.unregistered.com" in payload["error"]
    assert "olywork tools add" in payload["error"]


def test_a_vendors_own_error_is_never_rewritten(intercepting, ca, olywork):
    """The mapping fires only on olywork's `X-Olywork-Error` marker. A genuine 404 from the vendor, relayed
    through olywork, must reach the agent exactly as the vendor wrote it."""
    olywork.replies.append(httpx.Response(404, json={"error": {"message": "No such charge: ch_123"}}))
    with _agent(intercepting, ca) as agent:
        r = agent.get("https://api.stripe.com/v1/charges/ch_123")
    assert r.status_code == 404
    assert r.json() == {"error": {"message": "No such charge: ch_123"}}
    assert "olywork" not in r.text


def test_an_older_registry_without_the_marker_passes_through(intercepting, ca, olywork):
    """Against a registry that predates the marker nothing fires, and the reply is untouched."""
    olywork.replies.append(httpx.Response(403, json={"detail": "tool not in your access list"}))
    with _agent(intercepting, ca) as agent:
        r = agent.get("https://api.stripe.com/v1/charges")
    assert r.status_code == 403 and r.json() == {"detail": "tool not in your access list"}


def test_the_explained_error_keeps_the_connection_usable(intercepting, ca, olywork):
    """An explained refusal is still an ordinary HTTP response — the next call on the same connection
    has to work."""
    olywork.replies.append(httpx.Response(403, json={"detail": "nope"}, headers={"X-Olywork-Error": "1"}))
    with _agent(intercepting, ca) as agent:
        first = agent.get("https://api.stripe.com/v1/charges")
        second = agent.get("https://api.stripe.com/v1/customers")
    assert first.status_code == 403 and first.json()["source"] == "olywork"
    assert second.status_code == 200


# ---- the daemon's state file (`olywork serve`) --------------------------------------------------
def test_state_file_is_owner_only(tmp_path, monkeypatch):
    """`olywork shell --proxy` keeps its token in the subshell's environment; a DAEMON has to write it
    down so another terminal can find it. That file is the whole extra risk of `olywork serve`."""
    monkeypatch.setenv("OLYWORK_CONFIG", str(tmp_path / "config.json"))
    path = lp.write_state(18791, "tok", os.getpid(), "https://olywork.example", "acme", ["b.com", "a.com"])
...
def test_a_dead_daemon_does_not_look_alive(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_CONFIG", str(tmp_path / "config.json"))
    lp.write_state(18791, "tok", 999_999, "https://olywork.example", "", [])
...
def test_a_live_daemon_is_reported(tmp_path, monkeypatch):
    monkeypatch.setenv("OLYWORK_CONFIG", str(tmp_path / "config.json"))
    lp.write_state(18791, "tok", os.getpid(), "https://olywork.example", "", ["a.com"])
