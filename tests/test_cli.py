...
def _isolate_cli_config(tmp_path, monkeypatch):
    """Never let a CLI test touch the real ~/.olywork/config.json. Some commands persist config
...
def test_config_v2_roundtrip(tmp_path, monkeypatch):
    monkeypatch.setattr(cli, "CONFIG_PATH", tmp_path / "config.json")
    cli._save_config({"base_url": "https://olywork.com", "token": "T", "email": "me@x.dev",
...
def test_legacy_multiorg_config_migrates(tmp_path, monkeypatch):
    ...
    (tmp_path / "config.json").write_text(json.dumps({
        "base_url": "https://olywork.com", "active_org": "team-a",
...
def test_load_config_defaults_to_production_not_localhost(tmp_path, monkeypatch):
    """A fresh CLI (no ~/.olywork/config.json) must default to https://olywork.com, NOT localhost.
    The first-run bug was: install.sh's `olywork config --base-url` failed silently, so login
...
def test_is_loopback_url_detects_localhost_variants():
    ...
    assert cli._is_loopback_url("https://olywork.com") is False
...
def test_login_exits_with_helpful_message_when_localhost_unreachable(monkeypatch):
    ...
    assert "olywork config --base-url" in error_msg
...
def test_pick_active_org_prefers_the_tokens_baked_org(monkeypatch):
    ...
    arbitrary other team (`olywork login --token <olywork key>` used to activate `oauth-test`)."""
    import base64
...
def test_client_sends_token_and_active_org(monkeypatch):
    ...
    with cli._client(cfg) as c:
        assert c.headers["X-Olywork-Token"] == "TK" and c.headers["X-Olywork-Org"] == "team-a"
...
def test_admin_client_prefers_admin_token():
    ...
    with cli._admin_client(cfg) as c:
        assert c.headers["X-Olywork-Token"] == "ENV"
    del cfg["admin_token"]
    with cli._admin_client(cfg) as c:
        assert c.headers["X-Olywork-Token"] == "USER"
...
def test_call_sends_explicit_authorization_method_as_olywork_control_header(monkeypatch):
    fake = _FakeClient()
    monkeypatch.setattr(cli, "_client", lambda cfg: fake)
    monkeypatch.setattr(cli, "_show", lambda r: None)
    ...
    cli.cmd_call(args, {"base_url": "http://x"})
    assert fake.calls[0][4]["X-Olywork-Authorization-Method"] == "delegated-admin"
...
def test_secret_add_env_var_parses_and_strips_quotes(tmp_path, monkeypatch):
    """`secret add --env-var` reads ONE var from an .env via olywork's parser — a quoted value
...
def test_scan_upload_import_verbs():
    """`olywork scan` is the read-only preview (forced dry_run, no prompts); `olywork upload` is the real
    thing; `olywork import` stays a working alias of upload (old docs/scripts must not break)."""
    p = cli.build_parser()
    s = p.parse_args(["scan"])
...
def test_onboard_setup_source_picks_scan_dirs(tmp_path, monkeypatch):
    """`olywork onboard --path setup --source global` scans the machine-wide agent skill folders
    ...
    from olywork import agents as ag
...
def test_only_resolvable_gaps():
    ...
    assert not cli._only_resolvable_gaps(mk(["olywork.json secret file missing: token.json"]))    # file gap → not
...
def test_load_catalog_prefers_newer_bundled_over_older_server(tmp_path, monkeypatch):
    ...
    from olywork import providers as prov
    monkeypatch.setattr(cli, "CONFIG_PATH", tmp_path / "config.json")
...
def test_subcommand_help_is_not_the_grouped_top_level_page():
    """add_subparsers clones the PARENT's class by default, which would make every `olywork X -h`
    ...
    for name in ("cli", "call", "connections", "audit", "tool"):
        ...
        assert sub_help.startswith(f"usage: olywork {name}"), name
...
def test_catalog_verbs_parse_without_displacing_a_platform_slug():
    """`search`/`get` are positional verbs, not subparsers — so `olywork catalog tiktok` still browses
...
_SEARCH_BODY = {
    ...
    "hints": ["olywork catalog get justoneapi.tiktok.video.comments   # params, cost, example response"],
}
...
def test_catalog_search_table_prices_in_usd_and_points_at_get(monkeypatch, capsys):
    ...
    assert "olywork catalog get justoneapi.tiktok.video.comments" in out
...
def test_catalog_get_renders_params_siblings_and_the_command(monkeypatch, capsys):
    body = {
        ...
        "siblings": [{"id": "tikhub.tiktok.video.comments", "provider": "tikhub", "provider_display": "TikHub",
                      ...
                      "verified": "2026-07-28", "docs_url": "", "has_example": True, "input": None}],
        "call_template": "olywork call justoneapi /api/tiktok/get-post-comment/v1 --query awemeId=76662",
    ...
    assert "olywork call justoneapi /api/tiktok/get-post-comment/v1 --query awemeId=76662" in out
...
def test_catalog_get_needs_an_id_and_404s_helpfully(monkeypatch, capsys):
    ...
    assert "find one with: olywork catalog search" in capsys.readouterr().out
...
def test_onboard_catalog_calls_when_olywork_serves_it(monkeypatch, capsys):
    """The happy path: olywork's own key covers it, so the price is stated and the call is made."""
    c = _onboard_catalog(monkeypatch, {"tier": "platform", "estimated_cost_usd": 0.001})
    out = capsys.readouterr().out
    assert "on olywork's key" in out and "$0.001" in out
    assert "GET /call/tikhub.tiktok.user.profile" in c.seen      # it really called
    assert "olywork balance" in out                                  # and showed what it cost
...
def test_onboard_catalog_says_not_metered_for_your_own_key(monkeypatch, capsys):
    ...
    assert "olywork balance" not in out          # nothing was spent, so don't show a balance


def test_onboard_catalog_dead_end_names_the_one_command_that_fixes_it(monkeypatch, capsys):
    """Before platform providers are switched on (OLYWORK_PLATFORM_PROVIDERS empty) this is what every
    ...
    out = capsys.readouterr().out
    assert "olywork connections connect --provider tikhub" in out
...
def test_show_prints_failure_diagnostics_on_stderr_and_keeps_stdout_the_body(capsys):
    ...
    relayed = httpx.Response(403, content=body, headers={
        "content-type": "application/json", "X-Olywork-Call-Id": "c2075d8d79eb4436aafc310e81081c1b"})
    with pytest.raises(SystemExit):
        cli._show(relayed)
    ...
    refused = httpx.Response(402, content=b'{"detail":{"error":"route_max_cost"}}', headers={
        "content-type": "application/json", "X-Olywork-Error": "1"})
    with pytest.raises(SystemExit):
        cli._show(refused)
    _, err = capsys.readouterr()
    assert "HTTP 402" in err and "olywork refused" in err
...
def test_show_prints_the_charge_and_call_id_for_a_metered_success(capsys):
    ...
    metered = httpx.Response(200, content=b'{"results":[{"x":1}],"next_token":"abc"}', headers={
        "content-type": "application/json", "X-Olywork-Cost-Micro": "6667",
        "X-Olywork-Call-Id": "125117d50cd3470cb25ba11f3d9789ad"})
    cli._show(metered)
    out, err = capsys.readouterr()
    assert json.loads(out) == {"results": [{"x": 1}], "next_token": "abc"}
    assert err.strip() == "olywork: charged $0.006667 · call id 125117d50cd3470cb25ba11f3d9789ad"

    own_key = httpx.Response(200, content=b'{"ok":true}', headers={
        "content-type": "application/json", "X-Olywork-Call-Id": "deadbeef"})
    cli._show(own_key)
    _, err = capsys.readouterr()
    assert err == ""
    ...
    replay = httpx.Response(200, content=b'{}', headers={
        "content-type": "application/json", "X-Olywork-Cost-Micro": "6667",
        "X-Olywork-Idempotent-Replay": "true", "X-Olywork-Call-Id": "c1"})
    cli._show(replay)
    _, err = capsys.readouterr()
