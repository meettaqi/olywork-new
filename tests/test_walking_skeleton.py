...
async def test_dashboard_served_at_root(clients: AsyncClient):
    ...
    r = await clients.get("/")
    assert r.status_code == 200
    # The product is branded `olywork` everywhere since the rename; asserting the old name left main
    # red and every PR failing CI behind it.
    assert "olywork" in r.text and "Sign in" in r.text
    r = await clients.get("/app")
    assert r.status_code == 200
    # the app root div may carry extra attributes (e.g. v-cloak) — match the id, not the exact tag
    assert "olywork" in r.text and 'id="app"' in r.text
...
async def test_meta_reports_the_released_version(clients: AsyncClient):
    ...
    assert body["olywork_version"], "a release check must be able to read the version from outside"
    assert body["olywork_version"] != body["app_version"], "these are different questions"
    assert body["olywork_version"][0].isdigit(), f"expected a version, got {body['olywork_version']!r}"
...
def test_migrations_fail_fast_rather_than_queueing_the_world():
    ...
    env_src = files("olywork").joinpath("alembic", "env.py").read_text()
