...
The data lives in src/olywork/catalog/*.yaml (see docs/context/architecture/catalog.md). These tests
assert the API CONTRACT the dashboard and `olywork catalog` are written against, plus the two things
...
async def test_every_endpoint_carries_a_domain_and_a_call_line(clients: AsyncClient):
    """The domain decides which section a row files under, and the `olywork call` line is the reason
    ...
    for slug in ("tiktok", "web"):
        ...
        for e in eps:
            ...
            assert e["call_template"].startswith(f"olywork call {e['id']}")
...
async def test_search_finds_the_job_across_providers_best_first(clients: AsyncClient):
    ...
    assert top[0] == "olywork.tiktok.video.comments", "the routed endpoint for the job comes first"
    assert set(top[1:]) == {
    ...
    first = rows[1]  # the first CHILD; rows[0] is the generated olywork.* row, whose provider is olywork itself
    ...
    assert any(h.startswith(f"olywork catalog get {rows[0]['id']}") or h.startswith(f"olywork catalog get {first['id']}") for h in body["hints"])
...
async def test_endpoint_detail_answers_everything_in_one_call(clients: AsyncClient):
    ...
    assert tmpl.startswith("olywork call justoneapi.tiktok.video.comments")
...
async def test_call_template_carries_method_and_body_for_a_post(clients: AsyncClient):
    ...
    tmpl = body["call_template"]
    assert tmpl.startswith("olywork call dataforseo.web.backlinks.summary --method POST")
...
async def test_platform_eligibility_refuses_everything_it_cannot_prove():
    """The predicate behind spending olywork's own key. Asymmetric on purpose: an unknown price must
    ...
    # eligible one would put olywork's own key behind a route the provider has already removed.
    ...
    # A plan-gated route works upstream but olywork's own subscription cannot serve it — a customer
...
def test_call_template_falls_back_to_documented_examples(tmp_path):
    ...
    assert cs.call_template(ep) == "olywork call moz.web.thing --query site=moz.com --query 'limit=<integer>'"
...
def test_gtm_catalog_paths_and_declared_parameters_are_the_same_contract():
    ...
    ``accounts/…/containers/…`` through one olywork placeholder encodes the hierarchy as ``%2F``.
...
def test_an_unpriced_route_is_still_refused():
    """The original rule, unchanged: no usd → refuse, so olywork never pays a provider and charges $0."""
    from olywork.domain.catalog import store as cs
    cat = cs.load()
...
# ---- shared-plan rates: a price olywork SET must say so, everywhere ----------------------------
...
def test_every_shared_plan_rate_prints_its_fee_and_break_even():
    """A `kind: olywork_shared_plan` entry is the catalog asserting OUR OWN price for a flat-fee
...
def test_the_shared_plan_check_actually_BITES(tmp_path):
    ...
    bad = """
credit_rates_usd:
  no_price:   {usd: null, kind: olywork_shared_plan, basis: "olywork shared-plan rate. $10/mo. break-even at 1,000 calls/mo", source: "x", checked: "2026-08-14"}
  no_fee:     {usd: 0.001, kind: olywork_shared_plan, basis: "olywork shared-plan rate. break-even at 1,000 calls/mo", source: "x", checked: "2026-08-14"}
  no_breakeven: {usd: 0.001, kind: olywork_shared_plan, basis: "olywork shared-plan rate. Vendor sells flat $10/mo", source: "x", checked: "2026-08-14"}
  wrong_start: {usd: 0.001, kind: olywork_shared_plan, basis: "Vendor sells flat $10/mo; break-even at 1,000 calls/mo", source: "x", checked: "2026-08-14"}
  typo_kind:  {usd: 0.001, kind: olywork_shared_pla, basis: "whatever", source: "x", checked: "2026-08-14"}
  prose_only: {usd: 0.001, basis: "olywork shared-plan rate without the marker", source: "x", checked: "2026-08-14"}
...
def test_a_shared_plan_rate_converts_like_any_credit():
    ...
    call on olywork's shared plan. cost_view needs ZERO changes — this asserts the pilot entry flows
...
def test_the_trial_kind_check_actually_BITES(tmp_path):
    """Every dishonest olywork_trial shape refused by the REAL check: a non-zero 'trial', a zero with
    ...
    bad = """
credit_rates_usd:
  nonzero:  {usd: 0.001, kind: olywork_trial, trial_calls_per_team_day: 20, basis: "olywork trial rate. x", source: "x", checked: "2026-08-15"}
  no_cap:   {usd: 0, kind: olywork_trial, basis: "olywork trial rate. x", source: "x", checked: "2026-08-15"}
  bad_cap:  {usd: 0, kind: olywork_trial, trial_calls_per_team_day: 0, basis: "olywork trial rate. x", source: "x", checked: "2026-08-15"}
  wrong_basis: {usd: 0, kind: olywork_trial, trial_calls_per_team_day: 20, basis: "free on our key", source: "x", checked: "2026-08-15"}
  fine_trial: {usd: 0, kind: olywork_trial, trial_calls_per_team_day: 20, basis: "olywork trial rate. Served at $0 on olywork's free key", source: "x", checked: "2026-08-15"}
"""
    (tmp_path / "fx.yaml").write_text(bad)
    original = cv.CATALOG
    try:
...
# ---- method metadata: an endpoint olywork enforces the wrong verb on cannot be called at all -------
def test_no_tikhub_ads_route_is_recorded_as_a_GET():
    """olywork enforces the recorded method, so a wrong one is not a cosmetic error — it makes the
...
async def test_search_pulls_the_routed_parent_in_when_a_child_matches(clients: AsyncClient):
    ...
    assert "olywork.people.email.find" in ids, ids
    assert ids.index("olywork.people.email.find") < ids.index("leadsforge.people.email.find")
    rows = (await clients.get("/catalog/search", params={"q": "find leads"})).json()["results"]
    assert rows[0]["id"] == "olywork.people.search", [r["id"] for r in rows[:3]]
...
async def test_search_caps_a_routed_group_at_a_few_children(clients: AsyncClient):
    ...
    rows = (await clients.get("/catalog/search", params={"q": "find leads"})).json()["results"]
    parent = next(r for r in rows if r["id"] == "olywork.people.search")
    kids = [r for r in rows if r["capability"] == "people.search" and r.get("kind") != "routed"]
    assert len(kids) <= 5 and parent["children_hidden"] >= 1
    assert "olywork.people.email.find" in {r["id"] for r in rows}, "the next job fits on the page now"
