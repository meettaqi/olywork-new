"""The universal /login page + /auth/cli/approve — `olywork login` opens /login?cli=<id>, which
...
from olywork.models import Membership, Org, Tool, User
...
async def web(monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_GOOGLE_CLIENT_ID", "")  # explicit: a dev's .env may configure Google
...
async def test_cli_orgs_sorts_team_before_personal(web):
    ...
    async with session_maker() as s:
        o = Org(name="Olywork", slug="olywork"); s.add(o); await s.flush()
    ...
    assert [o["slug"] for o in orgs] == ["olywork", "dev-personal"]
...
async def test_email_door_completes_the_handshake(web):
    ...
    code = start.json()["dev_code"]  # OLYWORK_EMAIL_DEV_MODE=true in conftest
    verify = await web.post("/auth/email/verify", json={"email": "new@x.dev", "code": code})
    assert verify.status_code == 200 and web.cookies.get("olywork_session")  # verify set the session
    lid, code = await _start(web)
    r = await web.post("/auth/cli/approve", json={"login_id": lid, "code": code})
    assert r.status_code == 200
    d = (await web.get(f"/auth/cli/poll?login_id={lid}")).json()
...
async def test_login_page_confirms_a_fragment_code_instead_of_typing(web):
    """`olywork login` puts the code in the URL fragment; the page's JS swaps the typed input for a
...
async def test_wrong_code_attempts_are_capped(web):
    ...
    from olywork.routers.auth import CLI_APPROVE_MAX_TRIES
    uid = await _seed_user()
    web.cookies.set("olywork_session", sess.make_session(uid))
    lid, code = await _start(web)
