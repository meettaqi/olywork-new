...
tests; enforcement is verified live on macOS in the run path. See src/olywork/fsjail.py."""

from __future__ import annotations

from olywork import fsjail


def test_macos_profile_denies_writes_except_scratch_and_dev():
    p = fsjail.macos_profile("/tmp/olywork-fsjail-abc")
    assert p.startswith("(version 1)")
    assert "(allow default)" in p              # reads/exec/net stay open
    assert "(deny file-write*)" in p
    assert '(allow file-write* (subpath "/tmp/olywork-fsjail-abc"))' in p   # the private scratch
