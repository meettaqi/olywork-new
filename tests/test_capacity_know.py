...
from olywork.domain.capacity.policy import (
    AGGREGATORS, LatestState, default_policy, ensure_policies, latest_state, policy_population,
)
from olywork.domain.capacity.sweep import STATE_NS, run_sweep, snapshot_from
from olywork.domain.capacity.view import LatestStateView
from olywork.models import CapacityPolicy, CapacitySnapshot
...
async def test_import_creates_one_policy_per_account_and_flags_unknowns_without_overwriting():
    ...
    async with session_maker() as db:
        db.add(CapacityPolicy(provider="findymail", capacity_type="credits", funding_mode="manual",
                              source="api", owner_email="taqi@example.com"))
        await db.commit()
    async with session_maker() as db:
    ...
    assert rows["findymail"].owner_email == "taqi@example.com"  # the hand-edited row survived
...
async def test_sweep_one_failing_collector_does_not_stop_the_others(monkeypatch):
    await reset_db()
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_FINDYMAIL", "k1")
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_LEADSFORGE", "k2")
