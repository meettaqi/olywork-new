...
from olywork import crypto, oauth
from olywork.application import connect as connect_use_cases
from olywork.application.connect import _backfill_provider_extra_tools
from olywork.config import get_settings
from olywork.infra.db import session_maker
from olywork.models import Secret, Tool
...
@pytest.fixture
def olywork_google_app(monkeypatch):
    monkeypatch.setenv("OLYWORK_GOOGLE_CLIENT_ID", "olywork-google-cid")
    monkeypatch.setenv("OLYWORK_GOOGLE_CLIENT_SECRET", "olywork-google-csec")
    ...
    monkeypatch.setenv("OLYWORK_GOOGLE_ADS_CLIENT_ID", "olywork-ads-cid")
    monkeypatch.setenv("OLYWORK_GOOGLE_ADS_CLIENT_SECRET", "olywork-ads-csec")
...
def test_refreshable_credentials_are_always_fresh():
    """olywork mints a new access token on demand, so a short expiry is an implementation detail —
...
# ---- auto-provisioning -------------------------------------------------------------------
async def test_registry_connect_autoprovisions_a_callable_tool(clients: AsyncClient, olywork_google_app):
...
async def test_reconnect_rebinds_instead_of_duplicating(clients: AsyncClient, olywork_google_app):
...
async def test_byo_connect_provisions_no_tool(clients: AsyncClient):
    """Without a registry provider olywork doesn't know the upstream, so it must not invent one."""
    await _connect_byo(clients)
    assert (await clients.get("/tools")).json() == []


# ---- resource selection + revoke ---------------------------------------------------------
async def test_set_and_read_back_the_selected_resource(clients: AsyncClient, olywork_google_app):
...
async def test_another_orgs_connection_is_not_reachable(clients: AsyncClient):
    ...
    hdr = {"X-Olywork-Token": other.json()["token"]}
    r = await clients.get(f"/connections/{st['secret_id']}/resources", headers=hdr)
    assert r.status_code == 404


# ---- the chosen resource must be human-readable -------------------------------------------
async def test_selecting_a_resource_stores_its_readable_name(clients: AsyncClient, olywork_google_app):
    ...
    r = await clients.post(f"/connections/{sid}/resource", json={
        "resource_ref": "properties/384078430", "resource_name": "ai-taqi.com",
    })
    assert r.status_code == 200
    assert r.json()["resource_name"] == "ai-taqi.com"
    conns = {c["id"]: c for c in (await clients.get("/connections")).json()}
    assert conns[sid]["resource_name"] == "ai-taqi.com"
    assert conns[sid]["resource_ref"] == "properties/384078430"  # the id is still what we call with


async def test_discovery_backfills_a_missing_resource_name(clients: AsyncClient, olywork_google_app, monkeypatch):
...
async def test_successful_discovery_marks_the_connection_working(clients: AsyncClient, olywork_google_app, monkeypatch):
...
@pytest.fixture
def olywork_meta_app(monkeypatch):
    monkeypatch.setenv("OLYWORK_META_CLIENT_ID", "olywork-meta-cid")
    monkeypatch.setenv("OLYWORK_META_CLIENT_SECRET", "olywork-meta-csec")
    monkeypatch.setenv("OLYWORK_INSTAGRAM_CLIENT_ID", "olywork-instagram-cid")
    monkeypatch.setenv("OLYWORK_INSTAGRAM_CLIENT_SECRET", "olywork-instagram-csec")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
async def test_business_owned_pages_join_the_facebook_picker(clients: AsyncClient, olywork_meta_app, monkeypatch):
...
async def test_business_owned_instagram_accounts_join_the_picker(clients: AsyncClient, olywork_meta_app, monkeypatch):
...
async def test_instagram_selection_stores_and_binds_the_linked_page_token(
    clients: AsyncClient, olywork_meta_app, monkeypatch, ig_id, expected_page, expected_token,
):
    """The caller chooses an Instagram id, while Olywork privately derives the linked Page token.
...
async def test_instagram_selection_subscription_failure_is_atomic(
    clients: AsyncClient, olywork_meta_app, monkeypatch,
):
    _meta_test_provider(
...
async def test_a_failing_business_walk_leaves_the_primary_listing_intact(clients: AsyncClient, olywork_meta_app, monkeypatch):
...
async def test_revoke_removes_the_provider_tool_it_provisioned(clients: AsyncClient, olywork_google_app):
...
# ---- olywork's own credentials, not the user's ------------------------------------------------
async def test_platform_credential_is_bound_from_settings_not_an_org_secret(
    clients: AsyncClient, olywork_google_app, monkeypatch
):
    """Google Ads needs a developer token that takes WEEKS of Google approval to obtain. Making
    each user supply their own would defeat the point of a hosted registry — so olywork supplies it,
    and it must never land in the tenant's secret store where they could read or extract it."""
    from olywork import oauth_providers as P
    from olywork.config import get_settings

    monkeypatch.setenv("OLYWORK_GOOGLE_ADS_DEVELOPER_TOKEN", "olywork-dev-token")
    get_settings.cache_clear()
    try:
        assert P.GOOGLE_ADS.can_autoprovision, "with olywork's token, Ads needs nothing from the user"
        ...
        assert not any("developer-token" in n for n in names), \
            "olywork's credential must not be copied into the org's secrets"
    finally:
        get_settings.cache_clear()


async def test_without_a_platform_token_the_user_is_asked(clients: AsyncClient, olywork_google_app, monkeypatch):
    """A self-hosted deployment with no developer token of its own falls back to prompting."""
    from olywork import oauth_providers as P
    from olywork.config import get_settings

    monkeypatch.setenv("OLYWORK_GOOGLE_ADS_DEVELOPER_TOKEN", "")
    get_settings.cache_clear()
    try:
...
async def test_id_only_listings_are_enriched_with_real_names(clients: AsyncClient, olywork_google_app, monkeypatch):
...
async def test_a_failed_name_lookup_keeps_the_row(clients: AsyncClient, olywork_google_app, monkeypatch):
...
async def test_widening_access_upgrades_the_connection_it_targets(clients: AsyncClient, olywork_google_app):
...
async def test_a_second_account_is_added_not_swapped(clients: AsyncClient, olywork_google_app):
...
async def test_a_second_account_gets_its_own_tool(clients: AsyncClient, olywork_google_app):
...
async def test_reconnect_cannot_be_aimed_at_another_provider(clients: AsyncClient, olywork_google_app):
...
async def test_enabling_write_keeps_read(clients: AsyncClient, olywork_google_app):
...
async def test_reconnect_rebinds_the_tool_to_the_same_secret(clients: AsyncClient, olywork_google_app):
    first = await _connect_byo(clients, provider="google-search-console", capability="read",
...
async def test_identity_providers_record_who_connected(clients: AsyncClient, olywork_google_app, monkeypatch):
...
async def test_a_failed_identity_lookup_still_connects(clients: AsyncClient, olywork_google_app, monkeypatch):
...
async def test_oauth_providers_reject_the_token_endpoint(clients: AsyncClient, olywork_google_app):
...
def test_slack_is_offerable_without_deployment_credentials():
    """The user brings their own bot, so olywork needs no Slack app of its own — it must not show
...
async def test_slack_style_ok_false_is_reported_not_swallowed(clients: AsyncClient, monkeypatch):
    ...
    async with session_maker() as db:
        ...
        from olywork import crypto as _crypto
        from olywork.models import Secret as _S
...
async def test_split_host_connect_provisions_the_admin_tool_too(clients: AsyncClient, olywork_google_app):
...
async def test_split_host_reconnect_rebinds_extras_without_duplicating(clients: AsyncClient, olywork_google_app):
...
async def test_upgrade_backfills_missing_split_host_tool_idempotently(
        clients: AsyncClient, olywork_google_app):
...
async def test_revoke_removes_the_extra_tool_too(clients: AsyncClient, olywork_google_app):
...
async def test_pick_resource_stamps_ready_made_example(clients: AsyncClient, olywork_google_app):
    """The pick is the moment olywork finally knows the property id every runReport needs — render it
...
async def test_pick_resource_without_template_changes_no_examples(clients: AsyncClient, olywork_google_app):
