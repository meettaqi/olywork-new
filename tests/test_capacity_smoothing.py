"""Step D′ of docs/PROVIDER-CAPACITY-PLAN.md — burst smoothing on olywork's own platform keys:
...
from olywork.infra.upstream.limiter import Limiter, limiter
...
async def test_burst_429_with_a_short_retry_after_is_re_sent_once_on_the_same_hold(
    ...
    assert r.headers["X-Olywork-Smoothed"] == "retry=1"
...
async def test_still_429_after_the_retry_is_relayed_as_is(clients: AsyncClient, platform_on, monkeypatch):
    ...
    assert r.headers["X-Olywork-Cost-Micro"] == "0"
...
async def test_no_retry_for_a_post_a_long_retry_after_or_a_quota_429(clients: AsyncClient, platform_on, monkeypatch):
    ...
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 429 and len(seen) == 1 and "retry=" not in r.headers.get("X-Olywork-Smoothed", "")
...
async def test_concurrent_platform_calls_over_the_limit_relay_no_429_and_hold_no_db(
    ...
    waits = sorted(int(r.headers["X-Olywork-Smoothed"].split("=")[1]) for r in rs if "X-Olywork-Smoothed" in r.headers)
...
async def test_a_smoothed_call_holds_no_db_connection_while_it_waits(clients: AsyncClient, platform_on, monkeypatch):
    ...
    assert r.status_code == 200 and r.headers["X-Olywork-Smoothed"].startswith("wait=")
    elapsed_ms = int((time.monotonic() - t_drained) * 1000)
    assert int(r.headers["X-Olywork-Smoothed"].split("=")[1]) >= 300, f"setup took {elapsed_ms} ms"
