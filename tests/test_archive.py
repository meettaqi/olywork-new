...
from olywork import archive, audit
from olywork.application.call import service as call_service
from olywork.archive import cache_key, content_hash, policy, storable
from olywork.models import ArchiveKey, ArchiveSnapshot
...
def test_key_ignores_caller_noise_headers():
    ...
    noisy = cache_key("GET", "p.e", "https://api.x/q?a=1", headers={
        ...
        "X-Olywork-Token": "t", "Accept-Encoding": "gzip", "traceparent": "00-x",
    })
    assert quiet == noisy
...
async def test_admin_archive_report(clients: AsyncClient, shadow, monkeypatch):
    monkeypatch.setenv("OLYWORK_ADMIN_TOKEN", "ADM-TOKEN")
    get_settings.cache_clear()
    try:
        monkeypatch.setattr(get_settings(), "archive_mode", "shadow")
        monkeypatch.setitem(catalog_store.load().by_id[EP], "cache", "transient")
        ...
        r = await clients.get("/admin/archive", headers={"X-Olywork-Token": "ADM-TOKEN"})
        assert r.status_code == 200, r.text
        d = r.json()
...
async def test_max_age_zero_forces_live(clients: AsyncClient, serve):
    await clients.get(f"/call/{EP}?aweme_id=7")
    await archive.drain()
    r = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Max-Age": "0"})
    assert r.status_code == 200 and "x-olywork-cache" not in r.headers
...
async def test_refresh_worker_refreshes_a_due_demanded_key(clients: AsyncClient, serve):
    ...
    # olywork's platform key rode the provider's own injection shape (tikhub: Bearer header).
...
async def test_admin_archive_keys_endpoint(clients: AsyncClient, serve, monkeypatch):
    ...
    try:
        ...
        r = await clients.get(f"/admin/archive/keys?endpoint_id={EP}",
                              headers={"X-Olywork-Token": "ADM-TOKEN"})
        assert r.status_code == 200, r.text
        d = r.json()
...
async def test_archive_panel_page_serves(clients: AsyncClient):
    ...
    assert r.status_code == 200
    assert "Archive" in r.text and "OLYWORK_ADMIN_TOKEN" in r.text  # the shell + its token gate
...
async def test_a_result_is_scoped_to_the_team(clients: AsyncClient, shadow):
    ...
    denied = await clients.get(f"/calls/{row['id']}/result", headers={"X-Olywork-Token": outsider})
...
async def test_endpoint_stats_match_direct_aggregation(clients: AsyncClient, shadow, monkeypatch):
    ...
    from olywork.models import ArchiveEndpointStat
    async with session_maker() as s:
...
async def test_big_bodies_compress_and_serve_back_identical(clients: AsyncClient, serve, monkeypatch):
    ...
    assert r2.headers.get("x-olywork-cache") == "hit" and r2.content == big == r1.content
...
async def test_locked_key_refreshes_identity_map_state(clients: AsyncClient, shadow):
    ...
    from olywork.infra.db import background_session_maker
