"""Shell mode (`olywork shell`) — the MVP / Phase 1 transparent CLI interception.
...
def test_shim_script_is_well_formed():
    s = shell.shim_script("stripe-tool", "/usr/local/bin/olywork")
    assert s.startswith("#!/bin/sh\n")
    # runs on the CLEAN PATH so `olywork run` finds the real bin, never this shim (loop avoidance)
    assert 'exec env PATH="$OLYWORK_SHELL_REALPATH"' in s
    # the literal `--` fences olywork's parsing from the user's args, which _run_local then strips
    assert "run stripe-tool -- \"$@\"" in s


def test_shim_script_server_route_adds_the_tier_flag():
    s = shell.shim_script("stripe-tool", "/usr/local/bin/olywork", route="server")
    # the tier flag goes BEFORE the tool name (olywork's own rule)
    assert "run --server stripe-tool -- \"$@\"" in s


def test_shim_execs_olywork_run_with_clean_path_and_verbatim_args(tmp_path):
    """The heart of it: the shim must call `olywork run <tool> -- <args>` verbatim, on the clean PATH,
    and hand back the CLI's exit code. Uses a fake `olywork` that records its argv + PATH, then exits 7."""
    argv_out, path_out = tmp_path / "argv.out", tmp_path / "path.out"
    fake_olywork = tmp_path / "fakeolywork"
    fake_olywork.write_text(
        "#!/bin/sh\n"
    ...
    fake_olywork.chmod(0o755)

    shim_dir = tmp_path / "bin"
    shim_dir.mkdir()
    shell.write_shims(str(shim_dir), [("stripe", "stripe-tool", "local")], str(fake_olywork))
    assert stat.S_IMODE((shim_dir / "stripe").stat().st_mode) == 0o755

    env = dict(os.environ, OLYWORK_SHELL_REALPATH="/clean/only", PATH="/usr/bin:/bin")
...
def test_server_route_shim_execs_olywork_run_server(tmp_path):
    """A server-routed shim must call `olywork run --server <tool>` through a real shell."""
    ...
    shim_dir = tmp_path / "bin"
    shim_dir.mkdir()
    shell.write_shims(str(shim_dir), [("stripe", "stripe-tool", "server")], str(fake_olywork))
    env = dict(os.environ, OLYWORK_SHELL_REALPATH="/usr/bin:/bin")
...
def test_shim_script_bypasses_completion_to_the_real_bin():
    s = shell.shim_script("gh", "/usr/local/bin/olywork", real_bin="/opt/homebrew/bin/gh")
    # cobra completion (__complete*) execs the real bin directly — never olywork
    assert 'case "$1" in __complete*) exec /opt/homebrew/bin/gh "$@" ;; esac' in s
    # a normal invocation still routes through olywork run
    assert "run gh -- \"$@\"" in s


def test_completion_call_bypasses_olywork_real_bin_runs(tmp_path):
    """Through a real shell: `gh __complete …` execs the real bin directly (no olywork → no audit/cap),
    while `gh <normal>` still routes through olywork run."""
    real_log, olywork_log = tmp_path / "real.out", tmp_path / "olywork.out"
    real_bin = tmp_path / "realgh"
    real_bin.write_text(f'#!/bin/sh\nprintf "REAL %s\\n" "$*" >> {real_log}\n')
    real_bin.chmod(0o755)
    fake_olywork = tmp_path / "fakeolywork"
    fake_olywork.write_text(f'#!/bin/sh\nprintf "OLYWORK %s\\n" "$*" >> {olywork_log}\n')
    fake_olywork.chmod(0o755)

    shim = tmp_path / "bin" / "gh"
    shim.parent.mkdir()
    shim.write_text(shell.shim_script("gh", str(fake_olywork), real_bin=str(real_bin)))
    shim.chmod(0o755)
    env = dict(os.environ, OLYWORK_SHELL_REALPATH="/usr/bin:/bin")

    subprocess.run([str(shim), "__complete", "sta"], env=env)   # completion → real bin
    subprocess.run([str(shim), "repo", "list"], env=env)        # normal → olywork run
    assert real_log.read_text() == "REAL __complete sta\n"      # completion never hit olywork
    assert olywork_log.read_text() == "OLYWORK run gh -- repo list\n"  # normal did
...
def test_registered_bin_resolves_shim_in_a_real_shell(tmp_path):
    ...
    env = dict(os.environ, PATH=f"{shim_dir}{os.pathsep}{realpath}", OLYWORK_SHELL_REALPATH=realpath)
...
def test_start_session_wires_path_and_tears_down(tmp_path, monkeypatch):
    ...
    rc = shell.start_session([("stripe", "stripe", "local")], "/usr/local/bin/olywork", ttl_minutes=30)
...
def test_stop_outside_a_session_exits(monkeypatch):
    monkeypatch.delenv("OLYWORK_SHELL", raising=False)
...
def test_stop_signals_the_controller(monkeypatch):
    sent: dict = {}
    monkeypatch.setenv("OLYWORK_SHELL", "1")
    monkeypatch.setenv("OLYWORK_SHELL_PID", "4242")
...
def test_start_session_publishes_the_proxy_env_and_stops_it(tmp_path, monkeypatch):
    ...
    shell.start_session(
        [("stripe", "stripe", "local")], "/usr/local/bin/olywork",
        extra_env={"HTTPS_PROXY": "http://olywork:tok@127.0.0.1:18791", "SSL_CERT_FILE": "/x/ca-bundle.pem"},
        on_close=lambda: stopped.append(True),
        captured_hosts=["api.stripe.com"],
    )
    assert captured["env"]["HTTPS_PROXY"] == "http://olywork:tok@127.0.0.1:18791"
    assert captured["env"]["SSL_CERT_FILE"] == "/x/ca-bundle.pem"
    assert stopped == [True]


def test_extra_env_cannot_break_the_shims(tmp_path, monkeypatch):
    """Applied AFTER our own variables: an add-on that set PATH or a OLYWORK_SHELL* marker would stop
    ...
    shell.start_session([("stripe", "stripe", "local")], "/usr/local/bin/olywork",
...
def test_cmd_shell_start_seeds_the_allow_list_from_the_tool_listing(monkeypatch, tmp_path):
    ...
    from olywork import localproxy as lpx
    ...
    class _Handle:
        def env(self, olywork_host):
            seen["olywork_host"] = olywork_host
    ...
    env, stop, hosts = cli._start_local_proxy(args, {"base_url": "https://olywork.example", "token": "tok",
    ...
    assert seen["cfg"].olywork_token == "tok" and seen["cfg"].org == "acme"
    assert seen["cfg"].base_url == "https://olywork.example"
    assert seen["olywork_host"] == "olywork.example"                # the registry never comes back through us
...
# ---- olywork serve (the daemon front door) -----------------------------------------------------
def test_serve_env_prints_shell_lines_and_the_way_back(tmp_path):
    ...
    way out, because `olywork serve stop` cannot reach into a shell that already has the variables."""
    env = {"HTTPS_PROXY": "http://olywork:a b@127.0.0.1:18791", "SSL_CERT_FILE": "/x/ca.pem"}
    lines = cli._serve_export_lines(env)
    assert "export SSL_CERT_FILE=/x/ca.pem" in lines
    assert "export HTTPS_PROXY='http://olywork:a b@127.0.0.1:18791'" in lines   # quoted: the token is opaque
...
def test_serve_start_refuses_a_second_one(monkeypatch, capsys):
    ...
    assert "already running" in str(exc.value) and "olywork serve stop" in str(exc.value)
...
def test_serve_start_needs_login(monkeypatch):
    with pytest.raises(SystemExit) as exc:
        cli.cmd_serve_start(argparse.Namespace(port=None, renew_ca=False, foreground=False),
                            {"base_url": "http://x"})
    assert "olywork login" in str(exc.value)
...
def test_serve_status_says_what_is_captured(monkeypatch, capsys):
    ...
    monkeypatch.setattr(lpx, "running", lambda: {
        "port": 18791, "pid": 4242, "base_url": "https://olywork.example", "org": "acme",
...
def test_serve_status_and_env_when_nothing_runs(monkeypatch, capsys):
    ...
    assert "olywork serve start" in str(exc.value)
...
# ---- `olywork <command>` — per-launch opt-in, nothing global -----------------------------------
def test_bare_word_runs_a_program_but_a_typo_stays_a_olywork_error(monkeypatch):
    """`olywork claude` should launch claude. `olywork toool ls` must stay an ordinary olywork error, not an
...
def test_a_real_olywork_command_is_never_shadowed(monkeypatch, tmp_path):
    """If someone has a program called `call` or `org` on their PATH, olywork's own command still wins."""
    fake = tmp_path / "call"
    fake.write_text("#!/bin/sh\n")
    fake.chmod(0o755)
    monkeypatch.setenv("PATH", f"{tmp_path}:{os.environ['PATH']}")
    seen: dict = {}
    monkeypatch.setattr(cli, "cmd_call", lambda args, cfg: seen.update(ran="olywork-call"))
    monkeypatch.setattr(cli, "cmd_with", lambda args, cfg: seen.update(ran="with"))
    monkeypatch.setattr(cli, "_load_config", lambda: {"token": "t", "base_url": "http://x"})
    cli.main(["call", "https://api.example.com/x"])
    assert seen == {"ran": "olywork-call"}
...
def test_with_refuses_a_program_that_does_not_exist():
    with pytest.raises(SystemExit) as exc:
        cli.cmd_with(argparse.Namespace(command="definitely-not-a-real-binary", args=[], quiet=True),
                     {"token": "t", "base_url": "http://x"})
    assert "not a olywork command" in str(exc.value)
...
def test_with_starts_its_own_proxy_and_stops_it(monkeypatch):
    ...
    system picks, so two `olywork claude` sessions can never collide."""
    from olywork import localproxy as lpx
    ...
    class _Handle:
        port = 41234
        def env(self, olywork_host):
            return {"HTTPS_PROXY": "http://olywork:tok@127.0.0.1:41234"}
        def stop(self):
    ...
    monkeypatch.setattr("olywork.shell._run_subshell", lambda argv, env: events.append(("ran", argv[0], env["HTTPS_PROXY"])) or 0)
...
def test_with_attaches_to_a_running_daemon_and_leaves_it_up(monkeypatch):
    """A `olywork serve` daemon is someone's deliberate choice — borrowing it must not shut it down."""
    from olywork import localproxy as lpx

    events = []
    monkeypatch.setattr(lpx, "running", lambda: {
        "port": 18800, "pid": 1, "token": "daemon-tok", "base_url": "https://olywork.example",
        "org": "", "hosts": ["api.stripe.com"]})
    monkeypatch.setattr(cli, "_start_proxy_handle",
                        lambda *a, **k: pytest.fail("must not start a second proxy"))
    monkeypatch.setattr("olywork.shell._run_subshell",
                        lambda argv, env: events.append(env["HTTPS_PROXY"]) or 7)

    with pytest.raises(SystemExit) as exc:
        cli.cmd_with(argparse.Namespace(command="sh", args=[], quiet=True),
                     {"token": "t", "base_url": "https://olywork.example"})
    assert exc.value.code == 7                                  # the command's own exit code
    assert events == ["http://olywork:daemon-tok@127.0.0.1:18800"]
...
def test_with_runs_the_program_for_real_and_passes_args_and_exit_code(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(lpx, "running", lambda: {
        "port": 18800, "pid": 1, "token": "tok", "base_url": "https://olywork.example",
        "org": "", "hosts": []})
    script = f'printf "%s\\n" "$@" > {out}; printf "%s" "$HTTPS_PROXY" >> {out}; exit 9'
    with pytest.raises(SystemExit) as exc:
        cli.cmd_with(argparse.Namespace(command="sh", args=["-c", script, "_", "a b", "--flag"],
                                        quiet=True), {"token": "t", "base_url": "https://olywork.example"})
...
def test_the_install_hint_matches_how_olywork_was_installed(monkeypatch):
    """`pip install "tools-registry[proxy]"` is right for exactly ONE of the four ways people install
    olywork — and the installer's own way is not it. A uv-tool or Homebrew venv is not on the ambient
    ...
    for prefix, label in {"/opt/homebrew/Cellar/olywork/0.6.0/libexec": "Homebrew",
