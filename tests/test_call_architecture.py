...
from olywork import bootstrap
from olywork.application import billing
from olywork.application.call import authorize, overflow, reserve, service, settle
from olywork.domain import money
from olywork.domain.capacity import marks as capacity_marks
from olywork.domain.governance import usage as usage_policy


_SRC = Path(__file__).parents[1] / "src" / "olywork"
...
_DATAPLANE_DERIVED_WRITES = {
    ...
    # signature on olywork's own key marks the provider exhausted in ratestore (the shared Ephemeral
...
def test_call_runtime_import_edges_point_inward() -> None:
    call_forbidden = ("olywork.api", "olywork.bootstrap", "olywork.routers", "fastapi", "starlette")
    ...
    async_forbidden = ("olywork.api", "olywork.routers", "olywork.application", "olywork.audit")
...
def test_startup_manifests_keep_dataplane_and_control_work_separate() -> None:
    assert bootstrap.ROLE_BACKGROUND_TASKS == {
        "all": ("olywork.adsconv.worker",),
        "dataplane": (),
        "control": ("olywork.adsconv.worker",),
    }
    for checks in bootstrap.ROLE_STARTUP_CHECKS.values():
        assert "olywork.api._backfill_provider_extra_tools" not in checks
        assert "olywork.api._bootstrap_single_user" not in checks
    assert "olywork.mcp.mcp_lifespan" in bootstrap.ROLE_STARTUP_CHECKS["dataplane"]
    assert "olywork.mcp.mcp_lifespan" not in bootstrap.ROLE_STARTUP_CHECKS["control"]
