"""Tier 0 server-side CLI execution (`olywork run --server`).
...
def test_spawn_preexec_toggles_with_the_setting(monkeypatch):
    # The rlimit preexec is applied only when OLYWORK_RUN_RLIMITS is on (default). Off → no preexec.
    from olywork import runner
    from olywork.config import get_settings

    monkeypatch.setenv("OLYWORK_RUN_RLIMITS", "false")
    get_settings.cache_clear()
    assert runner._spawn_preexec() is None
    monkeypatch.setenv("OLYWORK_RUN_RLIMITS", "true")
    get_settings.cache_clear()
...
async def test_run_scrubs_server_env(clients: AsyncClient, monkeypatch):
    # A var set on the olywork process must NOT reach the child (the env is built fresh, not copied).
...
async def test_run_missing_cli_rejected(clients: AsyncClient):
    await _register_runnable(clients, name="ghost", bin="olywork-nonexistent-bin-xyz")
...
async def test_run_cross_org_isolation(clients: AsyncClient):
    ...
    r = await clients.post("/users", json={"email": "other@olywork.com"})
    tok2 = r.json()["token"]
    r = await clients.post("/run", json={"tool": "sh-skill", "args": []}, headers={"X-Olywork-Token": tok2})
    assert r.status_code == 404
