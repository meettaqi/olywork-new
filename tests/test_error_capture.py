"""Admin-only evidence kept for a failed relayed call: caller input and provider/olywork explanation.
...
2. It never retains olywork's own credential. That key is shared across every tenant, so a leak is not
   one customer's problem — and providers routinely quote the offending request back in a 400/401.
3. It captures BOTH failure shapes: the provider answering badly, and olywork failing to reach it.
"""
...
from olywork.application.call import evidence as call_evidence
from olywork.application.call import service as call_service
from olywork.application.call.types import ReservationFailed, UpstreamResponse
...
ADMIN = {"X-Olywork-Token": ADMIN_TOKEN}       # /admin/* authenticates with the env token, not a member
...
def test_constant_binding_format_is_not_a_secret_rendering(monkeypatch):
    ...
    key = "cr.test-Q7"
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_CRUSTDATA", key)
...
def platform_on(monkeypatch):
    monkeypatch.setenv("OLYWORK_ADMIN_TOKEN", ADMIN_TOKEN)
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_TIKHUB", PLATFORM_KEY)
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_SPYFU", SPYFU_KEY)
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_DATAFORSEO", "PLATFORM-DFS-KEY-def456")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "tikhub,spyfu,dataforseo")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
# ---- shape 2: olywork never reached the provider ---------------------------------------------------
async def test_olywork_s_own_502_is_explained_too(clients: AsyncClient, platform_on, monkeypatch):
    ...
    row = await _row(clients)
    assert row["error_response"].startswith("olywork: ")
...
async def test_olywork_side_502_keeps_own_tool_evidence(clients: AsyncClient, monkeypatch):
    import httpx
...
async def test_a_olywork_refusal_stores_nothing(clients: AsyncClient):
...
async def test_a_decoded_basic_credential_never_survives(clients: AsyncClient, platform_on,
    ...
    echoes olywork's credential in a form where neither the base64 blob nor `Basic <blob>` appears —
    so only decomposing it can catch this."""
    import base64
    login, password = "olywork-ops@example.com", "pw.7Kq2"
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_DATAFORSEO",
...
async def test_a_lowercase_percent_encoded_key_never_survives(clients: AsyncClient, platform_on,
    ...
    from urllib.parse import quote
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_TIKHUB", "a/b+c=dQ")
    get_settings.cache_clear()
    monkeypatch.setattr(call_service, "relay", _fake_relay(
...
async def test_a_bodyless_failure_still_leaves_a_row(clients: AsyncClient, platform_on, monkeypatch):
    ...
    # The required param has to be present or olywork refuses before relay — the empty half under test
...
async def test_a_cap_refusal_says_WHICH_cap(clients: AsyncClient, platform_on, monkeypatch):
    ...
    it was is in olywork's own detail, and 878 refusals a week were discarding it."""
    async def _boom(*a, **k):
...
def test_unknown_telemetry_costs_a_column_not_the_whole_row():
    ...
    from olywork.audit import _known_fields
