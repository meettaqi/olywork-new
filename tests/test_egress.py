...
no firewall touched. Enforcement is verified live on macOS in the setup path. See src/olywork/egress.py."""

from __future__ import annotations

from olywork import egress
...
def test_collect_hosts_includes_registry_and_catalog_dedup_sorted():
    ...
    hosts = egress.collect_hosts("https://olywork.com", catalog)
    assert hosts == ["api.github.com", "api.stripe.com", "olywork.com"]
...
def test_pf_ruleset_scopes_user_and_drops_the_rest():
    r = egress.pf_ruleset(["1.2.3.4", "5.6.7.8", "2001:db8::1"], "olywork-run")
    assert "pass out quick proto { tcp udp } from any to any port 53 user olywork-run" in r  # DNS allowed
    assert "to { 1.2.3.4 5.6.7.8 } port 443 user olywork-run" in r                           # v4 allow-list
    assert "2001:db8::1" in r                                                              # v6 kept
    assert r.strip().endswith("block drop out quick from any to any user olywork-run")        # default drop last
    assert "127.0.0.1" not in r                                                            # loopback NOT allowed


def test_pf_ruleset_with_no_ips_still_drops():
    r = egress.pf_ruleset([], "olywork-run")
    assert "block drop out quick from any to any user olywork-run" in r
