...
from olywork.models import CreditBlock, Hold, LedgerEntry, Org
...
async def test_referee_instant_grant_failure_after_staging_never_raises(
    ...
    r = await c.post("/users", json={"email": "ref-boom-a@olywork.com"})
    referrer_user = r.json()["id"]
    r = await c.post("/users", json={"email": "ref-boom-b@olywork.com"})
    referred_org, referred_user = r.json()["org_id"], r.json()["id"]
    async with session_maker() as db:
    ...
    # "olywork" logger; re-enable it so caplog sees the warning regardless of test order.
    monkeypatch.setattr(_logging.getLogger("olywork"), "disabled", False)
    with caplog.at_level(_logging.WARNING, logger="olywork"):
        async with session_maker() as db:
            row = (await db.execute(select(Referral))).scalars().first()
...
async def test_sweep_grant_failure_after_staging_never_raises(c: AsyncClient, monkeypatch, caplog):
    ...
    r = await c.post("/users", json={"email": "sweep-boom-ref@olywork.com"})
    referrer_user = r.json()["id"]
    rows = []
    for i in range(2):  # TWO due rows: the second iteration's log is the one a loop-local copy misses
        r = await c.post("/users", json={"email": f"sweep-boom-{i}@olywork.com"})
    ...
    monkeypatch.setattr(ledger, "grant", grant_then_boom)
    monkeypatch.setattr(_logging.getLogger("olywork"), "disabled", False)  # see the referee test
    with caplog.at_level(_logging.WARNING, logger="olywork"):
        async with session_maker() as db:
...
async def test_a_grant_failure_cannot_fail_signup(c: AsyncClient, monkeypatch):
    ...
    r = await c.post("/users", json={"email": "promo-fails@olywork.com"})
    assert r.status_code == 200, r.text
...
async def test_a_grant_failure_after_staging_still_returns_the_signup(c: AsyncClient, monkeypatch):
    ...
    from olywork.routers.signup_cookies import REFERRAL_COOKIE
    from olywork.models import Referral

    _, ann_token = await _org(c, "ref-ann@olywork.com")
    ...
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["email"] == "promo-fails-late@olywork.com"
...
async def test_margin_is_applied_at_reserve_and_settle(c: AsyncClient, monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_PLATFORM_MARGIN", "0.5")
    try:
        ...
        monkeypatch.delenv("OLYWORK_PLATFORM_MARGIN")
        get_settings.cache_clear()
...
async def test_balance_shows_the_wallet_to_members_and_the_history_to_admins(c: AsyncClient):
    """POLICY CHANGE (2026-08, from Taqi's report): this route used to be admin-only, which meant a
    machine identity could not read the balance it was spending — while every 402 already handed it
    `balance_micro`, and both llms.txt and skill.md tell an agent to run `olywork balance` after a call.
    ...
    org_id, owner = await _org(c, "owner@olywork.com")
    r = await c.post(f"/orgs/{org_id}/invites", json={"email": "m@olywork.com", "role": "member"},
                     headers=_h(owner))
    code = r.json()["code"]
    member = (await c.post("/invites/accept",
                           json={"code": code, "email": "m@olywork.com"})).json()["token"]
    ...
    other_org, other = await _org(c, "stranger@olywork.com")
...
def test_cli_balance_renders_credit_holds_and_ledger(monkeypatch, capsys):
    """`olywork balance` is a rendered table, so it must show sub-cent amounts as money (a call costs
