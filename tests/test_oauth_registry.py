...
other half — name a provider and olywork's own approved app supplies the credentials, requesting
...
def olywork_google_app(monkeypatch):
    """olywork's own Google client — what a deployment sets to offer registry connects."""
...
async def test_providers_endpoint_lists_the_registry(clients: AsyncClient, olywork_google_app):
...
async def test_registry_connect_uses_olyworks_own_app(clients: AsyncClient, olywork_google_app):
    ...
    q = _consent_query(d)
    assert q["client_id"] == ["olywork-google-cid"]
    assert q["redirect_uri"][0].endswith("/oauth/callback")
    assert q["state"] == [d["state"]]


async def test_the_broadest_capability_is_the_default(clients: AsyncClient, olywork_google_app):
...
async def test_choosing_read_narrows_the_request(clients: AsyncClient, olywork_google_app):
...
async def test_capabilities_are_cumulative(clients: AsyncClient, olywork_google_app):
...
async def test_secret_name_defaults_to_the_service(clients: AsyncClient, olywork_google_app):
    d = (await clients.post("/oauth/start", json={"provider": "google-search-console"})).json()
    st = (await clients.get(f"/oauth/status/{d['state']}")).json()
    assert st["name"] == "google-search-console"


async def test_an_explicit_name_still_wins(clients: AsyncClient, olywork_google_app):
    d = (await clients.post(
...
async def test_unknown_provider_is_404(clients: AsyncClient, olywork_google_app):
...
async def test_unconfigured_provider_is_a_clear_422(clients: AsyncClient, monkeypatch):
    """A deployment that never set olywork's Google client should say so, not fail mid-consent."""
    monkeypatch.setenv("OLYWORK_GOOGLE_CLIENT_ID", "")
    monkeypatch.setenv("OLYWORK_GOOGLE_CLIENT_SECRET", "")
    get_settings.cache_clear()
    try:
...
def test_multi_method_provider_is_configured_when_any_method_is_available(monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_INSTAGRAM_CLIENT_ID", "")
    monkeypatch.setenv("OLYWORK_INSTAGRAM_CLIENT_SECRET", "")
    monkeypatch.setenv("OLYWORK_META_CLIENT_ID", "meta-cid")
    monkeypatch.setenv("OLYWORK_META_CLIENT_SECRET", "meta-secret")
    get_settings.cache_clear()
    try:
...
def test_one_review_setting_controls_the_instagram_connect_experience(
    ...
    monkeypatch.setenv("OLYWORK_OAUTH_REVIEW_PENDING", pending)
    get_settings.cache_clear()
    try:
        row = next(item for item in P.listing() if item["service"] == "instagram")
        methods = {item["name"]: item for item in row["authorization_methods"]}
...
async def test_unknown_capability_is_422(clients: AsyncClient, olywork_google_app):
...
# Facebook's consent screen shows only that bare app name. Without a notice on the olywork side, the
...
def test_meta_providers_disclose_the_crewlet_app_name():
    ...
    for service in META_SERVICES:
        ...
        assert "Olywork Dev Inc" in notice, service
    page = next(
