...
from olywork import audit, crypto, localproxy
from olywork.domain import money as ledger
from olywork.application.call import idempotency as call_idem
from olywork.application.call import reserve as call_reserve
from olywork.application.call import service as call_service
from olywork.application.call.intake import CallMeta
...
from olywork.domain.governance import budgets as budget_policy
from olywork.models import CallRecord, CreditBlock, Hold, LedgerEntry, Membership, Org, TagSpend, User
...
def platform_on(monkeypatch):
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_TIKHUB", "PLATFORM-TIKHUB-KEY")
...
def _request_with_meta(value: str) -> Request:
    return Request({
        ...
        "path": f"/call/{EP}",
        "query_string": b"",
        "headers": [(b"x-olywork-meta", value.encode("latin-1"))],
    })
...
async def test_attack_1_all_ingress_paths_reject_storage_key_delimiters(
    ...
    for raw in ("customer=a\x1fb", "customer=a,b"):
        response = await clients.get(f"/call/{EP}?aweme_id=1", headers={"X-Olywork-Meta": raw})
...
async def test_attack_2_usage_identity_survives_five_tags_zero_release_settle_and_org_overlap(
    ...
    response = await clients.get(f"/call/{EP}?aweme_id=five", headers={"X-Olywork-Meta": header})
...
async def test_attack_4_concurrent_prechecks_overshoot_is_bounded_not_exact(
    ...
    responses = await asyncio.gather(*(
        clients.get(
            ...
            headers={"X-Olywork-Meta": "customer=race"},
        )
...
async def test_review_pin_bypass_matrix_never_attributes_a_different_value(
    ...
    attempts = (
        ({"X-Olywork-Token": token}, 200),
        ({"X-Olywork-Token": token, "X-Olywork-Meta": " CUSTOMER = cust_A "}, 200),
        ({"X-Olywork-Token": token, "X-Olywork-Meta": "customer=CUST_A"}, 403),
        ({"X-Olywork-Token": token, "X-Olywork-Meta": "customer=cust_A, workspace=ws_9"}, 200),
        ([("X-Olywork-Token", token), ("X-Olywork-Meta", "customer=cust_A"),
          ("X-Olywork-Meta", "customer=cust_B")], 200),
        ([("X-Olywork-Token", token), ("X-Olywork-Meta", "customer=cust_B"),
          ("X-Olywork-Meta", "customer=cust_A")], 403),
    )
    successes = 0
...
async def test_review_distinct_memberships_never_share_idempotent_bodies(
    ...
    async def relay_by_token(request, upstream_url, tool, secrets, client, drop_params=None,
        ...
        body = next(
            value for name, value in request.raw_headers if name.lower() == b"x-olywork-token")
    ...
    common = {"X-Olywork-Meta": "customer=same", "Idempotency-Key": "same-key"}
    first_a = await clients.get(f"/call/{EP}?aweme_id=idem", headers={**common, "X-Olywork-Token": token_a})
    first_b = await clients.get(f"/call/{EP}?aweme_id=idem", headers={**common, "X-Olywork-Token": token_b})
    replay_a = await clients.get(f"/call/{EP}?aweme_id=idem", headers={**common, "X-Olywork-Token": token_a})
    replay_b = await clients.get(f"/call/{EP}?aweme_id=idem", headers={**common, "X-Olywork-Token": token_b})
...
async def test_review_replay_keeps_the_original_call_id(clients: AsyncClient, platform_on):
    headers = {"X-Olywork-Meta": "customer=cust_A", "Idempotency-Key": "review-call-id"}
    ...
    assert replay.headers["X-Olywork-Idempotent-Replay"] == "true"
    assert replay.headers["X-Olywork-Call-Id"] == first.headers["X-Olywork-Call-Id"]
...
async def test_review_primary_call_cap_survives_audit_loss(
    ...
    headers = {"X-Olywork-Meta": "customer=cust_A"}
...
def test_review_local_proxy_keeps_meta_but_replaces_identity():
    cfg = localproxy.ProxyConfig(token="proxy", olywork_token="real-token", org="real-org")
    out = localproxy._forward_headers(cfg, [
        ("X-Olywork-Meta", "customer=cust_A"), ("X-Olywork-Token", "stolen"),
        ("X-Olywork-Org", "victim"), ("X-Custom", "kept"),
    ])
    grouped: dict[str, list[str]] = {}
    for key, value in out:
        grouped.setdefault(key.lower(), []).append(value)
    assert grouped["x-olywork-meta"] == ["customer=cust_A"]
    assert grouped["x-olywork-token"] == ["real-token"]
    assert grouped["x-olywork-org"] == ["real-org"]
