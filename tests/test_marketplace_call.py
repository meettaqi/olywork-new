"""Direct marketplace calls: `olywork call <catalog-endpoint-id>` with no registered tool.
...
injected via a virtual, never-persisted tool (tier 2), else — for an endpoint olywork is willing to spend
its own money on — OLYWORK'S OWN key, metered against the org's prepaid balance (tier 4), and only then
...
def platform_on(monkeypatch):
    ...
    for name, value in PLATFORM_KEYS.items():
        monkeypatch.setenv(f"OLYWORK_PLATFORM_KEY_{name}", value)
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", ",".join(k.lower() for k in PLATFORM_KEYS))
...
def minimax_platform_on(monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_MINIMAX", "PLATFORM-MINIMAX-KEY")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "minimax")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
async def test_tier3_no_credential_is_an_actionable_404(clients: AsyncClient):
    ...
    assert "olywork connections connect --provider tikhub" in detail
    assert "olywork secret add tikhub" in detail          # tikhub is a pasted-key provider
...
async def test_provider_name_404_points_at_the_marketplace(clients: AsyncClient):
    """`olywork call tikhub /path` (no such tool) keeps failing, but no longer dead-ends."""
...
# ---- tier 4: olywork's own key, billed to the org balance ------------------------------------------
async def test_tier4_relays_with_the_platform_key_and_charges_the_balance(clients: AsyncClient, platform_on):
    """The keyless first call: no credential in the org, and the endpoint is served anyway — on olywork's
    ...
    assert r.status_code == 200, r.text
    d = r.json()
    assert d["auth"] == f"Bearer {PLATFORM_KEYS['TIKHUB']}"   # olywork's key, injected the provider's way
    ...
    assert r.headers.get("X-Olywork-Cost-Micro") == str(EP_MICRO), dict(r.headers)
...
async def test_an_unmetered_call_carries_NO_cost_header(clients: AsyncClient, platform_on):
    ...
    assert "X-Olywork-Cost-Micro" not in r.headers
...
async def test_tier2_shadows_tier4(clients: AsyncClient, platform_on):
    ...
    the balance is untouched. (Silently switching them onto olywork's key would move their data, their
...
async def test_provider_not_allow_listed_is_still_tier3(clients: AsyncClient, monkeypatch):
    """The kill switch: keys configured, but the provider isn't named in OLYWORK_PLATFORM_PROVIDERS."""
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_TIKHUB", PLATFORM_KEYS["TIKHUB"])
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "dataforseo")   # tikhub deliberately absent
    get_settings.cache_clear()
    try:
        r = await clients.get(f"/call/{EP}?aweme_id=7")
        assert r.status_code == 404
        assert "olywork connections connect" in r.json()["detail"]
...
async def test_allow_listed_without_a_key_is_still_tier3(clients: AsyncClient, monkeypatch):
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_TIKHUB", "")
...
async def test_ineligible_endpoints_fall_through_to_tier3(clients: AsyncClient, platform_on, monkeypatch, why, patch):
    """`platform_eligible` is the fence: olywork spends its own money only where the price is
    ...
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 404, f"{why} must not be served on olywork's key"
    assert await _balance(clients) == 1_000_000
...
async def test_empty_balance_is_a_402_an_agent_can_act_on(clients: AsyncClient, platform_on):
    ...
    assert "olywork topup --auto on" in d["message"]


async def test_caller_max_cost_header_refuses_a_direct_call_before_the_reserve(clients: AsyncClient, platform_on):
    """`X-Olywork-Route-Max-Cost` on a plain /call/: a hard ceiling the caller sets, enforced before any
    ...
    assert r.status_code == 200, r.text
    assert r.headers["X-Olywork-Cost-Micro"] == str(EP_MICRO)
    assert await _balance(clients) == 1_000_000 - EP_MICRO


async def test_a_balance_refusal_is_a_olywork_refused_event_not_a_vendor_402(
    clients: AsyncClient, platform_on, posthog_events,
):
    """olywork said no before any upstream trip, and the event says so as data, not as a status code."""
    org_id = (await clients.get("/orgs")).json()[0]["org_id"]
    ...
    assert p["outcome"] == "olywork_refused" and p["refused_by"] == "balance"
...
async def test_402_with_autotopup_on_names_the_policy_not_a_missing_card(clients: AsyncClient, platform_on):
    ...
    assert "olywork connections connect --provider tikhub" in d["message"]
...
async def test_hunter_zero_result_search_costs_nothing(clients: AsyncClient, platform_on, monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_HUNTER", "PLATFORM-HUNTER-KEY")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "tikhub,scrapecreators,dataforseo,brightdata,hunter")
    get_settings.cache_clear()
...
async def test_daily_cap_fails_closed(clients: AsyncClient, platform_on, monkeypatch):
    """The per-org daily ceiling on olywork's keys — the blast radius of a runaway agent. Unlike the soft
    per-user call cap, it refuses rather than letting spend through."""
    monkeypatch.setenv("OLYWORK_PLATFORM_DAILY_CAP_USD", "0.0015")   # 1500 micro = one call, not two
    get_settings.cache_clear()
    try:
...
async def test_demo_orgs_can_never_spend(clients: AsyncClient, platform_on):
    ...
    async with session_maker() as db:
        ...
        await db.commit()
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 404, "a public-demo org gets the tier-3 dead-end, not olywork's key"
    assert await _balance(clients) == 1_000_000
...
async def test_a_user_may_not_forge_a_platform_binding(clients: AsyncClient, platform_on):
    """The other door onto olywork's keys: a tool the caller registers themselves. `relay` resolves
...
def test_local_run_cannot_export_a_platform_binding():
    """`olywork run --local` hands credentials to the member's own machine, so it may only ever release
...
async def test_the_same_key_can_belong_to_two_different_CALLERS(clients: AsyncClient):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import IdempotentCall, Membership
...
async def test_deleting_a_team_takes_its_remembered_answers(clients: AsyncClient):
    ...
    from olywork.domain.governance.teams import ORG_SCOPED_MODELS
    from olywork.models import IdempotentCall
...
async def test_no_key_means_nothing_changes(clients: AsyncClient, platform_on):
    ...
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 200
    assert "X-Olywork-Idempotent-Replay" not in r.headers
...
async def test_a_labelled_retry_is_answered_WITHOUT_reaching_the_provider(clients: AsyncClient,
    ...
    assert r.headers.get("X-Olywork-Idempotent-Replay") == "true"
    assert r.headers.get("X-Olywork-Cost-Micro") == "4200", "and what it originally cost"
...
async def test_one_callers_label_is_invisible_to_another(clients: AsyncClient, platform_on):
    ...
    clients.headers["X-Olywork-Token"] = other_token
    r = await clients.get(f"/call/{EP}?aweme_id=7", headers={"Idempotency-Key": "shared-label"})
    if prev:
        clients.headers["X-Olywork-Token"] = prev
    assert r.status_code == 200
    assert "X-Olywork-Idempotent-Replay" not in r.headers, "another caller must not read this answer"
...
async def test_the_SAME_LABEL_TWICE_bills_once_and_calls_the_provider_once(clients: AsyncClient,
    ...
    assert "X-Olywork-Idempotent-Replay" not in first.headers, "the first call is not a replay"
    ...
    assert second.headers.get("X-Olywork-Idempotent-Replay") == "true"
...
async def test_an_unmetered_call_is_not_stored(clients: AsyncClient):
    ...
    protect, and olywork has no business holding their response."""
    from sqlmodel import select
...
async def test_a_stale_label_reused_later_starts_fresh(clients: AsyncClient, platform_on):
    ...
    assert r.status_code == 200, r.text
    assert "X-Olywork-Idempotent-Replay" not in r.headers, "yesterday's answer must not be served"
...
def test_the_billability_truth_table():
    ...
    shared plan key it is olywork's own saturation, and billing it would charge teams for our
...
async def test_a_stale_catalog_method_never_charges_a_per_call_endpoint(
    ...
    """A relayed 405 is olywork's stale method metadata, not caller input.

    The catalog chooses the method and rejects a caller override before relay. Even a provider whose
    pricing says ``per_call`` therefore cannot turn its rejection of OLYWORK'S method into team spend.
...
# ---- trial pools: $0 on olywork's key, capped per team per day (fx.yaml kind: olywork_trial) -------

@pytest.fixture()
def trial_on(monkeypatch):
    """Tier 4 for a TRIAL provider: olywork's free-tier key in the env, provider allow-listed."""
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_FINNHUB", "trial-pool-test-key")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "tikhub,scrapecreators,dataforseo,finnhub")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
def test_no_x_endpoint_is_published_as_free():
    """Nothing on X's v2 API is free to olywork any more: X bills the app owner per use, so every
    ...
    assert not free, f"X is pay-per-use — these publish a price olywork cannot honour: {free[:10]}"
...
def x_billed(monkeypatch):
    ...
    monkeypatch.setenv("OLYWORK_OAUTH_BILLED_PROVIDERS", "x")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
async def _connect_x(clients: AsyncClient) -> None:
    """A REGISTRY X connection: `secret.provider` set is what marks the bill as olywork's (a BYO
    connect leaves it empty and is never metered)."""
    import json as _json

    from olywork import crypto
    from olywork.models import Secret
...
def test_the_owned_read_discount_is_never_claimed():
    ...
    olywork's, so no X entry may quote that rate as a per-CALL own-account price — the way `/2/users/me`
    did until 2026-08-18, under-billing the reads olywork pays the most for."""
    from olywork.domain.catalog import store as catalog_store
...
async def test_a_byo_x_connection_is_never_metered(clients: AsyncClient, x_billed, monkeypatch):
    ...
    `secret.provider`, its upstream bill is already theirs, and olywork must not charge for it."""
    import json as _json
...
def test_hunter_people_enrich_accepts_linkedin_handle_without_email():
    ...
    so olywork refused every handle-keyed call with a 400 before Hunter ever saw it — 2,369 times for
...
def test_body_limit_reads_nested_paging():
    ...
    from olywork.application.call.resolve import _body_limit
...
def test_influencersclub_settle_counts_accounts():
    ...
    from olywork.application.call.settle import _observed_cost_micro
...
async def test_concurrent_settles_never_lose_a_block_draw(clients: AsyncClient):
    ...
    import asyncio
    from olywork.domain import money
    from olywork.infra.db import session_maker
    from olywork.models import CreditBlock
    ...
    from olywork.domain.money import with_margin
