...
def test_contract_roundtrip_to_payload(tmp_path):
    ...
    c["health"] = {"path": "me"}
    (d / "olywork.json").write_text(json.dumps(c))
...
# ---- bug-hunt regressions: a stale/hand-edited olywork.json fails clearly, not with a traceback ---
def test_malformed_olywork_json_raises_clear_value_error(tmp_path):
    d = tmp_path / "svc"
    d.mkdir()
    (d / "olywork.json").write_text("{ not valid json")
...
def test_contract_env_sourced_secret_reads_from_environment(tmp_path, monkeypatch):
    """A olywork-import contract with an env-sourced secret (no file) must be readable by
