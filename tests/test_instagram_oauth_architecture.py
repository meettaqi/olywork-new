...
def instagram_apps(monkeypatch):
    monkeypatch.setenv("OLYWORK_INSTAGRAM_CLIENT_ID", "instagram-cid")
    monkeypatch.setenv("OLYWORK_INSTAGRAM_CLIENT_SECRET", "instagram-csec")
    monkeypatch.setenv("OLYWORK_META_CLIENT_ID", "meta-cid")
    monkeypatch.setenv("OLYWORK_META_CLIENT_SECRET", "meta-csec")
...
async def test_catalog_call_explicitly_selects_each_instagram_authorization(
    ...
    direct = await clients.get(
        ...
        headers={"X-Olywork-Authorization-Method": "instagram-login"},
    )
    ...
    page_call = await clients.get(
        "/call/instagram.x.user-messages",
        params={"page_id": "PAGE-DIRECT", "platform": "instagram"},
        headers={"X-Olywork-Authorization-Method": "facebook-page"},
    )
...
async def test_page_only_access_dry_run_preserves_connect_guidance(
    ...
    assert response.json() == {
        ...
        "connect_command": "olywork connections connect --provider instagram --capability page-tools",
        ...
        "detail": (
            ...
            "olywork connections connect --provider instagram --capability page-tools"
        ),
    }
