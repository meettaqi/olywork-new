...
The olywork CLI reports which coding agent it runs inside (X-Olywork-Client). The registry stamps it on
...
from olywork.models import CallRecord, Membership, Org, User


def _h(t: str, client: str | None = None) -> dict:
    h = {"X-Olywork-Token": t}
    if client is not None:
        h["X-Olywork-Client"] = client
    return h
...
def test_detect_runtime_ignores_config_location_vars(monkeypatch):
    ...
    from olywork.cli import _detect_runtime
    for var in ("OLYWORK_CLIENT", "CLAUDECODE", "CODEX_SANDBOX", "CODEX_SANDBOX_NETWORK_DISABLED",
    ...
    monkeypatch.setenv("OLYWORK_CLIENT", "my-agent")
    assert _detect_runtime() == "my-agent"


def test_olywork_token_env_overrides_the_config_file(monkeypatch):
    """Per-PROCESS identity: OLYWORK_TOKEN/OLYWORK_ORG in a runtime's env make the CLI act as that
    agent while ~/.olywork/config.json stays the human's — several agents on one machine, each with
    its own scope. Without this, every runtime shared the machine-global config identity."""
    from olywork.cli import _client
    monkeypatch.setenv("OLYWORK_TOKEN", "agent-token")
    monkeypatch.setenv("OLYWORK_ORG", "test-team")
    c = _client({"base_url": "http://x", "token": "human-token", "active_org": "other"})
    assert c.headers["X-Olywork-Token"] == "agent-token"
    assert c.headers["X-Olywork-Org"] == "test-team"
    monkeypatch.setenv("OLYWORK_URL", "http://dev-registry:1")
    c = _client({"base_url": "http://x", "token": "human-token", "active_org": "other"})
    assert str(c.base_url).startswith("http://dev-registry:1"), "OLYWORK_URL must ride with the token"
    monkeypatch.delenv("OLYWORK_TOKEN"); monkeypatch.delenv("OLYWORK_ORG"); monkeypatch.delenv("OLYWORK_URL")
    c = _client({"base_url": "http://x", "token": "human-token", "active_org": "other"})
    assert c.headers["X-Olywork-Token"] == "human-token"
    assert c.headers["X-Olywork-Org"] == "other"
...
async def test_call_emits_one_tool_called_event(env, posthog_events):
    ...
    assert p["call_ref"] == r.headers["X-Olywork-Call-Id"]
