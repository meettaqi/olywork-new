...
from olywork import agent_pages
...
async def test_chatgpt_page_is_served_with_the_crawler_essentials(clients: AsyncClient):
    ...
    assert "ChatGPT" in title and ("olywork" in title or "olywork.com" in title)
...
async def test_chatgpt_page_install_block_has_the_setup_line(clients: AsyncClient):
    html = (await clients.get("/agents/chatgpt")).text
    assert ("set up olywork" in html or "set up olywork" in html) and ("olywork.com/llms.txt" in html or "olywork.com/llms.txt" in html)
...
async def test_agent_pages_are_hosted_only(monkeypatch):
    """The install copy describes olywork.com's own hosted listing and grant — true of olywork.com, false of
    every self-hosted registry. So off the reference hosts the page 404s and leaves the sitemap."""
    monkeypatch.setenv("OLYWORK_PUBLIC_URL", "https://registry.example.internal")
...
async def test_use_case_page_is_served_with_the_crawler_essentials(clients: AsyncClient):
    ...
    title = re.search(r"<title>(.*?)</title>", html).group(1)
    assert ("olywork" in title or "olywork.com" in title) and "providers" in title and "API" in title
    assert "noindex" not in html
...
async def test_use_case_page_answers_the_four_questions_in_order(clients: AsyncClient):
    ...
    why_hdr = "Why go through olywork" if "Why go through olywork" in html else "Why go through olywork.com"
    order = ["best way to ask", why_hdr,
...
async def test_use_case_page_compares_one_row_per_provider_with_every_endpoint_collapsed(clients: AsyncClient):
    ...
    # as a fake "olywork" provider row. Match that filter here.
    ...
    assert "olywork call " in html or "olywork call " in html
...
async def test_reliability_section_appears_only_with_traffic(clients: AsyncClient):
    ...
    import inspect
    from olywork.routers.web import use_case_job_page
...
async def test_use_case_page_prices_come_from_the_catalog(clients: AsyncClient):
    ...
    from olywork.routers.web import _usd_short
    html = (await clients.get(USECASE)).text
...
async def test_no_em_dashes_in_the_hand_written_copy():
    ...
    body = "\n".join(l for l in src.splitlines()
                     ...
                     and "set up olywork —" not in l
                     and "set up olywork —" not in l)  # the literal setup line in install_steps
...
def test_related_cards_resolve_to_the_job_s_own_category():
    ...
    from olywork.routers.web import _related_link, _use_case_page_for
...
async def test_workflow_page_is_served_with_the_crawler_essentials(clients: AsyncClient):
    ...
    title = re.search(r"<title>(.*?)</title>", html).group(1)
    assert "olywork" in title or "olywork.com" in title
    assert "noindex" not in html
    ld = _ld(html)
...
async def test_workflow_pages_are_hosted_only(monkeypatch):
    monkeypatch.setenv("OLYWORK_PUBLIC_URL", "https://registry.example.com")
    get_settings.cache_clear()
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://registry") as c:
...
async def test_pages_off_the_shared_shell_carry_adtrack(clients: AsyncClient, path: str):
    ...
    Without it no `olywork_ad` cookie is set, so `org.ad_gclid` stays NULL and `adsconv.queue()`
...
async def test_routed_rows_never_surface_a_provider_named_olywork(clients: AsyncClient):
    """PR #242's `kind: routed` meta-rows delegate to children that are already listed, so on any
    public surface they double-count and print a vendor named "olywork" (the brand is olywork.com, and
    olywork is not a vendor). `_pub` is the one filter every public page reads, and the provider grid
    feeds both the sitemap and /catalog's prerender — so /tools/olywork must not exist."""
    from olywork.routers.web import _provider_rows, _pub
    cat = catalog_store.load()
    ...
    assert "olywork" not in {r["service"] for r in _provider_rows()}
    assert (await clients.get("/tools/olywork")).status_code == 404
    assert "/tools/olywork<" not in (await clients.get("/sitemap.xml")).text
