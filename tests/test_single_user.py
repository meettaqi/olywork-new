...
from olywork import api, maintenance
from olywork.domain.identity import session as sess
from olywork.__main__ import _prepare_serve
from olywork.api import LOCAL_ORG_NAME, LOCAL_USER_EMAIL, app
...
from olywork.routers import web
...
def test_local_mode_needs_sqlite_and_a_loopback_url():
    ...
    assert _settings(public_url="https://olywork.com").single_user_ok is False, \
        "a public domain must never serve a no-login dashboard"
    assert _settings(database_url="postgresql+asyncpg://u@h/db",
                     public_url="https://olywork.com").single_user_ok is False
...
async def test_the_minted_token_actually_works(local):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://registry") as c:
        r = await c.get("/tools", headers={"X-Olywork-Token": local.read_text().strip()})
...
async def test_selfhost_script_is_served_and_templated():
    ...
    assert "OLYWORK_SINGLE_USER=true" in r.text
