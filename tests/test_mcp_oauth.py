"""olywork as an OAuth authorization server — the metadata, the refusals, and client registration.
...
other MCP server must not spend a olywork balance just because it happens to be validly signed by us.
"""
...
from olywork.domain.identity import mcp_oauth, session
from olywork.routers import auth as auth_routes
...
async def test_connect_demo_defaults_off(monkeypatch):
    monkeypatch.delenv("OLYWORK_CONNECT_DEMO_ENABLED", raising=False)
...
async def test_a_token_for_ANOTHER_resource_is_refused():
    ...
    a olywork balance merely because we signed the token."""
...
async def test_our_own_token_is_accepted_and_carries_the_team():
    ...
    ours = mcp_oauth.mcp_resource_url()
    tok = mcp_oauth.make_access_token(user_id=7, org_id=3, audience=ours, scope="olywork:call")
    claims = mcp._oauth_claims(tok)
    assert claims is not None
    assert claims["sub"] == 7 and claims["org"] == 3
    assert claims["aud"] == ours and claims["scope"] == "olywork:call"


async def test_a_session_cookie_is_not_an_access_token():
    """olywork mints session cookies and identity tokens with the same HMAC construction. Without a type
...
async def test_dynamic_registration_mints_a_client(clients):
    ...
    body = r.json()
    assert body["client_id"].startswith("olywork-client-")
...
async def test_redirect_matching_is_EXACT_not_prefix():
    ...
    from olywork.domain.identity.mcp_oauth import redirect_uri_allowed
...
async def test_loopback_redirect_allows_ANY_port_but_not_other_drift():
    ...
    to vary — olywork's pure exact-match rejected `http://localhost:3118/callback` against a registered
...
async def test_cimd_refuses_unsafe_urls(url):
    ...
    from olywork.domain.identity.mcp_oauth import fetch_client_id_metadata
...
async def _signed_in(clients, email="oauth-user@olywork.com"):
    ...
    if prev:
        clients.headers["X-Olywork-Token"] = prev
    from sqlmodel import select

    from olywork.domain.identity import session as _sess
    ...
    clients.cookies.set("olywork_session", cookie)   # on the client: httpx deprecates per-request
...
async def test_the_whole_flow_end_to_end(clients):
    ...
    params = {"client_id": client_id, "redirect_uri": "https://client.test/cb",
              ...
              "resource": mcp_oauth.mcp_resource_url(), "scope": "olywork:call"}
...
async def test_v2_dcr_flow_mints_only_a_v2_audience(clients):
    ...
    client_id = await _register(clients)
    _, org_id = await _signed_in(clients, "oauth-v2@olywork.com")
    verifier, challenge = _pkce()
...
async def test_v2_scope_selects_v2_when_claude_omits_the_resource(clients):
    ...
    client_id = await _register(clients)
    _, org_id = await _signed_in(clients, "oauth-v2-no-resource@olywork.com")
    verifier, challenge = _pkce()
...
async def test_a_code_can_be_redeemed_only_ONCE(clients):
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "once@olywork.com")
    verifier, challenge = _pkce()
    params = {"client_id": client_id, "redirect_uri": "https://client.test/cb",
              "response_type": "code", "code_challenge": challenge,
...
async def test_the_WRONG_verifier_cannot_redeem_a_stolen_code(clients):
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "pkce@olywork.com")
    _, challenge = _pkce()
...
async def test_you_cannot_approve_for_a_team_you_are_not_in(clients):
    ...
    client_id = await _register(clients)
    cookie, _ = await _signed_in(clients, "outsider@olywork.com")
    other = await clients.post("/users", json={"email": "stranger@olywork.com"})
    prev = clients.headers.get("X-Olywork-Token")
    clients.headers["X-Olywork-Token"] = other.json()["token"]
...
async def test_a_GET_never_grants_anything(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import OAuthCode

    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "getonly@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256", "org_id": org_id},
...
async def test_the_consent_page_says_what_it_costs_in_WORDS(clients):
    """A screen that lists `olywork:call` and calls that informed consent is a formality. The two things
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "consent@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
    ...
    assert "consent@olywork.com" in page, "and which account they are granting from"
...
async def test_the_page_warns_when_a_client_registered_ITSELF(clients):
    ...
    client_id = await _register(clients)
    cookie, _ = await _signed_in(clients, "warned@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256"})
...
async def test_json_is_still_available_for_a_non_browser_client(clients):
    ...
    client_id = await _register(clients)
    cookie, _ = await _signed_in(clients, "jsonclient@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256"},
...
async def test_CANCEL_tells_the_client_no_rather_than_hanging(clients):
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "declines@olywork.com")
    _, challenge = _pkce()
    r = await clients.post("/oauth/authorize", data={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256", "org_id": org_id,
...
async def test_a_CROSS_ORIGIN_submission_is_refused(clients):
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "csrf@olywork.com")
    _, challenge = _pkce()
    r = await clients.post("/oauth/authorize", data={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256", "org_id": org_id,
...
async def test_the_consent_page_cannot_be_framed(clients):
    ...
    was something else. olywork sets this globally; asserting it here is what notices if that changes."""
    client_id = await _register(clients)
    cookie, _ = await _signed_in(clients, "framed@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256"})
...
async def test_an_oauth_token_actually_CALLS_the_tools(clients):
    ...
    to a user.
    """
    access, org_id = await _grant(clients, "usable@olywork.com")
...
async def test_the_token_spends_from_the_team_ON_THE_GRANT(clients):
    ...
    access, granted_org = await _grant(clients, "multi@olywork.com")
...
async def test_revoking_your_tokens_kills_an_existing_grant(clients):
    """`token_version` is olywork's kill switch for a leaked credential. An OAuth grant must honour it
    too, or the one button a user has for "make it stop" would quietly miss this door."""
    access, _ = await _grant(clients, "revoker@olywork.com")
    async with mcp_session(clients) as c:
    ...
    async with session_maker() as db:
        user = (await db.execute(
            select(User).where(User.email == "revoker@olywork.com"))).scalar_one()
...
async def test_a_grant_comes_with_a_refresh_token(clients):
    ...
    body, _, _ = await _grant_full(clients, "refresher@olywork.com")
...
async def test_refreshing_ROTATES_the_token_and_the_new_one_works(clients):
    ...
    body, client_id, _ = await _grant_full(clients, "rotate@olywork.com")
...
async def test_replaying_a_SPENT_refresh_token_kills_the_whole_family(clients):
    ...
    body, client_id, _ = await _grant_full(clients, "replay@olywork.com")
    first = body["refresh_token"]
...
async def test_a_refresh_cannot_quietly_change_teams(clients):
    ...
    body, client_id, org_id = await _grant_full(clients, "steady@olywork.com")
...
async def test_a_refresh_token_is_bound_to_its_client(clients):
    ...
    body, _, _ = await _grant_full(clients, "bound@olywork.com")
...
async def test_revoking_ends_the_family_and_never_leaks_whether_a_token_existed(clients):
    ...
    body, client_id, _ = await _grant_full(clients, "revoke@olywork.com")
...
async def test_refresh_tokens_are_stored_HASHED(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import OAuthRefresh

    body, _, _ = await _grant_full(clients, "hashed@olywork.com")
    async with session_maker() as db:
    ...
    assert all(len(r.token_hash) == 64 for r in rows), "sha256 hex, as everywhere else in olywork"
...
async def test_a_resource_we_do_not_serve_is_refused_UP_FRONT(clients):
    ...
    identifier our metadata declares, which is a reasonable thing for a client to do. olywork accepted
    ...
    """
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "wrongres@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256",
...
async def test_omitting_the_resource_is_fine(clients):
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "nores@olywork.com")
    verifier, challenge = _pkce()
    r = await clients.post("/oauth/authorize", data={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256", "org_id": org_id,
...
async def test_a_null_origin_from_a_redirect_chain_is_not_treated_as_cross_site(clients):
    ...
    """
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "nullorigin@olywork.com")
    _, challenge = _pkce()
...
async def test_a_signed_out_user_is_returned_to_the_consent_screen(clients):
    ...
    olywork" landed on the dashboard and the authorization was silently dropped.
    ...
    parked = (sent_away.cookies.get("olywork_oauth_return") or "").strip('"')
    ...
    cookie, _ = await _signed_in(clients, "returner@olywork.com")
    clients.cookies.set("olywork_oauth_return", parked)
...
async def test_the_team_picker_shows_which_team_can_PAY(clients):
    ...
    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "picker@olywork.com")
    _, challenge = _pkce()
    r = await clients.get("/oauth/authorize", params={
        "client_id": client_id, "redirect_uri": "https://client.test/cb", "response_type": "code",
        "code_challenge": challenge, "code_challenge_method": "S256"})
...
async def test_a_team_with_no_balance_is_labelled_not_hidden(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import Org

    client_id = await _register(clients)
    cookie, org_id = await _signed_in(clients, "broke@olywork.com")
    async with session_maker() as db:
...
async def _as(email: str) -> dict:
    """Headers that act as a given user — an identity token, minted the way `olywork login` does."""
    from sqlmodel import select
    ...
    return {"X-Olywork-Token": _sess.make_identity(user.id, token_version=user.token_version)}


async def test_the_team_on_a_grant_can_be_moved_without_reconnecting(clients):
    """The consent screen's choice was invisible and permanent: an agent reported a slug, `olywork org
...
async def test_a_rolling_deploy_family_without_authority_can_be_listed_and_moved(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import OAuthGrant, OAuthRefresh

    email = "rolling-gap@olywork.com"
...
async def test_refresh_repairs_missing_authority_with_the_original_consent_time(clients):
    ...
    body, client_id, _ = await _grant_full(clients, "rolling-refresh@olywork.com")
...
async def test_a_grant_cannot_be_moved_to_a_team_you_are_not_in(clients):
    ...
    body, _, _ = await _grant_full(clients, "insider@olywork.com")
    me = await _as("insider@olywork.com")
    grant = (await clients.get("/oauth/grants", headers=me)).json()[0]["grant"]

    outsider = (await clients.post("/users", json={"email": "outsider@olywork.com"})).json()
    theirs = (await clients.get("/orgs", headers={"X-Olywork-Token": outsider["token"]})).json()[0]
...
async def test_only_the_grants_own_user_can_move_it(clients):
    ...
    await _grant_full(clients, "owner@olywork.com")
    grant = (await clients.get("/oauth/grants",
                               headers=await _as("owner@olywork.com"))).json()[0]["grant"]

    stranger = (await clients.post("/users", json={"email": "stranger@olywork.com"})).json()
    theirs = (await clients.get("/orgs", headers={"X-Olywork-Token": stranger["token"]})).json()[0]
    r = await clients.post(f"/oauth/grants/{grant}/team", json={"team": theirs["slug"]},
                           headers={"X-Olywork-Token": stranger["token"]})
...
async def test_balance_tells_an_oauth_caller_how_to_move_the_team(clients):
    ...
    body, _, _ = await _grant_full(clients, "labelled@olywork.com")
    async with mcp_session(clients) as c:
        out = await _call_tool(c, "balance", {}, token=body["access_token"])
    assert out["identity"] == "labelled@olywork.com" and out["team_name"]
    assert "use-team" in out["hint"]


async def test_the_listed_grant_id_is_the_one_use_team_accepts(clients, monkeypatch):
    """The id printed by `olywork mcp grants` is an ARGUMENT, not prose. It is 22 characters and the
...
async def test_a_grant_dies_with_the_membership_it_was_consented_under(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import Membership, User

    body, client_id, org_id = await _grant_full(clients, "departing@olywork.com")
    async with session_maker() as db:
        user = (await db.execute(select(User).where(
            User.email == "departing@olywork.com"))).scalar_one()
...
async def test_a_rotation_started_before_a_move_cannot_drag_the_team_back(clients):
    ...
    from olywork.infra.db import session_maker
    from olywork.models import OAuthRefresh as _RT
...
async def test_a_team_you_cannot_use_is_indistinguishable_from_one_that_does_not_exist(clients):
    """Told apart, this route reports whether an arbitrary slug exists on olywork — an oracle any
    signed-in account could walk."""
    await _grant_full(clients, "prober@olywork.com")
    me = await _as("prober@olywork.com")
    grant = (await clients.get("/oauth/grants", headers=me)).json()[0]["grant"]

    outsider = (await clients.post("/users", json={"email": "elsewhere@olywork.com"})).json()
    theirs = (await clients.get("/orgs", headers={"X-Olywork-Token": outsider["token"]})).json()[0]
...
async def test_an_expired_grant_is_not_presented_as_authorized(clients):
    ...
    body, client_id, _ = await _grant_full(clients, "expired-list@olywork.com")
    me = await _as("expired-list@olywork.com")
    async with session_maker() as db:
...
async def test_an_expired_grant_cannot_be_moved(clients):
    ...
    body, _, _ = await _grant_full(clients, "expired-move@olywork.com")
    me = await _as("expired-move@olywork.com")
...
async def test_rotation_does_not_change_the_grant_date(clients):
    ...
    body, client_id, _ = await _grant_full(clients, "grant-date@olywork.com")
    me = await _as("grant-date@olywork.com")
