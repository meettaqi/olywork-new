...
from olywork.models import CallRecord, DenyRule, Membership, Org, Secret, Tool, User

ADMIN = "ENV-ADMIN-SECRET"


def _h(t: str) -> dict:
    return {"X-Olywork-Token": t}


@pytest.fixture
async def c(monkeypatch):
    monkeypatch.setenv("OLYWORK_ADMIN_TOKEN", ADMIN)
...
async def test_onboard_seeds_a_populated_team(c):
    ...
    async with session_maker() as s:
        ...
        assert roles["ada@demo.olywork.local"] == "admin"
        assert roles["ben@demo.olywork.local"] == "member"
        assert roles["cora@demo.olywork.local"] == "viewer"
...
async def test_demo_teammate_cannot_request_a_login_code(c):
    r = await c.post("/auth/email/start", json={"email": "ada@demo.olywork.local"})
...
async def test_seed_tool_and_accept_teammate(c):
    ...
    # demo teammate that auto-joins. Use an IDENTITY token (like the dashboard session) so X-Olywork-Org
    ...
    oh = {**_h(tok), "X-Olywork-Org": org["org"]}
    # seed the tool
    ...
    await c.post(f"/orgs/{org['org_id']}/invites", json={"email": "alex@demo.olywork.local", "role": "member"}, headers=oh)
    at = await c.post("/onboard/accept-teammate", json={"email": "alex@demo.olywork.local"}, headers=oh)
    assert at.status_code == 200
    members = {m["email"]: m["role"] for m in (await c.get(f"/orgs/{org['org_id']}/members", headers=oh)).json()}
    assert members.get("alex@demo.olywork.local") == "member"  # joined instantly
    ...
    missing = await c.post(
        ...
        json={"email": "missing@demo.olywork.local"},
        headers=oh,
    )
...
async def test_reset_sweeps_deny_rules_naming_a_demo_teammate(c):
    ...
    oh = {**_h(tok), "X-Olywork-Org": org["org"]}
    await c.post(f"/orgs/{org['org_id']}/invites", json={"email": "alex@demo.olywork.local", "role": "member"}, headers=oh)
    assert (await c.post("/onboard/accept-teammate", json={"email": "alex@demo.olywork.local"}, headers=oh)).status_code == 200
    async with session_maker() as s:
        alex = (await s.execute(select(User).where(User.email == "alex@demo.olywork.local"))).scalar_one()
