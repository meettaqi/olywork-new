...
The agent passes the REAL upstream URL (it already knows the API); olywork resolves the tool by
...
async def test_passthrough_resolves_by_url_and_injects(clients: AsyncClient):
    ...
    assert r.status_code == 200, r.text
    d = r.json()
    assert d["auth"] == "Bearer SEK"          # credential injected by olywork
...
async def test_completed_relay_closes_the_upstream_response(clients: AsyncClient):
    ...
    try:
        ...
        token = clients.headers["X-Olywork-Token"].encode("latin-1")
        scope = {
            "type": "http",
            ...
            "query_string": b"",
            "root_path": "",
            "headers": [(b"host", b"registry"), (b"x-olywork-token", token)],
...
async def test_caller_headers_and_cookies_passthrough_and_token_stripped(clients: AsyncClient):
    ...
    assert "x-olywork-token" not in h             # our control header never leaks upstream


async def test_control_infra_headers_and_olywork_cookie_stripped(clients: AsyncClient):
    await _register(clients, "sec", "https://api.sec.com")
    r = await clients.get(
        "/call/https://api.sec.com/echo",
        headers={
            "X-Olywork-Org": "olywork", "ngrok-skip-browser-warning": "1",
            "X-Forwarded-For": "1.2.3.4", "X-Forwarded-Proto": "https", "Via": "1.1 edge",
            "X-Keep": "yes", "Cookie": "olywork_session=SECRET; keep=1; olywork_oauth_state=xyz",
        },
    )
    h = r.json()["headers"]
    for leak in ("x-olywork-org", "ngrok-skip-browser-warning", "x-forwarded-for", "x-forwarded-proto", "via"):
        assert leak not in h, f"{leak} leaked upstream"
    assert h["x-keep"] == "yes"        # unrelated caller header preserved
    assert h["cookie"] == "keep=1"     # olywork's own cookies scrubbed, other cookies kept
...
async def test_encoded_slash_preserved_named_form(clients: AsyncClient):
    ...
    r = await clients.put("/call/npmreg/@olywork%2folywork", content=b"{}")
    assert r.status_code == 200, r.text
    assert r.json()["raw_path"].endswith("/@olywork%2folywork")
...
async def test_encoded_slash_preserved_passthrough_form(clients: AsyncClient):
    ...
    r = await clients.put("/call/https://registry.npm2.test/@olywork%2folywork", content=b"{}")
    assert r.status_code == 200, r.text
    assert r.json()["raw_path"].endswith("/@olywork%2folywork")


async def test_olywork_marks_its_own_call_errors(clients: AsyncClient):
    """A caller cannot otherwise tell olywork's 404 ("no tool registered for that host") from the
    vendor's own 404 — both are a status and some JSON. `X-Olywork-Error` is the distinction the local
    ...
    assert r.status_code == 404
    assert r.headers.get("x-olywork-error") == "1"
    ...
    assert ok.status_code == 200 and "x-olywork-error" not in ok.headers   # a real relay is not tagged

    other = await clients.get("/tools/by-name/ghost")                   # not a /call/ path
    assert other.status_code == 404 and "x-olywork-error" not in other.headers
