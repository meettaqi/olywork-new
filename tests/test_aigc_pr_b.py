...
from olywork.routers import call as call_router
...
def test_async_wait_succeeds_after_unknown_status_and_warns_once():
    ...
    assert outcome["recovery"] == "olywork call vendor.task.status -p task_id=task-1"
...
def test_async_wait_dynamic_url_enforces_hosts_and_fetches_by_terminal_id():
    ...
    assert outcome["code"] == 0
    assert outcome["fetch_command"] == "olywork call vendor.result -p file_id=file-9"
...
def test_call_await_writes_only_terminal_raw_body_to_stdout(monkeypatch, capsys):
    ...
    submitted = response(202, {"task_id": "task-7"}, {
        "X-Olywork-Async": json.dumps(descriptor), "X-Olywork-Cost-Micro": "100000",
    })
    ...
    assert "resume: olywork call vendor.task.status -p task_id=task-7" in captured.err
    assert "generation reservation: $0.1" in captured.err
    assert "retrieve the result (file bytes, or JSON with a download URL): olywork call vendor.result -p file_id=file-7" in captured.err
    assert "result lifetime: 9h" in captured.err


def test_call_await_routes_dynamic_poll_urls_back_through_olywork(monkeypatch, capsys):
    descriptor = static_descriptor()
    descriptor["poll"] = {"url_from": "urls.get", "url_hosts": ["api.replicate.com"]}
    submitted = response(202, {
        "task_id": "task-8", "urls": {"get": "https://api.replicate.com/v1/predictions/task-8"},
    }, {"X-Olywork-Async": json.dumps(descriptor)})
...
def test_router_attaches_static_async_metadata_without_reading_body(monkeypatch):
    ...
    assert json.loads(headers["X-Olywork-Async"]) == descriptor
