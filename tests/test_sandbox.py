...
from olywork import sandbox
from olywork.routers.onboard import SANDBOX_RATE_MAX
from olywork.api import app
from olywork.infra.db import reset_db, session_maker
from olywork.models import Secret, Tool, User
...
async def test_call_synthesizes_and_injects_without_network(anon):
    """The safety boundary: the call injects for REAL (visible) but never leaves olywork."""
...
async def test_skill_samples(anon):
    ...
    assert ph["key"] == "POSTHOG_KEY" and "SKILL.md" in ph["files"] and "olywork.json" in ph["files"]
...
async def test_skill_install_script(anon):
    ...
    assert "/call/https://app.posthog.com" in body                    # calls through the olywork proxy
...
async def test_gc_reaps_a_sandbox_that_made_an_idempotent_call(anon):
    ...
    from olywork.models import IdempotentCall, Membership, Org
...
async def test_a_failing_reaper_does_not_close_the_front_door(anon, monkeypatch):
    ...
    from olywork.application.onboard import sandbox as onboard_sandbox
