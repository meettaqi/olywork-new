...
from olywork.routers.orgs import count_today
from olywork.timeutil import utcnow_naive as _utcnow_naive
from olywork.infra.db import session_maker
from olywork.models import CallRecord, Membership, RunRecord, User


async def _set_cap(cap: int, email: str = "tim@olywork.com") -> int:
...
async def test_runs_and_calls_share_one_gate(clients: AsyncClient):
    ...
    src = (Path(__file__).parents[1] / "src" / "olywork" / "api.py").read_text()
    assert src.count("await _enforce_daily_cap(caller, db)") == 2  # local-run grant + server run
    await _mk_echo_tool(clients)
    org_id = await _set_cap(2)
    await _set_counter(org_id, "tim@olywork.com", 2, date.today())  # two prior events today
    blocked = await clients.get("/call/echo/anything")
    assert blocked.status_code == 429 and "2/2" in blocked.json()["detail"]
    assert await _counter(org_id, "tim@olywork.com") == (2, date.today())  # refused = not counted
...
async def test_cap_is_per_member_not_global(clients: AsyncClient):
    ...
    bob = await clients.get("/call/echo/anything", headers={"X-Olywork-Token": btok})
...
async def test_yesterdays_usage_does_not_count_today(clients: AsyncClient):
    await _mk_echo_tool(clients)
    org_id = await _set_cap(1)
    await _set_counter(org_id, "tim@olywork.com", 1, date.today() - timedelta(days=1))  # yesterday's
    assert (await clients.get("/call/echo/anything")).status_code == 200  # a new day starts from 0
    assert await _counter(org_id, "tim@olywork.com") == (1, date.today())  # ...and this was its first
...
async def test_setting_a_cap_seeds_the_counter_from_todays_journal(clients: AsyncClient):
    ...
    for _ in range(3):
        assert (await clients.get("/call/echo/anything")).status_code == 200
    await audit.drain()
    await _seed_run(org_id, "tim@olywork.com")  # a server run counts too: 4 events in the journal
    uid = [x["user_id"] for x in (await clients.get(f"/orgs/{org_id}/members")).json()
           if x["email"] == "tim@olywork.com"][0]
    assert (await clients.patch(f"/orgs/{org_id}/members/{uid}/cap", json={"daily_call_cap": 5})).status_code == 200
    assert await _counter(org_id, "tim@olywork.com") == (4, date.today())
...
async def test_counter_agrees_with_the_journal_after_real_calls(clients: AsyncClient):
    ...
    await audit.drain()
    async with session_maker() as s:
        journal = await count_today(s, org_id, "tim@olywork.com")
    assert await _counter(org_id, "tim@olywork.com") == (journal, date.today()) == (4, date.today())


async def _get_org_id(email: str = "tim@olywork.com") -> int:
    async with session_maker() as s:
        uid = (await s.execute(select(User.id).where(User.email == email))).scalar_one()
...
async def test_usage_endpoint_rolls_up_by_user_tool_and_day(clients: AsyncClient):
    org_id = await _get_org_id()
    await _seed_call(org_id, "tim@olywork.com")  # 2 calls + 1 server run = 3 events
    await _seed_call(org_id, "tim@olywork.com")
    await _seed_run(org_id, "tim@olywork.com")
    u = (await clients.get(f"/orgs/{org_id}/usage")).json()
    assert u["totals"] == {"call": 2, "local_run": 0, "server_run": 1, "total": 3}
    assert u["by_user"][0]["user_email"] == "tim@olywork.com" and u["by_user"][0]["total"] == 3
...
async def test_usage_endpoint_is_admin_only(clients: AsyncClient):
    ...
    assert (await clients.get(f"/orgs/{org_id}/usage", headers={"X-Olywork-Token": vtok})).status_code == 403
...
async def test_set_cap_is_admin_only_and_validated(clients: AsyncClient):
    ...
    assert (await clients.patch(f"/orgs/{org_id}/members/{mid}/cap", json={"daily_call_cap": 1},
                                headers={"X-Olywork-Token": mtok})).status_code == 403


async def test_list_members_shows_cap_and_used_today(clients: AsyncClient):
    org_id = await _set_cap(7)
    await _seed_call(org_id, "tim@olywork.com")
    tim = [x for x in (await clients.get(f"/orgs/{org_id}/members")).json() if x["email"] == "tim@olywork.com"][0]
