...
base64/gzip-encode the body and mark it with `X-Olywork-Body-Encoding`; the edge then sees only opaque
...
from olywork.bootstrap_http import _decode_request_body
...
async def test_skills_accepts_encoded_sql_recipe(clients: AsyncClient):
    ...
    r = await clients.post(
        ...
        headers={"X-Olywork-Body-Encoding": "base64", "Content-Type": "application/json"},
    )
...
async def test_proxy_relays_decoded_body_upstream(clients: AsyncClient):
    ...
    r = await clients.post(
        ...
        headers={"X-Olywork-Body-Encoding": "base64", "Content-Type": "text/plain"},
    )
    assert r.status_code == 200, r.text
...
async def test_bad_encoding_is_400(clients: AsyncClient):
    r = await clients.post(
        ...
        headers={"X-Olywork-Body-Encoding": "gzip", "Content-Type": "application/json"},
    )
