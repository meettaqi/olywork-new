"""OAuth-billed metering: providers whose upstream bill lands on olywork's app (X's pay-per-use).

A registry X connect is the org's own credential (tier 1/2), but every call on it spends olywork's
prepaid X credits — so with the provider allow-listed (`OLYWORK_OAUTH_BILLED_PROVIDERS=x`) the call
...
# only when the caller owns the developer app, and on a registry connect the app is olywork's, so a
...
async def _connect_x(clients: AsyncClient, *, provider: str = "x") -> None:
    ...
    async with session_maker() as db:
        secret = Secret(org_id=org_id, name="x", owner="tim@olywork.com", kind="oauth",
                        value=crypto.encrypt(json.dumps(blob)), provider=provider)
        db.add(secret)
        await db.flush()
        db.add(Tool(org_id=org_id, name="x", owner="tim@olywork.com",
...
async def test_kill_switch_off_stays_free(clients: AsyncClient):
    """Default deploy = no OLYWORK_OAUTH_BILLED_PROVIDERS: an X connect call is served and nothing
    ...
    assert "x-olywork-cost-micro" not in {k.lower() for k in r.headers}
...
async def test_empty_page_settles_at_zero(clients: AsyncClient, billed_on, monkeypatch):
    ...
    r = await clients.get(f"/call/{POSTS}?id=42&max_results=10")
    assert r.status_code == 200, r.text
    assert int(r.headers["x-olywork-cost-micro"]) == 0
...
async def test_url_passthrough_is_priced_off_the_matched_endpoint(
    ...
    assert r.status_code == 200, r.text
    assert int(r.headers["x-olywork-cost-micro"]) == PROFILE_MICRO
...
async def test_unmatched_passthrough_gets_the_default_read_rate(
    ...
    """A route the catalog doesn't know still costs olywork money — the provider-level default
    ...
    assert r.status_code == 200, r.text
    assert int(r.headers["x-olywork-cost-micro"]) == 3 * POST_MICRO
    settle = next(e for e in await _entries(clients) if e["kind"] == "settle")
...
async def test_post_create_is_per_call(clients: AsyncClient, billed_on, monkeypatch):
    ...
    assert int(r.headers["x-olywork-cost-micro"]) == CREATE_MICRO
...
async def test_post_with_url_prices_13x(clients: AsyncClient, billed_on, monkeypatch):
    ...
    assert r.status_code == 201, r.text
    assert int(r.headers["x-olywork-cost-micro"]) == CREATE_LINK_MICRO
