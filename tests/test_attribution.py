"""First-touch traffic-source attribution: web/sitetrack.js → `olywork_utm` cookie → Org.utm_*.
...
async def test_signup_persists_the_utm_cookie(clients):
    r = await clients.post("/users", json={"email": "sponsor@example.com"}, cookies={"olywork_utm": COOKIE})
...
async def test_org_creation_persists_the_utm_cookie(clients):
    ...
    r = await clients.post("/orgs", json={"name": "sponsored team"}, cookies={"olywork_utm": COOKIE})
    assert r.status_code == 200, r.text
    org = await _org(r.json()["org_id"])
...
async def test_referrer_only_cookie_records_just_the_host(clients):
    ...
    r = await clients.post("/users", json={"email": "ref@example.com"},
                           cookies={"olywork_utm": quote("|||||news.ycombinator.com", safe="")})
    assert r.status_code == 200, r.text
    org = await _org(r.json()["org_id"])
...
async def test_sitetrack_is_inert_without_a_posthog_key(clients, monkeypatch):
    ...
    assert "olywork_utm" in r.text     # cookie half always present
