...
from olywork.application.call import authorize, evidence, idempotency, intake, reserve, resolve, service, settle
from olywork.application.call.types import (
    AuthorizationFailed,
    CallContext,
    CallFailure,
    CallInput,
...
def test_gateway_failure_kinds_keep_production_blame() -> None:
    ...
    assert _BLAME_BY_KIND["injection_failed"] == "olywork"
    assert _BLAME_BY_KIND["ssrf_refused"] == "olywork"
...
def test_upstream_relay_is_framework_neutral() -> None:
    relay_module = importlib.import_module("olywork.infra.upstream.relay")
...
def test_reservation_failure_keeps_mechanism_and_blame_separate() -> None:
    ...
    assert (short.blame, unavailable.blame) == ("caller", "olywork")
...
def test_resolution_result_and_failures_are_framework_neutral() -> None:
    ...
    assert ResolutionFailed(
        "injection_failed", status_code=502, detail="configuration").blame == "olywork"
