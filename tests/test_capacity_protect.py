...
from olywork import archive, audit, ratestore
...
from olywork.application.call.types import CallFailure, UpstreamResponse
from olywork.infra.db import session_maker
from olywork.domain.capacity import marks as capacity_marks
from olywork.domain.capacity.marks import DEFAULT_LOCK, LOCK_NS, MAX_LOCK, Lock
...
from olywork.models import Hold, LedgerEntry
...
async def test_exhausted_platform_account_is_refused_before_any_hold(clients: AsyncClient, platform_on):
    ...
    assert r.headers["X-Olywork-Error"] == "1"
    assert "X-Olywork-Call-Id" in r.headers
    body = r.json()["detail"]
...
async def test_one_signature_is_a_strike_that_a_2xx_erases(clients: AsyncClient, platform_on, monkeypatch):
    ...
    assert "X-Olywork-Error" not in r.headers
    assert r.headers["X-Olywork-Cost-Micro"] == "0" and await _balance(clients) == before
...
async def test_a_failed_strike_never_fails_the_call(clients: AsyncClient, platform_on, monkeypatch):
    ...
    r = await clients.get(f"/call/{EP}?aweme_id=7")
    assert r.status_code == 402 and r.headers["X-Olywork-Cost-Micro"] == "0"


def test_provider_capacity_is_a_olywork_blamed_typed_failure():
    exc = CallFailure("provider_capacity", status_code=503, detail={"error": "x"})
    assert exc.blame == "olywork" and exc.status_code == 503
