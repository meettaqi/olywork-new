...
from olywork.domain.catalog import stats as catalog_stats
...
from olywork.infra.db import _background_engine, _engine, session_maker
from olywork.infra.upstream.relay import relay as upstream_relay
from olywork.models import CallRecord, Hold
...
async def test_a_saturated_pool_answers_a_typed_503_not_an_anonymous_500(
    ...
    assert r.json()["olywork_saturated"] is True
    assert r.headers.get("Retry-After") == "2"


@pytest.mark.skipif(
    not os.environ.get("OLYWORK_TEST_DB_URL"), reason="requires the Postgres test database"
)
...
async def test_a_settle_that_loses_the_pool_once_retries_and_still_charges(
    ...
    assert r.headers.get("X-Olywork-Cost-Micro") == str(EP_MICRO)
