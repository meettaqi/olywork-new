"""Caller tags (`X-Olywork-Meta`) — the attribution half of builder-pays billing.

A builder reselling olywork runs their whole user base through ONE org, ONE balance and ONE token, and
...
from olywork import audit, crypto
...
from olywork.routers import call as call_routes
from olywork.config import get_settings
from olywork.infra.db import session_maker
from olywork.models import Membership, Org, TagSpend, User
...
async def test_a_bag_without_the_primary_dimension_still_records_its_tags(clients: AsyncClient):
    ...
    await _mk_echo_tool(clients)
    await clients.get("/call/echo/x", headers={"X-Olywork-Meta": "workspace=ws_9"})
    row = await _newest_call(clients)
...
async def test_oversized_and_malformed_bags_are_refused(clients: AsyncClient):
    ...
    for bad in (
        ...
        r = await clients.get("/call/echo/x", headers={"X-Olywork-Meta": bad})
...
async def test_email_shaped_values_are_refused(clients: AsyncClient):
    ...
    await _mk_echo_tool(clients)
    r = await clients.get("/call/echo/x", headers={"X-Olywork-Meta": "customer=jane@example.com"})
...
async def test_a_refused_bag_never_reaches_the_provider(clients: AsyncClient):
    ...
    r = await clients.get("/call/echo/x", headers={"X-Olywork-Meta": "a=1,b=2,c=3,d=4,e=5,f=6"})
    assert r.status_code == 422
    await audit.drain()
...
async def test_a_malformed_bag_does_not_burn_an_idempotency_label(clients: AsyncClient):
    ...
    bad = await clients.get("/call/echo/x", headers={**hdr, "X-Olywork-Meta": "customer"})
    assert bad.status_code == 422
    good = await clients.get("/call/echo/x", headers={**hdr, "X-Olywork-Meta": "customer=cust_1"})
    assert good.status_code == 200, good.text


# ---- egress ------------------------------------------------------------------------------------
async def test_no_olywork_header_reaches_the_upstream(clients: AsyncClient):
    """Customer ids must not be handed to third-party providers. Asserted by PREFIX, not by name:
    the old per-name list had already missed `x-olywork-client`, which leaked to every provider."""
    await _mk_echo_tool(clients)
    r = await clients.get("/call/echo/anything", headers={
        "X-Olywork-Meta": "customer=cust_8123",
        "X-Olywork-Client": "claude-code",
        "X-Olywork-Future": "not-invented-yet",     # a header that does not exist yet must also be dropped
    })
    assert r.status_code == 200, r.text
    seen = {k.lower() for k in r.json()["headers"]}
    assert not [k for k in seen if k.startswith("x-olywork-")], f"olywork headers leaked upstream: {seen}"
...
async def test_tags_do_not_disturb_the_relayed_request(clients: AsyncClient):
    ...
    tagged = (await clients.get("/call/echo/thing?q=1",
                                headers={"X-Olywork-Meta": "customer=cust_1"})).json()
...
# Tier 4 (olywork's own key, metered against the org balance) is the only path that spends money, so the
...
async def test_a_metered_call_attributes_its_spend_to_every_tag(clients: AsyncClient, platform_on):
    ...
    r = await clients.get(f"/call/{EP}?aweme_id=7",
                          headers={"X-Olywork-Meta": "customer=cust_A, workspace=ws_9"})
    assert r.status_code == 200, r.text
...
async def test_a_released_call_bills_no_tag(clients: AsyncClient, platform_on, monkeypatch):
    ...
    org_id = await _org_id(clients)
    await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=cust_A"})
    async with session_maker() as db:
...
async def test_tag_totals_reconcile_with_org_spend(clients: AsyncClient, platform_on):
    ...
    for tags in ("customer=cust_A, workspace=ws_1",
        ...
        assert (await clients.get(f"/call/{EP}?aweme_id=7",
                                  headers={"X-Olywork-Meta": tags})).status_code == 200
...
async def test_caller_tags_cannot_overwrite_ledger_provenance(clients: AsyncClient, platform_on):
    """A hostile bag must not rewrite the money journal. olywork's own keys merge LAST now — before this
    change a caller could zero `charged_micro` or forge the `tier` that reconcile.py reads."""
    org_id = await _org_id(clients)
    r = await clients.get(f"/call/{EP}?aweme_id=7",
                          headers={"X-Olywork-Meta": "charged_micro=0, margin=9, tier=free"})
    assert r.status_code == 200, r.text
...
async def test_no_budget_row_means_unlimited(clients: AsyncClient, platform_on):
    ...
    org_id = await _org_id(clients)
    r = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=brand_new"})
    assert r.status_code == 200, r.text
...
async def test_a_blocked_tag_is_refused(clients: AsyncClient, platform_on):
    ...
    ok = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=cust_B"})
...
async def test_a_blocked_tag_is_refused_before_any_cached_answer(clients: AsyncClient, platform_on):
    ...
    org_id = await _org_id(clients)
    hdr = {"X-Olywork-Meta": "customer=cust_A", "Idempotency-Key": "retry-1"}
    ...
    assert "X-Olywork-Idempotent-Replay" not in again.headers
...
async def test_a_spend_cap_refuses_and_names_its_dimension(clients: AsyncClient, platform_on):
    ...
    first = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=cust_A"})
    assert first.status_code == 200, first.text          # exactly at the cap
    second = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=cust_A"})
...
async def test_stacked_budgets_refuse_independently(clients: AsyncClient, platform_on):
    ...
    hdr_a = {"X-Olywork-Meta": "customer=cust_A, workspace=ws_1"}
    hdr_b = {"X-Olywork-Meta": "customer=cust_B, workspace=ws_1"}
...
async def test_the_workspace_ceiling_bites_once_its_users_together_exceed_it(clients: AsyncClient,
    ...
    for who in ("cust_A", "cust_B"):
        r = await clients.get(f"/call/{EP}?aweme_id=7",
                              headers={"X-Olywork-Meta": f"customer={who}, workspace=ws_1"})
        assert r.status_code == 200, r.text
    blocked = await clients.get(f"/call/{EP}?aweme_id=7",
                                headers={"X-Olywork-Meta": "customer=cust_C, workspace=ws_1"})
...
async def test_two_users_can_reuse_one_idempotency_label(clients: AsyncClient, platform_on):
    ...
    a = await clients.get(f"/call/{EP}?aweme_id=7",
                          headers={**hdr, "X-Olywork-Meta": "customer=cust_A"})
    b = await clients.get(f"/call/{EP}?aweme_id=7",
                          headers={**hdr, "X-Olywork-Meta": "customer=cust_B"})
    assert a.status_code == 200 and b.status_code == 200, (a.text, b.text)
    assert "X-Olywork-Idempotent-Replay" not in b.headers, "cust_B was served cust_A's stored answer"
...
async def test_one_user_still_replays_their_own_label(clients: AsyncClient, platform_on):
    ...
    hdr = {"Idempotency-Key": "retry-1", "X-Olywork-Meta": "customer=cust_A"}
    first = await clients.get(f"/call/{EP}?aweme_id=7", headers=hdr)
    again = await clients.get(f"/call/{EP}?aweme_id=7", headers=hdr)
    assert first.status_code == 200 and again.status_code == 200
    assert again.headers.get("X-Olywork-Idempotent-Replay") == "true"
    org_id = await _org_id(clients)
    async with session_maker() as db:
        assert await ledger.tag_invoice_since(db, org_id, "customer", "cust_A", _EPOCH) == EP_MICRO
...
async def test_untagged_idempotency_is_unchanged(clients: AsyncClient, platform_on):
    ...
    first = await clients.get(f"/call/{EP}?aweme_id=7", headers=hdr)
    again = await clients.get(f"/call/{EP}?aweme_id=7", headers=hdr)
    assert first.status_code == 200 and again.headers.get("X-Olywork-Idempotent-Replay") == "true"
...
async def test_a_reused_label_on_a_different_request_still_says_so(clients: AsyncClient, platform_on):
    ...
    not olywork's internal scoped form."""
    hdr = {"Idempotency-Key": "retry-1", "X-Olywork-Meta": "customer=cust_A"}
...
async def test_usage_by_tag_is_admin_only(clients: AsyncClient):
    ...
    r = await clients.get(f"/orgs/{org_id}/usage/by-tag", headers={"X-Olywork-Token": token})
    assert r.status_code == 403


async def test_every_call_returns_a_join_key(clients: AsyncClient, platform_on):
    """X-Olywork-Call-Id is what lets a builder line our records up against their own, row for row."""
    r = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=cust_A"})
    assert r.status_code == 200
    ref = r.headers["X-Olywork-Call-Id"]
    assert ref
...
async def test_the_join_key_is_scoped_to_the_team(clients: AsyncClient, platform_on):
    ...
    denied = await clients.get(f"/calls/{ref}", headers={"X-Olywork-Token": outsider})
...
async def test_calls_can_be_windowed_and_paged(clients: AsyncClient):
    await _mk_echo_tool(clients)
    for _ in range(3):
        await clients.get("/call/echo/x", headers={"X-Olywork-Meta": "customer=cust_A"})
    await audit.drain()
...
async def test_a_tag_value_cannot_smuggle_the_idempotency_separator(clients: AsyncClient):
    ...
    for hostile in ("customer=A\x1fB", "customer=a b", "customer=a/b", "customer=a,b", "customer=<script>"):
        r = await clients.get("/call/echo/x", headers={"X-Olywork-Meta": hostile})
...
async def test_ordinary_customer_ids_still_pass(clients: AsyncClient):
    ...
    for good in ("cust_8123", "ws-9", "acct.42", "tenant:eu-west",
        ...
        r = await clients.get("/call/echo/x", headers={"X-Olywork-Meta": f"customer={good}"})
...
async def test_the_collision_is_closed_end_to_end(clients: AsyncClient, platform_on):
    ...
    crafted = await clients.get(
        ...
        headers={"X-Olywork-Meta": "customer=A\x1fB", "Idempotency-Key": "C"})
...
async def test_a_reaped_stale_hold_bills_nobody(clients: AsyncClient):
    ...
    from olywork.models import Hold
...
async def test_a_provider_credential_refusal_is_never_billed(clients: AsyncClient, platform_on,
    ...
    expired platform key into real spend on a team's balance — and, for a builder reselling olywork,
    ...
    for status in (401, 402, 403, 407, 408, 429):
        ...
        assert r.headers.get("X-Olywork-Cost-Micro") == "0", f"{status} was billed"
...
async def test_unlimited_until_a_default_is_set(clients: AsyncClient, platform_on):
    ...
    org_id = await _org_id(clients)
    for _ in range(3):
        r = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=anyone"})
        assert r.status_code == 200, r.text
...
async def test_a_default_applies_to_every_value_without_an_override(clients: AsyncClient, platform_on):
    ...
    for who in ("alice", "bob"):                      # neither has ever been seen before
        ok = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": f"customer={who}"})
        assert ok.status_code == 200, ok.text
        capped = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": f"customer={who}"})
...
async def test_an_override_beats_the_default_in_both_directions(clients: AsyncClient, platform_on):
    ...
    for _ in range(3):                                 # vip sails past the default
        assert (await clients.get(f"/call/{EP}?aweme_id=7",
                                  headers={"X-Olywork-Meta": "customer=vip"})).status_code == 200
    tight = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=tight"})
...
async def test_being_seen_does_not_create_an_override(clients: AsyncClient, platform_on):
    ...
    org_id = await _org_id(clients)
    assert (await clients.get(f"/call/{EP}?aweme_id=7",
                              headers={"X-Olywork-Meta": "customer=seen_first"})).status_code == 200
    await clients.put(f"/orgs/{org_id}/budgets/customer", json={"daily_cap_micro": 1})
    after = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "customer=seen_first"})
...
async def test_the_budget_list_shows_policy_not_bookkeeping(clients: AsyncClient, platform_on):
    ...
    for who in ("a", "b", "c"):                        # three registry rows, no limits
        await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": f"customer={who}"})
    assert (await clients.get(f"/orgs/{org_id}/budgets")).json() == []
...
async def test_tag_keys_reports_what_was_sent_not_only_what_is_budgetable(clients: AsyncClient,
    ...
    await clients.get(f"/call/{EP}?aweme_id=7",
                      headers={"X-Olywork-Meta": "customer=c1, feature=email-finder"})
...
async def test_setting_a_limit_declares_the_dimension(clients: AsyncClient, platform_on):
    ...
    ok = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "workspace=ws_9"})
    assert ok.status_code == 200, ok.text
    capped = await clients.get(f"/call/{EP}?aweme_id=7", headers={"X-Olywork-Meta": "workspace=ws_9"})
...
async def test_the_integration_skill_is_served_and_templated(clients: AsyncClient):
    ...
    for must in ("X-Olywork-Meta", "X-Olywork-Call-Id", "X-Olywork-Cost-Micro", "X-Olywork-Error",
