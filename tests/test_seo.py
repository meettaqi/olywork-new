...
`public_url` rather than on the literal olywork.com: a self-hosted registry must advertise itself.
"""
...
async def test_sitemap_redirects_from_a_legacy_host(clients: AsyncClient):
    """A sitemap served on olywork.olywork.com but full of olywork.com URLs is a cross-submission a
    crawler may discard wholesale. Send it to the canonical copy instead."""
    for path in ("/robots.txt", "/sitemap.xml"):
        r = await clients.get(path, headers={"Host": "olywork.olywork.com"},
...
def test_no_shelf_is_published_that_the_app_grid_hides():
    ...
    """
    from olywork.routers.catalog import _platform_rows
...
def _spa() -> str:
    from olywork.routers.web import _WEB_DIR
...
async def test_hub_links_stay_off_a_self_hosted_registry(monkeypatch):
    """The job, workflow and agent pages exist on olywork.com only (`_hosted`), so a self-hosted
    ...
    from olywork.routers.web import INDEXNOW_KEY
