...
Before this, a call olywork refused BEFORE the relay fell into two buckets: money refusals were
...
from olywork.infra.upstream.relay import relay as upstream_relay
...
async def test_unknown_tool_is_recorded_as_a_resolution_refusal(clients: AsyncClient):
    ...
    assert rec.user_email == "tim@olywork.com"  # identity was known — the token was fine
...
async def test_bad_token_is_recorded_anonymously_as_an_auth_refusal(clients: AsyncClient):
    ...
    r = await clients.get("/call/echo-tool/echo", headers={"X-Olywork-Token": "garbage"})
...
async def test_refusals_reach_the_caller_in_the_audit_log(clients: AsyncClient):
    """`olywork audit` is where an org goes to ask "why did my call fail" — the column must be in
...
async def test_a_credential_dead_end_names_a_sibling_olywork_can_already_serve(
    clients: AsyncClient, monkeypatch: pytest.MonkeyPatch,
):
    """The 2026-08-19 report's worst refusal: one org spent 268 calls on
    `meta-ad-library.meta-ads.library.search`, which olywork holds no key for, while
    `scrapecreators.…-search-ads` — the SAME capability, on a key olywork already has — answered 192
    ...
    """
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_SCRAPECREATORS", "platform-scrapecreators-key")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "scrapecreators")
    ...
    assert "olywork connections connect --provider meta-ad-library" in detail
    ...
    assert "callable now on olywork's key" in detail
    ...
    # A provider olywork holds no key for is still worth naming, but must not claim to be callable.
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "")
    A.get_settings.cache_clear()
    unkeyed = (await clients.get(f"/call/{endpoint}", params=query)).json()["detail"]
    assert sibling in unkeyed
    assert "callable now on olywork's key" not in unkeyed
