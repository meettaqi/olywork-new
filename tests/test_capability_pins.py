...
async def test_a_pin_cannot_be_sidestepped_to_spend_OLYWORKS_money(clients: AsyncClient):
    """The boundary, pinned so nobody later assumes a pin blocks a vendor everywhere.

    A pin gates the catalog ID, and a catalog id is the ONLY route to olywork's own key — so the money
    ...
    assert by_url.status_code == 404                      # no org tool → olywork's key is unreachable
...
async def test_a_capability_can_never_restructure_a_url(clients: AsyncClient):
    ...
    As `/orgs/{id}/pins/{capability}`, `olywork org unpin ..` DELETED THE TEAM: every normalizing HTTP
...
async def test_two_admins_pinning_at_once_leave_one_row(clients: AsyncClient):
    ...
    from olywork.models import CapabilityPin
