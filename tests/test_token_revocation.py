...
from olywork.infra.db import reset_db
...
async def test_revoke_kills_old_identity_token_and_issues_a_fresh_one(client):
    ...
    assert (await client.get("/invites/mine", headers={"X-Olywork-Token": fresh})).status_code == 200  # fresh works
...
async def test_revoke_signs_out_other_browser_sessions_but_keeps_the_caller(client):
    ...
    old_cookie = verify.cookies.get("olywork_session")
    assert old_cookie
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://registry",
                           cookies={"olywork_session": old_cookie}) as device_b:
...
async def test_typed_credentials_and_safe_legacy_boundary_end_to_end(client):
    ...
    for cookie in (live_session, expired_session):
        assert (await client.get(
            "/invites/mine", headers={"X-Olywork-Token": cookie},
        )).status_code == 401
    ...
    created = (await client.post(
        "/orgs", json={"name": "Legacy Team"}, headers={"X-Olywork-Token": tok},
    )).json()
    ...
    assert (await client.get(
        "/tools", headers={"X-Olywork-Token": stale_pinned},
    )).status_code == 200
    assert (await client.get(
        "/invites/mine", headers={"X-Olywork-Token": stale_orgless},
    )).status_code == 401
    assert (await client.get(
        "/invites/mine", headers={"X-Olywork-Token": stale_pinned + "x"},
    )).status_code == 401

    # token_version remains the kill switch for permanent and supported legacy identity keys.
    assert (await client.post(
        "/auth/revoke-tokens", headers={"X-Olywork-Token": stale_pinned},
    )).status_code == 200
    assert (await client.get(
        "/invites/mine", headers={"X-Olywork-Token": stale_pinned},
    )).status_code == 401
