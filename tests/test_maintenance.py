...
def _env(tmp_path: Path, *, single_user: bool = False) -> tuple[dict[str, str], Path, Path]:
    ...
    env = os.environ.copy()
    env.update({
        "OLYWORK_DATABASE_URL": f"sqlite+aiosqlite:///{database}",
        "OLYWORK_PUBLIC_URL": "http://localhost:18790",
        "OLYWORK_SINGLE_USER": "true" if single_user else "false",
        "OLYWORK_SINGLE_USER_TOKEN_FILE": str(token_file),
        "OLYWORK_CLAUDE_CONNECTOR_ENABLED": "false",
    })
...
def _upgrade(env: dict[str, str]) -> subprocess.CompletedProcess[str]:
    return _run(["-m", "olywork", "upgrade"], env)
...
def _create_unstamped_schema(env: dict[str, str]) -> None:
    script = textwrap.dedent(
        ...
        from olywork import models
        from olywork.infra.db import _engine, dispose_engine
...
def test_real_serve_path_can_query_database_after_pre_serve_maintenance(tmp_path):
    ...
    port = _free_port()
    base_url = f"http://127.0.0.1:{port}"
    env.update({"PORT": str(port), "OLYWORK_PUBLIC_URL": base_url})
        ...
        try:
            ...
            if ready and token_file.exists():
                request = urllib.request.Request(
                    f"{base_url}/tools",
                    headers={"X-Olywork-Token": token_file.read_text().strip()},
                )
                try:
...
def test_empty_database_upgrade_uses_pure_alembic_without_provisioning_a_user(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    assert "olywork schema: alembic upgrade head (empty database)" in result.stdout
    assert "olywork upgrade complete" in result.stdout
    with sqlite3.connect(database) as db:
...
def test_raw_asgi_refuses_a_database_behind_head(tmp_path):
    ...
    assert "python -m olywork upgrade" in server_stderr
...
def test_upgrade_refuses_an_unstamped_pre_adoption_database(tmp_path):
    ...
    assert "python -m olywork upgrade" in result.stderr
...
def test_upgrade_applies_a_pending_revision_from_one_behind_head(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    assert "olywork schema: alembic upgrade head (stamped database)" in result.stdout
    assert _alembic_version(database) == _alembic_head()
...
def test_app_lifespan_does_not_run_release_backfills(tmp_path):
    ...
    script = textwrap.dedent(
        ...
        from olywork.bootstrap import create_app
