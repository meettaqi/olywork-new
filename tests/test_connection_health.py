...
The failure this exists to prevent: a credential olywork cannot renew itself (LinkedIn issues no
...
from olywork import crypto, health, oauth
...
async def test_a_refreshable_credential_never_asks_for_reconnect(clients: AsyncClient):
    """olywork renews these itself; nagging about their 1-hour expiry would be pure noise."""
    sid = await _connect(clients)
    rows = {r["secret_id"]: r for r in (await clients.get("/health")).json()}
...
async def test_a_failed_refresh_records_why(clients: AsyncClient, olywork_google_app):
