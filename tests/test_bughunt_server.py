...
from olywork import audit, crypto, oauth
...
from olywork.models import Secret, User
...
async def test_otp_code_dies_after_max_wrong_attempts(c):
    from olywork.routers.auth import MAX_OTP_ATTEMPTS
...
async def test_proxy_response_is_sandboxed(c):
    ...
    # a browser navigating to /call/… must not execute upstream HTML/JS under olywork's origin
    assert r.headers.get("x-content-type-options") == "nosniff"
    assert r.headers.get("content-security-policy") == "sandbox"


async def test_upstream_setcookie_cannot_overwrite_olywork_cookies(c):
    from starlette.responses import Response
    from httpx import ASGITransport, AsyncClient as AC
    evil = FastAPI()

    @evil.api_route("/{p:path}", methods=["GET"])
    async def h(p):  # an upstream that tries to plant olywork's own cookies
        resp = Response(content=b"{}", media_type="application/json")
        resp.raw_headers.append((b"set-cookie", b"olywork_session=EVIL; Path=/"))
    ...
    assert not any("olywork_session" in x for x in setc)  # our session cookie is NOT overwritable by an upstream
...
async def test_numeric_slug_resolves_as_slug_not_id():
    from olywork.domain.identity.access import _resolve_org
...
async def test_revoke_only_pending_invites(c):
    ...
    async with session_maker() as db:
        from olywork.models import Invite
...
def test_safe_webhook_url_blocks_internal():
    from olywork.health import safe_webhook_url
...
async def test_oauth_callback_replay_keeps_done(c):
    from olywork.models import PendingOAuth
    async with session_maker() as db:
