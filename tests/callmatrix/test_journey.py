...
async def test_j1_signup_to_topup_and_recovery(
    ...
    monkeypatch.setattr("olywork.email.send_otp", capture_otp)
    ...
    # A catalog call uses olywork's key and settles its reserved estimate.
    ...
    assert platform_call.headers["X-Olywork-Cost-Micro"] == str(charged)
    ...
    # `EP_MICRO` passes only while OLYWORK_PLATFORM_MARGIN is 0.
    ...
    event = {"id": "evt_journey_topup", "type": "payment_intent.succeeded", "data": {"object": {
        ...
        "metadata": {
            "olywork_org_id": str(org_id), "olywork_kind": "topup", "olywork_auto": "0",
        },
    }}}
    ...
    assert viewer_call.headers.get("X-Olywork-Error") == "1"
