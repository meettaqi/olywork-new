...
async def assert_outcome(
    ...
    assert (response.headers.get("X-Olywork-Error") == "1") is expect.olywork_error
    if expect.cost_micro is None:
        assert "X-Olywork-Cost-Micro" not in response.headers
    else:
        assert response.headers.get("X-Olywork-Cost-Micro") == str(expect.cost_micro)
    ...
    assert call_id, "every /call response must carry X-Olywork-Call-Id"
    return row
