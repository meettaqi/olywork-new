...
from olywork.domain.identity import access as identity_access
...
async def test_e4_member_tool_acl(
    ...
    await _register_echo(matrix_clients)
    org_id = (await matrix_clients.get("/orgs")).json()[0]["org_id"]
    owner_token = matrix_clients.headers["X-Olywork-Token"]
    ...
    before = await snapshot(matrix_clients, fake_provider)
    matrix_clients.headers["X-Olywork-Token"] = agent.json()["token"]
    try:
        response = await matrix_clients.get("/call/echo/blocked")
    finally:
        matrix_clients.headers["X-Olywork-Token"] = owner_token
...
async def test_e5_public_demo_catalog_call_is_resolution_refusal(
    ...
    owner_token = matrix_clients.headers["X-Olywork-Token"]
    before = await snapshot(matrix_clients, fake_provider)
    matrix_clients.headers["X-Olywork-Token"] = published.json()["token"]
    try:
        response = await matrix_clients.get(f"/call/{EP}?aweme_id=7")
    finally:
        matrix_clients.headers["X-Olywork-Token"] = owner_token
...
async def test_d2_concurrent_same_key_loser_gets_409(
    ...
    assert loser.headers["X-Olywork-Error"] == "1"
    await assert_outcome(
    ...
    loser_call_id = loser.headers.get("X-Olywork-Call-Id")
    if loser_call_id is not None:
        assert loser_row["call_ref"] == loser_call_id
    assert loser_call_id, "every /call response must carry X-Olywork-Call-Id"
...
# F group: the relay preserves caller bytes while stripping olywork's own session boundary.
...
async def test_f3_olywork_cookie_is_scrubbed_both_directions(
    matrix_clients: AsyncClient, fake_provider: FakeProvider,
) -> None:
    await _register_echo(matrix_clients)
    before = await snapshot(matrix_clients, fake_provider)
    ...
    response = await matrix_clients.get(
        "/call/echo/cookies",
        headers={
            "Cookie": "olywork_session=caller-secret; ordinary=keep-me",
            "X-Custom-Proof": "kept",
            "X-Fake-Set-Cookie": "olywork_session=evil; Path=/; HttpOnly",
        },
    )
    ...
    assert "olywork_session" not in hit.headers["cookie"]
    assert all("olywork_session" not in value.lower() for value in response.headers.get_list("set-cookie"))
...
async def test_g1_ten_concurrent_calls_compete_for_three_call_balance(
    ...
    assert all(response.headers.get("X-Olywork-Error") != "1" for response in responses if response.status_code == 200)
    assert all(response.headers.get("X-Olywork-Error") == "1" for response in responses if response.status_code == 402)
    assert all(response.headers.get("X-Olywork-Cost-Micro") == str(charged)
    ...
    assert all(response.headers.get("X-Olywork-Call-Id") for response in responses), (
        "every concurrent outcome must carry X-Olywork-Call-Id")
...
async def test_h1_saturation_503_still_carries_a_call_id_and_a_row(
    ...
    with caplog.at_level(logging.WARNING, logger="olywork.audit"):
        response = await matrix_clients.get("/call/echo/ping")
    await audit.drain()

    assert response.status_code == 503
    assert response.json()["olywork_saturated"] is True
    assert response.headers["Retry-After"] == "2"
    call_id = response.headers.get("X-Olywork-Call-Id")
    assert call_id, "a saturation 503 must still carry X-Olywork-Call-Id"
    ...
    # A 5xx is olywork failing, not olywork refusing — `_refusal_kind` maps it to None.
    assert row["refused_by"] is None
    (event,) = await posthog_events()
    assert event["distinct_id"] == "tim@olywork.com"
    expected = {
        "status_code": 503,
...
async def test_h2_unexpected_500_is_reported_once_as_olywork_owned(
    ...
    async with AsyncClient(
        ...
        headers={"X-Olywork-Token": matrix_clients.headers["X-Olywork-Token"]},
    ) as remote_client:
...
async def test_h3_pool_saturation_before_identity_uses_an_intake_event(
    ...
    assert response.status_code == 503
    assert response.json()["olywork_saturated"] is True
    assert response.headers.get("X-Olywork-Call-Id")
    assert await posthog_events() == []
    (event,) = await posthog_events("call_intake_failed")
    assert event["distinct_id"] == "olywork-server"
    expected = {
        "status_code": 503,
        "outcome": "gateway_failed",
        "failure_kind": "db_pool",
...
async def test_h4_catalog_saturation_releases_idempotency_claim(
    ...
    assert first.json()["olywork_saturated"] is True
    assert first.headers.get("X-Olywork-Call-Id")
    assert second.status_code == 200, second.text
    assert len(fake_provider.hits) == 1
    rows = (await matrix_clients.get("/calls")).json()
    failed = next(row for row in rows if row["status_code"] == 503)
    assert failed["tool_name"] == EP
    assert failed["call_ref"] == first.headers["X-Olywork-Call-Id"]
    (event,) = [
