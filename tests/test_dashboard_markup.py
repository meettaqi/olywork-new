...
def test_oauth_entry_opens_the_existing_sign_in_modal_without_minting_a_sandbox():
    ...
    assert "Sign in to continue connecting Olywork" in INDEX
...
def test_ref_entry_opens_sign_in_without_minting_a_sandbox():
    ...
    assert "localStorage.getItem('olywork-sbx')" not in INDEX
...
def test_connection_rows_retain_the_shared_production_layout():
    ...
    assert "<span class=\"mono\" :title=\"'olywork call '+c.name\">{{c.name}}</span>" in INDEX
...
def test_platform_provider_navigation_does_not_wait_for_connection_registry_state():
    ...
    assert "seen.filter(s=>s!=='olywork')" in block
...
def test_the_expansion_carries_a_paste_ready_call_line():
    """Finding the endpoint was never the goal — running it is. The `olywork call` line is served on
...
def test_the_run_actions_lead_the_expansion_tab_bar():
    ...
    Try-it is the primary (ink-fill) CTA for a key/token provider — trying on olywork's key is what most
    visitors want — with the own-key path demoted to a secondary "Bring your own key". For an OAuth
    provider olywork can't serve on its own key, so Connect is the primary instead and Try-it is
...
def test_try_it_selects_registry_authorization_and_updates_every_call_surface():
    ...
    assert 'X-Olywork-Authorization-Method: ${this.epTryAuthMethod}' in logic
    ...
    assert "opts.headers['X-Olywork-Authorization-Method']=this.epTryAuthMethod" in runner
...
def test_the_topup_modal_offers_four_presets_plus_other_and_an_auto_toggle():
    ...
    assert "I authorize olywork to charge my saved card <b>${{autoAmount}}</b>" in modal
    assert "Processing fee" not in modal, "olywork charges no fee; do not copy one from elsewhere"
