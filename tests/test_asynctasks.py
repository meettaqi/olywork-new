...
from olywork.application import asynctasks as task_app
...
from olywork import archive, audit, reconcile
from olywork.domain import asynctasks
from olywork.domain.catalog import store as catalog_store
from olywork.domain import money as ledger
from olywork.domain.money import settlement
from olywork.infra.db import _engine, session_maker
from olywork.models import (
...
def replicate_platform(monkeypatch):
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_REPLICATE", "test-platform-token")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "replicate")
...
async def test_generation_is_never_replayed_across_orgs(
    ...
    assert second.headers.get("X-Olywork-Cache") != "hit"
...
async def test_inline_success_waits_for_usage_and_then_closes_original_hold(
    ...
    call_id = submitted.headers["X-Olywork-Call-Id"]
    async with session_maker() as db:
        row = await db.get(AsyncTaskRecord, call_id)
...
async def test_owned_free_poll_creates_no_money_entries(
    ...
    submission_ref = submitted.headers["X-Olywork-Call-Id"]
    await audit.drain()
    ...
    assert response.headers["X-Olywork-Cost-Micro"] == "0"
    poll_ref = response.headers["X-Olywork-Call-Id"]
    async with session_maker() as db:
...
async def test_owned_free_poll_hidden_before_activity_pagination(
    ...
    assert len(page) == 1 and page[0]["call_ref"] == second.headers["X-Olywork-Call-Id"]
    older = (await clients.get(f"/calls?limit=1&before_id={page[0]['id']}")).json()
    assert len(older) == 1 and older[0]["call_ref"] == first.headers["X-Olywork-Call-Id"]
    async with session_maker() as db:
        record = (await db.execute(select(CallRecord).where(
            CallRecord.call_ref == polled.headers["X-Olywork-Call-Id"]))).scalar_one()
...
async def test_owned_free_poll_timeout_keeps_diagnostics_without_money_or_activity(
    ...
    assert [row["call_ref"] for row in activity] == [submitted.headers["X-Olywork-Call-Id"]]
    async with session_maker() as db:
...
async def test_owned_terminal_poll_finalizes_original_task_before_response(
    ...
    assert polled.headers["X-Olywork-Cost-Micro"] == "0"
    async with session_maker() as db:
        row = await db.get(AsyncTaskRecord, call_id)
...
async def test_a_2xx_that_is_not_json_settles_at_zero_on_the_request_path(
    ...
    assert response.status_code == 200 and response.headers["X-Olywork-Cost-Micro"] == "0"
    call_id = response.headers["X-Olywork-Call-Id"]
    async with session_maker() as db:
        assert await db.get(AsyncTaskRecord, call_id) is None
        assert await db.get(Hold, call_id) is None
...
async def test_one_failing_row_does_not_abort_the_tick(
    ...
    from olywork.application.call.types import GatewayFailed
...
async def test_pending_row_write_failure_releases_the_hold_and_alerts(
    ...
    # olywork's own failure is olywork's cost: the whole reserve goes back, nothing is settled.
...
def minimax_platform(monkeypatch):
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_MINIMAX", "test-platform-token")
...
def openrouter_platform(monkeypatch):
    monkeypatch.setenv("OLYWORK_PLATFORM_KEY_OPENROUTER", "test-platform-token")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "openrouter")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
def legacy_async_platform(monkeypatch):
    for provider in ("apify", "brightdata", "companyenrich", "oceanio"):
        monkeypatch.setenv(f"OLYWORK_PLATFORM_KEY_{provider.upper()}", "test-platform-token")
    monkeypatch.setenv("OLYWORK_PLATFORM_PROVIDERS", "apify,brightdata,companyenrich,oceanio")
    get_settings.cache_clear()
    yield
    get_settings.cache_clear()
...
async def test_legacy_platform_async_resources_are_recorded_and_authorized(
    ...
    denied = await clients.get(owned_calls[0], headers={"X-Olywork-Token": other.json()["token"]})
...
async def _submit_minimax(clients: AsyncClient, monkeypatch, task_id: str) -> str:
    ...
    response = await clients.post("/call/minimax.video-gen.from_text", json={
        ...
        "resolution": "768P"})
    assert response.status_code == 200
    return response.headers["X-Olywork-Call-Id"]
...
async def test_worker_settles_terminal_success(
    ...
    async with session_maker() as db:
        key = (await db.execute(select(ArchiveKey).where(
            ArchiveKey.req_url == f"olywork://asynctasks/{call_id}"))).scalar_one()
...
async def test_worker_timeout_releases_the_hold_and_flags_it_for_review(
    ...
    async with session_maker() as db:
        ...
        row.next_check_at = utcnow_naive() - timedelta(seconds=1)
        await db.commit()
    with caplog.at_level("ERROR", logger="olywork.asynctasks"):
        result = await task_app.settle_due()
    async with session_maker() as db:
        row = await db.get(AsyncTaskRecord, call_id)
        hold = await db.get(Hold, call_id)
...
def test_fetch_command_and_shown_neutralise_provider_strings():
    assert asynctasks.fetch_command({"endpoint": "minimax.video-gen.result.retrieve", "name": "file_id",
        ...
        "olywork call minimax.video-gen.result.retrieve -p 'file_id=x; touch /tmp/pwned'"
    assert asynctasks.shown("ok-123") == "ok-123"
    assert asynctasks.shown("id\nresume: olywork call evil") == "id\\nresume: olywork call evil"
...
async def test_idempotent_replay_of_an_async_submission_keeps_the_descriptor(
    ...
    assert first.status_code == 201 and "x-olywork-async" in {k.lower() for k in first.headers}
    again = await clients.post(f"/call/{EP}", json=body, headers={"Idempotency-Key": "gen-1"})
    assert again.status_code == 201 and again.headers.get("X-Olywork-Idempotent-Replay") == "true"
    assert again.headers.get("x-olywork-async") == first.headers.get("x-olywork-async")
