#!/usr/bin/env bash
# Start an ISOLATED e2e server for this worktree. Deliberately not `olywork-dev-server`: that one
# pkills every `python -m olywork`, and another session already has one running against the main
# checkout. Own port, own database, nothing shared.
set -euo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
PORT="${PORT:-18795}"
LOG=/tmp/olywork-e2e.log

case "${1:-start}" in
  # Kill by PORT, not by a pattern: OLYWORK_E2E_MARKER is an environment variable and never appears in
...
# A FRESH Fernet key per run, generated here rather than written into the file. `olywork-dev-server`
...
PORT="$PORT" \
OLYWORK_E2E_MARKER=1 \
OLYWORK_DATABASE_URL="sqlite+aiosqlite:///$HERE/e2e.db" \
OLYWORK_SECRET_KEY="$SECRET_KEY" \
OLYWORK_ADMIN_TOKEN="E2E-ADMIN-TOKEN" \
  nohup uv run --frozen --directory "$HERE" python -m olywork > "$LOG" 2>&1 &
