...
nav_html = """<div class="navwrap"><nav class="nav">
  ...
  <div class="links" style="display:flex;gap:32px;align-items:center;">
    ...
    <a href="/app?ref={ref}" style="background:var(--ink);color:#fff;padding:10px 24px;border-radius:99px;font-weight:600;">Start free</a>
  </div>
</nav></div>"""
...
# Replace navwrap in web.py
web_py = re.sub(r'<div class="navwrap"><nav class="nav">.*?</nav></div>', nav_html.replace("{ref}", "{ref}"), web_py, flags=re.DOTALL)
web_py = re.sub(r'<footer>.*?</footer>', footer_html.replace("{hub_links}", "{hub_links}").replace("{_GH}", "{_GH}"), web_py, flags=re.DOTALL)
