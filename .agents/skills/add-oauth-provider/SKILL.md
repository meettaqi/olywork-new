...
description: Add a provider to olywork's OAuth registry (the ones olywork holds its own approved app for). Use when asked to "add YouTube/Notion/Meta OAuth", "support connecting X", "add a new OAuth provider", or when a connect flow, capability picker, channel/account picker, or provider health probe needs building. Covers the code changes, the platform-side approval steps, and the pitfalls that don't announce themselves.
---
...
skill is the **registry** path: olywork owns the registered app, so the user picks a
provider and supplies nothing. Only add a provider here if olywork holds — or is
willing to go get — the platform approval behind it.

Read `src/olywork/oauth_providers.py`'s module docstring first. The rule it states is
...
### 1. Credentials → `src/olywork/config.py`

Add `<name>_client_id` / `<name>_client_secret` to `Settings`, loaded from
`OLYWORK_<NAME>_CLIENT_ID` / `_SECRET`. **Reuse an existing pair when the platform
...
### 2. The provider → `src/olywork/oauth_providers.py`
...
### 5. Dashboard → `src/olywork/web/index.html`
...
3. **Redirect URI** must match `{OLYWORK_PUBLIC_URL}/oauth/callback` exactly.
...
connection then breaks weekly — fatal for olywork, and invisible until day 8. Confirm
...
**`OLYWORK_PUBLIC_URL` decides the redirect_uri, not the port you're browsing.**
