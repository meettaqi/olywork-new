...
description: Traps to avoid when querying Google Analytics 4 through olywork — cases where the GA4 Data API returns a confident wrong answer instead of an error. Use whenever answering questions about site traffic, visitors, pageviews, channels, conversions, or revenue from GA4.
---

# GA4 via olywork — what will silently mislead you
...
Property id: `olywork connections ls` → the `google-analytics` row's `resource_ref`.
...
To list properties, use olywork, not the proxy:

```bash
olywork connections resources <connection-id>   # hits the Admin API server-side
```
