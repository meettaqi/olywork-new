...
description: Traps to avoid when querying Google Search Console through olywork — cases where the API returns a confident wrong answer instead of an error, especially around totals and recent-date trends. Use whenever answering questions about organic search clicks, impressions, rankings, or index status.
---

# Search Console via olywork — what will silently mislead you
...
Site id: `olywork connections ls` → `resource_ref`. If it's empty, **no site is pinned** — list them
with `olywork connections resources <id>` and say which one you chose and why. Don't present an
