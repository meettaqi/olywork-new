...
```bash
uv run --frozen olywork call google-ads "v22/customers/2277522568:generateKeywordHistoricalMetrics" \
...
Google Ads is an **own-key tool, so this costs nothing**. Use the tool-scoped form (`olywork call
...
```bash
uv run --frozen olywork catalog search "<the job in plain words>"
uv run --frozen olywork catalog get <endpoint-id>          # params, cost, verified, siblings
```
...
**Use-case page** — add to `USE_CASE_PAGES` in `src/olywork/agent_pages.py`, keyed by
...
starting "Using olywork, …"), `prompt_why` (4 × `(lead, one line)`), `result_noun`, `what_is_heading`,
...
`set up olywork — https://olywork.com/llms.txt` into that client's chat, and hand it the team token
(header `X-Olywork-Token`) only if it asks. That is what the dashboard's own onboarding shows, so the
...
uv run --frozen python -m olywork          # then open the page and READ it in a browser
```
...
- `olywork.com`, never bare `olywork`, in titles, H1s and anchors.
- Never claim olywork.com chooses or fails over between providers. It compares; the agent picks, or the
  reader tells it how.
- Observed reliability stats may be published per vendor (Taqi, 2026-08-21) with the "live traffic,
