...
description: Build a olywork provider skill — the endpoint map + mistake map that lets an agent do real work on a platform API through olywork's proxy. Use when adding a skill for a connected provider (Google Ads, LinkedIn, Meta Ads, TikTok, X, Instagram, Search Console), or when an existing provider skill needs verifying or extending.
---
...
refresh, and the audit trail, which is where olywork bugs surface.
...
```bash
sqlite3 olywork-dev.db "select method,path,status_code from callrecord order by id desc limit 5"
```
...
# <Platform> via olywork
  one line: you call X through olywork's proxy, you never hold a credential
...
- **Resource id format** — exact prefix, exact encoding. Read it from `olywork connections ls`
  → `resource_ref`. If empty, `olywork connections resources <id>` then `olywork connections use`.
...
- **Scope vs capability** — what the connection actually granted (`olywork connections ls` →
  `scopes`, `capabilities`, `missing_capabilities`), not what the platform offers.

## Also record olywork friction

This exercise doubles as a olywork test. When an agent would plausibly guess wrong about *olywork's own
interface*, note it — those are product bugs, not skill content. Found so far:

- `olywork connections list` is invalid; the subcommand is `ls`
- `olywork connections resources` takes a numeric id, not a provider name
...
- [ ] olywork friction recorded separately from platform friction
