---
name: dev-local
description: One-command local dev stack for tools-registry. Use when asked to "start the dev server", "run olywork locally", "test login locally", "bring the stack up", or before any manual/browser test against localhost.
---

# dev-local — the local olywork stack in one command

`scripts/dev-local.sh` runs the FastAPI server in a tmux session with dev-safe
settings, and gives you a sandboxed CLI that never touches `~/.olywork/config.json`.

| Service | Command (managed by the script) | Port |
|---|---|---|
| olywork server | `uv run python -m olywork --reload` + `OLYWORK_EMAIL_DEV_MODE=true`, own sqlite `olywork-dev.db` | 18790 |
...
scripts/dev-local.sh cli <args>  # working-tree olywork CLI, sandboxed HOME, pre-pointed at localhost
scripts/dev-local.sh reset       # down + wipe olywork-dev.db and the CLI sandbox
```
...
`~/.olywork/config.json` (usually pointing at production) is never overwritten.
