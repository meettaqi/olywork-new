...
description: Use when setting up, changing or debugging olywork's own conversion tracking — the ad-click capture, the AdConversion outbox, or the Data Manager uploader — and whenever asked whether conversions are "working", "live" or "verified". Also use before claiming any part of the pipeline is proven.
---

# olywork's ad conversion tracking — what breaks, and what "verified" actually means

olywork records three conversions against the ad click that produced them: **signup**, **first
...
|---|---|---|---|
| 1 | Script is served | `curl -s https://olywork.com/adtrack.js \| wc -c` | HTTP 200 with a **0-byte body** — the route serves an empty script when `adsconv.enabled()` is false |
...
curl -s https://olywork.com/adtrack.js | wc -c

# 2. does every destination load it?
for p in / /resources /use-cases/seo-data-for-ai-agents; do
  printf '%-42s %s\n' "$p" "$(curl -s "https://olywork.com$p" | grep -c adtrack.js)"; done

# 3-5. the whole chain, in production, without spending anything
curl -s -X POST https://olywork.com/users -H 'Content-Type: application/json' \
  -H 'Cookie: olywork_ad=gclid%7CPROBE1%7Cp2' -d '{"email":"you+probe@example.com"}'
...
**A numeric `transactionId` is rejected.** `"2"` fails, `"olywork-2"` succeeds, every other field
...
**The scope belongs to olywork, not to customers.** The uploader authenticates with a platform refresh
...
It also misses `olywork cli run` / `olywork with`, which bypass `/call/` entirely.

## Changing the uploader

1. Read `docs/context/architecture/ads-conversions.md` first.
2. Change `src/olywork/adsconv.py` only — the outbox, chokepoints, dedupe and money rules stay put.
