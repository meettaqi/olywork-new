...
**If the tools error or you cannot see them**, the plugin needs a token. The human sets `OLYWORK_TOKEN`:

1. Sign in at https://olywork.com (GitHub / Google / email code)
2. Copy the token from Settings → API Token
3. Set `OLYWORK_TOKEN` in the environment Cursor reads (or add it in Cursor's plugin configuration)
