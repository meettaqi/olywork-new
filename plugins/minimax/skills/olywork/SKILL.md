...
This plugin ships the skill, so you have this page — but not yet the `olywork` command. Set it up
**once**:

```bash
curl -fsSL https://olywork.com/install.sh | sh   # 1. the CLI (skip if `olywork --version` already works)
olywork login                                   # 2. sign in — opens a browser
```
...
Do **not** run `olywork mcp install` here: it writes configs for other agents, not for MiniMax.
