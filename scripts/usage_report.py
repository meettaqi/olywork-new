...
(Render dashboard -> olywork-db -> Access Control) before doing anything else. Nothing here writes:
every statement is a SELECT, and the connection is opened read-only.

It deliberately uses RAW SQL rather than the ORM. Importing `olywork.models` binds the query to
...
----------------------------------------------------------------------------------------------------
  refused    `refused_by IS NOT NULL` — OLYWORK said no before a byte went upstream: no balance, a daily
...
DB_ID = os.environ.get("OLYWORK_PROD_DB_ID", "dpg-d94fp3d7vvec73di5rqg-a")
...
with c as (
  select
    ...
    -- the team registered themselves or a shape olywork could not resolve at all.
...
QUERIES: dict[str, str] = {
    ...
    # improve without opening anything: `req` is a request olywork rejected, `res` is something it
    ...
    #
    # `reason` distinguishes three owners, not two: olywork refused it, olywork could not reach the
    ...
    "errdetail": BASE + """
      select ep, cataloged,
             ...
             -- re-attributed olywork's own 502s to the provider — the same false-diagnosis class this
             ...
             case when refused_by is not null then refused_by
                  when error_response like 'olywork:%' then '(olywork never reached it)'
    ...
    "tiers": BASE + """
      select case credential_tier
               when 'platform'   then 'platform — olywork''s key, metered'
...
def print_summary(data: dict, days: int, top: int) -> None:
    ...
    fails = data["failures"]
    if fails:
        print("\n\033[1mtop provider failures\033[0m  (the provider answered badly — olywork's refusals are excluded)")
        for f in fails[:12]:
...
def svg_stacked(daily: list[dict], unit: str = "day") -> str:
    """Volume as ok / provider-failed / olywork-refused. Hand-built bars — no library, no CDN."""
    if not daily:
    ...
    return (f'<svg viewBox="0 0 {W} {H}" width="100%" height="{H}" role="img" '
            ...
            '<span><b style="background:var(--warn)"></b>olywork refused</span></div>')
...
PLAIN = {
    ...
    "resolution": ("not found", "no such endpoint, or olywork has no key to call it with"),
    ...
    "(olywork never reached it)": ("olywork could not reach the provider",
...
def _detail_rows(detail: list[dict], cols: int) -> str:
    ...
    identical 400 can be the provider rejecting a relayed call or olywork refusing to relay it at all,
    ...
    for d in detail:
        ...
        who = ('<span class="pill crit">provider</span>' if upstream
               else '<span class="pill warn">olywork</span>')
        ...
        if said or sent:
            ...
            rows.append('<tr class="evrow"><td></td><td colspan="5"><div class="ev dim">'
                        ...
                        'answer are the team\'s to inspect, not olywork\'s</div></td></tr>')
    return (f'<tr class="detail" hidden><td colspan="{cols}">'
            ...
            f'a bare id never left olywork</span></th></tr></thead>'
...
def build_html(data: dict, days: int, top: int, since: dt.datetime, unit: str = "day") -> str:
    ...
    kpis = [
        ...
        ("Refused by olywork", f"{pct(s['refused'], calls)}", f"{s['refused']:,} calls", "warn" if s["refused"] else ""),
...
<title>olywork Call Ledger</title>
<style>{CSS}</style>
</head><body>
<div class="wrap">
  <header>
    <div><h1>olywork call ledger</h1>
  ...
  <div class="card"><h2>Tool requests — all {len(reqs)} open{req_new_label}</h2>
    ...
    <b>Read them against the catalog before building anything.</b> A request for something olywork
  ...
  <div class="card"><h2>Searches that matched nothing — {len(misses)} distinct
    ...
    <b>Read each against the catalog before adding anything:</b> a miss for something olywork already
  ...
  <div class="card"><h2>Catalog endpoints — all {len(cataloged)}</h2>
    ...
    <b>Provider said no</b> — olywork passed the call on and the provider rejected it. Their end.<br/>
    <b>Bad request</b> — wrong HTTP method, or a missing/invalid parameter. olywork stopped it; nothing
    was sent and nothing was charged. <span class="dim">(stored as <code>request</code>)</span><br/>
    <b>Not found</b> — no such endpoint, or it exists but olywork has no key to call it with. Also
  ...
  <div class="card"><h2>Not a catalog endpoint — all {len(other)}</h2>
    ...
    itself</b> (real hosts — google-ads, search-console, render, vercel), and <b>calls olywork could not
  ...
  <div class="cols">
    <div class="card"><h2>Why olywork said no</h2>{refusals}
  ...
  <footer>
    <b>Stopped by olywork</b> = refused before anything went upstream (bad request, not found, out of
