#!/usr/bin/env python3
"""Generate every plugin's SKILL.md from the one source: `src/olywork/web/skill.md`.

Five shop windows, four bootstraps, one source:

- **codex**  -> `plugin/skills/olywork/SKILL.md` — ships an MCP connector, so its bootstrap points at
  the five tools and tells the reader NOT to reach for a terminal.
- **claude** -> `skills/olywork/SKILL.md` — skills-only by design (nothing about it waits on a
  ...
  Code ignores.
- **cursor** -> `plugins/olywork/skills/olywork/SKILL.md` — same CLI bootstrap as claude; Cursor
  prescribes its own layout, where the plugin root is never the repo root.
- **dsh**    -> `dsh/skills/olywork/SKILL.md` — DeepSeek Harness, the only surface that ships the
  connector AND the CLI path in one install: the MCP row is disabled until `OLYWORK_TOKEN` exists, so
  its bootstrap has to cover both states and steer away from `olywork mcp install`, which cannot write
  a dsh profile. See docs/DSH-PLUGIN.md.
- **minimax** -> `plugins/minimax/skills/olywork/SKILL.md` — skills-only (MiniMax forbids credentials in the
  package), CLI bootstrap with NO `olywork mcp install` step. See docs/MINIMAX-PLUGIN.md.
...
Each plugin is a shop window, and what it ships is the SAME skill `olywork skill bootstrap` already
...
2. A short section is prepended, and it is the ONLY thing that differs between the two variants —
   ...
   `install.sh` -> `olywork login` -> `olywork mcp install`, in that order (step 3 exits without writing
...
SOURCE = ROOT / "src" / "olywork" / "web" / "skill.md"
PUBLIC_BASE = "https://olywork.com"

CODEX_TARGET = ROOT / "plugin" / "skills" / "olywork" / "SKILL.md"
CLAUDE_TARGET = ROOT / "skills" / "olywork" / "SKILL.md"
...
# Unlike Claude Code, the plugin root is never the repo root — `source` must be `./plugins/olywork`.
CURSOR_TARGET = ROOT / "plugins" / "olywork" / "skills" / "olywork" / "SKILL.md"
...
DSH_TARGET = ROOT / "dsh" / "skills" / "olywork" / "SKILL.md"
...
MINIMAX_TARGET = ROOT / "plugins" / "minimax" / "skills" / "olywork" / "SKILL.md"
...
```bash
curl -fsSL {BASE}/install.sh | sh   # the CLI, pointed at olywork
...
# first, and fall back to CLI only if they are not there. The MCP config uses `${env:OLYWORK_TOKEN}` for
...
# ahead of `olywork login` is a silent no-op an agent would move straight past.
CLI_BOOTSTRAP = """
...
```bash
curl -fsSL {BASE}/install.sh | sh   # 1. the CLI (skip if `olywork --version` already works)
...
```bash
export OLYWORK_TOKEN=<token from {BASE} → sign in → copy token>
```
...
# step 3: `olywork mcp install` writes configs for Claude Code, Cursor and opencode, none of which is a
...
# `olywork mcp install` writes a verified config for it. Cursor now ships MCP alongside the skill, so it
...
# the only one where `olywork mcp install` is the wrong move — it writes configs for other agents, not
# for a dsh profile.
VARIANTS = {
...
def render(variant: str) -> str:
    ...
    if stamp_version:
        ...
        # parent directory, which `olywork` already does). Claude Code ignores the extra key, so
    ...
    #    sets OLYWORK_ROUTED_DISCOVERY=off. A plugin copy is static and cannot know a deployment's
