#!/usr/bin/env bash
# smoke-domain.sh — live verification of the olywork.olywork.com → olywork.com cutover, phase-aware.
#
#   scripts/smoke-domain.sh pre     # after the INERT deploy (env still on the old domain)
#   scripts/smoke-domain.sh post    # after flipping OLYWORK_PUBLIC_URL to https://olywork.com
#
...
OLD="https://olywork.olywork.com"
NEW="https://olywork.com"
PHASE="${1:-}"
...
for base in "$OLD" "$NEW"; do
  ...
  # OLYWORK_SMOKE_TOKEN to a real token to make this check authoritative (expects 200).
  auth_args=()
  [ -n "${OLYWORK_SMOKE_TOKEN:-}" ] && auth_args=(-H "Authorization: Bearer $OLYWORK_SMOKE_TOKEN")
  ...
  if [ -n "${OLYWORK_SMOKE_TOKEN:-}" ]; then
    [ "$c" = 200 ] && note PASS "$base/mcp/ authed initialize → 200 (host+origin accepted)" \
                   || note FAIL "$base/mcp/ authed initialize → $c (expected 200)"
  else
    case "$c" in
      200|401) note PASS "$base/mcp/ initialize → $c (transport alive; set OLYWORK_SMOKE_TOKEN for full proof)" ;;
...
# olywork.com marketing pages serve in place in EVERY phase (never a redirect source: loop safety).
