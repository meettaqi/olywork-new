#!/usr/bin/env bash
#
# dev-local.sh — bring the olywork dev stack up in one command.
#
# One service: the FastAPI server (`python -m olywork`, port 18790) in a tmux window,
# started with OLYWORK_EMAIL_DEV_MODE=true (the OTP shows in the login page / API
# response — no mail sender needed) and its own sqlite DB (olywork-dev.db), so your
# real data and .env stay untouched.
#
# The `cli` subcommand runs the WORKING-TREE olywork CLI in a sandboxed HOME
# (scripts/.dev-home), pre-pointed at localhost — it never touches
# ~/.olywork/config.json, which usually points at production.
#
# Usage:
...
PORT=18790
DEV_DB="$ROOT/olywork-dev.db"                 # *.db is gitignored
...
SERVER_ENV="OLYWORK_EMAIL_DEV_MODE=true OLYWORK_CONNECT_DEMO_ENABLED=true OLYWORK_DATABASE_URL=sqlite+aiosqlite:///$DEV_DB"
SERVER_CMD="cd $ROOT && set -a && . $DEV_KEYS && set +a && env $SERVER_ENV uv run python -m olywork --reload"
...
ensure_dev_keys() {
  ...
  {
    printf 'OLYWORK_SECRET_KEY=%s\n' "$(cd "$ROOT" && uv run python -m olywork keygen)"
    printf 'OLYWORK_SESSION_SECRET=%s\n' "$(openssl rand -hex 24)"
  } > "$DEV_KEYS"
}
...
cli() {
  ...
  mkdir -p "$DEV_HOME/.olywork"
  [ -f "$DEV_HOME/.olywork/config.json" ] || printf '{"base_url":"http://localhost:%s"}\n' "$PORT" > "$DEV_HOME/.olywork/config.json"
  (cd "$ROOT" && HOME="$DEV_HOME" uv run olywork "$@")
}
