...
The collectors moved to `olywork.domain.capacity.collectors` (the worker's `capacity sweep` persists
...
from olywork import reconcile  # noqa: E402
from olywork.domain import money as ledger  # noqa: E402
from olywork.config import get_settings  # noqa: E402
from olywork.infra.db import session_maker  # noqa: E402
from olywork.domain.capacity.collectors import (  # noqa: E402,F401 — re-exported for older callers
...
async def main(days: int, as_json: bool) -> int:
    ...
    try:
        ...
        print(f"no ledger in {get_settings().database_url}: {exc.orig}\n"
              "Point OLYWORK_DATABASE_URL at the deployment's database (the balances below still work).",
              file=sys.stderr)
