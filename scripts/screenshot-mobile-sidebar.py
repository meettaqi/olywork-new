...
def start_server():
    ...
    env = os.environ.copy()
    env["OLYWORK_DATABASE_URL"] = "sqlite+aiosqlite:///screenshot-test.db"
    env["OLYWORK_EMAIL_DEV_MODE"] = "true"
    env["OLYWORK_SECRET_KEY"] = "screenshot-test-key-not-for-production"
    env["OLYWORK_SESSION_SECRET"] = "screenshot-session-secret"
    
    proc = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "olywork.api:app", "--host", "127.0.0.1", "--port", str(PORT)],
        env=env,
