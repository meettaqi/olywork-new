...
Connects with `OLYWORK_DATABASE_URL` / `--dsn` (a postgres:// DSN). With `--render`, reuses
...
async def main() -> int:
    ...
    ap.add_argument("--dsn", default=os.environ.get("OLYWORK_DATABASE_URL", ""), help="postgres DSN")
    ...
    if args.render:
        ...
        if not args.dsn:
            print("no DSN: pass --dsn, set OLYWORK_DATABASE_URL, or use --render", file=sys.stderr)
            return 2
