...
Usage (credential comes ONLY from the environment — never from a file or argv):
    OLYWORK_CATALOG_CRED='<secret>' uv run python scripts/catalog_verify.py <service> [--id <endpoint-id>] [--write]
    OLYWORK_CATALOG_CRED='<secret>' uv run python scripts/catalog_verify.py <service> --extended [--id …]

For each endpoint in src/olywork/catalog/<service>.yaml it sends `test_request` to base_url+path with
the provider's auth shape and required protocol headers (taken from olywork.oauth_providers —
...
src/olywork/catalog/examples/<endpoint-id>.json. Prints one PASS/FAIL line per endpoint.
...
def main() -> int:
    ...
    ap.add_argument("--via-olywork", metavar="TOOL",
                    help="route through a olywork registry's /call proxy (tool name) using ~/.olywork/config.json "
                         "instead of a raw credential — for OAuth providers whose token olywork holds")
    args = ap.parse_args()

    cred = os.environ.get("OLYWORK_CATALOG_CRED")
    if not cred and not args.via_olywork:
        print("OLYWORK_CATALOG_CRED not set (or use --via-olywork <tool>)", file=sys.stderr)
        return 2

    sys.path.insert(0, str(ROOT / "src"))
    from olywork.oauth_providers import REGISTRY
    ...
    if args.via_olywork:
        cfg = json.loads((Path.home() / ".olywork" / "config.json").read_text())
        call_base = cfg["base_url"].rstrip("/") + f"/call/{args.via_olywork}"
        headers["X-Olywork-Token"] = cfg["token"]  # olywork's own auth header, not Bearer
        if cfg.get("active_org"):
            headers["X-Olywork-Org"] = str(cfg["active_org"])
    else:
        secret = cred
