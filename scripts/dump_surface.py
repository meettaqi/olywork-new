#!/usr/bin/env python3
"""Write deterministic snapshots of olywork's public application surface."""
...
def _configure_test_environment() -> None:
    """Match tests/conftest.py before importing olywork, whose settings load at import time."""
    worker = os.environ.get("PYTEST_XDIST_WORKER", "")
    db_dir = os.path.join(tempfile.gettempdir(), "olywork-tests")
    os.makedirs(db_dir, exist_ok=True)
    default_db = f"sqlite+aiosqlite:///{db_dir}/olywork-test{'-' + worker if worker else ''}.db"
    os.environ["OLYWORK_DATABASE_URL"] = os.environ.get("OLYWORK_TEST_DB_URL", default_db)
    os.environ["OLYWORK_EMAIL_DEV_MODE"] = "true"
    os.environ["OLYWORK_RESEND_API_KEY"] = ""
    os.environ["OLYWORK_RUN_ALLOWED_BINS"] = (
        "sh,echo,true,false,cat,sleep,olywork-nonexistent-bin-xyz"
    )
...
def _lifespan() -> dict[str, Any]:
    ...
    return {
        "background_tasks": [
            {
                "condition": "olywork.adsconv.enabled()",
                "task": "olywork.adsconv.worker",
            }
        ],
        "mounted_lifespans": [
            {
                "condition": "olywork.bootstrap._mcp is not None",
                "task": "olywork.mcp.mcp_lifespan",
            }
        ],
        "shutdown": [
            {
                "action": "cancel",
                "condition": "olywork.adsconv.worker was started",
                "task": "olywork.adsconv.worker",
            },
            {"action": "await", "task": "olywork.audit.drain"},
            {"action": "await", "task": "olywork.analytics.drain"},
            {"action": "await close", "task": "app.state.http"},
        ],
        "startup": [
            {"action": "await", "task": "olywork.infra.db.verify_db"},
