# Olywork (OpenRouter for Tools)

![olywork — the tool catalog for your agent](docs/assets/olywork-hero.png)
...
Live at [olywork.com](https://olywork.com) — anyone can self-host.
...
public API at all (invite-only, partner-only, app-review-only). olywork carries those accounts and
bills fractions of a cent per call.

## Two kinds of tool, one token

- **The catalog** — external endpoints olywork can serve **on its own key**, metered against your
...
- **Your own tools** — anything a teammate registered: a paid API account, an OAuth connection, a
  vendor CLI, a `SKILL.md`. **Your own key always wins over olywork's, and those calls are never
  metered.**
...
Visit [**olywork.com**](https://olywork.com) — the dashboard,
...
curl -fsSL https://olywork.com/install.sh | sh
...
olywork balance                                     # exactly what that cost

# (or `olywork onboard` for the guided walkthrough)
```

Your token identifies you on every call (`X-Olywork-Token` header) and is the same for all tools.
Discover what your team has shared: `olywork tool ls` · check credential health: `olywork health`.

### Or install it as a Claude Code plugin

```
/plugin marketplace add olywork/olywork
/plugin install olywork@olywork
```

Installs with no token and no configuration. The skill loads as `olywork:olywork` and, on its first run,
walks your agent through the rest — the CLI, sign-in, then `olywork mcp install` — so you end up with
the command line **and** olywork's tools. Other agents: `npx skills add olywork/olywork -s olywork`
...
The Claude Connectors Directory surface is `https://olywork.com/mcp/v2/`. It exposes only curated
...
```bash
olywork catalog                                    # every platform, busiest first
olywork catalog search "find a work email"         # by the job, not the vendor
olywork catalog get hunter.people.email.find       # params, PRICE, example response
olywork call hunter.people.email.find --query domain=reddit.com --query full_name="Alexis Ohanian"
```
...
3. neither → **olywork's own key**, billed to the team's prepaid balance.

Your own credential always beats olywork's, so connecting a key you already pay for makes those calls
free of the balance rather than duplicating them. An endpoint olywork has no published price for is
**refused**, not served free — you are told to connect your own key instead. Where several providers
serve one capability, `olywork catalog search` shows them side by side with prices; **choosing is
yours** — olywork does not silently pick or fail over between providers for you. (When olywork's own
account for a provider is out it may serve the *same* endpoint through an olywork-owned relay account,
disclosed on the response; a team can opt out.) The exception you opt into: `olywork.<capability>` routed endpoints, where olywork picks the provider for you and names it.

```bash
olywork balance          # credit left, calls in flight, recent spend
...
The zero-thought path — point olywork at a project and it figures out what's shareable:

```bash
olywork scan     # read-only preview: the keys, skills & CLIs upload would register
olywork upload   # register them (encrypted server-side); idempotent, --replace to update
```

`olywork upload` scans the `.env` (matching keys against ~80 known providers), every skill
...
olywork upload env --select openai,stripe,resend     # or straight from the .env
```

**Use** — the agent-native way: build the **real** upstream request and prefix it with the proxy.
olywork resolves the tool by host, injects the credential, and relays everything else faithfully
(your `X-Olywork-Token` is stripped before the upstream sees it):

```
Real request:   GET https://api.intercom.io/conversations?per_page=5
Through olywork: GET https://olywork.com/call/https://api.intercom.io/conversations?per_page=5
                    header:  X-Olywork-Token: <your token>
```

Or the CLI shorthand — and `olywork calls` for the audit log:

```bash
olywork call intercom conversations --query per_page=5
olywork call stripe v1/balance
```

### 2. CLIs

**Share** — automatic: `olywork upload` detects installed catalog CLIs (`stripe`, `gh`, `vercel`, …)
and registers them; a recipe-only catalog CLI skill (e.g. `stripe-cli`) auto-becomes runnable too.

**Use** — `olywork run` executes the vendor CLI **with the org's credential injected**, so you never
hold the key or log in:

```bash
olywork run stripe -- get /v1/balance
olywork run gh -- pr list
olywork run --server agentmail-cli inboxes list   # runs on the registry server: the key never reaches you
```

`--local` (default) runs on your machine; `--server` runs on the registry and streams output back.
For a whole session, `olywork shell start` opens a subshell where every registered CLI injects
automatically — just use `stripe`, `gh`, … normally; `exit` reverts. `olywork runs` is the audit log.
...
```bash
olywork upload skills --dir ~/.claude/skills --all   # register a folder of skills in one pass
```

**Use** — pull any shared skill into your agent; its API calls go through olywork with your token,
so the key stays on the server, never in the skill:

```bash
olywork skill install seo-blog-writer      # writes into ./.claude/skills/  (--all for the library)
```
...
olywork skill init --dir ./my-skill          # drafts olywork.json (guesses base_url, finds secrets)
olywork skill add  --dir ./my-skill          # registers recipe + secrets + tool, atomically

# OAuth via the browser (mints the first token, olywork holds it and auto-refreshes)
olywork oauth connect gsc --client-secret client_secret.json \
...
```bash
olywork org create "Acme"                        # make a team, become owner
olywork org invite teammate@acme.com             # invite by email (pick role + tool access)
olywork org join <code> --email you@acme.com     # accept an invite (creates you if new)
olywork org ls | use <slug> | members            # switch orgs, see the roster
olywork org access <member> --tools a,b          # per-member tool access (admin+)
```

## Going deeper

- [`USAGE.md`](USAGE.md) — the full `olywork` CLI reference.
- [`/llms.txt`](https://olywork.com/llms.txt) — the agent-onboarding file: call
protocol, discovery, auth, CLI, skills. One fetch teaches an agent the whole registry.
- **The dashboard** at [olywork.com](https://olywork.com) — full CRUD, a guided
...
The proxy endpoint is `/call/{...}`; all endpoints take the `X-Olywork-Token` header.
...
That runs the server in tmux with hot-reload, its own sqlite DB (`olywork-dev.db`), and email OTP dev
mode (sign-in codes shown on the page — no mail sender needed). Day-to-day:

```bash
scripts/dev-local.sh cli login   # sandboxed CLI: never touches your real ~/.olywork/config.json
...
uv run python -m olywork upgrade  # prepare schema + run idempotent release tasks without serving
uv run python -m olywork          # serve on 0.0.0.0:18790 (add --reload for dev)
uv run python -m olywork keygen   # print a fresh Fernet key for OLYWORK_SECRET_KEY
```

> **Installing to run a server (not from source):** the base package is the **CLI only**. To run a
> registry, install the server extra — `pip install "olywork[server]"` (or `"tools-registry[server]"`) — which adds FastAPI, the
> database drivers, and encryption. `pip install olywork` alone gives just the `olywork` command for
> talking to an existing registry.

The team instance is hosted on **Render** (web service + Postgres) at `olywork.com`.

## Configuration

Environment variables (prefix `OLYWORK_`, read from `.env`):


| Var                                       | Default                         | Purpose                                                                                                                                                                    |
| ----------------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OLYWORK_DATABASE_URL`                       | `sqlite+aiosqlite:///./olywork.db` | DB URL (SQLite for dev, Postgres in prod)                                                                                                                                  |
| `OLYWORK_SECRET_KEY`                         | *(empty)*                       | Fernet key for secrets-at-rest; empty → an ephemeral key is minted (secrets won't survive a restart)                                                                       |
| `OLYWORK_PUBLIC_URL`                         | `https://olywork.com`           | olywork's public base, used to build the OAuth callback URI                                                                                                                |
| `OLYWORK_SESSION_SECRET`                     | *(empty)*                       | signs the dashboard session cookie; falls back to `OLYWORK_SECRET_KEY`. Set a real value in prod                                                                              |
| `OLYWORK_GITHUB_CLIENT_ID` / `_SECRET`       | *(empty)*                       | GitHub OAuth sign-in (callback `<public_url>/auth/github/callback`); empty hides the button                                                                                |
| `OLYWORK_GOOGLE_CLIENT_ID` / `_SECRET`       | *(empty)*                       | Google OAuth sign-in (redirect `<public_url>/auth/google/callback`); empty hides the button                                                                                |
| `OLYWORK_INSTAGRAM_CLIENT_ID` / `_SECRET`    | *(empty)*                       | Instagram App ID and secret for direct Instagram Login (redirect `<public_url>/oauth/callback`)                                                                           |
| `OLYWORK_META_CLIENT_ID` / `_SECRET`         | *(empty)*                       | Meta app credentials for Facebook Pages, Meta Ads, and optional Instagram `page-tools`                                                                                     |
| `OLYWORK_OAUTH_REVIEW_PENDING`               | `instagram-login,page-messages` | Registry review keys awaiting production access. Remove `page-messages` after Page messaging approval; set empty after direct Instagram approval.                         |
| `OLYWORK_RESEND_API_KEY` / `OLYWORK_EMAIL_FROM` | *(empty)*                       | transactional email via Resend (OTP codes + invites); From must be a Resend-verified sender                                                                                |
| `OLYWORK_ADMIN_TOKEN`                        | *(empty)*                       | cross-tenant **super-admin** bearer; authorizes every `/admin/*` endpoint. Empty disables the env path (only `is_superadmin` users reach `/admin`). Keep it long + secret. |
| `OLYWORK_EMAIL_DEV_MODE`                     | `false`                         | when true, `/auth/email/start` returns the OTP in its response (no mail sender needed) — **dev/local only**, never in prod.                                                |


No `.env` is needed for local dev — every setting has a working default (ephemeral key, sqlite).

> **⚠️ Back these up before moving or redeploying:** the Fernet key (`OLYWORK_SECRET_KEY`) and the
> database (Postgres in prod; `olywork.db` for a local sqlite run). Lose the Fernet key and every
...
2. olywork's own control + edge-forwarding headers (`x-olywork-token`, `x-olywork-org`,
`ngrok-skip-browser-warning`, `x-forwarded-*`, `via`, …) and olywork's session cookie — all stripped,
never leak upstream,
3. the injected credential(s).

**OAuth, three ways to get the first token:** *manual upload* (drop in a `token.json`) ·
*auto-refresh* (if the token carries `refresh_token` + client creds, olywork keeps it fresh, you never
re-upload) · *hosted connect flow* (`olywork oauth connect` → browser consent → olywork captures the
token itself).
...
URL-passthrough + faithful relay, OAuth refresh + connect flow, health checks, `olywork run`/shell,
...
```
olywork/
├── src/olywork/            # the package (api, cli, proxy, injectors, oauth, health, convert, models, …)
...
├── USAGE.md             # full olywork CLI reference
└── pyproject.toml
```

Per-subsystem design docs are **fragments** in `docs/context/`, each citing its `src/olywork/*`
...
permission (`taqi@olywork.com`). **Using the hosted olywork.com API** inside your own product —
with pass-through billing via `X-Olywork-Meta` and `usage/by-tag` — is allowed without permission;
