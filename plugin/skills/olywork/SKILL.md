...
**If you can see `catalog_search`, `catalog_get`, `call`, `balance` and `my_tools`** — olywork is
connected. Use those; there is nothing to install. The rest of this page is written around the `olywork`
command line, so read `olywork catalog search` as `catalog_search`, `olywork call` as `call`, and so on.
...
is the thing olywork exists to avoid. (Registering olywork's MCP server into Codex is manual: it needs a
`~/.codex/config.toml` entry with an env-var indirection. `https://olywork.com/llms.txt` has the shape.)
...
| `call` | make the call; olywork injects the credential and relays the answer |
| `balance` | the team's prepaid balance |
| `my_tools` | what this team registered and you can call without holding the key |

If the connector is present but the tools error, it has no token yet: the human sets `OLYWORK_TOKEN`
(from https://olywork.com → sign in → copy token) for this plugin.

Either way, the rest of this page is the part that matters — **when** olywork is the right move, and
