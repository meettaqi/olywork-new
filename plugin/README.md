# olywork — Codex / ChatGPT plugin

A **distribution wrapper**, not a second product. It ships the same skill that
`olywork skill bootstrap` already installs into `~/.codex/skills/`, packaged so people find olywork by
...
> (`.claude-plugin/` + `skills/olywork/`) and declares **no connector in its manifest**, so it installs
...
> (`plugins/olywork/`), and DeepSeek Harness is an npm bundle (`package.json` + `dsh/`) whose MCP row is
...
> and [`docs/MINIMAX-PLUGIN.md`](../docs/MINIMAX-PLUGIN.md) (skills-only, from `plugins/minimax/`).
    ...
    ├── skills/olywork/        GENERATED — do not edit by hand
...
Note `interface.brandColor` is still clay `#e0703f` — the product colour on olywork.com. That is an
accent beside a monochrome mark, not a conflict, but change both together if the brand moves.

## The skill is generated

`src/olywork/web/skill.md` is the one source. It is served at `/skill.md`, written into every agent by
`olywork skill bootstrap`, and rendered into this plugin by:
...
- **A bootstrap section is prepended.** Every other install path implies the CLI already exists —
  `olywork skill bootstrap` only runs because `olywork` is installed. This is the one path where the skill
  can land on a machine with no olywork at all, and without it a first run ends in `command not found`.
...
{
  "name": "olywork-local",
  "interface": { "displayName": "olywork (local dev)" },
  "plugins": [
    {
      "name": "olywork",
      "source": { "source": "local", "path": "./devs/olywork/tools-registry-oss/plugin" },
...
```bash
codex plugin list                   # olywork@olywork-local — not installed
codex plugin add olywork@olywork-local     # the @marketplace suffix is REQUIRED
...
actually loaded — it should appear as `olywork:tools-registry`:
...
`pyproject.toml` (a test enforces this), and that the copy still describes what olywork does: it
**compares** providers and the agent chooses. olywork does not route automatically and does not fail
