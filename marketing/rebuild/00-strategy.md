...
Every page is a **recipe** — a runnable script with real endpoint ids, real prices and a run receipt — and every page says plainly what olywork does not do (send, warm, route, de-anonymize) and where the catalog has a gap.
...
- olywork **compares** providers; the script does the waterfall. Never "routes", never "fails over".
