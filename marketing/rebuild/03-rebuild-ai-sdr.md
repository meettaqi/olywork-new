...
page_id: r3
slug: /rebuild/ai-sdr/
seo_title: "Build Your Own AI SDR: Open Source, Priced Per Lead | olywork.com"     # 58
meta_description: "An AI SDR is a loop: signal, list, enrich, research, draft, send, triage. Here is the open-source version with olywork as the data layer — every call priced, sending left to your sequencer."  # 176 → trim
...
So this page is the 70%. It is the AI SDR as a loop you own — the shape every serious build guide converges on — with olywork as the data layer and your existing sequencer as the hands. It is not a product that sends.
...
| Send + sequence | Instantly / Smartlead / Lemlist, your key | own tool via olywork, never metered | — |
...
olywork call apify.linkedin.search.jobs --query 'title=Sales Development Representative' --query 'posted=week' \
  | jq -r '.[] | .company_domain' | sort -u > companies.txt

while read -r D; do
  P=$(olywork call icypeas.people.search --method POST --data "{\"query\":{\"currentCompanyWebsite\":{\"include\":[\"$D\"]},\"currentJobTitle\":{\"include\":[\"VP Sales\",\"Head of Sales\",\"CRO\"]}},\"pagination\":{\"size\":1}}" | jq -c '.items[0]')
  NAME=$(echo "$P" | jq -r '.firstname + " " + .lastname'); LI=$(echo "$P" | jq -r '.linkedinUrl')
  EMAIL=$(./waterfall.sh "$NAME" "$D")                 # from /rebuild/clay
  POSTS=$(olywork call tikhub.x.linkedin-web-v2-get-user-posts --query "url=$LI" | jq -r '[.posts[:3][].text] | join(" | ")')
...
- **Route between providers.** The script picks the order; olywork shows the options.
