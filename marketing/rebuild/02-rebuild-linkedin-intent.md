...
seo_title: "Gojiberry & Trigify, Rebuilt: LinkedIn Intent From $0.002 a Post | olywork.com"   # 66 → trim to "Rebuild Gojiberry: LinkedIn Intent Signals From $0.002 | olywork.com"
...
POST_URL="$1"
olywork call scrapecreators.x.v1-linkedin-post --query "url=$POST_URL" \
  | jq -r '.comments[].linkedinUrl' | sort -u > commenters.txt

while read -r P; do
  PROFILE=$(olywork call scrapecreators.linkedin.user.profile --query "url=$P")
  TITLE=$(echo "$PROFILE" | jq -r '.headline'); CO=$(echo "$PROFILE" | jq -r '.current_company.url')
  COMPANY=$(olywork call scrapecreators.x.v1-linkedin-company --query "url=$CO")
  SIZE=$(echo "$COMPANY" | jq -r '.employee_count'); DOMAIN=$(echo "$COMPANY" | jq -r '.website')
  RAISED=$(olywork call aviato.companies.funding_rounds --query "domain=$DOMAIN" | jq -r '.rounds[0].date // ""')
