# Build plan — the outbound cluster on olywork.com
...
| Item | Why | Where |
|---|---|---|
| Fix `_observed_cost_micro` for findymail / fiber | drafts say "a miss is free"; the ledger bills the miss | `src/olywork/…` per memory note; money code — ledger rules apply |
...
- **Strip every em-dash** — `test_workflow_copy_has_no_em_dashes` will fail the drafts as written. Also the brand rule: olywork.com, never bare "olywork" in copy.
...
| Week | X (Taqi's account) | Elsewhere |
|---|---|---|
...
| +1 | Price-receipt image #1: email verification, 3 providers | Open the YALC PR (olywork provider adapter); list in `gtmagents/gtm-agents` |
...
| weekly after | one receipt image, replies via the @olywork_ai loop ("when unsure, skip") | — |
...
- **`olywork skill install rebuild-clay` does not exist.** Removed from the Clay draft; a packaged skill is a separate deliverable.
- **Bare "olywork" and em/en dashes in reader copy** violate CLAUDE.md and `test_no_em_dashes_in_the_hand_written_copy`; strip at port time (the shipped FAQ copy is already clean).
...
- **D1(a) is not a shipping option as written.** The `WORKFLOWS` renderer and `tests/test_agent_pages.py` require a dated run, `rows_in`, a row-outcome CSV under `src/olywork/workflow_runs/`, a narrative, ≥4 failure modes, exactly 4 FAQ entries and 4 related labels. Aggregate-only receipts mean changing the renderer, spec, docs and tests together as a contract change, or shipping D1(b) with the anonymised CSV. Decision still Taqi's; the default flips to D1(b).
