# Independent review: olywork.com programmatic SEO plan
...
- disclosure of olywork's own economic relationship and the exact hosted/BYOK charging difference;
...
- Hosted claims create a self-hosting problem. Every `_page()` canonical is based on `public_url`, but the spec demands literal “olywork.com,” ChatGPT's public plugin, hosted keys, and a $1 grant on every deployment. Those statements are false on a self-hosted registry. Gate pSEO routes/content to the hosted deployment or derive host-appropriate copy; do not globally hardcode olywork.com.
