# olywork.com programmatic pages — the idea behind Pipedream, measured, and olywork's version

Written 2026-08-21. Sources: DataForSEO ranked-keyword pull for pipedream.com (4,000 US keywords,
four calls), live SERPs for olywork's target terms (ScrapeCreators, own key), blotato.com's sitemap,
Taqi's wiki (`wiki/topics/olywork.com SEO.md` §8 is the operative position, `Blotato growth
teardown.md`, `SEO.md`, `Distribution.md`), olywork.com Search Console 2026-07-21→08-18, and the
...
The eight principles, and olywork's counterpart to each:
...
1. **The data is the content; the page is a view.** Every page is a function of the registry
   ...
   *olywork:* the catalog YAML is that registry. `/catalog/<slug>` already renders from it; the new
...
2. **Titles are literal queries, composed from entity names.** Each level targets a different query
   shape. *olywork:* `<platform> api pricing` (shelf) · `<platform> mcp server` (mcp) · `<agent>` +
...
3. **Unbounded tail, concentrated head.** Pairs exist for any combination, but every page links the
   same usage-ranked top-24 apps and the footer's 8 money apps. *olywork:* we have the usage data —
...
4. **Two-hop crawl path to every entity** via an unpaginated A–Z directory. *olywork:* `/catalog`
...
5. **Every page is a funnel.** "TRY IT" / "USE THIS ACTION" deep-links into the builder with the
   component preselected. *olywork:* every row opens connect/install for the visitor's agent; the
...
6. **Thin-page risk is offset by unique machine data** — full source, version, key, doc link per
   component; changes with every PR. *olywork:* price per call, params, `test_request`, verified date,
...
7. **The open-source registry is the content pipeline and the backlink engine.** Every npm package
   ...
   *olywork:* the repo is public (423 stars). Each catalog YAML and each MCP-directory / ChatGPT-plugin
...
8. **Structure beats polish.** No canonicals, raw markdown in meta descriptions, self-pairs,
   ...
   transfer: AI crawlers don't render JS, so olywork's pages must be server-rendered (wiki §8 agrees).
...
## 2. The precedent in olywork's own category: blotato.com
...
real wedge). The sitemap, pulled today, is the structure Taqi sketched, already shipped:
...
## 3. What olywork.com's own data adds
...
- **Live SERPs:** `linkedin mcp server` / `reddit mcp server` → GitHub repos, mcpservers.org,
  PulseMCP, Docker Hub, Apify. Thin, winnable, and those directories are themselves places olywork
...
- **Product facts that are now true:** olywork is a public plugin in ChatGPT's directory (search
  "olywork" → Install, verified by Taqi 2026-08-20); OAuth MCP connector for Claude; `olywork mcp
  install` for Claude Code / Cursor / opencode.

## 4. What the wiki already decided (do not relitigate)

- `olywork.com` in every public title/heading/anchor, never bare `olywork` (immunology SERP).
- **No programmatic page per endpoint** — "the single most obvious idea here and the one most
  likely to earn a site-wide penalty" (`olywork.com SEO.md`, What not to do).
...
- Any claim of a number comes from the catalog at build time; observed success/latency stats carry
  the "not a fair benchmark" caveat; printing reliability numbers for named vendors is Taqi's call.
- Pages must be server-rendered: GPTBot/ClaudeBot/PerplexityBot do not run the Vue SPA.

## 5. The structure

The principle, stated once: **olywork's entity graph is agent × platform × provider, and a page exists
...
### `/apps/<agent>` — "olywork.com in ChatGPT / Claude / Claude Code / Cursor"
...
client (ChatGPT: Plugins → search olywork → Install, with the screenshot; Claude: OAuth connect; Claude
...
platform's *own* name — `linkedin mcp server`, not "olywork.com linkedin". Build only for platforms with
...
comparison table, add the `olywork call` sample. No new URLs; this is the metadata-only fix the wiki
calls the fastest ROI on an existing site.

### `/use-cases/<job>` — the spoke layer (revised 2026-08-21 after Taqi's push-back)
...
olywork's equivalent is the **capability worded as a job**: 1,034 capabilities, 201 with ≥2 providers.
...
Taqi's categories (sales, instagram, seo…) are **section headings on the hubs**, not a URL level —
...
3. Submit olywork to the directories that *are* the SERP: mcpservers.org, PulseMCP, Glama, Smithery,
...
- Whether observed success/latency may appear on public per-vendor pages with the caveat, or only
  prices. Wiki says this is Taqi's call.
- Whether the catalog should add influencer discovery (the one how-to with measured demand that olywork
  cannot honestly serve today).
- `olywork` team balance is negative (−$0.11) after today's DataForSEO pulls; top up before the next
  measurement pass.
