# olywork.com programmatic pages — build spec
...
- **`olywork.com` in every title, H1 and anchor.** Never bare `olywork`.
- **Observed stats** (success rate, p50) appear only with the one-line caveat "measured on olywork.com
  traffic; inputs and sample sizes differ by provider — not a controlled benchmark", and only if
  Taqi approves per-vendor publication. Prices appear always.
...
The install is mostly **agent-agnostic** — the onboarding's "set up olywork — https://olywork.com/skill.md …"
...
ChatGPT: Plugins → Install; Claude.ai: Connectors → `https://olywork.com/mcp/` (OAuth); Claude Code /
Cursor / opencode: `olywork mcp install`. Hand-written above the fold, generated below.

| Element | Content | Source |
|---|---|---|
| Title | `olywork.com for ChatGPT — 2,800 tools your ChatGPT can call, no API keys` | count computed |
| Meta | `Install olywork.com from ChatGPT's Plugins directory and ChatGPT can call {N} APIs across {P} platforms — SEO, LinkedIn, Reddit, people search — priced per call, no provider signup.` | computed |
| H1 | `What do you want ChatGPT to do?` | fixed per agent |
| Install block | per agent: **ChatGPT** — "Plugins → search *olywork* → Install", with Taqi's screenshot; **Claude** — "Settings → Connectors → Add → `https://olywork.com/mcp/`" (OAuth); **Claude Code / Cursor** — `curl -fsSL https://olywork.com/install.sh \| sh` then `olywork mcp install`. A button that starts the flow where a deep link exists. | `llms.txt` §MCP, `skill.md` |
...
| JSON-LD | `SoftwareApplication` (name olywork.com, `applicationCategory` DeveloperApplication, `offers` $0 with the free grant — matches visible text) + `BreadcrumbList` + `FAQPage` (4 Qs that appear verbatim in an FAQ section: is it free, do I need API keys, which agents, what does a call cost) | — |
...
Revised 2026-08-21 after Taqi asked "why B?". With E (pricing/comparison) and D (use cases) in
...
| Element | Content | Source |
|---|---|---|
| Title | `LinkedIn MCP server — 8 providers, 43 endpoints, from $0.002/call \| olywork.com` | catalog |
...
| Install tabs | the same four install blocks as type A, collapsed; the `claude-code`/`cursor` tab includes `olywork catalog search "linkedin profile"` as the first command | — |
...
| One runnable call | `olywork call <cheapest verified endpoint> …` with its `test_request`, and the MCP equivalent ("ask your agent: …") | `test_request` |
| Who owns the key | the honest paragraph: olywork.com's key, metered per call; or the team's own key, never metered; olywork compares, **does not route** | `CLAUDE.md` rule |
...
| Element | Content | Source |
|---|---|---|
| Title | `Find someone's work email from a name and company — 9 providers, from $0.009/success \| olywork.com` | slug map + catalog |
...
| One runnable call | cheapest verified endpoint's `test_request` as `olywork call …` plus the MCP phrasing | `test_request` |
...
Taqi's call (2026-08-21): a real comparison page, not a retitled shelf. `/catalog/<slug>` stays the
app's comparison UI and gets a link to this page; this page is server-rendered, editorial, and is
the one page only olywork.com can write — we have run every provider.
...
| Element | Content | Source |
|---|---|---|
| Title | `LinkedIn API pricing (2026) — 8 providers compared per call, plus the official API \| olywork.com` | catalog; year computed |
...
| **The official API** | its real terms: free tier, approval process, what it will not give you (LinkedIn: nothing about other members), and how it runs on olywork with the team's own key, unmetered | catalog `tier`/`scope` + hand-kept |
...
| One call each way | the cheapest verified endpoint and the official one, as `olywork call …` | `test_request` |
...
volume**; observed stats are "measured on olywork.com traffic, not a controlled benchmark"; olywork
...
Per wiki §8. Activation pages; their job is "does olywork.com work with DataForSEO, and how", not rank.

Title `DataForSEO on olywork.com — 61 endpoints, call them with no DataForSEO account`; sections:
what's covered (capabilities by platform), prices as olywork charges them, the own-key path
(`olywork connections connect --provider dataforseo`, never metered), limits from the catalog, one
...
|---|---|---|---|
| 1 | `/apps/chatgpt`, `/apps/claude`, then the other 8 from the shared agent list | new route `apps_page()`, shared agent JSON, install blocks, five prompts, screenshots; sitemap rows; tests | indexed in 14 days; `chatgpt`/`claude` + "olywork.com" in GSC |
...
Prerequisites before step 1 ships: Taqi's call on publishing observed stats per vendor; top up the
...
- `src/olywork/api.py` — routes `apps_page`, `mcp_page`, `use_case_job_page`, `integration_page`; a
  shared `_providers_block(platform|capability)` used by B, D, E; index-rule helper; `_SITEMAP_PAGES`
  spreading all four maps.
- `src/olywork/catalog/usecase_slugs.yaml` (new) — `capability → slug, category, sentence`.
- `src/olywork/web/pseo.css` (new) — one skin for A/B/D/F, tokens lifted from `landing.html`.
- `src/olywork/web/media/install/chatgpt-plugins.png` etc. — the screenshots.
...
- `src/olywork/web/llms.txt`, `skill.md` — link the agent pages.
...
| Self-hosting | "olywork.com", the ChatGPT plugin, hosted keys and the $1 grant hardcoded | **pSEO routes gated to the hosted deployment** (or host-derived copy) | every one of those statements is false on a self-hosted registry |
...
## v4 — reader-first simplification (Taqi, 2026-08-21)
...
link. "Does olywork.com work with DataForSEO" is answered by the catalog's provider view; the vendor's
