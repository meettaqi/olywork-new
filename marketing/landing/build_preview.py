...
Takes src/olywork/web/usecase-<key>.html and makes it self-contained:
  ...
  - drops the Google Fonts <link> (the host's CSP blocks font CDNs, so it would fail silently —
    the page falls back through olywork's own stack: Georgia / system-ui / ui-monospace)
  - rewrites root-relative hrefs to https://olywork.com/... so every link in the preview goes somewhere real
...
def inline_logos(markup):
    ...
    olywork.com renders an empty tile, which is worse than no tile. These are small SVGs, so inlining
    ...
    return re.sub(r'src="(?:https://olywork\.to)?/logos/([a-z0-9-]+)\.svg"', sub, markup)
...
def build(key, dest):
    ...
    body = re.sub(r'(href|src)="/(?!/)', r'\1="https://olywork.com/', body)
    ...
    banner = (
        ...
        'Preview &middot; olywork.com/use-cases/seo-data-for-ai-agents/ &middot; not yet deployed</div>'
    )
