...
seo_title: "Social Data MCP: Reddit, Instagram, TikTok, YouTube | olywork.com"            # 53
...
```text
Using olywork, find 20 TikTok creators posting about home espresso with 50k to 500k
...
## The old way vs. the olywork.com way

| | The old way | With olywork |
|---|---|---|
...
| **Keys** | A developer account per platform: and TikTok's is invite-only, Instagram's needs app review, LinkedIn's is partner-only | One olywork token. Every tool in the catalog answers to it |
...
platforms do not sell a usable public API at any price to a small team. That is the wall olywork.com gets you
over.
...
> **What olywork.com does and does not do here.** It returns real posts with real engagement numbers. The
...
*Run on olywork.com, 17 Aug 2026, on the topic "AI agents". Every figure is from the Activity log of that run.*
...
There is also a result here we did not expect. olywork.com carries **both** X's official API and an
...
olywork.com is closer to OpenRouter for agent tools than to a data vendor: one base URL, one token, many
providers behind it.
...
exact price is shown before the call, and olywork.com adds no markup. New teams start with $1.00 of free
credit: around a thousand posts' worth of calls.

**Which agents does it work with?**: `S-OBJ-AGENTS`

**Is there a Reddit MCP?**
Yes. The olywork.com MCP server exposes Reddit search, subreddit posts, comments and trending topics
...
All through the same olywork.com MCP connection, no platform approval required.
...
paste the setup line from `/llms.txt` into Grok and it can use the olywork.com catalog.
...
view counts. Then show `olywork balance` and let the number speak: the gap between "$200/mo X API" and this
