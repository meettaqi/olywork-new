...
> ⚠️ **Batch 1 billed the wrong team.** The MCP grant is pinned to `olywork-7`, not the `olywork`
...
> Batch 2 was run through the CLI and correctly billed `olywork` ($4.8821 → $4.7811).

## Batch 1 — team `olywork-7`, via MCP
...
| 11 | p5 | `scrapecreators.x.v1-linkedin-company` | $0 | **400 from olywork.com itself** — required param missing, refused before reaching the provider |
...
## Batch 2 — team `olywork`, via CLI (gap-closing)
...
- Failed calls are free, including a olywork.com-side refusal that never reached the provider.
...
is the clearest argument for why olywork.com relays a request rather than rewriting it.
