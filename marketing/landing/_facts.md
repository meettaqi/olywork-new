...
```bash
curl -s https://olywork.com/catalog/platforms | python3 -c "..."   # counts
olywork catalog get <endpoint_id>                                  # price + measured stats
```
...
|---|---|---|---|
| `F-01` | **2,600+ tools · 40+ providers** | olywork.com hero (2,617 / 42); rounded down so it holds under every count | 2026-08-17 |
| `F-02` | $1.00 free credit on every new team | olywork.com; `llms.txt`; `config.py promo_grant_micro = 1_000_000` | 2026-08-17 |
| `F-03` | 0% additional fee — olywork.com does not mark up provider call prices | olywork.com ("0% additional fee — we earn on volume pricing with vendors, not on you") | 2026-08-17 |
| `F-04` | Credentials injected server-side; response relayed verbatim; every call audited | `llms.txt` | 2026-08-17 |
| `F-05` | Your own key always wins over olywork's, and those calls are never metered | `llms.txt`; olywork.com | 2026-08-17 |
| `F-06` | olywork.com does **not** choose a provider or fail over for you, by design | `llms.txt` line 182 | 2026-08-17 |
...
| `F-08` | Sign-in doors: GitHub, Google, email one-time code. No card required to create a team or spend the grant | olywork.com sign-in modal; `ledger.grant` fires at org creation | 2026-08-17 |
...
| `F-10` | 100% open source, AGPL — github.com/meettaqi/olywork-new | olywork.com footer | 2026-08-17 |
...
| `F-13` | A team can pin a capability to one provider: `olywork org pin <capability> --provider <p>` | `llms.txt` CLI reference | 2026-08-17 |

> **Why `F-01` is a rounded claim, not an exact one.** Three surfaces currently disagree: olywork.com's hero
...
> 40+ providers** is Taqi's call (2026-08-17) and is safe on the provider half under every count.
>
...
## Subscription list prices (the "instead of" anchors, all from olywork.com's catalog grid)
...
| `F-24` | olywork.com's own summed example: "$371/mo of seats → cents per call" |

## Measured endpoint data — `olywork catalog get`, 2026-08-17
...
the "What Agents Call olywork For" artifact. **24,921 catalog calls across every team.**
...
teams or four.** Our own `olywork` org contributes under 40 of them, so the demand is genuinely
external — but its concentration is unknown. Treat this as directional for targeting, not as proof of a
market. The team count needs `OLYWORK_ADMIN_TOKEN` and one `/admin/*` call.
...
- Anything about how many people or teams use olywork.com. Not instrumented.
