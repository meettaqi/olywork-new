# Message to send to the olywork.com developer

Copy from the line below. Findings are from ~20 real calls on 2026-08-17 across two teams
(`olywork` and `olywork-7`) while building landing-page proof sections. Everything here was
reproduced at least twice unless noted.

---

Hey — I ran about 20 real calls through olywork.com today (SEO, enrichment, social, ads, company data) and hit
...
```
$ olywork catalog get lusha.companies-signals
...
```
POST → olywork says: "tikhub.x.tiktok-ads-search-ads is GET — add --method GET"
GET  → provider returns 405 Method Not Allowed
```

So olywork enforces GET and the upstream rejects GET. Catalog metadata looks wrong. Its stats row also reads
...
**5. The MCP OAuth grant is pinned to a team that `olywork org ls` doesn't list, with no way to see or change it.**

MCP `balance` reported team `olywork-7`. My CLI shows only:

```
* olywork   olywork   admin (active)
  ai-taqi      AI Taqi      admin
```

I spent ~$0.05 out of `olywork-7` before anyone noticed, because from inside the agent there is no
...
- olywork.com hero: **2,617 endpoints / 42 providers**
...
**Two provider-side things, not olywork bugs, but maybe worth a catalog note:**
...
five failures including a olywork-side refusal that never reached the provider, and all were $0.00. Every
