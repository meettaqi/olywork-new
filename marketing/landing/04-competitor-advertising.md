...
seo_title: "Ad Library API: Meta, Google, TikTok, LinkedIn Ads | olywork.com"        # 52
...
```text
Using olywork, show me every ad Notion is running on Meta right now. Group them by
...
| **Keys** | Meta's Ad Library API needs identity verification; the rest have no official access at all | One olywork token. Every tool in the catalog answers to it |
...
enough for olywork.com to have measured them. The catalog shows you that rather than a rounded number with no
sample behind it.
...
*Run on olywork.com, 17 Aug 2026, against the SEO-software category. Every figure is from the Activity log.*
...
the provider (HTTP 400, `$0.00`). One omitted a required parameter and was refused by olywork.com *before it
...
estimated audience bands. That is the objection section playing out live: olywork.com does not fail over for
...
bursts. olywork.com is closer to OpenRouter for agent tools than to a data vendor: one base URL, one token,
...
call, and olywork.com adds no markup. New teams start with $1.00 of free credit: enough for several hundred
library searches.

**Is there a Meta Ad Library MCP?**
This is one. olywork.com is a single MCP server carrying the Meta ad library, Google Ads Transparency and
...
DataForSEO, Bright Data) that return structured data from it. Your agent calls a olywork.com endpoint
and gets the ads back as JSON.

**Is there a Facebook Ads Library MCP?**
Yes. The olywork.com MCP server exposes Meta/Facebook ad library search through multiple providers.
