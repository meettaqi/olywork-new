...
`olywork` team at time of writing.
...
`olywork-7` team ($0.02 left), not `olywork` ($4.78). Calls through an agent will bill the wrong
balance and may 402 mid-take. Confirm with:

```bash
olywork org ls          # expect: * olywork (active)
olywork balance         # expect: ~$4.78
```
...
**Two hard stops**
1. **Never show the token.** Don't run `olywork login`, `env`, `cat ~/.olywork/*`, or anything that prints
   `trg_live_…` on screen. `olywork balance` and `olywork call` are safe.
...
```bash
olywork balance
olywork catalog get serpstat.google.keywords.volume            # free
olywork catalog get apollo.companies.search                    # free
olywork catalog search "ad library"                            # free
...
**If a call fails on camera, keep rolling.** A 4xx costs nothing and olywork says so. Recovering live — reading
...
|---|---|---|---|
| 1 | `olywork catalog search "keyword search volume"` | The endpoint list with a COST column | 3s |
| 2 | `olywork catalog get serpstat.google.keywords.volume` | **The money frame.** The sibling table: serpstat $0.0005/kw · 100% (77) · 1.2s, seranking $0.00179 · 92% (20), dataforseo $0.09/call · 99% (165) · 3.9s | **6s — this is the shot** |
| 3 | The call below | 13 keywords with real volume, difficulty and intent | 5s |
| 4 | `olywork balance` | The charge: $0.010 | 4s |

```bash
olywork call serpstat.google.keywords.volume --method POST --data '{"id":"1","method":"SerpstatKeywordProcedure.getKeywordsInfo","params":{"keywords":["mcp server","serp api","agent skills","reddit api","tiktok api","web scraping api","rank tracking api","backlink api","keyword research api","email finder api"],"se":"g_us","withIntents":true}}' \
...
| 2 | `olywork catalog get hunter.people.email.find` | COST line: *1 credit, charged only when an email is found; a miss is free* | 5s |
...
| 5 | `olywork balance` | One charge, not two | 4s |

```bash
# 1 — free discovery
olywork call hunter.x.discover-companies --method POST \
...
olywork call hunter.people.email.find --query domain=YOURDOMAIN.com --query full_name="Your Name" \
  | jq '{email: .data.email, score: .data.score, verified: .data.verification.status}'

# 4 — the miss. This is the beat. Nothing is charged.
olywork call hunter.people.email.find --query domain=stripe.com --query full_name="Nonexistent Personname" \
  | jq '{email: .data.email}'
```
...
| 5 | `olywork balance` | **Four platforms, ~$0.006 total** | **6s — this is the shot** |

```bash
olywork call tikhub.tiktok.search.videos --query keyword="ai agents" --query count=10 \
  | jq -r '.data.search_item_list[]?.aweme_info | "\(.statistics.play_count)\t\(.desc[0:60])"' | head -5

olywork call tikhub.youtube.search.videos --query keyword="ai agents" --query language_code=en --query country_code=us \
  | jq -r '.data.videos[] | "\(.view_count)\t\(.published_time)\t\(.title[0:50])"' | head -5

olywork call tikhub.x.twitter-web-fetch-search-timeline --query keyword="ai agents" \
  | jq '{ok: .code, at: .time}'

olywork call scrapecreators.reddit.search.posts --query query="ai agents" --query sort=top --query timeframe=week \
...
| 5 | `olywork balance` | Four ad libraries, ~$0.0075 | 4s |

```bash
olywork call scrapecreators.x.v1-facebook-adlibrary-search-ads --query query="seo tool" --query country=US --query status=ACTIVE --query trim=true \
  | jq '{matching_ads: .searchResultsCount, sample: [.searchResults[0].snapshot | {page_name, cta_text, body: .body.text[0:80]}]}'

olywork call scrapecreators.x.v1-google-adlibrary-advertisers-search --query query=semrush \
  | jq '.advertisers'

olywork call scrapecreators.x.v1-linkedin-ads-search --query keyword="seo software" \
  | jq '{total: .totalAds, sample: [.ads[0] | {poster, promotedBy}]}'

olywork call scrapecreators.x.v1-tiktok-ad-library-search --query query="Semrush" \
...
| # | Type this | What lands on screen | Hold |
|---|---|---|---|
| 1 | `olywork catalog get apollo.companies.search` | **The money frame.** Eleven siblings with COST / WORKS / SPEED — akta free, thecompaniesapi $0.0019, apollo $0.026, pdl $0.38 | **8s — hold longest of any shot** |
...
olywork call leadmagic.x.company-funding --method POST --data '{"company_domain":"stripe.com"}' \
...
olywork call leadmagic.x.company-funding --method POST --data '{"company_domain":"daloopa.com"}'
```
...
|---|---|---|
| 402 mid-take | Billing the `olywork-7` team | `olywork org use olywork`, check `olywork balance` |
...
| `no endpoint '…' in the catalog` | Bad id | `olywork catalog search "<the job>"` and read the real id |
