...
## `S-COMPARISON-TABLE`
The five rows of *the old way vs. the olywork.com way*. Row 1 and row 5 are rewritten per page with that
vertical's providers and job; rows 2–4 are fixed.

| | The old way | With olywork.com |
|---|---|---|
| **What you pay for** | *(per page: the specific subscriptions)* | One prepaid balance. Per call, in fractions of a cent |
| **Keys** | One account, one login and one API key per provider, spread across machines and `.env` files | One olywork.com token. Every tool in the catalog answers to it |
| **Picking a provider** | You guess, or you use the one you already pay for | `catalog get` lists every provider for that job with price, the success rate olywork.com has measured, median speed and when it last answered |
...
## `S-PROOF-TABLE`
Every page carries this table. **Fields must be filled from a real olywork.com run and are not publishable
...
The credential is injected on the server. Your agent makes the real upstream request through olywork.com's
`/call/` endpoint; olywork.com adds the key and relays the provider's answer back verbatim. No provider key
...
for the whole team, `olywork org pin <capability> --provider <provider>` refuses calls to any other provider
...
those calls are never metered against your olywork.com balance. Your key always wins over olywork.com's. `F-05`

## `S-OBJ-FAILURE` What happens if a provider fails?
olywork.com does not silently reroute you. That is deliberate: only you know which inputs you actually hold,
so olywork.com relays your request rather than rewriting it. Failed calls are not billed. If a call
...
The installer registers olywork.com's MCP server into Claude Code, Cursor and opencode automatically. Any MCP
client that supports the authorization spec can connect with OAuth. Anything that can run a shell command,
Codex included, can use the `olywork` CLI or plain HTTP. `F-09`
...
through it, unmetered. What olywork.com removes is the rest: an account, a contract and a key for every
...
## `S-FINAL-CTA-TRUST`
$1.00 of free credit on every new team. No credit card, no provider signup. olywork.com is open source
(AGPL). `F-02` `F-08` `F-10`

## `S-INSTALL`
```text
set up olywork — https://olywork.com/llms.txt
```
Paste that into your agent, or run `curl -fsSL https://olywork.com/install.sh | sh`.

---

## Wording rules for anyone editing these pages

- **`olywork.com`, never bare `olywork`,** in every title, meta description, heading and link anchor. Inside body
  copy, `olywork.com` on first mention and wherever it could be read as the immunology term.
