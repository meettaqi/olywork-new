# olywork.com outcome landing pages
...
```bash
python3 build.py && python3 build_html.py    # → src/olywork/web/usecase-*.html
```

Served at `/use-cases/<slug>` off the `_USE_CASES` map in `src/olywork/api.py`, with `usecase.css` as the
...
From `wiki/topics/olywork.com SEO.md` and the `/seo-growth` + `/seo-playbook` method. Each cost something
to learn; do not quietly drop one.

**Always write `olywork.com`, never bare `olywork`, in every title, meta description, heading and backlink
anchor.** `olywork` is regulatory T cells — an immunology term that won a Nobel Prize, with Wikipedia, NIH
...
plan. olywork.com does **not** route or fail over — `llms.txt` says so explicitly — so "picking is the problem
...
olywork.com yet**, because install → auth → first call → week-two reuse is not instrumented, and spending
...
Copy is written and every number is verified against olywork.com and the live catalog as of **2026-08-17**.
