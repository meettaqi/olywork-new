# tools-registry (`olywork`) - guide for every coding agent

<!-- Editors: CLAUDE.md imports this file; Codex and Cursor read it directly. -->

olywork is the tool catalog for an agent: one base URL, one token, and the agent can call a curated
...
1. A team's own key always wins over olywork's, is never metered, and is never routed or overflowed.
...
4. Plain `/call/` is a faithful relay: the injected credential and the transport headers listed in
   `src/olywork/infra/upstream/relay.py` are the only rewrites. Never add upstream-specific modeling
...
- **Design docs are fragments.** `docs/context/` holds one per subsystem, each naming its
  `src/olywork/*` sources in frontmatter; `docs/context/README.md` is the generated index and
...
- **Agent-facing files are the product's front door**, not documentation: `src/olywork/web/llms.txt`
  and `src/olywork/web/skill.md` (installed into every agent by `install.sh`). They, `README.md` and
  this file must agree on how olywork works; a behavior change asks whether all four move.
- **Three files move together** or they drift: `src/olywork/web/tutorial.js` (the only interactive
  source) and its hand-kept prose mirrors `src/olywork/web/tutorial.md` and `docs/TUTORIAL.md`.
...
- **The call runtime is self-contained.** `src/olywork/application/call/` depends on no management
...
`olywork run` allow-list and rlimits (`runner.py`). Read the fragment before touching any of them.
...
uv run olywork --help                             # the CLI from this checkout
uv run python -m olywork                          # the server
...
- **The package is split.** The base install is the light CLI; the FastAPI/DB stack is the
  ...
  fails the build.
- **The dashboard** (`src/olywork/web/index.html`) is a single-file Vue app with no build step, so a
  broken view name fails silently. Verify in a browser.
- **Schema.** Alembic owns it (`src/olywork/alembic/versions/`); every schema change is a revision.
  Startup only verifies the revision and refuses to boot when behind; migrations run only via
  `python -m olywork upgrade`.
...
nobody can debug. Provider choice is the easiest thing to overstate: olywork compares providers, and
