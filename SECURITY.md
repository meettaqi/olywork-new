...
(the "Report a vulnerability" button on the Security tab), or email **<taqi@olywork.com>**.
...
olywork is built so that **reading the source does not help an attacker** — nothing load-bearing
...
- **Local runs are sandboxed.** `olywork run` on a member's machine executes the CLI as a locked-down
  `olywork-run` user, with an egress allow-list and an optional filesystem jail, so a shared key can't be
  read, exfiltrated over the network, or written to a member-readable file.
- **Server runs are resource-limited.** `olywork run --server` executes each CLI with a scrubbed environment
  (olywork's own secrets removed), a per-run throwaway home, an allow-list of runnable commands, output
...
2. **The CLI-login handshake is in-process.** The short-lived pairing state for `olywork login` lives in the
