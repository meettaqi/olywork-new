"""Provider catalog + env scanner — the brains behind `olywork upload` (env → auto-registered tools).
...
The auth shapes map straight onto olywork's binding model (see `convert.py` / `POST /tools`):
...
# app prefix like OLYWORK_RESEND_API_KEY still resolves via its "RESEND" component. Keep tokens
...
#
# `cli` (optional) is the provider's local-run behavior profile for `olywork run` (docs/CLI-RUN-PLAN.md):
...
CATALOG: list[dict] = [
    ...
    #     src/olywork/catalog/<service>.yaml and docs/MARKET-DATA-CATEGORY-RESEARCH.md) ---
...
def env_values(env_path: str, names: list[str]) -> dict[str, str]:
    """Read the VALUES for specific variables — used by `olywork upload` at registration time only
...
def build_binding(auth: dict) -> dict | None:
    """Translate an auth shape into a olywork tool binding (see convert.py / POST /tools). Returns None
    ...
    for d in detections:
        if d.kind == "matched":
            ...
            actions.append(Action(detection=d, supported=False, base_url=d.base_url,
                                  tool_name=_slug(d.provider or ""), reason="OAuth — use `olywork oauth connect` (Phase 2.5)"))
            ...
            actions.append(Action(detection=d, supported=False, secret_name=d.vars[0],
                                  reason="no provider match — resolve with `olywork upload --llm`, or register by hand: "
                                         "`olywork secret add <name> --env-var <VAR>` + `olywork tool add <name> --base-url …`"))
...
# --- Phase 4: LLM fallback for unknown_secret vars (opt-in via `olywork upload --llm`) ---------------
LLM_SYSTEM = (
...
# --- dev/test entrypoint (Phase 1): `python -m olywork.providers [--env-dir DIR]` --------------------
...
if __name__ == "__main__":
    import argparse, os
    ap = argparse.ArgumentParser(description="olywork provider env-scan (Phase 1 detect + Phase 2 plan)")
