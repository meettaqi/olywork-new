"""olywork — a thin CLI over the registry API. It owns NO logic of its own (charter: the API is the
only brain); every command is one HTTP call. Config lives in ~/.olywork/config.json.

Auth is identity-first: `olywork login` opens the browser, you authenticate with GitHub, and the CLI
stores a single **identity token** (first login also registers you). Then you work across all your
orgs — `olywork org ls` / `olywork org use <slug>` picks the active one, sent as `X-Olywork-Org`. Agents/CI
can instead `olywork login --token <token>` with a per-org token. `olywork logout` clears it.

    olywork config --base-url https://olywork.com
    olywork login                       # GitHub (register-or-login); or: olywork login --token <token>
    olywork org ls | org use <slug>
    olywork secret add / tool add / call / calls / health / skill / admin
"""
...
PRODUCTION_BASE_URL = "https://olywork.com"
...
class _RegistryClient(httpx.Client):
    ...
    olywork's own JSON 403s) it re-sends the request base64-encoded with `X-Olywork-Body-Encoding: base64`,
...
def _active_org_id(cfg: dict, c: httpx.Client, *, strict: bool = True) -> int | None:
    ...
    if r.status_code != 200:
        ...
        if strict and 401 in (r.status_code, me.status_code):
            sys.exit("olywork: not signed in, or this token is invalid/expired.\n"
                     "  Sign in:            olywork login\n"
                     "  Using OLYWORK_TOKEN?   check it is the token `org agent-new` printed.")
        return None
    orgs = r.json()
...
def _show(resp: httpx.Response) -> None:
    ...
    if resp.status_code >= 400:
        ...
        if resp.status_code == 402 and isinstance(body, dict) and isinstance(body.get("detail"), dict):
            print("(check the balance with `olywork balance`, add funds with `olywork topup`)", file=sys.stderr)
        ...
        if isinstance(body, dict) and "choose an org" in str(body.get("detail", "")):
            if _ORG_OVERRIDE:
                print(f"(the --org value {_ORG_OVERRIDE!r} isn't one of your teams — see `olywork org ls`)",
                      file=sys.stderr)
            else:
                print("(no valid active team — pick one with `olywork org use <slug>`; see `olywork org ls`)",
                      file=sys.stderr)
        sys.exit(1)
...
def _show_failure_diagnostics(resp: httpx.Response) -> None:
    ...
    quota 403s as a generic "cli_error" and never learned they were free (2026-09-04). `X-Olywork-Error`
    marks olywork's own refusals; its absence on a 4xx/5xx means the provider answered and olywork relayed
    it unchanged. stderr only — stdout stays the exact body for whatever parses it."""
    headers = getattr(resp, "headers", {}) or {}
    whose = "olywork refused the call" if headers.get("X-Olywork-Error") else "the provider answered; olywork relayed it unchanged"
    line = f"olywork: HTTP {resp.status_code} — {whose}"
    if call_id := headers.get("X-Olywork-Call-Id"):
        line += f"; call id {call_id} (quote it to support; `olywork calls` shows the record)"
    if cost := headers.get("X-Olywork-Cost-Micro"):
...
def cmd_login(args, cfg) -> None:
    if args.token:  # agent / CI: a token directly (a per-org token, or a dashboard identity token)
        ...
        if cfg.get("active_org"):
            ...
            print("✓ Token saved, but you're not in a team yet. Create one with "
                  "`olywork org create \"Your Team\"` or accept an invite, then retry.")
        return
    ...
    # default is production olywork.com. If someone explicitly points at localhost (for local dev) and
    ...
    if start_exc and _is_loopback_url(base):
        sys.exit(
            f"Cannot reach {base} — is a local olywork server running?\n"
            f"  If you meant to use the production registry, run:\n"
            f"    olywork config --base-url {PRODUCTION_BASE_URL}\n"
            f"  then retry `olywork login`.\n"
            f"  (error: {start_exc})"
        )
    ...
    for _ in range(180):  # ~3 min
        ...
        if d.get("token"):
            ...
            if not cfg.get("active_org"):
                ...
                # CLI (which supplies X-Olywork-Org itself) and fails everywhere it gets pasted.
                _pin_token_to_active_org(cfg)
            print(f"✓ Logged in as {cfg['email']}. Active org: {cfg.get('active_org')}")
            _maybe_offer_onboarding(cfg)
            return
    sys.exit("Login timed out — run `olywork login` again.")
...
def _splash() -> None:
    """`olywork onboard`'s opening beat (~1s, any key skips): the wordmark and tagline decrypt
...
def _onboard_active_org(cfg: dict) -> dict | None:
    ...
    None if the caller has no team (a path then points them at `olywork org create` / an invite)."""
...
def _maybe_offer_onboarding(cfg: dict) -> None:
    ...
    try:
        with _spinner("checking your account"):
            me = httpx.get(f"{base}/auth/me", headers={"X-Olywork-Token": cfg["token"], "ngrok-skip-browser-warning": "1"}, timeout=10).json()
    except Exception:
        return
    if me.get("onboarded"):
        return
    ...
    if ans in ("n", "no"):
        ...
        _dim("No problem — run `olywork onboard` whenever you like.")
        return
...
def _catalog_pick(cfg: dict, args) -> dict | None:
    ...
    if not hits:
        _dim(f"  Nothing matched “{query}”. Try `olywork catalog search <words>` with different words.")
        return None
    return hits[0]
...
def _run_catalog(cfg: dict, args) -> None:
    ...
    happens next — olywork's key (price shown, confirmed first), the team's own credential (free), or
    ...
    _cmd(f"olywork catalog get {ep_id}")
    ...
    if tier == "platform":
        _kv("served", "on olywork's key — you need no account with this provider")
        ...
        if a.get("metered"):
            ...
            # olywork's, so the call is metered — say so before the call, not on the invoice.
        ...
        _kv("served", "not yet")
        _dim(f"  {detail}")
        _tip("Nobody on your team has a key for this provider, and olywork is not serving it on its "
             "own key. Connect one and this same call works:")
        _cmd(f"olywork connections connect --provider {ep.get('provider') or '<provider>'}")
        return
    ...
    if r.status_code < 400:
        ...
        _tip("Most catalog endpoints need a parameter (a username, a domain). "
             f"`olywork catalog get {ep_id}` lists them, then add `--query name=value`.")

    if tier == "platform":
        _section("④ What it cost")
        _cmd("olywork balance")
        with _client(cfg) as c:
    ...
    _dim("\n  Next:  olywork catalog search \"<what you want to do>\"   ·   olywork claude   ·   olywork upload")
...
def _run_setup(cfg: dict, args) -> None:
    ...
    (value-internal via `olywork upload`), batch health-check, then print the teammate hand-off."""
    _brand("setup — upload your skills & env, share them safely")
    org = _onboard_active_org(cfg)
    if org is None:
        _dim('You\'re not in a team yet. Create one:  olywork org create "Your Team"')
        return
    _kv("team", org.get("name") or org.get("slug"))
    print("  This is the OTHER half of olywork: turning keys and skills you already have into tools")
    ...
    _dim("  (Only want to USE olywork? Ctrl-C and run `olywork onboard` — the catalog needs none of this.)")
    ...
    if scanned:
        ...
        try:
            ...
            if ok or bad:
                ...
                if unknown:
                    _dim(f"   unchecked = registered before catalog probes; re-upload to validate:  olywork upload env --dir . --replace")
            else:
                ...
                _dim("  Add probes + validate:  olywork upload env --dir . --replace   then   olywork health --run")
        except Exception:  # noqa: BLE001
            _dim("  (run `olywork health --run` to validate)")
    else:
        ...
        _dim("  cd into the repo that has your credentialed skills / .env, then re-run  olywork onboard --path setup")
        if source == "local":
            _dim("  or import your machine-wide skills:  olywork onboard --path setup --source global")
...
def _run_access(cfg: dict, args) -> None:
    ...
    install, then make one no-key test call. Consumers never pull keys — olywork injects server-side."""
    _brand("connect — your team's shared skills & tools")
    org = _onboard_active_org(cfg)
    if org is None:
        _dim("You're not in a team yet — ask an admin to invite you, then `olywork accept`.")
        return
    _kv("team", org.get("name") or org.get("slug"))
    ...
    if tools:
        print(f"  {_M}tools (call any with NO key — olywork injects server-side):{_R}")
        for t in tools[:15]:
    ...
    print()
    _dim("You're set — `olywork tool ls` / `olywork skill ls` anytime.")
...
def _onboard_test_call(cfg: dict, tools: list) -> None:
    ...
    if tp is None:
        _cmd(f"olywork call {tool['name']} <path>")
        _dim(f"  ({tool['name']} has no known test path — its root usually isn't a valid endpoint; "
             "pick a real path from its docs)"); return
    path, method = tp
    _cmd(f"olywork call {tool['name']} {path}".rstrip())
    try:
        with _spinner(f"calling {tool['name']} through olywork"), _client(cfg) as c:
            r = c.request(method, f"/call/{tool['name']}/{path}".rstrip("/"))
        col = _G if r.status_code < 400 else _M
        print(f"  → {col}{r.status_code}{_R} — olywork injected the credential; you never held it.")
...
def _demo_catalog_peek(cfg: dict) -> None:
    ...
    _cmd('olywork catalog search "backlinks for a domain"')
    try:
        with _client(cfg) as c:
    ...
    if not rows:
        _dim("    (the registry didn't answer — the catalog is still there, try `olywork catalog`)")
    _arrow("olywork holds the key; you pay fractions of a cent per call, from $1.00 of free credit.")
...
def _demo_teammate_call(cfg: dict) -> str | None:
    ...
    print(f"  {_M}A teammate on THEIR machine — no key on it — hits a REAL API through olywork:{_R}")
    if not tools:  # nothing callable yet → a recognizable Stripe example (illustrative, not executed)
        _cmd("olywork call https://api.stripe.com/v1/balance")
        _dim("  → olywork would inject your Stripe key server-side and relay Stripe's real response — the teammate never sees the key.")
        return None
    tool = tools[0]
    path, method = _testable_path(tool)
    endpoint = f"{tool['base_url'].rstrip('/')}/{path}"
    _cmd(f"olywork call {endpoint}")   # DISPLAY the real upstream URL so it's clearly a real API…
    try:
        ...
        print(f"  → {col}{r.status_code}{_R} — a real response from {tool.get('host') or tool['name']}. "
              "olywork injected the key server-side; the teammate never held it.")
    except Exception as exc:  # noqa: BLE001
        _dim(f"  (call failed: {exc})")
    return tool["name"]
...
def _demo_call_log(cfg: dict, called: str | None) -> None:
    ...
    domain, so they read as real) calling other shared tools. The real ledger is `olywork calls`."""
    ...
    _arrow("your real ledger:  olywork audit")
...
def _demo_next_steps(cfg: dict) -> None:
    ...
    _kv("do it", "olywork onboard   →   Set up (share yours) · Access (use the team's)")
...
def cmd_onboard(args, cfg) -> None:
    if args.reset:
        if not cfg.get("token"):
            sys.exit("Log in first:  olywork login")
        with _client(cfg) as c:
    ...
    if not cfg.get("token"):
        sys.exit("Log in first:  olywork login")
    if not cfg.get("active_org"):
        _pick_active_org(cfg)  # identity token needs an active org so requests carry X-Olywork-Org
...
def cmd_secret_add(args, cfg) -> None:
    ...
    if getattr(args, "env_var", None):
        # Read ONE named var from an .env using olywork's own parser (strips a balanced quote pair,
...
def _load_catalog(cfg) -> list:
    ...
    bundled CATALOG when offline. Keeps `olywork upload` working with no server + always up to date with one."""
    import hashlib
    from . import providers as prov
...
def cmd_import(args, cfg) -> None:
    """Dispatch to env-import and/or skill-import. Bare `olywork upload` does BOTH for the target dir;
    `olywork upload env` / `olywork upload skills` restrict it. `olywork scan` is the read-only preview
    (forces dry_run). Location: --dir (default cwd)."""
    from . import skills as sk
    if getattr(args, "cmd", None) == "import":
        print("note: `olywork import` is now `olywork upload` (same command; preview with `olywork scan`)", file=sys.stderr)
    ...
    if mode in (None, "skills"):
        if have_skills:
            ...
            # (`olywork upload skills --dir ./.claude/skills` from a repo root). Resolve it by walking up,
...
def _agent_skills_hint(base_dir: str) -> str:
    ...
    for d in candidates:
        ...
        try:
            ...
            n = 0
        if n:
            return f"\nfound {n} skill(s) under {d} — scan them with: olywork scan skills --skills-dir {d}"
    return ""
...
def _import_env(args, cfg, env_path: str) -> None:
    ...
    if args.dry_run:
        ...
        if chosen:
            print("Found — `olywork upload` registers these:" if scan else "DRY RUN — would register:")
            for a in chosen:
                b = a.binding or {}
                print(f"  ✓ {a.tool_name:<14} {a.base_url}   [{b.get('name')}: {b.get('format')}]  (secret {a.secret_name})")
        if oauth_dets:
            print("\nFound OAuth pairs — `olywork upload` connects them one by one:" if scan
        ...
        if oauth_dets:
            if not args.no_oauth and sys.stdin.isatty():
                ...
                print(f"\n{len(oauth_dets)} OAuth app(s) detected ({provs}) — not connected. "
                      f"Connect when ready:  olywork connections connect <name>")
        unknowns = [d for d in detections if d.kind == "unknown_secret"]
...
def _import_clis(args, cfg, env_path: str) -> None:
    ...
    register the ready ones on the right tier — server-side when olywork can hold the key, local when the
...
def _register_cli_tool(c, entry, cli, decision, envvar, val_getter) -> str:
    ...
    if decision["tier"] == "server":
        ...
        # GH_TOKEN) leak in and fail to resolve. An empty list overrides it — olywork injects nothing.
        profile["inject"] = []
...
def _import_add_cli(args, cfg) -> None:
    ...
    if any((e.get("cli") or {}).get("bin") == bin_ for e in _load_catalog(cfg)):
        sys.exit(f"'{bin_}' is already in the catalog — just run `olywork upload clis`.")
    ...
    print(f"✓ Registered '{name}'. Run it locally: `olywork cli run {name}`.")
    print(f"  ↗ {_detail_url(cfg, 'tool', name)}")
    if bindings:
        print(f"  (key stored — also a callable API tool; to run '{bin_}' on the SERVER too, an admin adds it to OLYWORK_RUN_ALLOWED_BINS.)")
...
def _print_cli_report(scanned, registered, report_only, verbose, cfg=None) -> None:
    ...
    if buckets["needs_key"] or buckets["needs_login"]:
        ...
        print("  then re-run: olywork upload clis")
    ...
    if ni and verbose:
        ...
        print(f"\n{len(ni)} more catalog CLIs aren't installed. List them with: olywork scan clis --status")
    if not report_only and registered:
        done = sum(1 for _n, _t, r in registered if r == "ok")
        print(f"\nRegistered {done} CLI tool(s). Run one with: olywork cli run <name>")
...
def _import_skills(args, cfg, skills_dir, env_path: str) -> None:
    ...
    # key exported in your shell) — include both so olywork finds it without asking when it's already there.
    ...
    catalog = prov.CATALOG if args.dry_run else _load_catalog(cfg)
    # Scan every dir but DEDUPE by skill name — `olywork skill install` mirrors a skill into BOTH
    ...
    if args.dry_run:
        print("\nFound skills — `olywork upload` imports these:" if getattr(args, "as_scan", False)
    ...
    with _client(cfg) as c:
        ...
        for d in chosen:
            ...
            print(f"  ✓ {d.name:<28} ({tag})" + ("  [wrote olywork.json]" if wrote else ""))
...
def _resolve_secret_ref(c, ref):
    ...
    if not hits:
        sys.exit(f"no secret named {ref!r} — add it first (olywork secret add {ref} --value …) or use its id")
    return hits[0]["id"]
...
# server's own `olywork.domain.asynctasks`, a stdlib-only leaf - one implementation, pinned light by
...
def await_async_task(descriptor: dict, submission: httpx.Response, call_fn, clock,
    ...
    try:
        ...
        # hand it to stdout as any other response, then say what olywork could not find in it.
    ...
    if poll.get("endpoint"):
        ...
        recovery = f"olywork call {target} -p {shlex.quote(param_name + '=' + task_id)}"
    else:
        target = extracted.poll_url
        params = []
        recovery = f"olywork call {shlex.quote(target)}"
...
def cmd_call(args, cfg) -> None:
    ...
    if authorization_method := getattr(args, "authorization_method", None):
        headers["X-Olywork-Authorization-Method"] = authorization_method
    ...
    # `olywork call <id> --data …` just work beats asking the caller to repeat what the catalog knows.
    ...
    with _client(cfg) as c:
        ...
        if not getattr(args, "await_task", False) or not submission.headers.get("X-Olywork-Async"):
        ...
        if submission.status_code >= 400:
            ...
            return
        try:
            descriptor = json.loads(submission.headers["X-Olywork-Async"])
        except (TypeError, ValueError):
            sys.exit("olywork: X-Olywork-Async is not valid JSON")

        def call_fn(target, poll_params):
            # Dynamic poll URLs still travel THROUGH olywork. Calling the absolute upstream URL from
        ...
        if extracted is not None:
            ...
            if poll.get("endpoint"):
                ...
                recovery = f"olywork call {poll['endpoint']} -p {shlex.quote(resume_name + '=' + task_id)}"
            elif extracted.poll_url:
                recovery = f"olywork call {shlex.quote(extracted.poll_url)}"
        if task_id not in (None, ""):
            print(f"async task submitted: {_shown(task_id)}", file=sys.stderr)
            if recovery:
                print(f"resume: {recovery}", file=sys.stderr)
        if reserved := submission.headers.get("X-Olywork-Cost-Micro"):
        ...
        try:
            ...
            print("olywork: waiting interrupted; the upstream task is still recoverable", file=sys.stderr)
            if recovery:
                print(f"resume: {recovery}", file=sys.stderr)
        ...
        if outcome.get("error"):
            print(f"olywork: {outcome['error']}", file=sys.stderr)
...
def cmd_audit(args, cfg) -> None:
    ...
    rows = [
        {"kind": "call", "id": f"c{x['id']}", "user_email": x.get("user_email"),
         ...
         # A metered async task (generation): how it settled and where its artifact is. `olywork calls`
...
def cmd_run(args, cfg) -> None:
    """`olywork run <tool> -- <args>` dispatcher over two execution tiers (docs/CLI-RUN-PLAN.md):
      --local  (default): run the CLI on THIS machine; the credential is isolated under the olywork-run user.
      --server          : run the CLI on the registry server (Tier 0), streaming stdout/stderr back.
    """
    # argparse.REMAINDER swallows any olywork flag placed AFTER the tool name, silently. Catch ONLY that
    # case — a tier flag typed after the tool but BEFORE the `--` separator — by reading the REAL command
    # line. A flag after `--` legitimately belongs to the vendor CLI (`olywork run db -- --timeout 30`) and
    ...
    if args.tool in argv:
        ...
        if misplaced:
            sys.exit(f"olywork: put {misplaced[0]} BEFORE the tool name: "
                     f"olywork run {misplaced[0]} {args.tool} -- <cli args>  (a flag after the tool name is "
                     f"passed to the CLI, not to olywork)")
...
# ---- local runs: `olywork run --local <tool> -- <cli args…>` (docs/CLI-RUN-PLAN.md) ----------------
# The credential must not be readable by another program of the same user. On Linux we run the CLI as a
# dedicated `olywork-run` user (installed once via `olywork setup-local-run`): a different uid cannot read the
# process's env/memory. The member's `olywork run` hands off to that user via sudo; the RUNNER (as olywork-run)
# fetches the credential and runs the CLI, so the vendor secret only ever exists under olywork-run. Without
# that setup (or on non-Linux) it falls back to running as the member, best-effort, with a warning.
_RUN_USER = "olywork-run"
_RUNNER_PATH = "/usr/local/bin/olywork-runner"
_RUN_PROOF_PATH = "/etc/olywork-run/proof"  # the isolated-runner proof — root-owned, readable ONLY by olywork-run
...
def _run_helper(tool, user_args, cfg) -> None:
    ...
    with the CLI's code. Runs as olywork-run on the isolated path, or as the member in best-effort mode."""
    ...
    if r.status_code >= 400:
        try:
            sys.exit(f"olywork: {r.json().get('detail', r.text)}")
        except json.JSONDecodeError:
            sys.exit(f"olywork: grant failed (HTTP {r.status_code})")
    grant = r.json()
    ...
    if path is None:
        ...
        sys.exit(f"olywork: {binary!r} is not on your PATH{hint}")
    ...
    # args (global/auth flags belong first). Under olywork-run the env is safe — a different uid can't read it.
    env = dict(os.environ)
    argv_extra: list[str] = []
            ...
            for e in errors:
                try:
                    if re.search(e.get("pattern", ""), stderr_text):
                        ...
                        if e.get("message"):
                            print(f"olywork: {e['message']}", file=sys.stderr)
                            if verdict == "credential_invalid":
                                print("olywork: marked the credential invalid — an org owner can rotate it.", file=sys.stderr)
                        break
                except re.error:
                    continue
...
def _traversable_by_others(path: str) -> bool:
    """Can a NON-owner/non-group user (like olywork-run) reach `path`? True only if every component has the
...
def _run_local(args, cfg) -> None:
    ...
    olywork-run user so the vendor credential never touches the member's uid; otherwise run as the member,
    ...
    if isolatable and os.path.exists(_RUNNER_PATH):
        # olywork-run can't enter a private (0700) home, so if the cwd isn't world-traversable, start the
        # runner from a neutral accessible dir — else its shell prints a getcwd error and the CLI runs in
        # a dir olywork-run can't use anyway. (Only on the ISOLATED path; best-effort runs as the member.)
        if not _traversable_by_others(os.getcwd()):
            # NB: macOS's per-user $TMPDIR (/var/folders/…) is also 0700 → unreachable by olywork-run; pick a
            ...
            for _d in ("/tmp", "/"):
                if _traversable_by_others(_d):
                    try:
                        ...
                        pass
        # Hand off to olywork-run. sudo connects the terminal, so input/output/signals/exit flow through.
        ...
        try:
            ...
            pass
        sys.exit("olywork: could not switch to the olywork-run user — is local-run set up? (sudo olywork cli setup)")
    if isolatable:
        print("  · best-effort (run `sudo olywork cli setup` once for full isolation)", file=sys.stderr)
...
_EGRESS_PLIST = (
    ...
    '  <key>Label</key><string>dev.olywork.egress</string>\n'
    '  <key>ProgramArguments</key><array><string>{loader}</string></array>\n'
    '  <key>RunAtLoad</key><true/>\n'
    '</dict></plist>\n'
)
_EGRESS_LOADER = "/usr/local/bin/olywork-egress-load"
...
def _member_registry_url(member: str) -> str | None:
    ...
    config is empty). olywork-run must be allowed to reach it, or the runner's /grant call is blocked."""
    try:
        import pwd
        home = pwd.getpwnam(member).pw_dir
        return json.loads((Path(home) / ".olywork" / "config.json").read_text()).get("base_url")
...
def _install_egress(registry_url: str | None) -> None:
    """Install the static egress allow-list (Option 1): olywork-run may reach ONLY the registry + the catalog
    ...
    if not ips:
        ...
        return
    os.makedirs("/etc/olywork-run", exist_ok=True)
    if sys.platform == "darwin":
        Path("/etc/olywork-run/egress.pf").write_text(egress.pf_ruleset(ips, _RUN_USER))
        ...
        Path("/etc/olywork-run/egress.nft").write_text(egress.nft_ruleset(ips, int(uid)))
        subprocess.run(["nft", "-f", "/etc/olywork-run/egress.nft"], capture_output=True)
        print(f"  egress: nftables allow-list active — {_RUN_USER} may reach {len(hosts)} host(s), all else dropped")
        print("    (to persist across reboot, load /etc/olywork-run/egress.nft from your nftables service)")


def _pick_macos_service_uid() -> int:
    """A free system uid/gid for the hidden olywork-run account (macOS service accounts sit below 500)."""
    ...
    for uid in range(380, 500):
        if uid not in used:
            return uid
    sys.exit("olywork: no free system uid in 380-499 for the olywork-run user")


def _create_run_user() -> None:
    """Create the dedicated no-login olywork-run system user (idempotent). A DIFFERENT uid is what makes the
    ...
    if sys.platform == "darwin":
        ...
        for key, val in (("UserShell", "/usr/bin/false"), ("RealName", "olywork local run"),
...
def cmd_setup_local_run(args, cfg) -> None:
    """One-time admin setup for isolated local runs (Linux + macOS): create the olywork-run system user,
    install a fixed root-owned runner that can ONLY invoke the hidden helper (never a shell), and add a
    narrow sudoers rule letting the member run ONLY that runner as olywork-run. Idempotent."""
    if not (sys.platform.startswith("linux") or sys.platform == "darwin"):
        sys.exit("olywork: setup-local-run supports Linux and macOS.")
    if os.geteuid() != 0:
        sys.exit("olywork: run this with sudo — it creates a system user and a sudoers rule:\n"
                 "  sudo olywork cli setup")
    member = args.member or os.environ.get("SUDO_USER")
    if not member:
        sys.exit("olywork: could not determine which OS user to allow — pass --member <user>")
    ...
    if not re.match(r"^[a-z_][a-z0-9_-]*\$?$", member):
        sys.exit(f"olywork: {member!r} is not a valid unix username")
    olywork_bin = shutil.which("olywork") or os.path.realpath(sys.argv[0])
    ...
    if proof:
        ...
        os.chmod(_RUN_PROOF_PATH, 0o400)  # only olywork-run (and root) can read it; the member cannot
    ...
    #    never get an arbitrary command as olywork-run). HOME=/tmp keeps any tool cache writable; it exports
    ...
    if subprocess.run(["visudo", "-cf", tmp], capture_output=True).returncode != 0:
        os.unlink(tmp)
        sys.exit("olywork: the generated sudoers rule failed validation — nothing installed")
    os.replace(tmp, "/etc/sudoers.d/olywork-run")
    print(f"installed sudoers rule for member {member!r}")

    # Isolation works BY olywork-run being unable to read into the member's files — which also means it can't
    # exec a olywork installed inside the member's private (0700) home. Catch that here with the exact fix,
    # instead of a confusing "Permission denied" at the first run.
    if subprocess.run(["sudo", "-u", _RUN_USER, "test", "-x", olywork_bin],
                      capture_output=True).returncode != 0:
        print(f"\n  ! {_RUN_USER} cannot execute olywork at {olywork_bin} — it's inside a private home dir.\n"
              f"    Install olywork at a system path (e.g. /usr/local/bin) so the isolated runner can reach\n"
              f"    it; until then, isolated local runs will fail (the proxy + `olywork call` are unaffected).",
              file=sys.stderr)

    # The network half of the sandbox: restrict olywork-run's egress to the registry + catalog API hosts,
    ...
    if not getattr(args, "no_egress", False):
        _install_egress(getattr(args, "registry", None) or _member_registry_url(member))
    print(f"\ndone — {member} can now run:  olywork cli run <tool> -- <args>   (the CLI runs as {_RUN_USER})")


# ---- shell mode: transparent CLI interception (`olywork shell`) -------------------------------
_PROXY_PKG = "cryptography>=43"


def _proxy_install_hint() -> tuple[str, list[str]]:
    """How to add the certificate library to **this** copy of olywork: `(human label, argv)`.

    `pip install "tools-registry[proxy]"` is the right answer for exactly one of the four ways people
    install olywork, and the installer's own way is not it: a uv-tool or Homebrew venv is not on the
...
def ensure_proxy_dependency(assume_yes: bool = False) -> None:
    ...
    worse than not shipping the feature — so olywork works out how it was installed and offers to do it,
    ...
    if not argv:
        sys.exit("olywork: the local proxy needs the `cryptography` package, and this environment has no "
                 "installer (no pip, no uv, no pipx) to add it with. Install it however you manage "
                 "this Python, or reinstall olywork with:  pip install \"tools-registry[proxy]\"")
    printable = " ".join(shlex.quote(a) for a in argv)
    print(f"\n{_A}▚ olywork{_R} {_M}— the local proxy needs one more piece: the certificate library that "
    ...
    if not assume_yes and not sys.stdin.isatty():
        sys.exit(f"olywork: install it for this {label} install with:\n  {printable}")
    if not assume_yes:
        try:
            ...
            sys.exit("\nolywork: cancelled.")
        if answer not in ("", "y", "yes"):
            sys.exit(f"olywork: no problem. When you want it:\n  {printable}")

    print(f"  {_M}{printable}{_R}", file=sys.stderr)
    try:
        code = subprocess.call(argv)  # noqa: S603 — argv list, no shell
    except FileNotFoundError:
        sys.exit(f"olywork: {argv[0]!r} is not on your PATH. Install it by hand with:\n  {printable}")
    if code != 0:
        sys.exit(f"olywork: that did not work (exit {code}). Try it by hand:\n  {printable}")
    ...
    try:
        ...
        sys.exit("olywork: installed, but still not importable here. Try the command again in a new shell.")
    print(f"  {_G}✓{_R} {_M}ready.{_R}\n", file=sys.stderr)


def _start_proxy_handle(cfg, tools: list[dict], *, port: int | None = None, renew_ca: bool = False):
    """Bring up the local proxy and return `(handle, hosts, olywork_host)`. Shared by both front doors:
    `olywork shell start --proxy` (dies with the subshell) and `olywork serve` (a daemon other terminals
    ...
    try:
        ...
        sys.exit(f"olywork: could not start the local proxy on port {pcfg.port} ({exc}). Another one may "
                 f"already be running — check `olywork serve status`, or pick another port "
...
def _start_local_proxy(args, cfg, tools: list[dict]):
    """`olywork shell start --proxy`: bring up the local proxy and return `(env, stop, hosts)` for the
    subshell.
    ...
    script that never heard of olywork. Both end at the same server-side injection."""
    handle, hosts, olywork_host = _start_proxy_handle(
        cfg, tools, port=args.proxy_port, renew_ca=args.renew_ca)
    return handle.env(olywork_host), handle.stop, hosts
...
def _proxy_tools(cfg) -> list[dict]:
    ...
    try:
        ...
        sys.exit(f"olywork: cannot reach the registry at {base} ({type(exc).__name__}). "
                 f"Check it is up, or `olywork config --base-url <url>`.")
    if r.status_code >= 400:
...
def _serve_export_lines(env: dict, unset: bool = False) -> str:
    ...
    `eval "$(olywork serve env)"` prints. `--unset` is the way back out: `olywork serve stop` cannot reach
...
def _serve_daemon(args, cfg) -> None:
    """The daemon body (`olywork serve start --foreground`, and what the detached child runs).
    ...
    print(f"▚ olywork proxy listening on 127.0.0.1:{handle.port} — {len(hosts)} host(s) captured, "
          f"registry {olywork_host}", file=sys.stderr, flush=True)
    try:
        done.wait()
    finally:
        handle.stop()
        lpx.clear_state()
        print("▚ olywork proxy stopped.", file=sys.stderr, flush=True)
...
def _print_serve_banner(live: dict) -> None:
    """What you see after `olywork serve start`.
    ...
    print(f"\n{_A}▚ olywork proxy{_R}  {_G}running{_R}")
    ...
    print(f"    {_TEAL}olywork shell start --proxy{_R}   {_M}a subshell that is already set up{_R}  {_G}← simplest{_R}")
    print(f"    {_TEAL}eval \"$(olywork serve env)\"{_R}   {_M}set up THIS terminal{_R}\n")
    print(f"  {_M}Check any time with{_R} {_TEAL}olywork serve status{_R}{_M} — it says whether your terminal "
          f"is using it.{_R}\n")
...
def cmd_serve_start(args, cfg) -> None:
    ...
    than in a `olywork shell` subshell. Same engine, different front door."""
    from . import localproxy as lpx
    if not cfg.get("token"):
        sys.exit("olywork: sign in first — `olywork login`.")
    live = lpx.running()
    if live:
        sys.exit(f"olywork: a proxy is already running on 127.0.0.1:{live['port']} (pid {live['pid']}). "
                 f"Stop it with `olywork serve stop`, or see `olywork serve status`.")
    if args.foreground:
    ...
    # Re-invoke THIS interpreter and THIS code, not whatever `olywork` is on PATH — a Homebrew copy one
    # version behind would be launched instead and would not even have this command.
    argv = [sys.executable, "-c", "from olywork.cli import main; main()", "serve", "start", "--foreground"]
    if args.port:
    ...
    if not live:
        ...
        try:
            ...
            why = ""
        sys.exit(f"olywork: the proxy did not come up.\n  {why}\n  (full log: {log})" if why
                 else f"olywork: the proxy did not come up. Its log is {log}")
    _print_serve_banner(live)


def cmd_with(args, cfg) -> None:
    """`olywork <command> …` — run one command with the team's credentials, and nothing else changed.

    This is the opt-in shape of the whole feature. olywork is the PARENT of what it launches, so the
    proxy environment applies to that process and its children only: `olywork claude` gets the team's
    ...
    If a `olywork serve` daemon is already up we attach to it and leave it running. Otherwise we start a
    private proxy on an operating-system-chosen port — so two `olywork claude` sessions never collide —
    ...
    if not exe:
        sys.exit(f"olywork: {cmd[0]!r} is not a olywork command and is not an executable on your PATH.")
    if not cfg.get("token"):
        sys.exit("olywork: sign in first — `olywork login`.")
    ...
    if live:                                   # a daemon is already serving; borrow it, leave it up
        ...
        handle, hosts, olywork_host = _start_proxy_handle(cfg, _proxy_tools(cfg), port=0)
        env = handle.env(olywork_host)
        source = "a private proxy, stopped when this exits"

    if not args.quiet:
        print(f"\n{_A}▚ olywork{_R} {_G}{cmd[0]}{_R}  {_M}— {len(hosts)} host(s) credentialed by your team; "
...
def cmd_serve_stop(args, cfg) -> None:
    ...
    live = lpx.running()
    if not live:
        sys.exit("olywork: no proxy is running.")
    try:
        os.kill(int(live["pid"]), signal.SIGTERM)
    except (OSError, ValueError) as exc:
        sys.exit(f"olywork: could not stop the proxy (pid {live.get('pid')}): {exc}")
    for _ in range(50):
        time.sleep(0.1)
        if not lpx.running():
            break
    print(f"\n{_A}▚ olywork proxy{_R}  {_M}stopped{_R}")
    # Only worth saying to a shell that actually still points at the dead port — otherwise it is noise
    # after every stop.
    if os.environ.get("HTTPS_PROXY", "").startswith("http://olywork:"):
        print(f"  {_M}This terminal still points at it — clear it with:{_R}")
        print(f"    {_TEAL}eval \"$(olywork serve env --unset)\"{_R}")
    print()
...
def cmd_serve_status(args, cfg) -> None:
    ...
    live = lpx.running()
    if not live:
        print(f"\n{_A}▚ olywork proxy{_R}  {_M}not running{_R}")
        print(f"  {_M}Start it with{_R}  {_TEAL}olywork serve start{_R}\n")
        return
    here = os.environ.get("HTTPS_PROXY", "").endswith(f"127.0.0.1:{live['port']}")
    mark = f"{_G}this terminal is using it{_R}" if here else \
        f"{_AM}this terminal is NOT using it{_R}  {_M}→{_R}  {_TEAL}eval \"$(olywork serve env)\"{_R}"
    print(f"\n{_A}▚ olywork proxy{_R}  {_G}running{_R}  {_M}(pid {live['pid']}){_R}")
...
def cmd_serve_env(args, cfg) -> None:
    """Print the shell lines that point a terminal at the running daemon: `eval "$(olywork serve env)"`."""
    from . import localproxy as lpx
    if args.unset:
        # Deliberately does NOT require a running proxy. `--unset` is exactly what you need AFTER
        # `olywork serve stop`, and refusing then leaves the shell wedged: its variables still point at a
    ...
    live = lpx.running()
    if not live:
        sys.exit("olywork: no proxy is running — start one with `olywork serve start`.")
    env = lpx.proxy_env(live["port"], live["token"], lpx.proxy_dir() / "ca-bundle.pem",
                        urlsplit(live["base_url"]).hostname or "")
...
def cmd_shell_start(args, cfg) -> None:
    ...
    types `stripe …`/`gh …` normally and olywork handles auth behind the scenes (docs/CLI-SHELL-MODE-PLAN.md).
    MVP: shims call `olywork run <tool>`, reusing the whole local-run path. With `--proxy`, calls the
    AGENT makes directly to a registered API are caught too (docs/LOCAL-PROXY-PLAN.md)."""
    from . import shell as sh
    if os.environ.get(sh.ENV_ACTIVE) == "1":
        sys.exit("olywork: you're already in a olywork shell — type `exit` to leave first.")
    if not cfg.get("token"):
        sys.exit("olywork: sign in first — `olywork login`.")
    with _client(cfg) as c:
        r = c.get("/tools")
    ...
    if not entries and not proxy_on:
        sys.exit("olywork: no runnable CLIs in this team yet. Register one with `olywork upload clis`, or "
                 "enable local runs on a tool: `olywork tool update <name> --local-run on`.")
    for w in warnings:
        print(f"  ! {w}", file=sys.stderr)
    ...
    olywork_bin = shutil.which("olywork") or os.path.realpath(sys.argv[0])
    sys.exit(sh.start_session(entries, olywork_bin, ttl_minutes=args.ttl,
                              extra_env=extra_env, on_close=stop_proxy, captured_hosts=captured))


def cmd_shell_stop(args, cfg) -> None:
    """Leave the olywork shell (equivalent to `exit`/Ctrl-D). Only meaningful inside a session."""
    from . import shell as sh
    sh.stop_session()
...
def cmd_skill_scaffold(args, cfg) -> None:
    ...
    if args.out:
        ...
        print(f"wrote {args.out} (fill in base_url + bindings, then: olywork skill push {args.out})")
    else:
        print(manifest)
...
def cmd_skill_init(args, cfg) -> None:
    ...
    print(f"then register it in your active org:  olywork skill add --dir {args.dir}")
...
def cmd_skill_add(args, cfg) -> None:
    ...
    except ValueError as exc:  # malformed olywork.json
        sys.exit(str(exc))
    if contract is None:
        sys.exit(f"no {CONTRACT_FILE} in {args.dir} — run 'olywork skill init --dir {args.dir}' first")
...
def cmd_skill_install(args, cfg) -> None:
    ...
    try:
        ...
        sys.exit(f"unknown agent {getattr(args, 'agent', None)!r} — see `olywork agents ls` for names")
    with _client(cfg) as c:
        ...
        if args.all:
            ...
            if not targets:
                sys.exit(f"no skill named {args.name!r} in this org (see `olywork skill ls`)")
        else:
        ...
        for b in targets:
            ...
            extra = f"  (tools: {', '.join(t['name'] for t in tools)} — call via `olywork call`)" if tools else ""
    ...
    if skipped_existing:
        ...
        print(f"  To replace one with the team's version:  {_B}olywork skill install <name> --force{_R}")
...
def cmd_mcp_grants(args, cfg) -> None:
    ...
    agent reports a slug, `olywork org ls` lists the teams of whoever is logged in HERE, and those can
    ...
    _dim(f"  point one at another team: olywork mcp use-team {rows[0]['grant']} <team-slug>")
...
def cmd_mcp_install(args, cfg) -> None:
    """Register the olywork MCP server into every supported agent on this machine, header-authed with
    ...
    # applies the same OLYWORK_TOKEN-beats-config precedence used above, so this validates exactly the
    ...
    if who.status_code == 401:
        sys.exit("That token was rejected (401 invalid token) — nothing was written. It's expired "
                 "or from a different server; run `olywork login` (or copy a fresh token from the "
    ...
    print(f"\nRegistered the olywork MCP server ({out['mcp_url']}) into {ok} agent(s). "
          f"Restart an agent to pick it up.")


def cmd_skill_bootstrap(args, cfg) -> None:
    """Fetch the official olywork skill from the server and drop it into every detected agent's
    ...
    try:
        ...
        sys.exit(f"could not fetch the olywork skill from {base_url}: {exc}")
    if not recipe.strip():
    ...
    for b in bases:
        ...
        print(f"  ✓ olywork → {dest / 'SKILL.md'}"); n += 1
        ...
        try:
            if legacy.is_dir() and [q.name for q in legacy.iterdir()] == ["SKILL.md"]:
                ...
                print(f"    (removed the old tools-registry skill folder — renamed to olywork)")
        except OSError:
            pass
    ...
    print(f"\nInstalled the olywork skill into {n} location(s)  ({tail}).")


def cmd_agents_ls(args, cfg) -> None:
    """Show every agent olywork knows how to install skills for, its project + global skills dirs, and
    ...
    if _JSON_OVERRIDE:
        ...
        return
    print(f"{_A}Agents olywork can install skills for{_R}  ({_G}●{_R} detected here · {_M}○{_R} not)\n")
    ...
    print(f"{_M}`olywork skill install <name>` writes to those; `--agent <name>` / `--all-agents` / `--global` to widen.{_R}")
...
def cmd_balance(args, cfg) -> None:
    ...
    if auto and auto.get("enabled"):
        ...
        print(f"  {_AM}auto top-up off — {auto['disabled_reason']}. Run `olywork topup --auto on` after "
...
def cmd_topup(args, cfg) -> None:
    ...
    confirm. Run `olywork balance` after paying.
    """
    if args.auto:
    ...
    print(f"\n  {_M}Your balance updates as soon as Stripe confirms the payment "
          f"(seconds). Check it with `olywork balance`.{_R}\n")


def _topup_auto(args, cfg) -> None:
    """`olywork topup --auto on|off` — the consent gate lives HERE, in front of the request.
    ...
    if not turning_on:
        print(f"\n  {_G}Auto top-up is off.{_R} Add funds by hand with `olywork topup`.\n")
        return
...
def _confirm_consent() -> bool:
    ...
    try:
        ...
        answer = questionary.confirm(
            "Authorize olywork to charge your saved card automatically?",
...
def cli_version() -> str:
    """The installed olywork version (from package metadata; falls back for an editable/source run)."""
    try:
...
def cmd_org_access(args, cfg) -> None:
    ...
    with _client(cfg) as c:
        ...
        if cur is None:
            sys.exit(f"olywork: user {args.user_id} is not a member of this org")
...
def cmd_org_agent_new(args, cfg) -> None:
    """Mint (or rotate) an agent's own token. NOTE: an "agent" here is an IDENTITY that calls olywork —
    unrelated to `olywork agents`, which lists the coding agents we can install skills for."""
    with _client(cfg) as c:
        org_id = _active_org_id(cfg, c)
        if org_id is None:
            sys.exit("no active org")
...
def cmd_org_budgets(args, cfg) -> None:
    ...
    if not rows:
        ...
        print(f"  {_M}e.g. olywork org budget set customer cust_8123 --daily 5.00{_R}\n")
        return
...
def cmd_org_overflow(args, cfg) -> None:
    """Allow or refuse the overflow relay for this team: when olywork's own account for a provider is
    out, a metered call may be served through a olywork-owned aggregator account on the SAME endpoint
    (disclosed via X-Olywork-Served-Via, the aggregator's real price). Off = the call is refused
    ...
    print(f"\n  overflow relay: {_A if on else _AM}{'on' if on else 'off'}{_R}"
          f"  {_M}(when olywork's own account is out, serve the same endpoint via a olywork-owned "
...
def cmd_org_pins(args, cfg) -> None:
    ...
    if not rows:
        _dim("  no pins — every member picks the provider for each job "
             "(olywork catalog get <id> compares them)")
        return
...
def _valid_capability(value: str) -> str:
    ...
    if not _CAPABILITY_RE.match(v):
        sys.exit(f"olywork: {value!r} is not a capability id (lowercase letters, digits, dots, dashes)."
                 f"\n  See one with:  olywork catalog get <endpoint-id>")
    return v
...
def _last_ok_cell(row: dict) -> str:
    ...
    a plain age is MEASURED — the last time a real call through olywork came back 2xx. A `✓` age is
...
def cmd_catalog(args, cfg) -> None:
    ...
    `olywork catalog tiktok` keeps working unchanged; no platform slug collides with either word."""
    rest = list(getattr(args, "rest", []) or [])
    if args.platform == "search":
        return _catalog_search(" ".join(rest), args, cfg)
    if args.platform == "get":
        if not rest:
            sys.exit("which endpoint? e.g. olywork catalog get tikhub.tiktok.video.comments\n"
                     "find one with: olywork catalog search <query>")
        return _catalog_get(rest[0], cfg)
    if args.platform == "request":
        return _catalog_request(" ".join(rest), cfg)
    if rest:
        sys.exit(f"unexpected argument {rest[0]!r} — did you mean `olywork catalog search {args.platform} {' '.join(rest)}`?")
            ...
            for cat in order + sorted(set(by_cat) - set(order)):
                ...
                for p in by_cat[cat]:
                    print(f"  {p['slug']:<{w}} {p['endpoints']:>9} {p['capabilities']:>12}  "
                          f"{', '.join(p['providers'])}")
            print(f"\n{len(rows)} platforms — `olywork catalog <platform>` for its endpoints")
            return
...
def _print_catalog_endpoint(e: dict, connected: set = frozenset(), idw: int = 46) -> None:
    ...
    # Verified dates and core/extended tier are maintenance metadata (`olywork catalog get <id>`),
    # not decision data — a user picking an endpoint needs the ID to call, works-now?, price.
    # The ID leads: it is the thing `olywork call` takes, and a row without it named a provider an
    ...
    if e.get("kind") == "routed":
        print(f"  ▸ {_clip(e['id'], idw):<{idw}} {'ROUTED':<7} {_cost_usd(e.get('cost')):<16} {mark}  "
              f"olywork picks among {len(e.get('routed_children') or [])} providers below — own keys first, then cheapest per hit")
        return
    # unified USD only (`_cost_usd`); the provider's own credits/CNY live in `olywork catalog get`
...
def _catalog_search(query: str, args, cfg) -> None:
    ...
    if not query.strip():
        sys.exit("search for what? e.g. olywork catalog search tiktok comments")
    with _client(cfg, auth=False) as c:
    ...
    if not rows:
        ...
        _dim("try different task words, or browse the shelves with `olywork catalog`")
        _dim(f"still missing? file it: olywork catalog request \"{query}\"   # requests steer what gets added next")
        return
    ...
    # under it. Draw that as a hierarchy — "let olywork choose" leads, the specific providers indent.
    ...
    def _close_group() -> None:
        ...
        if open_group and open_group.get("children_hidden"):
            _dim(f"    + {open_group['children_hidden']} more do this job — olywork catalog get {open_group['id']} lists them all "
                 f"(routed and by-id)")
    ...
    for e in rows:
        if e.get("kind") == "routed":
            ...
            _dim(f"    olywork picks among {len(e.get('routed_children') or [])} providers (own keys first, then cheapest "
...
def _catalog_request(text: str, cfg) -> None:
    ...
    if not text.strip():
        sys.exit('request what? e.g. olywork catalog request "Ahrefs backlinks"')
...
def _catalog_get(endpoint_id: str, cfg) -> None:
    """One endpoint, everything about it — the last stop before `olywork call`."""
    with _client(cfg, auth=False) as c:
    ...
    if r.status_code == 404:
        ...
        if near:
            ...
            _dim(f"  olywork catalog get {near[0]}")
        else:
            ...
            _dim(f"find one with: olywork catalog search {terms.strip()}")
        sys.exit(1)
    if r.status_code != 200:
        _show(r)
        return
    ...
    if routing and routing.get("plan"):
        # The QUOTE: what `olywork call` on this routed id will try, in order, at olywork's prices. Own
        ...
        _dim("  a miss tries the next one (ceiling $1 per call by default); --header 'X-Olywork-Route-Max-Cost: 0.05' to cap it,")
        _dim("  --header 'X-Olywork-Route-Waterfall: 0' to stop at the first miss,")
        _dim("  --header 'X-Olywork-Route-Strict-Filters: 1' to refuse (422, unbilled) rather than call a provider that ignores a filter you sent")
        also = routing.get("also") or []
        if also:
            print(f"\n{_A}ALSO{_R}  {_M}the same job from providers olywork does not route to (yet) — call them by id{_R}")
            for a in also:
    ...
    if sibs:
        ...
        if pinned:
            ...
            _dim(f"  only theirs are listed (admin: olywork org unpin {e.get('capability')}).")
        else:
            _dim("  the same job from another provider.")
        _dim("  WORKS/SPEED are what olywork has actually observed; HIT is how often the provider FOUND")
    ...
    # Which credential tier would serve THIS caller (registered tool / org credential / olywork's own
...
def _print_async(desc) -> None:
    ...
    `X-Olywork-Async` header carries, so an MCP or raw-HTTP agent can poll it by hand."""
    ...
    if poll.get("endpoint"):
        ...
        print(f"  poll         olywork call {poll['endpoint']} -p {param.get('name')}=<task id>"
    ...
    if result.get("path"):
        ...
        print(f"  result       olywork call {result['fetch']} -p {fp.get('name')}=<`{fp.get('value_from')}` "
              f"from the finished task>")
    if result.get("ttl_note"):
        print(f"  lifetime     {result['ttl_note']} - download promptly; olywork never stores media")
    _dim("  `olywork call … --await` does all of this and prints the final response; from a coding")
...
def cmd_oauth_connect(args, cfg) -> None:
    if args.provider:  # registry mode — olywork's own approved app supplies the credentials
        ...
        sys.exit("give --provider <service> to use olywork's app (see `olywork connections providers`), "
...
# The front page of `olywork --help`: five groups in this order, each row a command + one line.
...
HELP_GROUPS: list[tuple[str, list[tuple[str, str]]]] = [
    ...
    ("ON YOUR MACHINE — use the team's credentials locally", [
        ...
        # form (`olywork claude`), which is how anyone will actually type it.
        ("with", "Run any command with the team's credentials: `olywork claude`, `olywork node app.js`."),
    ...
    ("TEAM MANAGEMENT", [
        ...
        ("agents", "Coding agents olywork can install skills for."),
    ...
    ("CONFIG", [
        ...
        ("update", "Upgrade the olywork CLI in place."),
        ("version", "Print the installed olywork version."),
    ]),
]

_GLOBAL_OPTS = [
    ("-h, --help", "Show this help and exit."),
    ("--version", "Print the olywork version and exit."),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    lg = mk(sub, "login", "Sign in. Opens the browser sign-in page; --email for a terminal-only code; --token for agents/CI.",
            "olywork login                                     # browser (reuses a dashboard session, or GitHub/Google/email)",
            "olywork login --email you@company.com             # emailed 6-digit code",
            "olywork login --token <per-org-token>             # non-interactive (agents/CI)")
    ...
    mk(sub, "logout", "Clear the stored credentials for this machine.",
       "olywork logout").set_defaults(fn=cmd_logout)

    mk(sub, "update", "Upgrade the olywork CLI in place (re-runs the registry's installer).",
       "olywork update").set_defaults(fn=cmd_update)
    mk(sub, "version", "Print the installed olywork version.", "olywork version").set_defaults(fn=cmd_version)

    ob = mk(sub, "onboard", "First-run: Set up (share skills+keys) · Access (pull your team's) · Demo.",
            "olywork onboard                                   # you're asked which path",
            "olywork onboard --path access                     # pull your team's shared skills + a test call",
            "olywork onboard --path setup --source global      # share skills from ~/.claude/skills etc., not this repo",
            "olywork onboard --path demo --yes                 # non-interactive demo")
    ...
    ob.add_argument("--reset", action="store_true", help="clean up demo team(s)/example teammates an older olywork demo created")
    ...
    oi = mk(og, "invite", "Invite someone to the active team by email (choose their tool access).",
            ...
            "olywork org invite bob@company.com --tools stripe,gh   # only these tools",
            "olywork org invite bob@company.com --all-tools --local-run off",
            "olywork org invite bob@company.com --skill slideshow   # share invite: they land on its page (full access; add --tools to scope)")
    ...
    oa = mk(og, "access", "Set which tools a member may use + whether they can run locally (admin+).",
            "olywork org access 5 --tools stripe,gh", "olywork org access 5 --all-tools", "olywork org access 5 --local-run off")
    ...
    mk(og, "invites", "List pending invites for the active team (admin+).", "olywork org invites").set_defaults(fn=cmd_org_invites)
    orv = mk(og, "revoke", "Revoke a pending invite before it's used.", "olywork org revoke 3")
    orv.add_argument("invite_id", type=int, help="the invite id (from `org invites`)"); orv.set_defaults(fn=cmd_org_revoke)
    mk(og, "members", "List the active team's members and their roles.", "olywork org members").set_defaults(fn=cmd_org_members)
    oov = mk(og, "overflow", "Show or set whether a metered call may be served through olywork's overflow "
                             "relay when olywork's own account is out (admin+).",
             "olywork org overflow", "olywork org overflow off")
    oov.add_argument("state", nargs="?", choices=["on", "off"], help="omit to show"); oov.set_defaults(fn=cmd_org_overflow)
    osr = mk(og, "set-role", "Change a member's role (owner only).", "olywork org set-role 5 admin")
    ...
    oan = mk(og, "agent-new", "Mint (or rotate) a token so an agent calls olywork as ITSELF — its own "
                              "cap, tool access and audit trail (admin+).",
             "olywork org agent-new ci-bot", "olywork org agent-new ci-bot --tools stripe,gh --cap 500",
             "olywork org agent-new ci-bot   # run again to rotate: the old token dies")
    ...
    oan.add_argument("--pin", action="append", metavar="DIM=VALUE",
                     help="pin this token to a caller tag (repeatable), e.g. --pin customer=cust_A. "
                          "The pin beats whatever X-Olywork-Meta the holder sends, so a token handed to "
    ...
    mk(og, "agents", "List this team's agent identities and their limits (admin+). "
                     "(Different from `olywork agents`, which lists coding agents for skill install.)",
       "olywork org agents").set_defaults(fn=cmd_org_agents)
    # ---- per-tag budgets: what a team reselling olywork sets on ITS OWN customers -------------------
    obs = mk(og, "budgets", "Per-tag spend limits you've set on your own customers (admin+).",
             "olywork org budgets", "olywork org budgets --dim workspace")
    ...
    obset = mk(og, "budget-set",
               ...
               "olywork org budget-set customer cust_8123 --daily 5",
               "olywork org budget-set workspace ws_9 --daily 50",
               "olywork org budget-set customer cust_8123 --block")
    ...
    obset.add_argument("--note", help="a note to yourself (shown in `olywork org budgets`)")
    ...
    obrm = mk(og, "budget-rm", "Drop a tag's limit. Its usage keeps being recorded and invoiced.",
              "olywork org budget-rm customer cust_8123")
    ...
    opn = mk(og, "project-new", "Create a project — a sub-scope inside the team (admin+).",
             'olywork org project-new "Apollo"')
    ...
    opin = mk(og, "pin", "For this JOB, our team uses this provider — the rest are refused (admin+).",
              "olywork org pin people.email.find --provider hunter",
              "olywork org pins", "olywork org unpin people.email.find")
    opin.add_argument("capability", help="a capability id, e.g. people.email.find (see `olywork catalog get`)")
    opin.add_argument("--provider", required=True, help="the provider service id, e.g. hunter")
    opin.set_defaults(fn=cmd_org_pin)
    mk(og, "pins", "The team's pinned provider per capability.", "olywork org pins").set_defaults(fn=cmd_org_pins)
    oup = mk(og, "unpin", "Remove a pin — the job goes back to the caller's choice (admin+).",
             "olywork org unpin people.email.find")
    ...
    od2 = mk(og, "deny", "Block calls to a host / path / method — for the team, or one member (admin+).",
             "olywork org deny --method DELETE --note 'no deletes'",
             "olywork org deny --host api.stripe.com", "olywork org deny --path /admin --user 7")
    ...
    mk(og, "deny-ls", "List this team's deny rules (admin+).", "olywork org deny-ls").set_defaults(fn=cmd_org_deny_ls)
    odr = mk(og, "deny-rm", "Remove a deny rule.", "olywork org deny-rm 3")
    ...
    s = mk(sub, "secret", "Manage stored credentials (encrypted server-side, never returned).",
           "olywork secret add STRIPE_KEY --value sk_live_…", "olywork secret ls",
           ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    sa = mk(s, "add", "Store a secret (a value, an .env var, a file, or auto-found in a skill dir).",
            ...
            "olywork secret add AHREFS_API_KEY --env-var AHREFS_API_KEY   # read + parse it from ./.env",
            "olywork secret add gcp --file creds.json --kind secret_file")
    ...
    mk(s, "ls", "List your secrets (names + kinds; never values).", "olywork secret ls").set_defaults(fn=cmd_secret_ls)
    sr = mk(s, "rm", "Delete a secret by id.", "olywork secret rm 4")
    sr.add_argument("id", type=int, help="the secret id (from `secret ls`)"); sr.set_defaults(fn=cmd_secret_rm)
    suu = mk(s, "update", "Rename a secret, change its value, or its kind.", "olywork secret update 4 --value sk_live_new")
    ...
    ad2 = alias(sub, "add", "(alias) friendly shortcut for `tool add`. --secret takes a name or id.",
                epilog=_ex("olywork add stripe --base-url https://api.stripe.com --secret STRIPE_KEY",
                           "olywork add gh --base-url https://api.github.com --secret GITHUB_TOKEN --format 'Bearer {secret}'"))
    ad2.add_argument("name", help="a name for this tool (used in `olywork call <name>`)")
    ...
    t = mk(sub, "tool", "Manage tools (endpoint or CLI).",
           "olywork tool ls", "olywork tool add stripe --base-url https://api.stripe.com --secret 1",
           ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    ta = mk(t, "add", "Register a tool with full control over the credential binding(s).",
            "olywork tool add stripe --base-url https://api.stripe.com --secret 1",
            "olywork tool add ads --base-url https://api.x.com --bind 'secret=1' --bind 'secret=2,name=developer-token'")
    ...
    mk(t, "ls", "List registered tools (names, hosts, bindings).", "olywork tool ls").set_defaults(fn=cmd_tool_ls)
    tr = mk(t, "rm", "Delete a tool by id.", "olywork tool rm 2")
    tr.add_argument("id", type=int, help="the tool id (from `tool ls`)"); tr.set_defaults(fn=cmd_tool_rm)
    tu = mk(t, "update", "Change a tool's base URL, bindings, or health-check.", "olywork tool update 2 --base-url https://api.stripe.com/v2")
    ...
    tu.add_argument("--local-run", choices=["on", "off"], help="allow/forbid `olywork run` (local tier) for this tool (owner opt-in)")
    ...
    au = mk(sub, "audit", "Who called/ran what, when, and the result — proxy calls and CLI runs, one log.",
            "olywork audit --limit 20", "olywork audit --calls          # only proxy calls",
            "olywork audit --runs           # only CLI runs (server + local)")
    _limit_arg(au, "rows")
    aug = au.add_mutually_exclusive_group()
    aug.add_argument("--calls", action="store_true", help="only proxy calls (what `olywork calls` showed)")
    aug.add_argument("--runs", action="store_true", help="only CLI runs, both tiers (what `olywork runs` showed)")
    au.set_defaults(fn=cmd_audit)

    ca = alias(sub, "calls", "(alias) the proxy-call half of `olywork audit`.")
    ...
    def _run_args(parser):
        ...
        g.add_argument("--local", action="store_true", help="run on this machine (default; credential isolated under the olywork-run user)")
    ...
    def _setup_args(parser):
        ...
        parser.add_argument("--run-proof", default="", help="the server's OLYWORK_RUN_PROOF value — enables running "
                            "SHARED-key tools (ones you don't own) locally; without it only your own-key tools run")
        parser.add_argument("--registry", help="registry URL olywork-run must reach for /grant (default: the member's configured base_url)")
        parser.add_argument("--no-egress", dest="no_egress", action="store_true",
                            help="skip the network allow-list (olywork-run keeps unrestricted egress)")
    ...
    def _shell_subs(parser, prefix):
        ...
        st = mk(s2, "start", "Start the olywork shell (a subshell; registered CLIs are transparently injected).",
                f"{prefix} start",
        ...
        st.add_argument("--proxy", action="store_true",
                        help="also catch calls your AGENT makes directly to a registered API (a script "
                             "calling api.stripe.com). olywork adds the credential on the server; no key "
        ...
        mk(s2, "stop", "Leave the olywork shell (same as typing `exit` or Ctrl-D).",
           f"{prefix} stop").set_defaults(fn=cmd_shell_stop)

    wi = mk(sub, "with",
            "Run any command with your team's credentials injected — olywork is the parent, so ONLY that "
            "command is affected. Usually written without the word `with`: `olywork claude`.",
            "olywork claude", "olywork codex", "olywork node server.js", "olywork with -- python train.py")
    ...
    sv = mk(sub, "serve",
            ...
            "APIs are credentialed in YOUR OWN shell (the same thing `olywork shell start --proxy` does "
            "for a subshell).",
            "olywork serve start", 'eval "$(olywork serve env)"', "olywork serve status", "olywork serve stop",
            ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    svs = mk(sv, "start", "Start the proxy in the background (prints how to point a shell at it).",
             "olywork serve start", "olywork serve start --port 18800", "olywork serve start --foreground")
    ...
    mk(sv, "stop", "Stop the background proxy.", "olywork serve stop").set_defaults(fn=cmd_serve_stop)
    mk(sv, "status", "Is it running, on which port, and which hosts does it capture?",
       "olywork serve status").set_defaults(fn=cmd_serve_status)
    sve = mk(sv, "env", "Print the shell lines that point this terminal at the running proxy.",
             'eval "$(olywork serve env)"', 'eval "$(olywork serve env --unset)"')
    ...
    cn2 = mk(sub, "cli", "Run vendor CLIs with the org's credential injected (run · shell · setup).",
             "olywork cli run stripe -- get /v1/balance", "olywork cli shell start", "sudo olywork cli setup",
             ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    _run_args(mk(cn2, "run",
                 ...
                 "olywork cli run stripe -- get /v1/balance", "olywork cli run gh -- pr list",
                 "olywork cli run --server agentmail-cli -- inboxes list"))
    cr2 = mk(cn2, "runs", "Show the CLI-run audit log: who ran which tool's CLI, when, and the exit code.",
             "olywork cli runs --limit 20", "olywork audit           # calls + runs together")
    ...
    _shell_subs(mk(cn2, "shell",
                   ...
                   "olywork cli shell start", "olywork cli shell stop"), "olywork cli shell")
    _setup_args(mk(cn2, "setup",
                   "One-time admin setup: create the olywork-run user + install the isolated runner (run with sudo).",
                   "sudo olywork cli setup"))

    # The pre-`cli`-namespace spellings, kept working for existing scripts + agent instructions.
    _run_args(alias(sub, "run", "(alias) `olywork cli run`."))
    rns = alias(sub, "runs", "(alias) `olywork cli runs`."); _limit_arg(rns, "runs"); rns.set_defaults(fn=cmd_runs)
    _shell_subs(alias(sub, "shell", "(alias) `olywork cli shell`."), "olywork shell")
    _setup_args(alias(sub, "setup-local-run", "(alias) `olywork cli setup`."))

    # hidden: invoked as the olywork-run user by the installed runner (never called by a human directly)
    ...
    sk = mk(sub, "skill", "Register / manage skills (a recipe + its secrets + tool(s), as one bundle).",
            "olywork skill init --dir ./my-skill", "olywork skill add --dir ./my-skill", "olywork skill install seo-blog-writer",
            ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    sc = mk(sk, "scaffold", "Emit a skill-registration manifest stub for a folder (bindings need completing).", "olywork skill scaffold ./my-skill")
    ...
    si = mk(sk, "init", "Draft a olywork.json for a skill dir (guesses base_url, finds secrets).", "olywork skill init --dir ./my-skill")
    si.add_argument("--dir", required=True, help="the skill directory"); si.add_argument("--out", help="write the olywork.json here"); si.set_defaults(fn=cmd_skill_init)
    sad = mk(sk, "add", "Register a skill folder (recipe + secrets + tool) from its olywork.json.", "olywork skill add --dir ./my-skill")
    sad.add_argument("--dir", required=True, help="the skill directory (must contain olywork.json)"); sad.set_defaults(fn=cmd_skill_add)
    sp = mk(sk, "push", "Register a skill from a prepared manifest file.", "olywork skill push ./manifest.json")
    sp.add_argument("file", help="the manifest JSON file"); sp.set_defaults(fn=cmd_skill_push)
    mk(sk, "ls", "List registered skills (bundles).", "olywork skill ls").set_defaults(fn=cmd_skill_ls)
    skr = mk(sk, "rm", "Delete a skill (bundle) by id.", "olywork skill rm 1")
    ...
    ski = mk(sk, "install", "Pull a shared skill into every agent's skills dir (.agents/skills + .claude/skills).",
             "olywork skill install seo-blog-writer", "olywork skill install --all", "olywork skill install foo --agent cursor")
    ...
    ski.add_argument("--agent", help="install for one agent only (see `olywork agents ls`)")
    ...
    skb = mk(sk, "bootstrap", "Install the official olywork skill into every detected agent (used by the installer).",
             "olywork skill bootstrap", "olywork skill bootstrap --all-agents")
    ...
    # ---- mcp (register the olywork MCP server into detected agents, header-authed) ----
    mc = mk(sub, "mcp", "Register the olywork MCP server into the coding agents on this machine.",
            "olywork mcp install").add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    mci = mk(mc, "install", "Add olywork as a header-authed MCP server in every supported detected agent.",
             "olywork mcp install")
    mci.add_argument("--name", default="olywork", help="the MCP server name to register (default: olywork)")
    ...
    # ---- agents (which coding agents olywork installs skills for) ----
    ag = mk(sub, "agents", "List the coding agents olywork can install skills for (and which are detected here).",
            "olywork agents ls").add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    mk(ag, "ls", "Show every known agent + its skills dirs + detection status.",
       "olywork agents ls").set_defaults(fn=cmd_agents_ls)
    ...
    sc = mk(sub, "scan", "Scan a directory / machine (read-only): list the keys, skills & CLIs olywork would upload. Nothing leaves this machine.",
            "olywork scan                                      # both .env + skills in this dir",
            "olywork scan env                                  # just the .env keys",
            "olywork scan clis                                 # installed CLIs olywork can register",
            "olywork scan skills --dir ~/.claude/skills")
    ...
    def _upload_args(parser):
        ...
        parser.add_argument("--llm-token", help="LLM API token (or set OLYWORK_LLM_TOKEN)")
        ...
        _clis_flags(parser)
        parser.add_argument("--dry-run", action="store_true", help=argparse.SUPPRESS)  # works, but we teach `olywork scan`
    ...
    up = mk(sub, "upload", "Upload a directory / machine: register its .env provider keys, skill subdirs, AND/OR installed CLIs (encrypted server-side).",
            "olywork upload                                    # both .env + skills in this dir",
            "olywork upload env --select openai,stripe        # just picked provider keys",
            "olywork upload clis                               # register installed catalog CLIs",
            "olywork upload skills --dir ~/.claude/skills --all",
            "olywork scan                                      # preview first; nothing leaves the machine")
    _upload_args(up)
    ...
    usg = mk(sub, "usage", "What each of your caller tags consumed — what you invoice your own users "
             ...
             "olywork usage                       # by your primary tag (default: customer)",
             "olywork usage --by workspace",
             "olywork usage --by customer --days 7")
    ...
    tu = mk(sub, "topup", "Add funds to your team's balance, or set up automatic top-ups.",
            "olywork topup                                     # a Checkout link for the default amount",
            "olywork topup 100                                 # …for $100 (+10% bonus credit)",
            "olywork topup --auto on --threshold 5 --amount 20 # refill $20 whenever it drops below $5",
            "olywork topup --auto off")
    ...
    # `olywork topup 25 --auto on` ambiguous about which $25 was meant.
    ...
    he = mk(sub, "health", "Show tool/secret health, or run the checks now with --run.",
            "olywork health", "olywork health --run")
    ...
    ct = mk(sub, "catalog", "Browse the library of endpoints & tools for data access and integrations.",
            "olywork catalog                                   # platforms, busiest first",
            "olywork catalog tiktok                            # every provider's tiktok endpoints, by capability",
            "olywork catalog search tiktok comments            # find an endpoint by what it does",
            "olywork catalog get tikhub.tiktok.video.comments  # params, cost, example response, how to call it",
            "olywork catalog request \"Ahrefs backlinks\"        # missing? file it — requests steer what gets added")
    ...
    cnp = mk(sub, "connections", "Your connected accounts: connect providers, health, expiry.",
             "olywork connections                               # same as `connections ls`",
             "olywork connections connect --provider google-search-console",
             "olywork connections resources 12",
             "olywork connections use 12 sc-domain:example.com", "olywork connections rm 12")
    cnp.set_defaults(fn=cmd_connections_ls)  # bare `olywork connections` = list them
    cn = cnp.add_subparsers(dest="sub", required=False, metavar="<subcommand>")
    mk(cn, "ls", "List connections with health + expiry.", "olywork connections ls").set_defaults(fn=cmd_connections_ls)
    _connect_args(mk(cn, "connect", "Connect a provider: browser OAuth consent — or a pasted API key for key-based providers.",
                     "olywork connections connect --provider google-search-console          # olywork's app, read scope",
                     "olywork connections connect --provider google-search-console --capability write",
                     "olywork connections connect gsc --client-secret ./client_secret.json --scopes <scope>  # your own app"),
                 "olywork connections")
    mk(cn, "providers", "List providers olywork holds its own OAuth app for (what you can connect).",
       "olywork connections providers").set_defaults(fn=cmd_oauth_providers)
    ...
    cu = mk(cn, "use", "Select which resource this connection acts on.",
            "olywork connections use 12 sc-domain:example.com")
    ...
    cd = mk(cn, "rm", "Disconnect (bound tools stay, but stop working until reconnected).",
            "olywork connections rm 12")
    cd.add_argument("id", type=int); cd.set_defaults(fn=cmd_connections_rm)

    oa = alias(sub, "oauth", "(alias) `olywork connections connect` / `olywork connections providers`.",
               ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    mk(oa, "providers", "List providers olywork holds its own OAuth app for.",
       "olywork oauth providers").set_defaults(fn=cmd_oauth_providers)
    _connect_args(mk(oa, "connect", "Mint an auto-refreshed OAuth secret through browser consent.",
                     "olywork oauth connect --provider google-search-console"), "olywork oauth")

    # ---- super-admin ----
    ad = mk(sub, "admin", "Super-admin (cross-tenant): platform-wide view + control.",
            "olywork admin login --token <admin-token>", "olywork admin stats", "olywork admin orgs",
            ).add_subparsers(dest="sub", required=True, metavar="<subcommand>")
    al = mk(ad, "login", "Save the super-admin bearer token for later admin commands.", "olywork admin login --token <admin-token>")
    al.add_argument("--token", required=True, help="the cross-tenant admin token"); al.set_defaults(fn=cmd_admin_login)
    mk(ad, "stats", "Platform-wide counts and health.", "olywork admin stats").set_defaults(fn=cmd_admin_stats)
    mk(ad, "orgs", "List every org across all tenants.", "olywork admin orgs").set_defaults(fn=cmd_admin_orgs)
    ao = mk(ad, "org", "Inspect one org by id.", "olywork admin org 2")
    ao.add_argument("org_id", type=int, help="the org id"); ao.set_defaults(fn=cmd_admin_org)
    mk(ad, "users", "List every user.", "olywork admin users").set_defaults(fn=cmd_admin_users)
    mk(ad, "tools", "List every tool across all orgs.", "olywork admin tools").set_defaults(fn=cmd_admin_tools)
    ac = mk(ad, "calls", "The cross-tenant audit log.", "olywork admin calls --limit 100")
    ac.add_argument("--limit", type=int, default=50, help="how many recent calls (default: 50)"); ac.set_defaults(fn=cmd_admin_calls)
    mk(ad, "health", "Platform-wide health rollup.", "olywork admin health").set_defaults(fn=cmd_admin_health)
    ag = mk(ad, "grant", "Grant a user super-admin.", "olywork admin grant 5")
    ag.add_argument("user_id", type=int, help="the user id"); ag.set_defaults(fn=cmd_admin_grant)
    arv = mk(ad, "revoke", "Revoke a user's super-admin.", "olywork admin revoke 5")
    arv.add_argument("user_id", type=int, help="the user id"); arv.set_defaults(fn=cmd_admin_revoke)
    asu = mk(ad, "suspend-user", "Suspend (or --undo) a user platform-wide.", "olywork admin suspend-user 5", "olywork admin suspend-user 5 --undo")
    ...
    aru = mk(ad, "rm-user", "Delete a user platform-wide.", "olywork admin rm-user 5")
    aru.add_argument("user_id", type=int, help="the user id"); aru.set_defaults(fn=cmd_admin_rm_user)
    aso = mk(ad, "suspend-org", "Suspend (or --undo) an org platform-wide.", "olywork admin suspend-org 2", "olywork admin suspend-org 2 --undo")
    ...
    aro = mk(ad, "rm-org", "Delete an org platform-wide.", "olywork admin rm-org 2")
    ...
    acr = mk(ad, "credit", "Credit an org promotional balance (HTTP equivalent of scripts/manual_grant.py).",
             "olywork admin credit 2867 --amount-usd 100 --ref hs-1234 --reason 'goodwill comp'")
...
def _looks_like_a_program(argv: list[str], commands: set[str]) -> bool:
    """Is this `olywork <program> …` rather than a olywork command? (`olywork claude`, `olywork node server.js`.)

    Two conditions, both needed. The word must not be a olywork command — otherwise a stray `call` binary
    on someone's PATH would shadow `olywork call`. And it must actually exist on this machine — otherwise
    a typo like `olywork toool ls` would become a confusing exec attempt instead of an ordinary olywork error.

    `with`'s own flags may come first (`olywork -q node app.js`), because putting the flag where it reads
