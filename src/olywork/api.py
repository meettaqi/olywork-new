...
async def _bootstrap_single_user() -> None:
    ...
    print(f"\n  olywork is ready — no account needed."
...
def _olywork_version() -> str:
    ...
    `olywork.cli` here costs ~200ms on first call and pulls the entire CLI into the server process for
...
async def meta() -> dict:
    ...
    `olywork_version` and `app_version` answer DIFFERENT questions and both are worth having.
    `app_version` is a hash of index.html: it changes whenever the dashboard bundle does, which is
    what an open tab compares to offer a refresh. `olywork_version` is the released package version,
    ...
    return {"public_url": s.public_url.rstrip("/"), "github": bool(s.github_client_id),
            ...
            "olywork_version": _olywork_version(),
...
async def providers_catalog() -> dict:
    """Open: the provider catalog `olywork upload` uses to detect env keys → tools. Served so the CLI can
...
def _require_local_run(caller: Caller) -> None:
    ...
    if caller.role != "owner" and not caller.membership.local_run_enabled:
        raise HTTPException(status_code=403, detail=(
            "local execution is disabled for you — run on the server instead (`olywork run --server`), "
...
async def clear_capability_pin(
    ...
    sending it — which is DELETE /orgs/{id}, the delete-the-team route. `olywork org unpin ..` really
...
# ---- local runs (`olywork run`): grant + outcome report (docs/CLI-RUN-PLAN.md) -----------------
# Redact obvious credentials a user might type INLINE (`olywork run x -- --token sk_live_…`) before the
...
async def grant_local_run(
    ...
    if profile is None:
        raise HTTPException(status_code=409, detail=(
            f"olywork doesn't know how to inject credentials into {tool.name!r}. Add a \"cli\" block to the "
            'skill\'s olywork.json — template: {"cli": {"bin": "' + tool.name + '", '
    ...
    if not profile.get("enabled"):
        raise HTTPException(status_code=403, detail=(
            ...
            f"olywork tool update {tool.name} --local-run on"))
    ...
    if denied:
        ...
        raise HTTPException(status_code=403, detail=(
            ...
            "(cli.deny in olywork.json)."))
    # Runner-proof gate (Bug 1). Handing a member a secret they do NOT own — a shared-key tool they may
    # RUN but not SEE — is allowed only for the isolated olywork-run runner, which proves itself with a
    # value the member can't read (`X-Olywork-Run-Proof`). A direct member call has no proof, so the raw
    ...
    if needs_proof:
        ...
        supplied = request.headers.get("X-Olywork-Run-Proof") or request.headers.get("X-Olywork-Run-Proof", "")
        if not (proof and hmac.compare_digest(supplied, proof)):
            await _grant_audit(db, caller, tool.name, "DENY", _redact_argv(body.argv), 403, _client_of(request))
            raise HTTPException(status_code=403, detail=(
                "this tool uses another member's key — running it needs the isolated olywork-run runner "
                "(an admin sets it up once: `sudo olywork setup-local-run --run-proof …`). A direct grant "
    ...
    if ttl is not None and ttl <= 0:
        warnings.append("the injected token appears already expired — the run will likely fail; "
                        "an owner may need to reconnect it (olywork oauth connect)")
    ...
    return [
        {
            ...
            # non-null = olywork said no before anything went upstream (see models.CallRecord) — the
            # one field that tells "the provider failed" apart from "we refused" in `olywork audit`.
            "refused_by": c.refused_by,
            # The caller's own tags (X-Olywork-Meta), for a builder reconciling this row against their
...
async def get_call(
    ...
    """One call by the `X-Olywork-Call-Id` it returned — the join key for a builder's own records.
...
async def list_runs(
    ...
    """Audit log for CLI executions (`olywork run`, both tiers), scoped to the caller's org — each row
...
async def run_tool_server(
    ...
    """Run a tool's CLI **on the olywork server**, with its `cli.inject` secrets injected into the
    ...
    if exec_bin not in _allowed_server_bins():
        raise HTTPException(status_code=422, detail=(
            ...
            "server (an admin can allow more via OLYWORK_RUN_ALLOWED_BINS). Use `olywork run --local` instead."))
...
# Deployment and imports keep using `olywork.api:app`; the concrete assembly now lives in bootstrap.
