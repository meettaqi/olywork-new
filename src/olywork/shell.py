"""olywork shell — transparent CLI interception (shell mode), MVP / Phase 1.
...
`olywork run`. The mechanic (docs/CLI-SHELL-MODE-PLAN.md): put a private **shim directory first on PATH**
holding one wrapper per registered CLI. The shell resolves a command name against PATH in order, so a
registered CLI finds our shim first and is routed through olywork; any other command has no shim and
resolves normally — the "is this registered?" test is done for free by name resolution.

Phase 1 keeps it correct and simple: each shim runs `olywork run <tool> -- "$@"`, reusing the whole
local-run path (grant / deny / runner-proof / audit / metering). The in-memory session agent (no key
on disk, ~ms per call) is Phase 2. Nothing here holds a credential — `olywork run` fetches the grant per
...
shim invokes `olywork run` with `PATH=$OLYWORK_SHELL_REALPATH` — the original PATH captured BEFORE the shim
dir was prepended. So `olywork run`'s own `shutil.which(<bin>)` resolves the REAL binary, never the shim,
...
# Env vars published into the subshell. OLYWORK_SHELL marks an active session (blocks nesting); _DIR is
# the private session dir (shims live in <dir>/bin); _REALPATH is the clean PATH the shims hand to
# `olywork run`; _PID is the `olywork shell start` process a `olywork shell stop` signals to tear down.
ENV_ACTIVE = "OLYWORK_SHELL"
ENV_DIR = "OLYWORK_SHELL_DIR"
ENV_REALPATH = "OLYWORK_SHELL_REALPATH"
ENV_PID = "OLYWORK_SHELL_PID"

# Sourced by the subshell so the user's normal environment is intact, our shims stay FIRST on PATH
# even if their rc reset it, and the prompt shows a `(olywork)` marker. Kept per-shell because the prompt
# variable + config file differ. `$OLYWORK_SHELL_DIR` is read from the env we pass in.
_ZSH_RC = (
    "# olywork shell (generated) — restore the user's zsh env, keep our shims first, mark the prompt.\n"
    'ZDOTDIR="$HOME"\n'
    '[ -f "$HOME/.zshrc" ] && source "$HOME/.zshrc"\n'
    'export PATH="$OLYWORK_SHELL_DIR/bin:$PATH"\n'
    'PROMPT="%F{cyan}(olywork)%f $PROMPT"\n'
)
_BASH_RC = (
    "# olywork shell (generated) — restore the user's bash env, keep our shims first, mark the prompt.\n"
    '[ -f "$HOME/.bashrc" ] && source "$HOME/.bashrc"\n'
    'export PATH="$OLYWORK_SHELL_DIR/bin:$PATH"\n'
    'PS1="\\[\\e[36m\\](olywork)\\[\\e[0m\\] $PS1"\n'
)
...
def plan_shims(tools: list[dict], server_for: frozenset[str] = frozenset()) -> tuple[list[tuple[str, str, str]], list[str]]:
    ...
    `cli.bin` that is `cli.enabled` (owner opt-in for local runs — a non-enabled tool's `olywork run`
    would 403). A tool named in `server_for` (by bin OR tool name) routes to the SERVER (`olywork run
...
def shim_script(tool_name: str, olywork_bin: str, route: str = "local", real_bin: str | None = None) -> str:
    """A shim's contents: exec `olywork run [--server] <tool> -- "$@"` on a CLEAN PATH (so `olywork run`
    finds the real binary, not this shim). `--server` runs the CLI on the registry server (key never
    on the machine); default is local. The tier flag goes BEFORE the tool name (olywork's own rule) and
    the literal `--` fences olywork's parsing from the user's args, which `olywork run` then strips — so the
    ...
    and routing it through olywork would flood the audit and burn the daily cap one keystroke at a time.
    ...
    return (
        "#!/bin/sh\n"
        f"# olywork shell shim → tool {tool_name} (runs {where}). Injects the team credential via `olywork run`.\n"
        "# Generated by `olywork shell start`; do not edit — it vanishes when the shell exits.\n"
        f"{bypass}"
        f'exec env PATH="${ENV_REALPATH}" {shlex.quote(olywork_bin)} run {tier}{shlex.quote(tool_name)} -- "$@"\n'
    )


def write_shims(shim_dir: str, entries: list[tuple[str, str, str]], olywork_bin: str) -> None:
    ...
    exec it directly and olywork run never recurses into the shim."""
    for bin_, tool_name, route in entries:
        ...
        p.write_text(shim_script(tool_name, olywork_bin, route, real_bin))
        os.chmod(p, 0o755)
...
def _print_banner(entries: list[tuple[str, str, str]], ttl_minutes: int | None,
    ...
    out = sys.stderr
    print(f"\n{_CLAY}{_BOLD}▚ olywork shell{_RESET}{_MUTED} — you're now in a shell where your team's CLIs just work.{_RESET}", file=out)
    print(f"{_MUTED}  The tools below run with the team credential injected for you — no `olywork run`,{_RESET}", file=out)
    ...
    if captured_hosts:
        ...
        print(f"{_MUTED}  started in this shell — goes through olywork, which adds the credential on the{_RESET}", file=out)
...
def _teardown(session_dir: str) -> None:
    ...
    Phase 1 holds nothing; `olywork run` fetches each grant fresh and never persists it."""
...
def _run_subshell(argv: list[str], env: dict, ttl_seconds: int | None = None) -> int:
    ...
    SIGTERM / SIGHUP by stopping the child — so `olywork shell stop` (which signals this process) and a
    ...
    def _on_ttl():
        if proc.poll() is None:
            print("\n▚ olywork shell reached its time limit — closing.", file=sys.stderr)
            _stop(None, None)
...
def start_session(entries: list[tuple[str, str, str]], olywork_bin: str, ttl_minutes: int | None = None,
    ...
    `PATH`/`OLYWORK_SHELL*` and break the shims."""
    base = session_base_dir()
    session_dir = tempfile.mkdtemp(prefix="olywork-shell-", dir=base)  # 0700 by mkdtemp
    os.chmod(session_dir, 0o700)
    shim_dir = os.path.join(session_dir, "bin")
    os.makedirs(shim_dir)
    os.chmod(shim_dir, 0o700)
    write_shims(shim_dir, entries, olywork_bin)
    ...
    env[ENV_REALPATH] = real_path  # the clean PATH the shims hand to `olywork run` (loop avoidance)
    ...
    try:
        ...
        if on_close is not None:
            try:
                ...
                print(f"▚ olywork shell: could not stop the proxy cleanly ({exc}).", file=sys.stderr)
        _teardown(session_dir)
        print("▚ olywork shell closed.", file=sys.stderr)


def stop_session() -> None:
    """`olywork shell stop`, run from inside a session: signal the `olywork shell start` process to stop the
    subshell (a child process can't terminate its parent shell directly, so it asks the olywork parent to,
    ...
    if os.environ.get(ENV_ACTIVE) != "1":
        sys.exit("olywork: no olywork shell is active here. Start one with `olywork shell start`.")
    pid = os.environ.get(ENV_PID)
    if not pid:
        sys.exit("olywork: this olywork shell has no controller to signal — type `exit` (or Ctrl-D) to leave.")
    try:
        os.kill(int(pid), signal.SIGTERM)
    except (ValueError, ProcessLookupError, OSError) as exc:
        sys.exit(f"olywork: could not stop the shell ({exc}) — type `exit` (or Ctrl-D) to leave.")
