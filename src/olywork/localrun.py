"""Local CLI runs (`olywork run`) — the server-side grant logic.
...
#   env / argv  → server-injectable (olywork holds + injects the secret; either run tier works)
    ...
    for pat in profile.get("deny") or []:
        try:
            ...
            if re.search(pat, joined) or any(re.search(pat, a) for a in argv):
                return pat, ("this skill's olywork.json" if pat in own else "the olywork catalog defaults")
        except re.error:
            continue
    return None
...
async def render_grant(tool: Tool, profile: dict, db: AsyncSession, http) -> dict:
    ...
    if not inject:
        ...
        # so `olywork run` just execs the CLI. (Empty is DISTINCT from a broken profile, which is rejected
    ...
    for entry in inject:
        ...
        if (secret is None or secret.org_id != tool.org_id
            ...
            raise LookupError(
                ...
                '"from_binding" (the tool has multiple credentials, so olywork won\'t guess which one)'
        ...
        if secret.kind == "oauth":
            ...
            try:
                ...
                # Google-style blobs carry `token` before olywork's first refresh and `access_token`
