...
Two things make this safe even though the token can register any endpoint:
  1. **Calls never touch the network.** `synthesize()` runs the REAL injectors to compute exactly
     what olywork *would* send upstream, then returns a labelled dummy response. No SSRF, no open relay,
...
# ---- the ONE live wire ----------------------------------------------------------------------
# The seeded stripe tool above is special: when OLYWORK_DEMO_STRIPE_KEY is configured, a sandbox call
...
def synthesize(method: str, upstream_url: str, tool: Tool, secrets: dict[int, Secret],
    ...
    """Compute what olywork WOULD send upstream (via the real injectors) and return a labelled dummy
    ...
    data = SAMPLE_BODIES.get(tool.host,
                             {"ok": True, "message": "Authenticated by olywork - your key was injected in the proxy."})
    return {
        "sandbox": True,
        "note": "Dummy sandbox response - olywork did NOT call the real upstream. Sign up to reach live APIs.",
