...
`OLYWORK_RESEND_API_KEY` is set, sends are skipped (logged), so local/dev without a key is fine.
"""
...
_WRAP = (
    ...
    '<span style="color:#e4714a">▚</span> <span style="color:#19D0E8;font-weight:600">olywork</span>'
...
async def _send(to: str, subject: str, html: str, text: str) -> bool:
    ...
    if not s.resend_api_key:
        print(f"[email] no OLYWORK_RESEND_API_KEY — skipping send to {to} ({subject!r})")
        return False
...
async def send_topup_receipt(email: str, org_name: str, amount_micro: int, balance_micro: int,
    ...
    body = (
        ...
        '<p style="margin:18px 0 0;color:#8e8c86;font-size:12px;line-height:1.6">Run <span style="color:#19D0E8">olywork balance</span> to see what it\'s made of and where it goes.</p>'
    )
    ...
    text = (f"{how} confirmed: {added}{bonus_txt} added to {org_name}'s tools-registry balance.\n"
            f"Balance now: {left}. Run `olywork balance` for the detail.")
