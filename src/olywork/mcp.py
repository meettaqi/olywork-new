"""The MCP front door — olywork's own operations, exposed to an agent that has no terminal.

A coding agent reaches olywork through the CLI, and the skill tells it which commands to run. An agent
...
through olywork's own HTTP API in-process (`httpx.ASGITransport`), rather than reaching into the
...
# that sends, or a DELETE — olywork does not model the upstream, so it cannot promise the call is safe,
...
_INTERNAL_BASE = "http://olywork.internal"
...
class _StaticSurfaceCapabilities:
    """Do not advertise or serve change subscriptions for olywork's fixed MCP surface.

    MCP SDK 2.0 currently installs subscriptions/listen unconditionally, then derives every
    listChanged/resource-subscribe capability from that handler. olywork never changes its six-tool
...
async def _observed_stats(endpoint_ids: list[str]) -> dict[str, dict]:
    """What olywork has measured across real calls to these endpoints — `{}` if it can't be read.
    ...
    if reader is None:
        logging.getLogger("olywork.mcp").warning("endpoint observation reader is not configured")
        return {}
    try:
        return await reader.get_many(endpoint_ids)
    except Exception:  # noqa: BLE001 — telemetry must never take search down
        logging.getLogger("olywork.mcp").warning("endpoint stats unavailable", exc_info=True)
        return {}
...
def _oauth_claims(token: str) -> dict | None:
    ...
    server, and honouring it here would spend their olywork balance on a consent they never gave us.
    """
...
def _need_token() -> dict:
    ...
    return {
        ...
        "detail": (
            f"This MCP server needs a olywork token. Get one at {base} "
            "(sign in, then Settings -> copy token) and set it as the OLYWORK_TOKEN environment "
...
async def _internal_auth(token: str) -> dict[str, str]:
    """Turn whatever the caller presented into headers olywork's own API understands.
    ...
    if claims is None:
        ...
        # a bare MCP bearer where no X-Olywork-Org header can travel). If it does, surface that as
        # X-Olywork-Org so `_resolve_org` takes its "pinned" path instead of asking "which team?" — the
        ...
        auth_h = {"X-Olywork-Token": token, "X-Olywork-Token": token}
        if pinned:
            auth_h.update({"X-Olywork-Org": pinned, "X-Olywork-Org": pinned})
        return auth_h
...
async def _api(token: str, *, client_name: str = "mcp"):
    """An in-process client bound to olywork's own ASGI app, carrying the caller's identity."""
    ...
    async with httpx.AsyncClient(
        ...
        timeout=_TIMEOUT,
        # X-Olywork-Client is attribution, not auth: without it every MCP-originated call lands
        # in the audit trail as client="", indistinguishable from unreported CLI traffic.
        headers={**await _internal_auth(token), "X-Olywork-Client": client_name, "X-Olywork-Client": client_name},
    ) as client:
        yield client
...
async def _resolve_org(client: httpx.AsyncClient) -> tuple[int | None, str | None, dict | None]:
    ...
    bug here. A PER-ORG token (what `olywork org agent-new` mints) has its org baked in and `/auth/me`
    reports it. An IDENTITY token (what `olywork login` gives, which is what most people actually hold)
    ...
    pinned = client.headers.get("X-Olywork-Org") or client.headers.get("X-Olywork-Org")
    if pinned:
    ...
    if chosen is None:
        return None, None, {
            ...
            "hint": "ask the human which team to use, then set OLYWORK_TOKEN to that team's token "
                    "(olywork org agent-new) so the choice is unambiguous",
        }
...
async def _whose_grant(client: httpx.AsyncClient, slug: str | None, *, oauth: bool) -> dict:
    ...
    A slug on its own cannot be sanity-checked. `olywork-7` looks like a plausible team to an
    ...
    one login while the CLI is signed in as another, which is exactly why `olywork org ls` did not list
    ...
    try:
        ...
        logging.getLogger("olywork.mcp").warning("could not label the grant's team", exc_info=True)
    if oauth:
        # Only an OAuth grant HAS a team to move. A header token carries its own team already, and
        # `olywork mcp grants` would list nothing for it — sending that caller to a command with no
        ...
        out.setdefault("hint", "to spend from a different team, the human who authorised this "
                               "connection runs `olywork mcp grants` then `olywork mcp use-team "
...
@mcp.tool(
    description=(
        ...
        "Returns each endpoint's id, provider, price per call, and whether olywork can serve it "
...
async def _catalog_search_impl(
    ...
    # its children right under it (catalog_store.group_routed), so an agent sees "let olywork choose"
    ...
    for ep, score in ranked:
        ...
        results.append({
            ...
            # a generated routed row: olywork picks among N children (own keys first, then cheapest
            # per hit) and names the one that served — the children follow in this list
            **({"routed": f"olywork picks among {len(ep.get('routed_children') or [])} providers"
...
@mcp.tool(
    ...
    # A write, but a harmless one: it files a report on olywork itself — nothing upstream, nothing spent.
...
@mcp.tool(
    description=(
        "One endpoint in full: its parameters, the exact price per call, whether olywork serves it on "
        "its own key, and the other providers that answer the same capability — with the success "
        "rate and speed olywork has measured across real calls. Read this BEFORE call() so you can "
...
@mcp.tool(
    description=(
        "Call something through olywork. `params` is the request body for a POST (an object, or an "
        ...
        "'render/v1/services'. olywork injects the credential server-side and relays the provider's "
        "response unchanged, so you never hold an API key. Catalog calls on olywork's key are metered "
        ...
        "went out. olywork returns the stored answer, does not call the provider again, and charges "
        ...
        "run) the CLI: `olywork call <endpoint> --upload name=@/path/to/file`.\n\n"
...
async def _call_impl(endpoint_id: str, params: dict | list | None = None,
    ...
    # BOTH halves answer here, exactly as `olywork call` does: `/call/{rest}` resolves a team's own tool
    ...
    # login-customer-id is the canonical need) — with olywork's own auth/routing headers filtered so
    ...
    # `x-olywork-meta` joins the filtered set: caller tags decide who gets billed and budgeted, so they
    ...
    extra_headers = {k: str(v) for k, v in (headers or {}).items()
                     if k.lower() not in ("x-olywork-token", "x-olywork-token",
                                          "x-olywork-org", "x-olywork-org", "authorization",
                                          "idempotency-key", "x-olywork-meta", "x-olywork-meta",
                                          "x-olywork-authorization-method",
                                          "x-olywork-authorization-method")}
    if authorization_method:
        extra_headers["X-Olywork-Authorization-Method"] = authorization_method
        extra_headers["X-Olywork-Authorization-Method"] = authorization_method
    ...
    async with api_context as client:
        ...
        # "choose an org (send X-Olywork-Org)" 400 — a header hint an MCP caller cannot act on.
        ...
        inbound_meta = (
            (ctx.headers or {}).get("x-olywork-meta")
            or (ctx.headers or {}).get("X-Olywork-Meta")
            or (ctx.headers or {}).get("x-olywork-meta")
            or (ctx.headers or {}).get("X-Olywork-Meta")
        )
        if inbound_meta:
...
async def _balance_impl(ctx: Context, *, surface: _SurfacePolicy) -> BalanceOut:
    ...
    async with api_context as client:
        ...
        if problem:
            return problem
        r = await client.get(f"/orgs/{org_id}/balance", headers={"X-Olywork-Org": slug or "", "X-Olywork-Org": slug or ""})
...
@mcp.tool(
    description=(
        ...
        "olywork's catalog key, and those calls are never metered."
    ),
    annotations=_READS,
    structured_output=True
)
async def my_tools(ctx: Context) -> MyToolsOut:
    ...
    async with _api(token) as client:
        ...
        if problem:
            return problem
        r = await client.get("/tools", headers={"X-Olywork-Org": slug or "", "X-Olywork-Org": slug or ""})
        whose = await _whose_grant(client, slug, oauth=_oauth_claims(token) is not None)
    body = _body(r)
    if r.status_code != 200:
...
directory_mcp = MCPServer(
    ...
    title="Olywork",
    description=(
        "Search and call Olywork's curated catalog of external data APIs, with price and reliability "
...
@directory_mcp.tool(
    ...
    title="Search Olywork Catalog",
    description=(
        "Searches Olywork's catalog by capability or task words and returns matching endpoint ids, "
...
@directory_mcp.tool(
    ...
    description=(
        ...
        "successful call may deduct the displayed price from the team's Olywork balance."
    ),
...
@directory_mcp.tool(
    name="balance",
    title="Check Olywork Balance",
    description="Returns the connected team's Olywork balance, in-flight holds, team and identity.",
...
@directory_mcp.tool(
    ...
    title="Request a Catalog Capability",
    description=(
        "Records a request for a provider or capability that is missing from Olywork's catalog. "
        "This creates a request in Olywork and does not call an external provider or spend balance."
    ),
...
def _allowed_hosts() -> list[str]:
    ...
    actually uses, and the in-process host the API calls itself on. `OLYWORK_MCP_ALLOWED_HOSTS` adds
    ...
    # what makes an env-var rollback lossless (a olywork.com config must survive a revert too).
    ...
    hosts += ["localhost", "127.0.0.1", "olywork.internal", "olywork.internal"]
    allowed_env = os.environ.get("OLYWORK_MCP_ALLOWED_HOSTS") or os.environ.get("OLYWORK_MCP_ALLOWED_HOSTS") or ""
    hosts += [h.strip() for h in allowed_env.split(",") if h.strip()]
...
def _allowed_origins(resource_version: str = "v1") -> list[str]:
    ...
    uses, extended by `OLYWORK_MCP_ALLOWED_ORIGINS`. A browser-based MCP client — including a web page
    ...
    origins += [o.strip() for o in os.environ.get("OLYWORK_MCP_ALLOWED_ORIGINS", "").split(",") if o.strip()]
...
class RequireAuthForProtectedTools:
    ...
    **Every request, not just tool calls.** Every olywork tool needs auth, so there is nothing to browse
    ...
    def _auth_verdict(self, scope, body: bytes) -> str | None:
        ...
        **Eager, not lazy.** Every olywork tool needs auth, so there is nothing to browse anonymously —
    ...
    async def _challenge(self, send, *, invalid: bool = False) -> None:
        ...
        if invalid:
            ...
            payload = {"error": "unauthorized",
                       "error_description": ("this MCP server needs a olywork grant — authorize at "
