...
_CONTROL_ROUTE_KEYS: frozenset[RouteKey] = frozenset({
    ...
    ('/7c2e4a91b5d3f8e6olywork2026.txt', ('GET',), 'indexnow_key'),
    ...
    ('/.well-known/skills/olywork/SKILL.md', ('GET',), 'well_known_skill_md'),
...
ROLE_STARTUP_CHECKS: dict[AppRole, tuple[str, ...]] = {
    "all": (
        "olywork.infra.db.verify_db",
        "app.state.http",
        "olywork.mcp.mcp_lifespan",
    ),
    "dataplane": (
        "olywork.infra.db.verify_db",
        "app.state.http",
...
async def pool_gauge(*, sample_s: float = _POOL_GAUGE_SAMPLE_S,
    ...
    that minute, next to its capacity. The reading behind `OLYWORK_DB_POOL_OVERRIDES`: a pool whose
    peak sits at capacity is one whose waiters are timing out (api: `503 olywork_saturated`;
    ...
    while True:
        try:
            ...
            logging.getLogger("olywork").exception("db pool gauge pass failed")
...
def create_app(role: AppRole = "all") -> FastAPI:
    ...
    app.add_exception_handler(StarletteHTTPException, bootstrap_handlers._mark_olywork_own_errors)
    ...
    if _mcp is None and "olywork.mcp.mcp_lifespan" in startup_checks:
        startup_checks.remove("olywork.mcp.mcp_lifespan")
    app.state.role = role
