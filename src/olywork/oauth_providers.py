"""Curated OAuth provider registry — the providers olywork itself holds an approved app for.

The generic connect flow (`POST /oauth/start`) takes a caller-supplied client_id/secret/URIs —
BYO mode, for any OAuth2 provider. This registry is the OTHER half: providers where **olywork** owns
...
class OAuthProvider:
    """One provider olywork holds an app for.

    `client_id_setting` / `client_secret_setting` name attributes on Settings rather than raw env
    vars, so the credentials load from `.env` the same way every other olywork setting does.
    """
    ...
    # "oauth"  — olywork holds an approved app; the user consents and supplies nothing.
    ...
    #            their behalf, and a shared app would put olywork between them and their own data.
    ...
    # the user has not seen on olywork: the Meta app is registered as "Crewlet", a sibling product, so
    ...
    # TIER 4 ONLY: settings attribute holding olywork's own second credential for platform-served
    # calls, when the extra credential is PER-USER (extra_credential_setting stays empty so a
    # user's connect never rides olywork's half of the pair — Tomba rejects a mismatched key/secret).
    ...
    # a registry connect rides olywork's app and every call spends olywork's credits, so the proxy meters
    # it against the org's balance (api.py, same reserve→settle path as tier 4) when the deployment
    # allow-lists the provider (`OLYWORK_OAUTH_BILLED_PROVIDERS` — see config.oauth_billed_set).
    ...
    def platform_extra_credential(self) -> str:
        """olywork's own value for the second credential, if this deployment has one."""
    ...
    # admin host 404s in olywork ("no registered tool"). Observed live: 13 calls / 7 orgs stuck at
...
GOOGLE_ANALYTICS = OAuthProvider(
    ...
    # no tool registered (olywork 404). Both doors shut; this opens the correct one.
    extra_tools=(
        {"suffix": "admin",
...
SLACK = OAuthProvider(
    service="slack",
    display_name="Slack",
    # Bring-your-own-bot, not a olywork-owned OAuth app. A Slack bot is workspace-scoped and belongs
    # to the workspace it's installed in — a shared olywork app would sit between a team and their own
...
X = OAuthProvider(
    ...
    # X bills olywork's app per use (prepaid credits, no plans — docs.x.com/x-api/getting-started/
...
# business. Someone who came here for olywork would be asked to authorize a product they have never
# heard of. Say so before they click Connect, not after.
_META_CONSENT_NOTICE = (
    "You'll see Crewlet on Facebook: our Meta app by Olywork Dev Inc, shared with olywork."
)
...
META_ADS = OAuthProvider(
    ...
    # Returns act_<id> together with the account's name, so the picker shows "Olywork Pty Ltd"
...
# The user pastes an API key instead of consenting through an OAuth app olywork owns. Same connect
...
# key" copy. A key provider needs nothing from olywork, so it is always offerable (is_configured=True).
...
MOZ = OAuthProvider(
    ...
    # message enumerates the valid actions and /quota is absent — see src/olywork/catalog/moz.yaml.)
    probe_method="POST",
...
TOMBA = OAuthProvider(
    ...
    platform_extra_setting="platform_key_tomba_secret",  # tier 4 injects olywork's OWN pair
    extra_credential_note=(
...
INFLUENCERSCLUB = OAuthProvider(
    ...
    # the dashboard; the API's own OAuth credentials ride the same header, but olywork only takes the key.
    token_header="Authorization",
    token_format="Bearer {secret}",
...
# These list as `configured: false` until olywork holds each platform's client id/secret. Microsoft +
...
def credentials(provider: OAuthProvider) -> tuple[str, str]:
    """olywork's own client id/secret for this provider. Raises if the deployment hasn't set them —
    ...
    if not (client_id and client_secret):
        raise ValueError(
            ...
            f"(set OLYWORK_{provider.client_id_setting.upper()} and "
            f"OLYWORK_{provider.client_secret_setting.upper()})"
        )
    ...
    return [
        {
            ...
            # bills olywork's app per use), with the default rates — shown BEFORE consent, so nobody
...
def platform_bindings(provider) -> list[dict]:
    ...
    # A per-user credential PAIR (Tomba's key+secret headers) needs olywork's own second half on
