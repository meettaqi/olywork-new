...
(a shell where those CLIs run with the team credential injected automatically — no `olywork run`, no keys on
...
`https://olywork.com`; replace it with your own.
...
- **A "tool" can be an HTTP API or a command-line program.** `olywork call` proxies an API; `olywork run` runs a
...
- **Auto-import reads your machine.** `olywork scan clis` looks at which catalog CLIs are actually installed
  (and whether you are logged into them); `olywork upload clis` registers each on the right tier — the key you
  already have in your environment goes to the server-injected tier; a CLI you are logged into stays local.
  (`olywork scan` = read-only preview, `olywork upload` = register; `olywork import` is the old name for `upload`.)
- **Shell mode shadows CLIs on `PATH`.** `olywork shell start` puts a private folder first on your `PATH` with
  one small wrapper (a "shim") named after each **registered** CLI. When you type `gh`, the shell finds our
  wrapper first and routes it through olywork; when you type `ls` or `git`, there is no wrapper, so it runs
...
- **The credential is never in your shell.** In shell mode the secret exists only inside the one subprocess
  olywork spawns for a command, and only for that command. Your shell's environment never holds it.
- **The security sandbox is opt-in and layered.** On Linux and macOS, `sudo olywork setup-local-run` makes a
...
them into registered olywork tools in one pass, choosing the safe run tier for each — no hand-writing a
`olywork.json` per tool.

## Step 1 — Preview what would be registered (nothing is written)

`olywork scan clis` scans your machine: for each CLI in olywork's catalog it checks whether the
...
olywork can hold it and inject it; these can run on the server, where the key never touches a member's
machine. **Local tier** (`gh`, `gcloud`, …) — you are logged into the CLI itself (its own config on disk),
so olywork registers the tool without any secret and just runs the CLI you already authenticated. `az` uses a
...
**Notice:** the `▸ gh · audit #58` line (on standard error) shows olywork wrapped the run and recorded it; the
rest is `gh`'s own output. No key was needed — olywork just ran the `gh` you already logged into.
...
`olywork run stripe -- …` is explicit and safe, but typing `olywork run` before every command is friction. Shell
...
```
```text
▚ olywork shell — you're now in a shell where your team's CLIs just work.
...
**Notice:** you are now in a subshell. The prompt shows a `(olywork)` marker. The banner lists the registered
...
## Step 6 — Use a team CLI with no `olywork run`

At the `(olywork)` prompt, run a registered CLI by its normal name.
...
**Notice:** you typed `gh`, not `olywork run gh`. The shell found olywork's `gh` shim first (it is first on
`PATH`), which routed the command through olywork — hence the `▸ gh · audit #59` audit line — and then ran the
...
```bash
olywork shell start --server-for stripe --ttl 60
```
```text
▚ olywork shell — you're now in a shell where your team's CLIs just work.
  …
...
if the member cannot capture the key. `sudo olywork setup-local-run` (Linux and macOS) builds a sandbox with
three layers. Set it up once, as an administrator on the machine.

```bash
sudo olywork setup-local-run
```
```text
created hidden system user 'olywork-run' (uid 380)
...
**Notice:** it creates a dedicated, no-login user `olywork-run`, a narrow rule that lets you run **only** the
olywork runner as that user, and a network allow-list (below). From now on a local run executes as `olywork-run`,
not as you.

## Step 10 — Layer 1: isolation (you cannot read the key)

The CLI runs as `olywork-run`, a different user id. A different user's process environment and memory are
...
**Notice:** the command ran as **`uid=380(olywork-run)`**, not as you. The team credential only ever exists
inside that user's process, which your account cannot read (`ps` shows nothing, there is no `/proc` entry
you can open). One requirement: olywork itself must be installed at a system path (for example
`/usr/local/bin`), because `olywork-run`, by design, cannot read into your private home directory to run it.
...
restricts `olywork-run`'s outbound network to only the registry plus the tool API hosts in the catalog.

```bash
# run AS olywork-run for the demo:
...
**Notice:** `olywork-run` reached `api.github.com` (200) but was **blocked** from `example.com` (000, the
...
periodically with `sudo olywork setup-local-run --refresh-egress`.
...
writes to a private scratch folder that only `olywork-run` can read, removed after the run.

```bash
olywork run --fs-jail writetool -- /tmp/leak     # writetool's program is `touch`
ls /tmp/leak
```
```text
...
```bash
olywork run writetool -- /tmp/leak2
ls -l /tmp/leak2
```
```text
-rw-r--r--  1 olywork-run  wheel  0  /tmp/leak2
```
...
- **Output redaction.** When you run a tool whose key you do **not** own (a shared key), olywork scrubs the
...
Each tool's owner opts local runs in first — `olywork tool update <name> --local-run on` (off by default,
...
- **Auto-import**: `olywork scan clis` and `--replace` registered the real set (openai/stripe on
  the server tier; gh/gcloud/etc. local), and `olywork run gh -- --version` printed gh's version with no
  credential — confirming the local-tier fix.
- **Shell mode**: inside `olywork shell start`, `gh --version` produced an audit line (intercepted) while
  `git --version` produced none (untouched); `exit` restored `which gh` to the real binary; a
  `--server-for stripe` run appeared in `olywork runs` as `"where": "server"`; tab-completion created no audit
  rows.
- **Isolation**: `olywork run idtool` printed `uid=380(olywork-run)`, and the member could not read that
  process's environment.
- **Egress**: `olywork-run` reached `api.github.com` (200) and was blocked from `example.com` (000), with the
  member's own network unaffected.
- **Filesystem jail**: `olywork run --fs-jail … -- /tmp/leak` returned "Operation not permitted" and created
