...
We use the registry at `https://olywork.com`.
...
- **Two independent dials per member.**
  ...
  - **Local execution** (`local_run_enabled`): may this member run a CLI **on their own machine**
    (`olywork run`, the default local tier)? On by default. Turn it **off** and they can still run on the
    server (`olywork run --server`), but never locally.
- **Access gates every way of using a tool.** A withheld tool is unreachable through *all* doors: the HTTP
  proxy `olywork call`, the server run `olywork run --server`, and the local run `olywork run`. There is no side path.
...
| Situation | `olywork call <tool>` | `olywork run <tool>` (local) | `olywork run --server <tool>` |
|---|:--:|:--:|:--:|
...
**Notice:** prefix any command with `HOME=~/.olywork-personas/<name>` to act as that person. **Tom** is the team
...
```bash
HOME=~/.olywork-personas/tom olywork org invite sam@olywork.com --tools gh --local-run off
```
```json
{
...
## Step 2 — (Alternative) let olywork ask

If Tom runs the invite **without** the access flags, olywork asks him interactively — "give access to all
tools?" — and, if he says no, shows a checklist of every tool (all pre-ticked) to uncheck the ones to
withhold.

```bash
HOME=~/.olywork-personas/tom olywork org invite sam@olywork.com
```
```
...
```bash
HOME=~/.olywork-personas/sam olywork login --email sam@olywork.com
...
```bash
HOME=~/.olywork-personas/sam olywork call gh zen
```
```
...
```bash
HOME=~/.olywork-personas/sam olywork call stripe v1/balance
```
```json
{
...
**Notice:** the message tells Sam exactly how to get access. Because access gates *all* doors, `olywork run
stripe` and `olywork run --server stripe` are refused the same way.

## Step 6 — Local execution is off

`gh` is allowed, but Sam's `local_run_enabled` is **off**, so he cannot run it on his own machine. olywork
...
```
```
olywork: local execution is disabled for you — run on the server instead (`olywork run --server`), or ask an admin to enable local runs for your account
```

**Notice:** this is the *local* wall, separate from tool access. `gh` is allowed — Sam just can't run it
**locally**. `olywork run --server gh …` runs on the registry (where the key stays) and is allowed.
...
```bash
HOME=~/.olywork-personas/sam olywork org members
```
```json
{
...
```bash
HOME=~/.olywork-personas/tom olywork org access 63 --all-tools --local-run on
```
```json
{
  "user_id": 63,
...
use `--tools a,b`. A flag you *don't* pass keeps its current value — so `olywork org access 63 --local-run off`
...
```bash
HOME=~/.olywork-personas/tom olywork org access 63 --tools gh,gcloud
```
...
Everything above is also point-and-click at `https://olywork.com/` → **Team**.
...
ran `olywork org access … --all-tools --local-run on` — everything opened up. On top of that walk-through, the
