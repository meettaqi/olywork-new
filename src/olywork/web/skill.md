...
- **Endpoint:** `{BASE}`  ·  **CLI:** `olywork` (or `olywork`)  ·  the CLI is a thin client over the API.
...
If you reached olywork through `{BASE}/mcp/` — ChatGPT, Claude Code, Cursor — the CLI steps above do not
...
Over HTTP that is `GET {BASE}/call/<tool-name>/<path>` with `X-Olywork-Token: <your token>`. olywork looks
