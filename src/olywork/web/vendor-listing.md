# List your API in the olywork catalog — instructions for a coding agent

You are helping an API vendor get listed in olywork's tool catalog ({BASE}). The deliverable is a
**pull request to https://github.com/meettaqi/olywork-new** containing the listing, plus a way for
...
1. **Registry entry** — an `OAuthProvider(auth_kind="key", …)` in `src/olywork/oauth_providers.py`,
...
2. **Logo** — `src/olywork/web/logos/<service>.svg`, a **neutral lettermark** (do not reproduce the
   real brand mark).
...
4. **Core catalog file** — `src/olywork/catalog/<service>.yaml` with the 8–15 endpoints an agent
   would actually reach for, each with: a `capability` from `src/olywork/catalog/capabilities.yaml`
...
- Your pricing page URL and billing model; a machine-readable rate-card endpoint if you have one
  (that is the fastest path to olywork serving your endpoints on its own key, metered).
