...
//
//   1. A first-party `olywork_utm` cookie: first touch wins (never overwritten), 90 days, carrying
...
(function () {
  try {
    ...
    // and must not record "olywork.com" as where someone came from.
    if (referrer === window.location.hostname) referrer = '';
    var anyUtm = has('utm_source') || has('utm_medium') || has('utm_campaign');
    var already = /(^|;\s*)olywork_utm=/.test(document.cookie);
    if ((anyUtm || referrer) && !already) {
      ...
      document.cookie = 'olywork_utm=' + v + ';path=/;max-age=7776000;samesite=lax' +
