"""Filesystem jail for isolated local runs (`olywork run --fs-jail`) — the last channel of the sandbox.

Even with uid isolation (the member can't read olywork-run's memory) and egress control (olywork-run can't send
...
olywork-run, so the member can't traverse into it); everything else on disk is read-only to the process.
