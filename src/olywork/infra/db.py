...
def _apply_overrides(specs: dict[str, dict[str, int]], raw: str) -> dict[str, dict[str, int]]:
    ...
    for item in (part.strip() for part in raw.split(",") if part.strip()):
        ...
        if not sep or pool not in specs or floor is None:
            log.warning("ignoring OLYWORK_DB_POOL_OVERRIDES entry %r: unknown pool or field", item)
            continue
        try:
            parsed = int(value)
        except ValueError:
            log.warning("ignoring OLYWORK_DB_POOL_OVERRIDES entry %r: not an integer", item)
            continue
        if parsed < floor:
            log.warning("ignoring OLYWORK_DB_POOL_OVERRIDES entry %r: %s must be >= %d (SQLAlchemy "
...
def _new_engine(name: str):
    ...
    TimeoutError to a `503 olywork_saturated` with Retry-After) rather than wait 30 s for an anonymous
...
def _schema_info() -> tuple[str, frozenset[str]]:
    ...
    alembic_config = import_module("olywork.maintenance")._alembic_config
...
async def verify_db() -> None:
    ...
    # With no OLYWORK_SECRET_KEY, crypto uses a per-process ephemeral Fernet key, so every stored
    ...
    if not settings.secret_key and "sqlite" not in settings.database_url:
        raise RuntimeError(
            "OLYWORK_SECRET_KEY is not set on a non-SQLite database - stored secrets would be lost on "
            "the next restart. Set a key (olywork keygen) before starting."
        )
    if not settings.secret_key:
        logging.getLogger("olywork").warning(
            "OLYWORK_SECRET_KEY unset - using an EPHEMERAL key; stored secrets will not survive a restart."
        )
    ...
    if revision is None:
        raise RuntimeError(
            "Database schema is not initialized or was not adopted - run `python -m olywork upgrade`. "
