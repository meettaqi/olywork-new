...
(method, path, ALL query params incl. duplicates, caller headers, caller cookies, body bytes streamed):
  ...
  2. olywork's own control/infra headers + the edge's forwarding headers (`x-olywork-*`,
     `ngrok-skip-browser-warning`, `x-forwarded-*`, `x-real-ip`, `forwarded`, `via`) — stripped so
     they never leak upstream. olywork's own cookies (`olywork_session`, `olywork_oauth_state`) are scrubbed
...
# The edge's forwarding headers + third-party infra — never leak to the upstream. olywork's OWN headers
# are handled by the `x-olywork-` PREFIX rule below, not by name: this set used to enumerate them and the
# enumeration silently failed. `x-olywork-client` was never listed, so every provider we relay to has been
# receiving the caller's runtime name; `x-olywork-meta` would have leaked a builder's customer ids the same
...
_DROP_PREFIXES = ("x-olywork-", "x-olywork-")
...
def _is_dropped_request_header(name: str, extra: frozenset[str]) -> bool:
    ...
    return name in _DROP_REQUEST or name in extra or name.startswith(_DROP_PREFIXES)
_DROP_RESPONSE = _HOP_BY_HOP
_OLYWORK_COOKIES = frozenset({"olywork_session", "olywork_oauth_state", "olywork_session", "olywork_oauth_state"})  # our cookies, scrubbed from Cookie


def _scrub_olywork_cookies(headers: httpx.Headers) -> None:
    """Drop olywork's own cookies from the forwarded Cookie header so a dashboard `credentials:'include'`
    ...
    kept = [
        ...
        if c.strip() and c.split("=", 1)[0].strip().lower() not in _OLYWORK_COOKIES
    ]
    if kept:
...
async def relay(
    ...
    _scrub_olywork_cookies(headers)  # keep caller cookies, drop olywork's own session cookie
    ...
    for binding in tool.bindings:
        # A PLATFORM binding injects one of olywork's own credentials (Google Ads' developer token),
        # read from settings rather than an org secret. Keeping it out of the org's secret store
        # matters: it's olywork's credential, not the tenant's, so it must not be readable by them or
    ...
    # Set-Cookie for one of olywork's own cookies — a registered upstream must not be able to
    # overwrite the operator's olywork_session / olywork_oauth_state under olywork's origin (fixation).
    raw_headers = [
        (k.encode("latin-1"), v.encode("latin-1"))
        for k, v in upstream_resp.headers.multi_items()
        if k.lower() not in drop_resp and not _is_olywork_setcookie(k, v)
    ]
    # The proxy is for API calls, but a browser could navigate to /call/… (authorized by the session
    # cookie) and render arbitrary upstream text/html AS AN ACTIVE DOCUMENT under olywork's own origin —
...
def _is_olywork_setcookie(name: str, value: str) -> bool:
    return name.lower() == "set-cookie" and value.split("=", 1)[0].strip().lower() in _OLYWORK_COOKIES
