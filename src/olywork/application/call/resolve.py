...
AUTHORIZATION_METHOD_HEADER = "X-Olywork-Authorization-Method"
...
async def _resolve_call(rest: str, caller: Caller, db: AsyncSession) -> ResolvedTarget:
    ...
    - URL-passthrough (agent-facing): rest is the real upstream URL. Resolve the tool by host
      (indexed) + longest base_url prefix — the caller types no olywork vocabulary, just the API.
            ...
            if any(_prefix_match(t.base_url) for t in on_host):
                raise ResolutionFailed(
                    "tool_access_denied", status_code=403, detail=(
                        ...
                        "admin can grant it (dashboard → Team, or `olywork org access <you> …`)"))
            raise ResolutionFailed(
    ...
    if tool is None:
        ...
        # A bare provider name (`olywork call tikhub /path`) stays a miss, but points at the
        ...
        if oauth_providers.get(name) is not None or name in cat.provider_meta:
            detail += (f" — but {name!r} is a marketplace provider; call its endpoints directly: "
                       f"olywork catalog search <what you need> → olywork call <endpoint-id>")
...
# ---- direct marketplace calls: `olywork call <catalog-endpoint-id>`, no tool registration ----------
...
# persisted (no registry pollution), then (4) OLYWORK'S OWN key for the provider, metered against the
...
async def _marketplace_secret(
    ...
    NAMED exactly for it (`olywork secret add tikhub …`). Newest wins — a reconnect supersedes."""
    if methods:
...
class MarketplaceCall:
    ...
    olywork's own key is paying — what the ledger is holding for it. `call_tool` carries this from
    ...
    def metered(self) -> bool:
        """True when OUR money is at stake: olywork's platform key (tier 4), or an org credential that
        rides olywork's pay-per-use OAuth app (`billed_oauth`). Tiers 1/2 on a provider that bills the
...
# ---- oauth-billed metering: providers whose upstream bill lands on olywork's app -------------------
# X moved to pay-per-use (Feb 2026): the APP OWNER is billed per resource read / per post written,
# whoever's user token made the call. A registry connect rides olywork's app, so those calls spend
# olywork's prepaid credits and must be metered against the org's balance — the same reserve→settle
...
def _oauth_billed_provider(secrets: dict[int, Secret]):
    ...
    upstream bills olywork's app (`platform_billed`), and this deployment opted into charging
    (`OLYWORK_OAUTH_BILLED_PROVIDERS`, the kill switch — empty keeps today's free behavior)."""
...
def _capability_alternatives(ep: dict, *, limit: int = 3) -> list[str]:
    ...
    same `capability` string, on a key olywork already holds — sat one row away answering 192 of 208
    ...
    COMPARES, it does not route: olywork never fails over on the caller's behalf (see the charter),
    ...
    for alt in cat.for_capability(capability):
        ...
        # "Servable" is the caller's real question: not "does another row exist" but "can olywork
    ...
    for _, _, alt_id, usd, servable in ranked[:limit]:
        ...
        how = "callable now on olywork's key" if servable else f"needs your own {alt_id.split('.')[0]} credential"
...
def _marketplace_no_credential(
    ...
    lines.append(f"  connect one:  olywork connections connect --provider {service}")
    if provider.uses_pasted_secret:
        lines.append(f"  or add a key: olywork secret add {service} --env-var {service.upper().replace('-', '_')}_API_KEY")
    lines.append(f"  or register the tool yourself: olywork tool add {service} --base-url {provider.base_url} …")
    if ep is not None:
    ...
    raise ResolutionFailed(
        "capability_pinned", status_code=403, detail={
            ...
            "message": (f"this team uses {pin.provider!r} for {cap!r}"
                        ...
                        + f". An admin can change it: olywork org unpin {cap}"),
...
def _authorization_error(
    ...
    command = f"olywork connections connect --provider {ep['provider']}"
    if capability:
...
async def _resolve_marketplace_call(
    ...
    if cost is not None and capacity_view.is_exhausted(service, ep["id"]):
        # olywork's own account for this call is known to be out (the call-path lock, or the sweep).
