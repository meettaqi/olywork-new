...
When olywork's OWN account for a provider fails a tier-4 call (a balance/quota signature, a burst-429
that smoothing could not absorb, a connect error), serve the SAME vendor endpoint through a
olywork-owned aggregator account: reserve a child hold (own id `{call_ref}:overflow`) → one aggregator
...
log = logging.getLogger("olywork.overflow")
...
def _trigger(mk: MarketplaceCall, status: int, headers, body: bytes) -> str | None:
    """Why this call may overflow, or None. Only olywork-side failures qualify (plan §4.1)."""
    if status == 401:
        return None
...
def _capacity_503(mk: MarketplaceCall, aggregator: str, why: str) -> CallFailure:
    ...
    return CallFailure("provider_capacity", status_code=503, detail={
        ...
        "message": (f"olywork's own {mk.provider} account is out and the overflow relay ({aggregator}) "
                    f"could not serve {mk.endpoint_id} ({why}); nothing was charged.\n"
                    f"  use your own key: olywork secret add {mk.provider} --env-var "
...
def _audit_child(mk: MarketplaceCall, child: MarketplaceCall, call_ref: str, aggregator: str,
    ...
    audit.record_call(
        ...
        telemetry={"call_ref": call_ref, "endpoint_id": mk.endpoint_id, "provider": mk.provider,
                   ...
                   **({"error_response": f"olywork overflow: {note}"} if note else {})})
