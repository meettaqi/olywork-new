...
async def catalog_endpoint_access(
    *, endpoint_id: str, authorization_method: str, caller: Caller, db: AsyncSession,
    ...
    if cost is not None:
        ...
        return {
            "tier": "platform",
            "detail": (f"no key needed — uses olywork's {service} key, {price} "
                       f"from your team balance (olywork balance)"),
...
async def _routed_access(endpoint: dict, caller: Caller, catalog) -> dict:
    ...
    how = (
        ...
        f"olywork's {first.endpoint['provider']} key, ~${(first.price_micro or 0) / 1e6:g}"
    )
    dropped_note = ""
    if plan.dropped:
        dropped_note = (
...
def _billed_note(endpoint: dict, provider, service: str, catalog) -> str:
    ...
    if estimate:
        return (
            ...
            f"{service} bills olywork's app per use)"
        )
    return f" — metered from the team balance ({service} bills olywork's app per use)"
...
def _missing_access(
    ...
    *, existing: bool = False,
    ...
    connect = f"olywork connections connect --provider {service}"
    if capability:
        connect += f" --capability {capability}"
    hint = (
        f"connect with: {connect}" if not provider.uses_pasted_secret else
        f"connect with: {connect}, or olywork secret add {service} …"
    )
    detail = (
