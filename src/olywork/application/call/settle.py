...
# a team as real spend, and for a builder reselling olywork it would land on their end customers' bills.
...
def _observed_cost_micro(mk: MarketplaceCall, body: bytes, headers=None) -> int | None:
    ...
    if mk.cost_type == "per_success" and isinstance(doc, dict):
        ...
        # FAILED"}` — free on the vendor's own terms ("only a code-0 response is billed") — and olywork
...
async def _platform_settle(
    ...
    if billable and status_code >= 400 and mk.tier == "platform":
        ...
        # charging: billing it would take the caller's money for olywork's empty account, and once
    ...
    # delta vs olywork's direct price): folded into the SAME settle transaction, the one allowlisted
    ...
    try:
        ...
        logging.getLogger("olywork.ledger").error(
    ...
    async def _cleanup() -> None:
        ...
        if response is not None:
            try:
                ...
                logging.getLogger("olywork.proxy").error(
        ...
        try:
            ...
            logging.getLogger("olywork.idempotency").error(
...
async def _note_capacity_signal(mk: MarketplaceCall, status_code: int, headers, body: bytes) -> str | None:
    ...
    try:
        ...
        logging.getLogger("olywork.capacity").warning("capacity signal handling failed", exc_info=True)
...
async def _note_capacity_recovery(mk: MarketplaceCall) -> None:
    ...
    try:
        ...
        logging.getLogger("olywork.capacity").warning("capacity recovery handling failed", exc_info=True)
...
async def _record_first_call(org_id: int) -> None:
    ...
    try:
        ...
        logging.getLogger("olywork.adsconv").error(
