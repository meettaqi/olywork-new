...
def _outcome(status_code: int, refused_by: str | None, *, answered: bool) -> str:
    """Who produced the status: olywork before any upstream trip, olywork with no upstream answer, or the vendor."""
    if refused_by is not None:
        return "olywork_refused"
    if not answered:
...
"""The longest `retry-after` olywork will honour with one re-send (plan §4.4, decided 2026-08-28)."""
...
async def _execute_call(request: _ApplicationRequest, upstream_client: httpx.AsyncClient) -> UpstreamResponse:
    ...
    if request.context.input.catalog_only:
        ...
        try:
            ...
            # Not a tool → maybe a marketplace endpoint id (`olywork call tikhub.tiktok.video.comments`).
    ...
    if ep is not None and ep.get("kind") == "routed":
        # A first-party routed endpoint (olywork.<capability>): the router picks children and runs
        # each through THIS use case again (child contexts, own hold ids), then assembles one
        # answer. The parent owns the idempotency label and the X-Olywork-* stamping below.
        try:
    ...
    def _audit(status_code: int, *, observed_micro: int | None = None, charged_micro: int | None = None,
        ...
        request.state.call_audited = True  # the refusal fallback in _mark_olywork_own_errors stands down
        telemetry: dict = {"call_ref": call_ref}
        if meta.tags:
            # Own-tool calls carry tags too: a builder's usage report has to account for every call
            # their user made, not only the ones that spent olywork's money.
        ...
        if mk is not None:
            telemetry |= {
                ...
                **({"cached": True} if served_hit else {}),
                # An org credential riding olywork's pay-per-use OAuth app: tier stays tool/credential
    ...
    # connect rides olywork's pay-per-use app, so the org's "own" oauth secret is exactly what makes
    ...
    if billed_provider is not None:
        ...
        # org must never be able to spend olywork's upstream credits — refuse rather than relay free.
        if caller.org.public_demo:
            _audit(403, refused_by="policy")
            raise AuthorizationFailed("policy_denied", status_code=403, detail=(
                f"{billed_provider.display_name} calls are pay-per-use on olywork's app and the "
    ...
    if mk is not None and mk.skip_direct:
        ...
        pending = _audit(503, charged_micro=0, refused_by="capacity",
                         error_response="olywork: own account exhausted — served via overflow",
        ...
        _set_response_header(response, "X-Olywork-Cost-Micro", str(charged))
        _set_response_header(response, "X-Olywork-Call-Id", call_ref)
        _set_response_header(response, "X-Olywork-Served-Via", f"overflow:{outcome.aggregator}")
        return response

    # Metered — olywork's own money is about to be spent (tier 4's platform key, or a registry OAuth
        ...
        try:
            ...
            _audit(exc.status_code, charged_micro=0,
                   ...
                   error_response=(
                       ...
                       _redact_snippet(f"olywork: {exc.detail}", refusal_secrets,
                                       _ERROR_RESPONSE_MAX)))
            raise
    body = b""
    response: UpstreamResponse | None = None
    started = _now_ms()
    try:
        # olywork keeps oauth tokens fresh: refresh in place if stale, before injecting. Inside the
        ...
        try:
            ...
            raise
        except httpx.RequestError as exc:  # upstream down/timeout is a gateway fault, not olywork's 500
            raise _gateway_request_failure(exc) from exc
    except asyncio.CancelledError:
        await _finish_cancelled_call(request, mk, call_ref, response)
        raise
        ...
        if metered or free_poll:
            ...
            request.state.call_cost_micro = 0
        # No provider body exists on this branch. olywork's own detail is the explanation instead, and
        ...
        _audit(exc.status_code, charged_micro=0 if metered or free_poll else None,
               ...
               error_response=(
                   _ERROR_MASKING_FAILED if _renderings is None else
                   _redact_snippet(f"olywork: {exc.detail}", _renderings, _ERROR_RESPONSE_MAX)))
        raise
    ...
    if mk is not None and mk.metered:
        ...
        try:
            ...
            if deferred:
                try:
                    ...
                    # olywork could not record the task, so nobody will ever observe its outcome.
                    # The platform absorbs that: release the hold (status None = never a charge)
                    # and alert. Charging the basis here would bill a customer for olywork's own
                    # failure; the doctrine is the same as the 24-hour deadline's.
                    logging.getLogger("olywork.asynctasks").error(
        ...
        _set_response_header(response, "X-Olywork-Call-Id", call_ref)
        if served_via:
            _set_response_header(response, "X-Olywork-Served-Via", served_via)
        if smoothed:
            _set_response_header(response, "X-Olywork-Smoothed", " ".join(smoothed))
        return response
    ...
    _set_response_header(response, "X-Olywork-Call-Id", call_ref)
    if free_poll:
        _set_response_header(response, "X-Olywork-Cost-Micro", "0")
        if smoothed:
            _set_response_header(response, "X-Olywork-Smoothed", " ".join(smoothed))
    return response
