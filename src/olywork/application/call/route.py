"""Routed endpoints — `POST /call/olywork.<capability>` (docs/CAPABILITY-ROUTING-PLAN.md §3–§5).
...
core `output` (via the child's adapter), the child's `raw` body, and `_olywork: {served_by, tried}`.
...
status as its "no result" answer (`miss: {status: 404}`), which is a MISS. Child-local olywork authorization failures and
platform vendor 401/403 responses are errors because another child may work. A MISS (2xx,
`adapter.miss`) stops unless the
caller turned the waterfall off (`X-Olywork-Route-Waterfall: 0`). The waterfall is ON by default —
the endpoint's job is to find the thing — bounded by `X-Olywork-Route-Max-Cost` (default $1.00).
"""
...
WATERFALL_HEADER = "x-olywork-route-waterfall"
MAX_COST_HEADER = "x-olywork-route-max-cost"
PREFER_HEADER = "x-olywork-route-prefer"
EXCLUDE_HEADER = "x-olywork-route-exclude"
...
async def build_plan(ep: dict, identity_given: dict, caller, options: RouteOptions) -> Plan:
    ...
    for e, ad, v in raw:
        ...
        if tier == "platform" and not get_settings().platform_key_for(e["provider"]):
            ...
            continue
        if c.exhausted:
            dropped.append({"endpoint_id": e["id"], "why": "olywork's account for this provider is exhausted right now"})
        cands.append(c)
        ...
        for c in cands:
            if c.ignored:
                ...
                dropped.append({"endpoint_id": c.endpoint["id"], "strict": True,
                                "why": f"cannot express {', '.join(c.ignored)} (X-Olywork-Route-Strict-Filters); it takes {wants}"})
            else:
                kept.append(c)
        cands = kept
    ...
    if not plan.candidates:
        ...
        raise ResolutionFailed("route_no_candidate", status_code=503 if capacity_drop else 422, detail={
            ...
            "message": (f"no provider can serve {ep['id']} for this identity right now" if not strict_drop or capacity_drop else
                        ...
                        f"(X-Olywork-Route-Strict-Filters): " + "; ".join(f"{d['endpoint_id']} {d['why']}" for d in strict_drop)
