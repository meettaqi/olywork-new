...
_BLAME_BY_KIND: dict[str, Blame] = {
    ...
    # olywork's OWN vendor account for the provider is out (balance/quota) — a 503 the caller cannot
    # fix, answered before any hold exists, with the same-capability alternatives named.
    "provider_capacity": "olywork",
    # Routed endpoints (olywork.<capability>): the caller's identity fits no provider, or the
    ...
    "injection_failed": "olywork",
    "ssrf_refused": "olywork",
...
class GatewayFailed(CallFailure):
    """The provider did not produce a complete response or olywork refused the relay."""
