...
def _secret_renderings(tool: Tool, secrets: dict[int, Secret]) -> list[str]:
    ...
    olywork injects the value verbatim, but a PROVIDER may hand it back transformed, and a transform it
    ...
    * **the DECODED halves of a Basic credential.** `config.py` states that dataforseo's platform
      ...
      `{"received_username": …, "received_password": …}` echoes olywork's credential in a form where
...
def _safe_secret_renderings(tool: Tool, secrets: dict[int, Secret]) -> list[str] | None:
    ...
    try:
        ...
        logging.getLogger("olywork").warning("could not render credentials for error masking: %s", exc)
        return None
...
def _caller_request_snippet(
    ...
    """What the CALLER actually sent, redacted — the half of a failure olywork otherwise forgets.
