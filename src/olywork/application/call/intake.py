...
# A builder reselling olywork through one token stamps their OWN ids on each call —
# `X-Olywork-Meta: customer=cust_8123, workspace=ws_9` — so they can attribute, budget and invoice their
...
def _parse_call_meta(raw_header: str | None, caller: Caller | None = None) -> CallMeta:
    """`X-Olywork-Meta: k=v, k=v` → a validated bag. No header means today's behaviour exactly.
    ...
    if len(raw.encode()) > _META_MAX_HEADER:
        raise IntakeFailed(
            ...
            detail=f"X-Olywork-Meta is limited to {_META_MAX_HEADER} bytes")
    tags: dict[str, str] = {}
    for segment in raw.split(","):
        ...
        if not sep or not _META_KEY_RE.match(raw_key.strip().lower()):
            ...
            raise IntakeFailed(
                "metadata_invalid", status_code=422,
                detail=(f"X-Olywork-Meta must be `key=value` pairs; keys are 1-32 chars of [a-z0-9_] "
                        f"(got {segment.strip()!r})"))
        try:
            key, value = budget_policy._validate_tag_pair(
                raw_key, raw_value, where="X-Olywork-Meta")
        ...
        if key in tags:
            raise IntakeFailed(
                "metadata_invalid", status_code=422,
                detail=f"X-Olywork-Meta names {key!r} twice")
        tags[key] = value
    if len(tags) > budget_policy._META_MAX_KEYS:
        raise IntakeFailed(
            "metadata_invalid", status_code=422,
            detail=(f"X-Olywork-Meta is limited to {budget_policy._META_MAX_KEYS} keys "
                    f"(got {len(tags)})"))
