...
async def _free_connection_name(base: str, org_id: int, db: AsyncSession) -> str:
    ...
    The bare name matters: every skill and doc says `olywork call google-search-console`, and a tool
...
def _provider_bindings(provider, secret: Secret) -> list[dict]:
    ...
    that OLYWORK holds (Google Ads' developer token) gets it as a platform binding — read from settings
...
async def _backfill_provider_extra_tools() -> int:
    ...
    async with session_maker() as db:
        ...
        await db.commit()
        if created:
            logging.getLogger("olywork").info("backfilled %d provider companion tool(s)", created)
        return created
...
async def start_oauth_connection(
    *, org_id: int, owner: str, name: str, provider_name: str | None,
    ...
    async with session_maker() as db:
        ...
        olywork_callback = f"{get_settings().public_url.rstrip('/')}/oauth/callback"
        # The code must come back to olywork's OWN callback — a body-supplied redirect_uri pointing elsewhere
        # turns this into a consent-phishing URL builder (a legit provider link that routes the code away).
        if redirect_uri and redirect_uri.rstrip("/") != olywork_callback:
            raise ConnectError("invalid_provider", "redirect_uri must be olywork's own /oauth/callback")
        redirect_uri = redirect_uri or olywork_callback
    ...
    async with session_maker() as db:
        ...
        for tool in tools:
            ...
            if tool.name == provider_service or not bindings:
                await db.delete(tool)  # olywork's own auto-provisioned tool, or nothing left to inject
...
async def list_connection_health(*, org_id: int, visible_ids_for) -> list[dict]:
    async with session_maker() as db:
        ...
        # health.needs_reconnect rides along so a credential olywork cannot renew announces itself BEFORE
