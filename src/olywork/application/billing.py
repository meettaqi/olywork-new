...
log = logging.getLogger("olywork.billing")
...
INTEGRATION_ID = "olyworktopup-qwkmzhrb"


class BillingNotConfigured(Exception):
    """No `OLYWORK_STRIPE_SECRET_KEY`. The endpoints turn this into a 503 — top-ups are simply off on
...
def bonus_for_topup(amount_micro: int) -> tuple[int, int]:
    ...
    truncates the sub-micro remainder, which is at most a millionth of a dollar in olywork's favour.
...
def _require_configured() -> str:
    ...
    if not key:
        raise BillingNotConfigured("OLYWORK_STRIPE_SECRET_KEY is not set — top-ups are disabled")
    return key
...
async def get_or_create_customer(db: AsyncSession, org: Org, *, email: str = "") -> str:
    ...
    customer = await _sdk(
        ...
        **({"email": email} if email else {}),
        ...
        metadata={"olywork_org_id": str(org.id), "olywork_org_slug": org.slug},
    )
...
async def create_topup_checkout(
    ...
    session = await _sdk(
        ...
        payment_intent_data={
            ...
            "metadata": {"olywork_org_id": str(org.id), "olywork_kind": "topup", "olywork_auto": "0"},
        ...
        invoice_creation={
            ...
            "invoice_data": {
                ...
                "metadata": {"olywork_org_id": str(org.id), "olywork_kind": "topup"},
            },
        },
...
async def create_setup_checkout(db: AsyncSession, org: Org, *, return_base: str = "", email: str = "") -> dict:
    ...
    session = await _sdk(
        ...
        setup_intent_data={"metadata": {"olywork_org_id": str(org.id), "olywork_kind": "autotopup_card"},
                           "usage": "off_session"},
        metadata={"olywork_org_id": str(org.id), "olywork_kind": "autotopup_card"},
        integration_identifier=INTEGRATION_ID,
...
def _idempotency_key(org_id: int, amount_micro: int, crossing: int, payment_method: str) -> str:
    ...
    return "olywork_" + hashlib.sha256(raw.encode()).hexdigest()[:40]
    ...
    async with _lock(org_id):
        ...
        try:
            pi = await _sdk(
                ...
                confirm=True,
                metadata={"olywork_org_id": str(org_id), "olywork_kind": "topup", "olywork_auto": "1"},
...
async def _credit(db: AsyncSession, org_id: int, amount_micro: int, pi_id: str, *, auto: bool,
    ...
    if fresh:
        ...
        # a payer, not revenue — value-based bidding needs volume olywork does not have yet. Guarded like
    ...
    # reaper: there is no scheduler in olywork, and every top-up anywhere is a fine clock to advance a
...
async def _on_payment_succeeded(db: AsyncSession, pi: dict) -> dict:
    ...
    auto-charge; `olywork_kind` on the metadata is what tells our top-ups apart from anything else the
    account does (the landing-page demo charges, for one)."""
    meta = pi.get("metadata") or {}
    if meta.get("olywork_kind") != "topup":
        return {"handled": False, "reason": "not a olywork top-up"}
    ...
    if amount_micro <= 0:
        return {"handled": False, "reason": "zero amount"}
    result = await _credit(db, org_id, amount_micro, pi["id"], auto=meta.get("olywork_auto") == "1")
    pm = pi.get("payment_method")
...
async def _on_payment_reversed(db: AsyncSession, charge_or_dispute: dict, kind: str) -> dict:
    ...
    That split is deliberate and it is the reason the hold window exists. olywork has never reversed a
