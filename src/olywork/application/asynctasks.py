...
log = logging.getLogger("olywork.asynctasks")
DEFAULT_LIMIT = 50
...
async def _poll(row: AsyncTaskRecord, client: httpx.AsyncClient) -> tuple[int, bytes]:
    ...
    tool = Tool(org_id=row.org_id, name=row.endpoint_id, owner="olywork-worker",
...
async def _finish(call_id: str, outcome: str, document: object | None, now, *,
    ...
    async with session_maker() as db:
        ...
        if outcome == "success":
            ...
            # No terminal state in 24 hours means olywork does not know whether the caller got
            # anything. The platform absorbs that uncertainty: the hold goes back to the team in
            # full, the upstream charge (if any) is olywork's, and the row is flagged for a human.
