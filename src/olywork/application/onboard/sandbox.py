...
SANDBOX_DOMAIN = "sandbox.olywork.local"     # throwaway visitor identities live here (can never log in)
SANDBOX_TTL_MIN = 60
...
# a credential to nothing, and sandbox calls never leave olywork. The visitor edits/removes these and adds
...
def is_sandbox_user(user) -> bool:
    """A login-free sandbox visitor (`visitor-…@sandbox.olywork.local`). It may act ONLY inside its own
...
async def export_skill(db: AsyncSession, org: Org) -> dict:
    ...
    lines = [f"# {name}", "",
             "A olywork **skill**: a recipe + its endpoints + the secrets they need, bundled so a",
    ...
    lines += ["", "## Use", "```", "# an agent or you, after installing the skill:",
              f"olywork call {tools[0].base_url}/… " if tools else "olywork call <url>", "```",
              "", "The key is injected server-side by olywork - it never lands on the caller's machine."]
    skill_md = "\n".join(lines)

    return {
        "manifest": manifest,
        "olywork_json": json.dumps(manifest, indent=2),
        "skill_md": skill_md,
        "install": [
            "# save the manifest as olywork.json in a skill folder, then:",
            "olywork skill add ./my-api-skill",
...
# recipe (SKILL.md) + wiring (olywork.json) - the credential stays in the vault; the caller sends
# only their olywork token and olywork injects the key.
SAMPLE_SKILLS = {
    "posthog-insights": {
...
def _skill_md(name: str, sk: dict, base: str, token: str | None) -> str:
    tok = token or "$OLYWORK_TOKEN"
    out = [
        "---", f"name: {name}",
        f"description: {sk['label']}. Calls the API through olywork - the key is injected server-side, never held here.",
        "---", "", f"# {name}", "", sk["label"] + ".", "",
        "Call the API **through olywork**: you send only a token; olywork injects the real credential.",
        "Never put an API key in a command.", "", "## Actions",
    ]
    for e in sk["endpoints"]:
        out += [f"- **{e['note']}**", "  ```",
                f'  curl -s -X {e["method"]} {base}/call/{sk["base_url"]}/{e["path"]} -H "X-Olywork-Token: {tok}"', "  ```"]
    if not token:
        out += ["", "> Set `OLYWORK_TOKEN` to your olywork token (from `olywork login`, in `~/.olywork/config.json`)."]
    return "\n".join(out)


def _skill_olywork_json(name: str, sk: dict) -> str:
    return json.dumps({
...
def _skill_secret(sk: dict) -> str:
    return (f"# Referenced by NAME - the value never ships in the skill folder.\n"
            f"{sk['key']}=      # empty on disk. olywork holds the value and injects it server-side.")
...
def skill_files(name: str, base: str, token: str | None = None) -> dict:
    ...
    return {"SKILL.md": _skill_md(name, sk, base, token),
            "olywork.json": _skill_olywork_json(name, sk),
...
async def gc(db: AsyncSession) -> int:
    ...
    for u in visitors:
        ...
        try:
            ...
            logging.getLogger("olywork").exception("could not reap sandbox user %s; skipping", u.id)
            continue
        n += 1
    if n:
        await db.commit()
