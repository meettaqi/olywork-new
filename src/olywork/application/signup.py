...
async def _grant_signup_promo(db: AsyncSession, org: Org) -> None:
    ...
    try:
        ...
        try:
            ...
            logging.getLogger("olywork").warning("ad conversion queue failed for org %s: %s", org_id, exc)
        ...
        await db.rollback()
        logging.getLogger("olywork").warning("promo grant failed for org %s: %s", org_id, exc)
...
def _utm_attribution_from(raw_cookie: str) -> dict[str, str]:
    """First-touch traffic source from the `olywork_utm` cookie (set by web/sitetrack.js):
...
def _stamp_utm(org: Org, raw_cookie: str) -> None:
    """Persist the first-touch source on a brand-new team. Independent of the Google-Ads `olywork_ad`
...
async def _redeem_referral(
    ...
    try:
        ...
        logging.getLogger("olywork").warning("referral attribution failed for org %s: %s", org_id, exc)
