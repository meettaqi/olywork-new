...
# In-memory handshake state for `olywork login` (single-instance; short-lived, fine to lose on restart).
...
async def approve_cli_login(
    ...
    async with database.session_maker() as db:
        ...
        # The pairing code proves the approver is the same person who ran `olywork login` (the code is
...
def start_github_login(cli: str, callback_base: Callable[[], str]) -> SocialLoginStart:
    ...
    if cli:  # this is a `olywork login` handshake, not a browser session
...
def start_google_login(cli: str, callback_base: Callable[[], str]) -> SocialLoginStart:
    ...
    state = crypto.new_token()
    if cli:  # a `olywork login` handshake, not a browser session
        _prune_handshakes()
        _cli_states[state] = (cli, _utcnow_naive())
...
async def _provision_social_user(email: str, state: str) -> SocialLoginProof:
    async with database.session_maker() as db:
        ...
        await db.commit()
        # Browser session OR `olywork login` handshake — both go through the /login team picker now.
...
async def complete_github_login(
    ...
    try:
        ...
        gh = {"Authorization": f"Bearer {access}", "Accept": "application/json", "User-Agent": "olywork"}
...
async def current_identity(x_olywork_token: str, session_cookie: str) -> CurrentIdentity:
    async with database.session_maker() as db:
        membership = None
        if x_olywork_token:
            membership = await _membership_by_token(x_olywork_token, db)
            user = (await db.get(User, membership.user_id) if membership
                    else await _user_from_identity_token(x_olywork_token, db))
        ...
        if membership is not None:
            ...
            # /orgs/{id}/... route, and being unable to told `olywork balance` there was "no active org".
...
def _effective_mcp_resource(resource: str, scope: str) -> str:
    ...
    version = "v2" if (mcp_oauth.DIRECTORY_SCOPE in requested_scopes or mcp_oauth.LEGACY_DIRECTORY_SCOPE in requested_scopes) else "v1"
...
async def _refresh_grant(*, refresh_token: str, client_id: str, resource: str) -> dict:
    ...
    async with database.session_maker() as db:
        ...
        if row.retired_at is not None:
            ...
            logging.getLogger("olywork.auth").warning(
...
async def move_oauth_grant(*, user_id: int, family_id: str, team: str) -> dict:
    async with database.session_maker() as db:
        ...
        # whether an arbitrary slug exists on olywork — a slug-existence oracle any signed-in account could
        # walk. The caller's own teams are already listed to them by `olywork org ls`, so the distinction
        # buys them nothing they cannot see elsewhere.
        if org is None or org.suspended or member is None:
            raise OAuthGrantError(
                f"no team {team!r} on this account — see `olywork org ls` for the teams you can use",
            )
