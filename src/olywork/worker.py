"""`olywork-worker` — the scheduled, server-side maintainer commands (the `worker` profile).

    olywork-worker capacity sweep [--only provider,...] [--json]
...
def _need_server() -> None:
    try:
        ...
        print("olywork-worker needs the server extra: pip install 'olywork[server]'", file=sys.stderr)
        raise SystemExit(2)
