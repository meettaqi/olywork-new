...
async def catalog_platform(slug: str, include_hidden: int = 0) -> dict:
    ...
    return {
        ...
        # The capability grouping `olywork catalog` renders. The dashboard reads `domains` below, but
...
async def _observed_or_empty(
    ...
    try:
        ...
        logging.getLogger("olywork.catalog").warning("endpoint stats unavailable", exc_info=True)
        return {}
...
async def catalog_search(q: str = "", limit: int = 25,
    ...
    olywork has MEASURED and on price, so the cut stops being file order. `hints` carries the next
    ...
    if not q.strip():
        ...
        hints = [f"nothing matches {q!r} closely enough — try different task words, or browse `olywork catalog` for the platform shelves",
...
async def catalog_endpoint(
    ...
    # of "compare providers" that only olywork can answer (see endpoint_stats + CAPABILITY-CHOICE-PLAN).
    ...
    if ep.get("kind") == "routed":
        # The QUOTE (routing plan §3.5): the contract and the ranked children on olywork's key, priced
        ...
        routing = {
            ...
            "headers": {"X-Olywork-Route-Waterfall": "on by default: a miss tries the next provider; 0 = stop at the first miss",
                        "X-Olywork-Route-Max-Cost": "USD ceiling for the whole call (default 1.00)",
                        "X-Olywork-Route-Prefer": "provider[,…]", "X-Olywork-Route-Exclude": "provider[,…]",
                        "X-Olywork-Route-Strict-Filters": "1 = never call a provider that cannot apply a filter you sent (answers 422, nothing charged); off by default — the answer names what was ignored in X-Olywork-Ignored-Filters"},
        }
    return {
