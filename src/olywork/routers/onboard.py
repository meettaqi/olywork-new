...
# Per-IP limiter for the unauthenticated mint endpoint, in the DB (olywork.ratestore) so it survives a
...
async def stripe_webhook(request: Request) -> dict:
    """Stripe → olywork: a signed event from the demo sandbox account. Only `charge.succeeded` feeds
...
async def demo_sandbox_skill(
    ...
    """Export whatever the visitor built in their sandbox as a shareable **skill** (olywork.json manifest
...
async def skill_samples() -> list[dict]:
    """The hosted sample skills the landing offers — each with its files (SKILL.md/olywork.json/.secret)
...
async def skill_install(name: str, token: str = ""):
    ...
    recipe's calls; without it the recipe reads the token from `olywork login`."""
