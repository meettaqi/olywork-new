...
OAUTH_RETURN_COOKIE = "olywork_oauth_return"
LEGACY_OAUTH_RETURN_COOKIE = "olywork_oauth_return"
...
def _remember_oauth_return(resp, request: Request) -> None:
    ...
    resp.set_cookie(LEGACY_OAUTH_RETURN_COOKIE, target, httponly=True, samesite="lax",
                    secure=_is_https(request), max_age=600)
...
def _take_oauth_return(request: Request) -> str | None:
    ...
    target = (request.cookies.get(OAUTH_RETURN_COOKIE) or request.cookies.get(LEGACY_OAUTH_RETURN_COOKIE) or "").strip('"')
