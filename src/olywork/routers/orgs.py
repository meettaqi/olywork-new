...
async def _usage_rollup(db: AsyncSession, org_id: int, since: datetime) -> dict:
    ...
    # What those calls COST the team on olywork's own keys — read from the ledger (the authority on money)
...
async def register_user(body: UserIn, request: Request) -> dict:
    try:
        return await signup_use_cases.register_user(
            ...
            ad_cookie=request.cookies.get("olywork_ad") or "",
            utm_cookie=request.cookies.get("olywork_utm") or "",
...
async def list_orgs(
    ...
    return await teams.list_user_orgs(
        user_id=user.id,
        x_olywork_token=x_olywork_token,
        x_olywork_org=x_olywork_org,
        db=db,
    )
...
async def delete_org(
    ...
    `olywork org unpin ..` deleted a team during testing. A request arriving without the slug it is
...
async def list_cli_deny(
    ...
    whole "what is blocked" picture in one place. These live on the TOOL (olywork.json `cli.deny` +
