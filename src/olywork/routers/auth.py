...
# OTP code + its brute-force counter, and the /auth/email/start throttle, live in the DB (olywork.ratestore
...
async def _find_or_create_user(db: AsyncSession, email: str) -> User:
    ...
    `olywork org create`) — we never spawn a throwaway personal org they didn't ask for. Their identity
    token is user-scoped, so it works before they have any org (org chosen per-request via X-Olywork-Org).
    Caller commits."""
    try:
...
def _login_callback_base(request: Request) -> str:
    ...
    directions, so a login minted on olywork.com also survives a OLYWORK_PUBLIC_URL rollback."""
...
def _auth_page(headline: str, sub: str = "", *, ok: bool = True, status: int = 200) -> HTMLResponse:
    ...
    html = (
        ...
        f'<div class="logo">▚ olywork</div><div class="mark">{"✅" if ok else "⚠️"}</div>'
...
async def auth_cli_start() -> dict:
    """`olywork login` calls this FIRST. The SERVER mints both the login_id and a short pairing code and
...
_CLI_HTTP_ERRORS = {
    ...
    "expired": (400, "this login has expired — run `olywork login` again"),
    "too_many_wrong_codes": (400, "too many wrong codes — run `olywork login` again"),
...
def _login_page_html(login_id: str, *, session_email: str | None, github: bool, google: bool) -> str:
    ...
    # login they didn't start has no matching code). A `olywork login` link carries the code in the URL
    ...
    return (
        ...
        f'<p>to connect the <b>olywork</b> CLI to your account</p>'
...
// `olywork login` puts the pairing code in the URL FRAGMENT (#code=…) — it never reaches the server on
...
async function loadOrgs(){const box=document.getElementById('orgpick');if(!box)return;
 ...
 if(!orgs.length){  // brand-new user: no team yet → make them NAME one (never finish the CLI login team-less)
  ...
  const inp=document.createElement('input');inp.id='newteam';inp.placeholder='Team name, e.g. Olywork';inp.autocomplete='off';box.appendChild(inp);
...
async def auth_logout(request: Request) -> JSONResponse:
    ...
    # request). Bind it to same-origin: reject a request whose Origin isn't olywork's own.
    if not _same_origin(request):
        raise HTTPException(status_code=403, detail="cross-origin logout rejected")
    resp = JSONResponse({"ok": True})
    resp.delete_cookie(sess.COOKIE)
    resp.delete_cookie(sess.LEGACY_COOKIE)
    return resp
...
async def auth_invite_signin(
    ...
    try:
        landing = await auth_use_cases.invite_signin_landing(t, code, olywork_session or olywork_session)
...
async def auth_invite_signin_confirm(request: Request):
    ...
    resp.set_cookie(sess.COOKIE, proof.session_cookie, httponly=True,
                    samesite="lax", secure=_is_https(request), max_age=sess.TTL_SECONDS)
    resp.set_cookie(sess.LEGACY_COOKIE, proof.session_cookie, httponly=True,
                    samesite="lax", secure=_is_https(request), max_age=sess.TTL_SECONDS)
    return resp
...
def _consent_page(*, client_name: str, client_uri: str, user_email: str, teams: list,
    ...
    `olywork:call` and calls it informed is a formality, not a decision.
    ...
    warn = ("" if not unverified else
            '<p class="fine"><b>This application registered itself.</b> olywork has not reviewed it — '
    ...
    return HTMLResponse(
        ...
        f"<h1>{who} wants to use olywork</h1>{where}"
        ...
        f"</div></form>{warn}"
        f'<p class="fine">You can revoke this at any time from your olywork dashboard.</p>'
        f"</div></div></body></html>")
...
async def oauth_authorize_approve(
    ...
    if not _same_origin(request):
        raise HTTPException(status_code=403, detail=(
            "cross-origin authorization rejected — this form must be submitted from olywork's own "
...
async def auth_revoke_tokens(
    ...
    """Kill switch for a leaked token: invalidate every signed identity token (from `olywork login`) AND
    ...
    keeps the user active; unlike rotating OLYWORK_SESSION_SECRET it affects ONLY this user. We then re-issue
