...
async def org_balance(
    ...
    **Two audiences, one route.** Any MEMBER sees the figure and the in-flight holds: they are the
    ones spending it, every agent is told to run `olywork balance` after a call, and a 402 already hands
...
async def billing_stripe_webhook(request: Request) -> dict:
    """Stripe → olywork: the ONLY door through which a payment becomes balance.
