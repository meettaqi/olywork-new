...
class OAuthStartIn(BaseModel):
    ...
    REGISTRY: supply `provider` (+ optional `capability`) and olywork fills all of it from its own
    ...
    scopes: list[str] = []
    redirect_uri: str | None = None  # defaults to olywork's public callback
...
async def oauth_providers_list() -> list[dict]:
    """Providers olywork holds its own approved app for. `configured` is false when this deployment
...
async def set_extra_credential(
    ...
    account. olywork can't invent the latter, so the connect deliberately stops short of a tool. This
...
async def revoke_connection(
    ...
    only at call time with "a bound secret is missing". We remove the tool olywork auto-provisioned for
    this provider (that's olywork's creation, not the user's) and drop the dead binding from any tool
