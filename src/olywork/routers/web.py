...
def _price_label(cost: dict | None) -> str:
    ...
    rather than importing it: pulling olywork.cli into the server process costs ~200ms and drags the
...
def _page(title: str, description: str, path: str, body: str, ld: list[dict],
    ...
    shell was invisible to paid attribution: no `olywork_ad` cookie means
    ...
    attribution. `olywork_ad` and `/adtrack.js` are already documented in that policy, so this is not.
...
<meta property="og:image:alt" content="olywork.com: one key for the whole tool catalog, priced per call"/>
...
<footer class="ow-footer">
  <div class="ow-footer-inner">
    <div>
      ...
      <a href="/app?ref=footer" class="ow-btn-ink" style="font-size:13.5px;padding:9px 20px;">Start for free &#8594;</a>
    </div>
    <nav class="ow-footer-cols" aria-label="Footer">
  ...
  <div class="ow-footer-bottom">
    <span>&#169; 2025 Olywork. All rights reserved.</span>
    <span>Open source &#183; Pay-per-call &#183; No markup</span>
  </div>
</footer>
...
async def catalog_page(slug: str):
    ...
    desc = (f"{len(eps)} {label.lower()} API endpoints from "
            ...
            + ". Call them through one olywork key — no provider signup.")
    ld = [
        {"@context": "https://schema.org", "@type": "ItemList",
         "name": f"{label} — API endpoints on olywork",
...
def _uc_providers(cat, eps: list[dict], obs: dict) -> list[dict]:
    ...
    # appear as provider "olywork" and must not show as a row in comparison tables. Filter them here
    ...
    for prov, plat in sorted({(e["provider"], e["platform"]) for e in eps}):
        ...
        # A $0 trial row is a third fact again: served on olywork.com's own free-tier key with a daily
...
async def use_case_job_page(request: Request, job: str,
    ...
    if legacy and not as_md:
        ...
        # canonical/og:url so each page names the serving host, not hardcoded olywork.com.
        base = get_settings().public_url.rstrip("/")
    ...
    own_key_free = any(p["free"] for p in provs)
    why_olywork = agent_pages.WHY_OLYWORK_OWN_KEY if own_key_free else agent_pages.WHY_OLYWORK
    olywork_cards = "".join(f'<div class="card"><h4>{_esc_html(t)}</h4><p>{_esc_html(d)}</p></div>'
                         for t, d in why_olywork)
...
async def workflow_page(request: Request, slug: str,
    ...
    why_cards = "".join(f'<div class="card"><h4>{_esc_html(t)}</h4><p>{_esc_html(d)}</p></div>'
                        for t, d in spec["prompt_why"])
    olywork_cards = "".join(f'<div class="card"><h4>{_esc_html(t)}</h4><p>{_esc_html(d)}</p></div>'
                         for t, d in agent_pages.WHY_OLYWORK)
...
async def tools_provider(service: str, db: AsyncSession = Depends(get_session),
    ...
    "{provider} mcp" — the agent->olywork->provider flow, setup (agent one-liner first), a prompt
    to try, why-olywork cards, EVERY tool grouped by platform, alternatives and an FAQ that also
    ...
    setup = (
        ...
        f'<button class="copybtn" data-copy="set up olywork — {_esc_html(base)}/llms.txt">copy</button></div>'
        f"<pre>set up olywork — {_esc_html(base)}/llms.txt</pre></div>"
        ...
        f'<button class="copybtn" data-copy="claude mcp add --transport http olywork {_esc_html(base)}/mcp">copy</button></div>'
        f"<pre>claude mcp add --transport http olywork {_esc_html(base)}/mcp</pre></div>"
        '<div class="cards">'
    ...
    prompt = (f"Using olywork, {task_lines[0]}. Show me the price first." if task_lines
              else f"Using olywork, call {display}. Show me the price first.")
    tryit = (
        '<section id="ask"><div class="wrap"><div class="seclab">Try it</div>'
        ...
        {"@context": "https://schema.org", "@type": "HowTo",
         ...
         "step": [
             {"@type": "HowToStep", "position": 1, "name": "Give your agent the setup line",
              "text": f"set up olywork — {base}/llms.txt"},
             {"@type": "HowToStep", "position": 2, "name": "Or add the MCP server yourself",
              "text": f"claude mcp add --transport http olywork {base}/mcp"},
...
async def pricing_page():
    ...
    body = (f'<main class="wrap">{_PV_CSS}<div class="phead">'
            '<div class="crumbs"><a href="/">olywork</a> / pricing</div>'
            "<h1>Pricing</h1>"
...
side with measured success rate, speed and price. <b>Choosing is yours</b> — olywork compares, but it
...
def _spa_with_og(kind: str, name: str):
    ...
    safe = _esc_html(name)
    meta = (
        f"<title>{safe} · Olywork</title>\n"
        f'<meta property="og:title" content="{safe} — shared {label}"/>\n'
        f'<meta property="og:description" content="A {label} shared via Olywork. '
...
async def robots_txt():
    ...
    sitemap rather than olywork.com's."""
...
INDEXNOW_KEY = "7c2e4a91b5d3f8e6olywork2026"
...
async def install_sh():
    """`curl -fsSL {BASE}/install.sh | sh` — installs the olywork CLI and points it at this server.
...
def routed_discovery_on() -> bool:
    """`OLYWORK_ROUTED_DISCOVERY` — see catalog_store.group_routed. The agent-facing files honour it
...
async def tutorial_import_shell_md():
    """Focused tutorial: CLI auto-import (`olywork upload clis`) + shell mode (`olywork shell`) + the
...
async def integrate_md():
    """The BUILDER skill: how to put olywork inside your own product and bill your own customers for it.

    Distinct from `skill.md`, which teaches an agent to USE olywork. This one is pasted into a builder's
...
async def tutorial_js():
    """The shared interactive-tutorial data + highlighter (window.OLYWORK_TUTORIAL / olyworkHL).
...
def _legal_page(name: str) -> HTMLResponse:
    ...
    # og:url name the host actually serving it. A hardcoded olywork.com would tell a self-hosted
...
async def adtrack_js():
    """First-party ad-click capture (see the file itself): sets the `olywork_ad` cookie that
...
async def sitetrack_js():
    """First-touch traffic-source capture (`olywork_utm` cookie, always on — first-party, no PII) plus
    ...
    `OLYWORK_POSTHOG_KEY` the analytics half is inert (empty key) and only the cookie half runs."""
...
async def grokbot_page():
    """Landing page for the olywork plugin inside Grok Bot ("Grok Bot for Outreach") — a scroll
    animatic of the Bot working a lead list through olywork. Indexed like /people-search: canonical,
...
async def fable_page():
    """Landing page for the Claude Fable 5.1 + olywork launch ("Run your GTM from the terminal"):
...
async def well_known_skills_index():
    """Advertise olywork's own skill under the agentskills.io well-known convention.

    This makes THIS host a first-class skill source: an agent that supports the standard can install
    olywork from olywork.com directly, with no directory, no review queue and no third party in the middle
...
async def connect_demo_page():
    ...
    privileged about being served from olywork's own domain. The point is to watch the whole dance in a
...
async def connect_demo_callback():
    """Where olywork sends the browser back. Hands the code to the opener and closes."""
    if not get_settings().connect_demo_enabled:
        raise HTTPException(status_code=404, detail="connect demo is not enabled")
