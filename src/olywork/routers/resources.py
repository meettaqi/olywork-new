...
sys.modules.setdefault("olywork.routers.providers", _providers)
sys.modules.setdefault("olywork.routers.convert", _convert)
sys.modules.setdefault("olywork.routers.skills", _skills)
...
async def delete_secret(
    ...
    # deletable, breaking `olywork run` — guard those references too.
...
class ToolIn(BaseModel):
    ...
    cli: dict | None = None  # local-run profile for `olywork run` (docs/CLI-RUN-PLAN.md)
...
def _require_public_base_url(base_url: str) -> None:
    ...
    targets so a member can't turn `olywork call` into an SSRF (e.g. base_url=169.254.169.254). Reuses the
...
def _allowed_server_bins() -> set[str]:
    """The commands `olywork run --server` may execute: catalog-known CLIs + an admin allow-list. Blocks a
...
def _materialize_skill_files(files: list) -> str:
    ...
    root = Path(tempfile.mkdtemp(prefix="olywork-skill-")).resolve()
...
def _scan_uploaded_skills(root: str, catalog: list, env_names: set) -> list:
    ...
    CLI's own `skills._classify` — so the dashboard verdict is identical to `olywork upload skills`."""
...
async def analyze_skill_folder(
    ...
    classifier as `olywork upload skills`: recipe-only vs contract vs generated, plus readiness gaps."""
    _require_can_register(caller)
...
async def import_skill_folder(
    ...
    the uploaded files / provided env values) → register each as a bundle. Mirrors `olywork upload skills`."""
    _require_can_register(caller)
    if demo_sandbox.is_sandbox(caller.org):
        raise HTTPException(status_code=403, detail="skill import is disabled in the sandbox")
    _check_upload_size(body.files)
...
async def get_bundle(
    ...
    if bundle is None or bundle.org_id != caller.org_id:
        raise HTTPException(status_code=404, detail="bundle not found")
    if not await _bundle_allowed(caller, bundle, db):  # `olywork skill install` uses this route too
        raise HTTPException(status_code=403, detail=(
