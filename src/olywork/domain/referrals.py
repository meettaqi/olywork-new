...
in olywork by design (see domain/money's reaper), so `sweep()` is called from paths someone is already
...
async def ensure_code(db: AsyncSession, user: User) -> str:
    ...
    base = re.sub(r"[^a-z0-9]+", "", (user.email or "").split("@")[0].lower())[:12] or "olywork"
    if len(base) < 2:
        base = f"{base}olywork"
    for _ in range(5):
