...
async def validate_bindings(bindings: list[dict], *, org_id: int, caller_email: str,
    ...
    for b in bindings:
        # A `platform_setting` binding injects one of OLYWORK's own credentials (a tier-4 provider key, the
        ...
        if b.get("platform_setting"):
            raise ToolConfigError(
                "a binding may not name a platform_setting — olywork's own credentials are server-managed "
