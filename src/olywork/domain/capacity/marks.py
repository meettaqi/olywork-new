...
log = logging.getLogger("olywork.capacity")
...
async def strike(provider: str, *, endpoint_id: str | None, kind: str,
    ...
    """One balance/quota signature on olywork's own account. Returns the row as written (active or
...
async def clear(key: str, *, lock_id: str | None = None) -> bool:
    """A 2xx on olywork's own account. Clears pending strikes, or the active lock the call was
