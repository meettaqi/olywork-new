"""The free balance / quota collectors for every provider olywork holds a platform key for.
...
(`olywork.domain.capacity.sweep`) turns these dicts into `CapacitySnapshot` rows.
"""
...
async def _serpstat(c, key):
    ...
    # one that works live — see the discrepancy note in src/olywork/catalog/serpstat.yaml.
...
async def provider_balance(provider: str, client: httpx.AsyncClient | None = None) -> dict:
    ...
    if not key:
        return {"provider": provider, "value": None, "unit": "", "no_key": True,
                "note": f"no OLYWORK_{setting.upper()} in the env"}
