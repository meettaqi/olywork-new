...
def is_refreshable(blob: dict) -> bool:
    ...
    (user uploads a token and re-uploads when it expires) and olywork just injects it as-is."""
...
def expiry_of(blob: dict) -> datetime | None:
    """The token's absolute expiry as a UTC-naive datetime (matching how olywork stores timestamps)."""
    ts = _expires_at(blob)
    if ts is None:
        return None
...
def expiry_state(expires_at: datetime | None, refreshable: bool, now: datetime | None = None) -> str:
    ...
    A refreshable credential is always `fresh`: olywork mints a new access token on demand, so its
    short expiry is an implementation detail the user should never be nagged about. Only a
    credential olywork CANNOT renew by itself gets an expiry warning."""
    if refreshable:
        return "fresh"
...
def secret_is_refreshable(secret: Secret) -> bool:
    """Whether olywork can renew this credential unattended. Decrypts server-side; the blob never
