...
This is the half of "compare providers" that only olywork can do. Anyone can read a rate card; only the
    ...
    **405 is the exception**, and it is one the rule's own justification demands. "The caller sent
    ...
    rows = (await db.execute(
        ...
        .where(CallRecord.endpoint_id.in_(ids), CallRecord.created_at >= since,
               # olywork's own refusals (paywall 402s, caps, bad requests never relayed) are facts
