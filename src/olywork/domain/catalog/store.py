"""Reader for the endpoint catalog (`src/olywork/catalog/*.yaml`) — the operations layer.
...
`olywork catalog` is a thin client over the /catalog/* endpoints like every other CLI command.
...
# The YAML data ships at the package root (`src/olywork/catalog/`), not next to this module.
...
#   routed  — a GENERATED first-party endpoint (`olywork.<capability>`) that picks among the children
...
class Catalog:
    ...
    # service -> {"usd", "fee_usd_month"} for rates OLYWORK SET (fx.yaml `kind: olywork_shared_plan`).
    ...
    def platform_eligible(self, endpoint: dict) -> bool:
        ...
        # never an offer olywork may spend against, even if its historical price remains complete.
        if endpoint.get("status"):
            return False
        if endpoint.get("kind") == "routed":
            return bool(endpoint.get("routed_children"))
        # Blocked on olywork's own plan: the route works, the price is real, and the shared key
...
def _parse(directory: Path) -> Catalog:
    ...
    # GENERATED `olywork.<capability>` row per capability with ≥ 2 verified children (routing/synthetic).
    ...
    shared_plans = {
        ...
        for service, v in (fx_doc.get("credit_rates_usd") or {}).items()
        if isinstance(v, dict) and v.get("kind") == "olywork_shared_plan"
    }
    trial_pools = {
        str(service): int(v.get("trial_calls_per_team_day") or 0)
        for service, v in (fx_doc.get("credit_rates_usd") or {}).items()
        if isinstance(v, dict) and v.get("kind") == "olywork_trial"
    }
...
def _load_routing(directory: Path, by_id: dict[str, dict]):
    ...
    try:
        ...
        import logging
        logging.getLogger("olywork.catalog").warning("routing files failed to load", exc_info=True)
        return {}, {}
...
def _normalize(raw: dict, provider: str, directory: Path) -> dict:
    ...
    return {
        ...
        # hit. Without it olywork bills the estimate for a body the VENDOR gave away free.
        ...
        # A route that WORKS upstream but that olywork's own plan/subscription cannot serve (a
...
def endpoint_view(ep: dict, provider_display: str, cat: Catalog | None = None) -> dict:
    ...
    return {
        ...
        **({
        ...
        "async": ep.get("async"),
        # whether olywork may serve this route with its OWN key, billed to the caller's balance —
        ...
        # A generated routed row (`olywork.<capability>`) names the children it chooses among, so a
...
def group_routed(rows: list[dict], key=lambda r: r, max_children: int | None = None) -> list[dict]:
    ...
    where the group sits; within it, "let olywork choose" leads and the specific providers follow.
    ...
    `OLYWORK_ROUTED_DISCOVERY=off` turns the STEERING off and returns the rows untouched: routed
...
def near_ids(endpoint_id: str, cat: Catalog, limit: int = 3) -> list[str]:
    ...
    # wearing a spellcheck's clothes, which is exactly what the charter says olywork does not do:
    # olywork compares providers side by side and the CALLER chooses. (The first cut of this scored
...
def unknown_id_hint(endpoint_id: str, cat: Catalog) -> str:
    ...
    return f"search for it instead: olywork catalog search {tail!r}"
...
def rerank(rows: list[tuple[dict, float]], stats: dict[str, dict],
    ...
    """Re-order equal-scoring rows by what olywork has MEASURED, then by price.

    Relevance still decides first — a cheap reliable endpoint that doesn't do the job is not the
    answer. But below that, olywork is the only party that can rank on evidence, and until this existed
