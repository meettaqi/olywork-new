"""The generated `olywork.<capability>` catalog rows — never hand-written (plan §5)."""
...
def routed_endpoint(contract: Contract, children: list[dict], adapters: dict[str, Adapter], cost_view) -> dict | None:
    ...
    return {
        ...
        "input": {"bodyType": "json", "body": body,
                  "note": (f"Send everything you know about the subject — olywork derives the rest (full_name ⇄ first+last), "
                           f"matches each provider on the variants it accepts ({variants}) and sends each provider only "
                           f"the fields it wants. "
                           "On a miss olywork keeps trying providers, cheapest first, within X-Olywork-Route-Max-Cost "
                           f"(default ${contract.default_max_cost_usd or 1.00:g} per call; misses on per-success providers are free). Options ride as "
                           "headers, never in the body: X-Olywork-Route-Waterfall: 0 (stop at the first miss), "
                           "X-Olywork-Route-Max-Cost: <usd>, X-Olywork-Route-Prefer / X-Olywork-Route-Exclude: <provider>. The "
                           "response is {output, raw, _olywork: {served_by, tried}}; X-Olywork-Served-By names the child.")},
