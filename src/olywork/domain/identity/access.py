...
async def _resolve_org(ref: str, db: AsyncSession) -> Org | None:
    """Resolve an X-Olywork-Org header (a slug, or a numeric id) to an Org. Slug wins first: an
    ...
    # dependency; bound it so an out-of-range X-Olywork-Org just falls through to the 400.
...
async def require_member(
    ...
    if membership is not None:  # per-org token — the org is baked in
        ...
        # identity token (CLI `olywork login`) or a browser session — pick the org via X-Olywork-Org / X-Olywork-Org
        user = (await _user_from_identity_token(token, db)) if token else await _user_from_session(session_cookie, db)
        if user is None:
            raise HTTPException(status_code=401, detail="invalid token" if token else "not authenticated")
        # The X-Olywork-Org/X-Olywork-Org header wins; a team-pinned identity token (org baked into its claim) is the
        ...
        org_ref = org_header or ((sess.read_identity_claims(token) or {}).get("org", "") if token else "")
        org = await _resolve_org(org_ref, db)
        if org is None:
            raise HTTPException(status_code=400, detail="choose an org (send X-Olywork-Org)")
        membership = (
            await db.execute(
...
async def require_superadmin(
    ...
    if user is not None and user.is_superadmin and not user.suspended:
        await db.commit()
        return user.email
    if not token and not session_cookie:  # nothing presented → not authenticated
        raise HTTPException(status_code=401, detail="not authenticated")
...
PUBLIC_DEMO_DOMAIN = "public-demo.olywork.local"  # unroutable — the public identity can never log in
LEGACY_PUBLIC_DEMO_DOMAIN = "public-demo.olywork.local"
# NOTE: "agent" here is an IDENTITY — a coding agent / automation that calls olywork. It is NOT the
...
AGENT_DOMAIN = "agents.olywork.local"  # unroutable — an agent acts only by its token
LEGACY_AGENT_DOMAIN = "agents.olywork.local"


def _is_agent_email(email: str) -> bool:
    norm = _norm_email(email)
    return norm.endswith(f"@{AGENT_DOMAIN}") or norm.endswith(f"@{LEGACY_AGENT_DOMAIN}")


def _is_machine_email(email: str) -> bool:
    """An identity minted by an admin for a machine — never a person who can sign in."""
    norm = _norm_email(email)
    return _is_agent_email(norm) or norm.endswith(f"@{PUBLIC_DEMO_DOMAIN}") or norm.endswith(f"@{LEGACY_PUBLIC_DEMO_DOMAIN}")
