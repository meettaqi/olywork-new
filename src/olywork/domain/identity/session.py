...
browser session can never be replayed as an ``X-Olywork-Token`` bearer (or vice versa). Legacy tokens
...
COOKIE = "olywork_session"
LEGACY_COOKIE = "olywork_session"
...
# set OLYWORK_SESSION_SECRET / OLYWORK_SECRET_KEY.
...
def _make(
    ...
    *,
    ...
    # It exists so a copyable "API key" works as a bare bearer where no `X-Olywork-Org` header can travel
