"""olywork as an OAuth **authorization server** — the metadata, and the tokens it will issue.

Everywhere else olywork speaks OAuth as a *client*: `oauth.py` builds PKCE challenges to sign in with
...
on ours — the user consented to that server, not to olywork, and we would be spending their olywork balance
...
def mcp_resource_audiences(version: str = "v1") -> set[str]:
    ...
    grants minted on olywork.com must equally survive a OLYWORK_PUBLIC_URL rollback to the old name."""
...
def read_access_token(token: str, *, expected_audience: str) -> dict | None:
    ...
    spend a olywork balance. Every other check here is ordinary — signature, type, expiry — but this is
...
def new_client_id() -> str:
    ...
    return "olywork-client-" + secrets.token_urlsafe(24)
