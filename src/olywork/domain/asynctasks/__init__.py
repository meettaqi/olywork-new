...
def artifact(descriptor: dict, terminal: object) -> dict:
    ...
    more call and olywork never downloads media) for the caller to format. `ttl_note` passes through
...
def fetch_command(fetch: dict) -> str:
    """The `olywork call` line that retrieves a fetch-mode artifact, shell-safe: the value is the
    provider's, so it is quoted wherever a human might paste the line (CLI stderr, the dashboard)."""
    return f"olywork call {shlex.quote(str(fetch['endpoint']))} -p " \
