"""Skill-directory scanner for `olywork upload` (skill mode) — the skill analogue of providers.scan_env.
...
- **api_tool** — a skill that calls an authed external API. Either it already carries a `olywork.json`
...
class SkillDetection:
    ...
    health: dict | None = None                   # optional {path:...} health check (from a olywork.json)
...
def scan_skills(skills_dir: str, catalog: list[dict] | None = None,
    ...
    a single skill (so `olywork upload skills` works from inside one skill). `env_names` = vars available."""
...
def _catalog_cli_detection(name: str, d: Path, prov_entry: dict, env_names: set[str]) -> SkillDetection:
    ...
    the machine env at import (or asked for). Executed via `olywork run`, not the proxy. `env_from` on an
    ...
    for e in inject:
        ...
        if not env_var:
            continue  # can't discover this one from an env var; a hand-written olywork.json can supply it
...
def _classify(d: Path, catalog: list[dict], env_names: set[str]) -> SkillDetection:
    name = d.name
    # (1) an existing, complete olywork.json wins — use it verbatim (its secrets are local files).
    try:
        contract = convert.load_contract(d)
    except ValueError as exc:
        return SkillDetection(name=name, path=str(d), kind="skip", note=f"bad olywork.json: {exc}")
    if contract and contract.get("base_url"):
        ...
        det = SkillDetection(name=name, path=str(d), kind="contract", base_url=contract["base_url"],
                             ...
                             note="has olywork.json", cli=_contract_cli(contract))
        ...
        for s in contract.get("secrets", []):
            if s.get("file") and not convert.resolve_secret_path(d, s["file"]).exists():
                det.gaps.append(f"olywork.json secret file missing: {s['file']}")
        ...
        if not contract.get("bindings") and not contract.get("cli"):
            # a CLI tool legitimately has no HTTP bindings — its auth is the cli.inject, run via `olywork run`
            det.gaps.append("olywork.json has no bindings (tool would have no auth)")
    ...
    if not (base_m or secret_files or auth_envs):
        ...
        # at import, or asked for). `olywork run <name>` then works — no proxy involved.
...
def _flag_header_collisions(det: SkillDetection) -> None:
    ...
    if dup:
        det.gaps.append(f"ships multiple credentials that all map to one header ({', '.join(dup)}) — add a "
                        "olywork.json mapping each credential to its own header (e.g. Authorization + developer-token)")


# --- contract + payload (increment 2) ------------------------------------------------------------
def to_contract(det: SkillDetection) -> dict:
    """The olywork.json dict for a generated/contract skill (drops the internal `source` marker; keeps
...
def write_contract(det: SkillDetection, force: bool = False) -> str | None:
    """Persist olywork.json into the skill dir so the skill carries its own contract. Skips a skill that
...
_SKIP_NAMES = {"SKILL.md", "olywork.json", "olywork.json", ".DS_Store"}
...
# --- dev/test entrypoint: `python -m olywork.skills --dir DIR [--env-dir ENVDIR]` --------------------
if __name__ == "__main__":
    import argparse
    import os
    ap = argparse.ArgumentParser(description="olywork skill-dir scan (classification only)")
    ...
    labels = {"contract": "● API tool (has olywork.json)", "generated": "● API tool (generated contract)",
