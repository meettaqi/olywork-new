"""Server-side CLI execution (Tier 0) — run a registered tool's real CLI on the olywork host, with
...
credential cache; a scrubbed environment that never carries olywork's own process env (which holds
OLYWORK_SECRET_KEY); a timeout + output cap; and redaction of secret values from the returned output.
...
def _child_env(secret_env: dict[str, str], home: str) -> dict[str, str]:
    """A scrubbed environment: NOT the server's env (it holds OLYWORK_SECRET_KEY and every other
...
def resolve_exec_bin(tool: Tool) -> str:
    ...
    if not cli:
        raise RunError(
            f"tool {tool.name!r} has no CLI profile — it's an HTTP tool (use `olywork call`), or add a "
            '"cli" block to its olywork.json to make it runnable.')
...
async def run_tool(
    ...
    if resolved is None:
        raise RunError(f"CLI {entrypoint!r} is not installed on the olywork server")

    secret_env, secret_values = await _collect_secret_env(tool, db)

    home = tempfile.mkdtemp(prefix="olywork-run-")
    ...
    try:
        ...
        # exit. A normal `olywork run` finish still leaves any BACKGROUND grandchild the CLI spawned alive
    ...
    if timed_out:
        err = (err or b"") + f"\n[olywork] killed: exceeded {timeout_s}s timeout".encode()
