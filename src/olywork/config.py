...
# Every hostname the reference deployment has EVER answered to. olywork moved
# olywork.olywork.com → olywork.com (2026-08); installed CLIs, skill.md files, .mcp.json configs and
...
# a OLYWORK_PUBLIC_URL revert a complete rollback: grants and logins minted on either name survive the
...
PUBLIC_HOST_ALIASES: tuple[str, ...] = ("olywork.olywork.com", "olywork.com", "olywork.com", "www.olywork.com")

# The subset browsers are REDIRECTED AWAY FROM (marketing pages only; see api.py's middleware).
# Deliberately one-way — only ever the pre-move name, never olywork.com — so a browser that cached the
# old→new 301 can never meet a new→old redirect and loop, even while a rollback is in effect.
LEGACY_PUBLIC_HOSTS: tuple[str, ...] = ("olywork.olywork.com",)


def platform_setting_name(provider: str) -> str:
    """The Settings attribute holding olywork's own key for `provider` — the string a `platform_setting`
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_file=".env", env_prefix="OLYWORK_", extra="ignore")
    ...
    def settings_customise_sources(
        ...
        olywork_env = EnvSettingsSource(settings_cls, env_prefix="OLYWORK_")
        olywork_dotenv = DotEnvSettingsSource(settings_cls, env_file=".env", env_prefix="OLYWORK_")
        return (
            init_settings,
            olywork_env,
            env_settings,
    ...
    database_url: str = "sqlite+aiosqlite:///./olywork.db"
    ...
    # Fernet key (urlsafe base64, 32 bytes). Generate with `olywork keygen` (see crypto.py).
    ...
    # DEMO Stripe webhook signing secret (env OLYWORK_DEMO_STRIPE_WEBHOOK_SECRET) for the landing page's live
    # payments feed (see pubfeed.py). Empty = the /stripe/webhook endpoint is off (404).
    demo_stripe_webhook_secret: str = ""

    # The Stripe sandbox restricted key (env OLYWORK_DEMO_STRIPE_KEY) behind the landing sandbox's ONE
    ...
    # Cross-tenant super-admin bearer (env OLYWORK_ADMIN_TOKEN). Presenting it authorizes every
    ...
    # Isolated-runner proof for `olywork run --local` (env OLYWORK_RUN_PROOF). A grant that would return a
    # secret the CALLER does NOT own (a shared-key tool a member may run but not read) requires this
    # value in the `X-Olywork-Run-Proof` header — held ONLY by the root-installed olywork-run runner, never
    # by the member. Empty = shared-key local runs are refused (owned-secret runs still work). Set it
    # on the server AND install it via `olywork setup-local-run --run-proof` to enable shared local runs.
    run_proof: str = ""

    # `olywork run --server` allow-list. The server only executes an entrypoint that is a catalog-known CLI
    ...
    # safe, high-value guards. Set OLYWORK_RUN_RLIMITS=false to disable entirely.
    ...
    # olywork's OWN provider keys, spent on a caller's behalf and metered against their prepaid balance.
    ...
    # `platform_key_for("tikhub")` → `platform_key_tikhub` → env `OLYWORK_PLATFORM_KEY_TIKHUB`.
    ...
    # capped trial pools at $0 (fx.yaml kind: olywork_trial — api._enforce_trial_allowance is the brake).
    ...
    # naming any of these in OLYWORK_PLATFORM_PROVIDERS.
    ...
    # Overflow aggregators (docs/PROVIDER-CAPACITY-PLAN.md §4.3): olywork-owned accounts that serve the
    ...
    # olywork pays the probe, bounded by the daily budget. On: the child cycle serves the caller.
    ...
    # happens to hold a key can't start spending it by accident. `OLYWORK_PLATFORM_PROVIDERS=""` in the
    # Render dashboard turns the whole feature off without a redeploy.
    platform_providers: str = ""
    # Whether DISCOVERY steers to routed rows: `olywork.<capability>` first in search, and a routed
    ...
    # OAuth providers whose UPSTREAM bill lands on olywork's developer app rather than the connected
    ...
    # deploy must OPT IN to charging (`OLYWORK_OAUTH_BILLED_PROVIDERS=x`). BYO-app connections
    ...
    # The archive's rollout switch (see olywork/archive.py): "off" (default) | "shadow" (record +
    ...
    # refresh calls ONE provider may spend per UTC day. A refresh is olywork's own vendor spend with
    ...
    # olywork's own public base URL — used to build the OAuth callback (must be whitelisted in the
    # provider's OAuth app). Self-hosting? Set OLYWORK_PUBLIC_URL to your deployment's URL.
    public_url: str = "https://olywork.com"
    ...
    # Product analytics (dashboard onboarding + Try-it). Empty key = OFF, so self-hosted olywork instances
    # send nothing — only a deployment that sets OLYWORK_POSTHOG_KEY reports. The key is a PUBLIC posthog
    # ingestion key (safe to expose to the browser); host defaults to EU cloud.
    posthog_key: str = ""
    posthog_host: str = "https://eu.i.posthog.com"
    # Intercom Messenger (support chat; olywork's own workspace). Empty app_id = OFF, so self-hosted
    ...
    # olywork's own Google Ads developer token, from OUR approved manager account. Ads needs it on
    ...
    # olywork's OWN long-lived refresh token for the Data Manager conversion uploader — a PLATFORM
    ...
    # it), never against a customer's own OAuth connection: the uploader ships olywork's OWN marketing
    # conversions to olywork's OWN ad account, a different purpose from a customer connecting Ads to
    ...
    # must be explicitly enabled (OLYWORK_EMAIL_DEV_MODE=true) for local testing without a mail sender.
    ...
    single_user_token_file: str = "~/.olywork/local-token"
    ...
    def single_user_ok(self) -> bool:
        ...
        A stray OLYWORK_SINGLE_USER=true in production therefore does nothing.
        """
    ...
    def platform_provider_set(self) -> frozenset[str]:
        """The allow-listed tier-4 providers (comma-separated `OLYWORK_PLATFORM_PROVIDERS`)."""
    ...
    def platform_key_for(self, provider: str) -> str | None:
        """olywork's own key for `provider`, or None if tier 4 must not serve it. BOTH conditions have to
    ...
    def oauth_billed_set(self) -> frozenset[str]:
        ...
        `OLYWORK_OAUTH_BILLED_PROVIDERS`). Empty = the current free behavior."""
    ...
    def expose_dev_code(self) -> bool:
        ...
        real (Postgres) deploy. So even a stray OLYWORK_EMAIL_DEV_MODE=true in production can't leak codes."""
    ...
    # be a Resend-verified domain — olywork.com is verified (DKIM + SPF); olywork.olywork.com remains
    # verified as a fallback.
    resend_api_key: str = ""
    email_from: str = "olywork <no-reply@olywork.com>"
