...
def resolve_secret_path(base: Path, rel: str) -> Path:
    ...
    machines (both are gitignored, so each person creates their own) — so a olywork.json written against
    ...
    if parts and parts[0] in _SECRET_DIRS:
        for sd in _SECRET_DIRS:
            ...
            if alt.exists():
                return alt
    return exact
CONTRACT_FILE = "olywork.json"  # the sidecar contract a skill can carry so olywork auto-registers it
LEGACY_CONTRACT_FILE = "olywork.json"
...
# ---- olywork.json contract: heuristic generation + loading -----------------------------------
...
def _is_app_config(f: Path) -> bool:
    ...
    The auto-generator neither stores nor binds it (a hand-written olywork.json still may)."""
...
def generate_contract(skill_dir: str | Path) -> dict:
    """Heuristically build a olywork.json for a skill dir: auto-discover secrets (from .secret/.secrets),
    ...
    if cli_profile and not cli_profile.get("unsupported"):
        ...
        fill.append(f"cli — local runs: members can `olywork run {p.name}`; review the deny list + inject")
    else:
        contract_cli = None
    if len(secrets) > 1:
...
def load_contract(skill_dir: str | Path) -> dict | None:
    """Return the parsed olywork.json/olywork.json in a skill dir, or None if absent. A hand-edited, malformed file
    raises a clear ValueError rather than a bare JSONDecodeError traceback."""
    f = Path(skill_dir) / CONTRACT_FILE
    if not f.exists():
        f = Path(skill_dir) / LEGACY_CONTRACT_FILE
    if not f.exists():
        return None
    try:
        contract = json.loads(f.read_text())
    except json.JSONDecodeError as exc:
        raise ValueError(f"{f.name} is not valid JSON: {exc}") from exc
    if contract.get("cli") is not None:
        ...
        try:
            ...
            raise ValueError(f"{f.name} cli block: {exc}") from exc
    return contract


def contract_to_skill_payload(skill_dir: str | Path, contract: dict) -> dict:
    """Turn a olywork.json + its on-disk secret files into a POST /skills body (values loaded from disk).
    ...
    for s in contract.get("secrets", []):
        ...
        elif s.get("env"):                     # env-sourced (a olywork-import contract) → read from the environment
            import os
