...
# The pre-olywork.com hostnames must keep answering the API forever — every installed CLI, skill.md
...
# parks the whole authorization request in `olywork_oauth_return` and sends the browser through `/` to
    ...
    async def __call__(self, scope, receive, send):
        ...
        if request.method in ("GET", "HEAD") and host in _LEGACY_HOSTS:
            ...
            if always or (path in _REDIRECT_PATHS and sess.COOKIE not in request.cookies):
                ...
                # must keep serving in place, but "not-olywork.olywork.com" must not.
...
class _BodyDecodeMiddleware:
    """Pure-ASGI: when a request carries `X-Olywork-Body-Encoding` or `X-Olywork-Body-Encoding`, decode the body before routing. The
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] != "http":
            return await self.app(scope, receive, send)
        enc = next((v.decode("latin-1") for k, v in scope["headers"] if k.lower() in _BODY_ENC_HEADERS), None)
        if enc is None:
            return await self.app(scope, receive, send)
        chunks: list[bytes] = []
        consumed_messages = []
        ...
        try:
            ...
            return await JSONResponse({"detail": "invalid body-encoding body"}, status_code=400)(scope, receive, send)
        # Strip the marker, drop content-encoding, and fix content-length to the decoded size.
        headers = [(k, v) for k, v in scope["headers"]
                   if k.lower() not in _BODY_ENC_HEADERS and k.lower() not in (b"content-length", b"content-encoding")]
...
for _middleware in (
    ...
    _middleware.__module__ = "olywork.api"
