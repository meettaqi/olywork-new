...
- **cache**: the newest stored answer for a key, served instead of a vendor call while it is
  fresh. Serving arrives in a later PR and only when `OLYWORK_ARCHIVE_MODE=serve`.
...
Eligibility is three gates, in order, all of which must pass:
  ...
  3. Tier — only METERED PLATFORM calls (olywork's own vendor key) are recorded; those responses are
...
_log = logging.getLogger("olywork.archive")
...
async def store_terminal_response(
    ...
    try:
        ...
        url=f"olywork://asynctasks/{call_id}", caller_body=b"", headers={},
...
async def load_terminal_responses(tasks: list[tuple[str, str]]) -> dict[str, bytes]:
    ...
    hashes = {cache_key("GET", endpoint_id, f"olywork://asynctasks/{call_id}", b"", {}): call_id
...
async def _store_locked(
    *,
    ...
    async with background_session_maker() as s:
        ...
        if origin == "caller":     # a refresh is olywork asking itself — never demand
...
def caller_max_age_s(headers) -> int | None:
    """`X-Olywork-Max-Age` / `X-Olywork-Max-Age`: the caller's own freshness bar in seconds, tightening (never widening)
    the endpoint's window. Malformed values are ignored — a typo must not change behavior."""
    raw = headers.get("x-olywork-max-age") or headers.get("x-olywork-max-age")
    if raw is None:
        return None
    try:
        v = int(str(raw).strip())
...
async def lookup(
    *,
    ...
    own fetch time, and the window is min(endpoint TTL, caller X-Olywork-Max-Age). Returns the
...
# A refresh is olywork's own vendor spend with no caller attached, so two brakes bound it — the
...
async def refresh_once(client) -> int:
    """One pass: find due-and-demanded keys, re-ask each question on olywork's platform key, and
...
async def _refresh_call(client, key) -> bool:
    """Re-make one stored question on olywork's own key. Injection reuses the ONE authoritative
