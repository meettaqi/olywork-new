"""olywork local proxy — catch the agent's OWN outgoing calls, inject the credential on the SERVER.

Today olywork only works when the agent is told to use it (`olywork call …`, or the `/call/` URL). The
moment an agent writes its own script that talks to `api.stripe.com` directly, olywork is invisible and
...
re-address it to olywork's `/call/` passthrough). Still to come: P3 fetches the allow-list from
...
def _require_cryptography():
    ...
    try:
        ...
        raise ProxyDependencyError(
            ...
            "  Run this from a terminal and olywork offers to install it the right way for your install,\n"
...
# ---- where state lives --------------------------------------------------------------------
def proxy_dir() -> Path:
    """`~/.olywork/proxy/` — the CA and the trust bundle. Follows `OLYWORK_CONFIG` when set, so tests and
    agents that redirect the CLI's config also redirect the CA instead of writing into the real one."""
    cfg = os.environ.get("OLYWORK_CONFIG")
    base = Path(cfg).expanduser().parent if cfg else Path.home() / ".olywork"
    return base / "proxy"
...
class CertAuthority:
    ...
    def context_for(self, host: str) -> ssl.SSLContext:
        ...
        with self._lock:
            ...
            fd, path = tempfile.mkstemp(prefix="olywork-leaf-", suffix=".pem", dir=self.dir)
            try:
                os.fchmod(fd, 0o600)
...
def _system_ca_pem() -> bytes:
    ...
    the internet identical to olywork's. Falls back to OpenSSL's configured file."""
    try:
        import certifi
...
def build_bundle(ca_cert_pem: bytes, out_path: Path) -> Path:
    ...
    if body and not body.endswith(b"\n"):
        body += b"\n"
    body += b"# olywork local proxy CA (this machine only)\n"
    body += ca_cert_pem
...
def ensure_ca(directory: Path | None = None, *, days: int = CA_DAYS, renew: bool = False) -> CertAuthority:
    ...
    now = datetime.now(timezone.utc)
    subject = x509.Name([
        x509.NameAttribute(NameOID.COMMON_NAME, f"olywork local proxy CA ({_machine_label()})"),
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, "olywork (local, not a public authority)"),
    ])
    cert = (
        x509.CertificateBuilder()
...
def proxy_env(port: int, token: str, bundle_path: Path | str, olywork_host: str | None = None) -> dict[str, str]:
...
# `olywork shell --proxy` needs none of this: the proxy lives and dies with one subshell, and its token
# is passed straight into that shell's environment. A DAEMON is different — another terminal has to
# find it — so the port and token go to disk. That file is the whole extra risk of `olywork serve`, which
...
def pid_alive(pid: int) -> bool:
    ...
    in the state file would therefore read as "alive" — and `olywork serve stop` would signal everything
    in this terminal."""
...
class ProxyConfig:
    ...
    (non-negotiable #3). It stays empty unless a olywork identity is configured, so a proxy with no
    ...
    # member's own olywork token, exactly what the CLI carries.
    base_url: str = ""
    olywork_token: str = ""
    org: str = ""
    ...
    def intercepts(self, host: str) -> bool:
        ...
        olywork token means we would terminate TLS and then fail — worse than not intercepting, because
        the agent's own call would break for a reason it cannot see."""
        if not (self.ca and self.base_url and self.olywork_token):
            return False
...
def _log(cfg: ProxyConfig, message: str) -> None:
    ...
    if cfg.verbose:
        print(f"▚ olywork proxy: {message}", file=sys.stderr)
...
def authorized(headers: dict[str, str], token: str) -> bool:
    """Check `Proxy-Authorization: Basic base64(olywork:<token>)`. Compared with `compare_digest` so a
...
async def _respond(writer: asyncio.StreamWriter, status: int, reason: str, body: str = "",
    ...
    """A short plain-text answer straight from the proxy. Kept explicit so a olywork-side failure never
...
async def _blind_tunnel(cfg: ProxyConfig, client_r, client_w, host: str, port: int) -> None:
    ...
    try:
        ...
        await _respond(client_w, 502, "Bad Gateway", f"olywork proxy: cannot reach {host}:{port}")
        _close(client_w)
        return
...
async def _forward_plain(cfg: ProxyConfig, client_r, client_w, head: bytes, target: str) -> None:
    ...
    if not host:
        await _respond(client_w, 400, "Bad Request", "olywork proxy: no host in the request URL")
        _close(client_w)
        return
    try:
        up_r, up_w = await _open_upstream(host, port)
...
# ---- P2 · intercept an allow-listed host and forward it to olywork -----------------------------
...
_CALLER_MUST_NOT_SET = frozenset({
    "x-olywork-token", "x-olywork-org", "x-olywork-client", "x-olywork-body-encoding",
    "x-olywork-token", "x-olywork-org", "x-olywork-client", "x-olywork-body-encoding",
})
...
async def _read_chunked(reader: asyncio.StreamReader) -> bytes:
    """Decode a chunked request body into bytes. httpx re-frames it for the hop to olywork, so we hand
...
def _olywork_url(base: str, host: str, target: str) -> str:
    ...
    the whole point: the agent wrote `api.stripe.com` and knows nothing about olywork."""
...
def _forward_headers(cfg: ProxyConfig, pairs: list[tuple[str, str]]) -> list[tuple[str, str]]:
    ...
    else, plus olywork's control headers. `Accept-Encoding: identity` is added when the caller asked for
...
async def fetch_hosts(client, base_url: str, token: str, org: str = "") -> frozenset[str]:
    ...
    everything"."""
    headers = {
        "X-Olywork-Token": token, "X-Olywork-Token": token,
        "X-Olywork-Client": "local-proxy", "X-Olywork-Client": "local-proxy",
...
async def refresh_hosts_forever(cfg: ProxyConfig, client, interval: int = HOSTS_REFRESH_SECONDS) -> None:
    ...
    while True:
        ...
        hosts = await fetch_hosts(client, cfg.base_url, cfg.olywork_token, cfg.org)
        if hosts and hosts != cfg.hosts:
            _log(cfg, f"allow-list now {len(hosts)} host(s)")
            cfg.hosts = hosts


def olywork_error_message(status: int, host: str, detail: str) -> str:
    """Turn olywork's refusal into something the agent can act on.

    The raw status is misleading here: a 404 from olywork means "no tool registered for this host",
    ...
    if status == 404:
        return (f"olywork: no tool is registered for {host} in this team, so there is no credential to "
                f"inject. An admin can add one with `olywork tools add`. (This is olywork's answer, not "
                f"{host}'s.)")
    if status == 403:
        return (f"olywork: you do not have access to {host} through this registry — either the tool is "
                f"not on your list or a policy rule blocks it. Ask an admin of your team. "
                f"(olywork's answer, not {host}'s.)")
    if status == 401:
        return ("olywork: this shell's olywork token is not valid any more. Run `olywork login` and start a "
                "new `olywork shell`.")
    if status == 429:
        return (f"olywork: your daily call limit for this team is used up, so the call to {host} was not "
                f"made. An admin can raise it.")
    if status == 409:
        return (f"olywork: more than one registered tool matches {host} and olywork cannot tell which "
                f"credential you meant. An admin should narrow one tool's base URL.")
    return f"olywork refused this call ({status}): {detail}" if detail else f"olywork refused this call ({status})."
...
def _is_waf_block(status: int, content_type: str) -> bool:
    ...
    a legitimate SQL or HTML payload, for instance. olywork's own 403s are JSON, so an HTML 403 did not
    come from olywork. Same test the CLI's `_RegistryClient` makes."""
    return status == 403 and "html" in content_type.lower()


async def _send_to_olywork(client, method: str, url: str, headers: list[tuple[str, str]], body):
    """One request to the registry, with the edge-WAF escape hatch. A buffered body that comes back
    403-as-HTML is re-sent base64-encoded under `X-Olywork-Body-Encoding`, which the server decodes to
...
async def _explain_olywork_error(writer: asyncio.StreamWriter, resp, host: str, keep_alive: bool) -> bool:
    """Replace olywork's own refusal with an explanation, keeping the status code.

    Only ever runs when the answer carries `X-Olywork-Error` or `X-Olywork-Error`, the marker the server puts on its own
    ...
    try:
        ...
        await resp.aclose()
    body = json.dumps({
        "error": olywork_error_message(resp.status_code, host, str(detail)),
        "source": "olywork", "host": host, "olywork_detail": detail,
    }).encode()
    lines = [
...
async def _body_chunks(resp):
    """The response body as raw bytes, whether olywork's answer is still streaming or already in memory.
...
async def _write_response(writer: asyncio.StreamWriter, resp, method: str, keep_alive: bool) -> bool:
    """Stream olywork's answer back to the agent inside the intercepted TLS connection.
    ...
    into olywork was framed differently. The body bytes themselves are untouched — including a
...
async def _intercept(cfg: ProxyConfig, client_r, client_w, host: str, client) -> None:
    """An allow-listed host: terminate TLS with a leaf we sign, then relay each request to olywork's
    ...
    `api.stripe.com`; it never learns olywork exists, and no vendor key is anywhere on this machine."""
    ca = cfg.ca
    ...
    while True:                                  # one TLS connection may carry many requests
        ...
        try:
            ...
            await _respond(client_w, 400, "Bad Request", "olywork proxy: could not read the request")
            break

        wants_alive = "close" not in headers.get("connection", "").lower()
        url = _olywork_url(cfg.base_url, host, target)
        out_headers = _forward_headers(cfg, _header_pairs(head[:-4]))
        try:
            resp = await _send_to_olywork(client, method, url, out_headers, body)
        except httpx.HTTPError as exc:
            # olywork itself is unreachable. Say so plainly — an agent that reads "502 Bad Gateway" here
            # would blame the vendor and retry forever against a service that is perfectly healthy.
            _log(cfg, f"{host} → olywork unreachable ({type(exc).__name__})")
            await _respond(client_w, 502, "Bad Gateway",
                           f"olywork proxy: cannot reach the registry at {cfg.base_url} "
                           f"({type(exc).__name__}). The call to {host} was NOT made.")
            break
        _log(cfg, f"{host} {method} → olywork {resp.status_code}")
        if resp.headers.get("x-olywork-error") or resp.headers.get("x-olywork-error"):
            alive = await _explain_olywork_error(client_w, resp, host, wants_alive)
        else:
...
def olywork_client(timeout: float = 120.0):
    ...
    **`trust_env=False` is not optional.** httpx reads `HTTPS_PROXY` from its own environment, and
    inside a `olywork shell` that variable points at THIS proxy — the first intercepted call would come
    straight back to us and loop. It also means the bundle and `NO_PROXY` are irrelevant here: we
    talk to olywork over the ordinary system trust, as any client would."""
...
async def handle_client(cfg: ProxyConfig, listen_port: int,
    ...
    try:
        try:
            ...
            await _respond(writer, 431, "Request Header Fields Too Large", "olywork proxy: request head too large")
            _close(writer)
            return
        ...
        try:
            ...
            await _respond(writer, 400, "Bad Request", "olywork proxy: malformed request")
            _close(writer)
            return
        ...
        if not authorized(headers, cfg.token):
            ...
            await _respond(
                ...
                "olywork proxy: this proxy is private to one olywork shell session.\n"
                "  Use the HTTPS_PROXY value from that session (it carries the session token).",
                extra={"Proxy-Authenticate": 'Basic realm="olywork"'},
            )
            _close(writer)
            return
        ...
        if method == "CONNECT":
            ...
            if _is_self(host, port, listen_port):
                await _respond(writer, 400, "Bad Request", "olywork proxy: refusing to tunnel to myself")
                _close(writer)
                return
            # The allow-list decision, and the only place it is made: a registered tool's host is
            # intercepted so olywork can add the credential; everything else stays a tunnel we cannot
        ...
        await _respond(writer, 400, "Bad Request",
                       "olywork proxy: only CONNECT and absolute-form http:// requests are proxied")
        _close(writer)
...
@dataclass
class ProxyHandle:
    """A running proxy. `olywork shell` is synchronous, so the event loop lives in a daemon thread and
...
def start(cfg: ProxyConfig, client=None) -> ProxyHandle:
    ...
    def _run() -> None:
        ...
        try:
            ...
            ready["client"] = client if client is not None else olywork_client()
            have_identity = bool(cfg.base_url and cfg.olywork_token)
            if have_identity and not cfg.hosts:
                ...
                cfg.hosts = loop.run_until_complete(
                    fetch_hosts(ready["client"], cfg.base_url, cfg.olywork_token, cfg.org))
    ...
    thread = threading.Thread(target=_run, name="olywork-localproxy", daemon=True)
    thread.start()
    started.wait(timeout=10)
    if error:
        raise error[0]
    if "server" not in ready:
        raise RuntimeError("olywork proxy: the listener did not start")
