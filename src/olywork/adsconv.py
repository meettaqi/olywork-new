...
def _payload_and_rows(
    ...
    for row in rows:
        ...
        event = {
            ...
            # SAME conversion on Google's side instead of double-counting. The `olywork-` prefix is
            ...
            "transactionId": f"olywork-{row.id}",
        }
...
async def _auth_headers(client) -> dict[str, str]:
    ...
    `/call/` request, it's olywork spending its OWN platform connection, so there is no Tool/bindings
    row (and no per-tenant Secret) to resolve credentials from. It exchanges olywork's OWN long-lived
