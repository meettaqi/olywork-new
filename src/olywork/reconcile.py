...
async def shared_plan_recovery(db: AsyncSession, since: datetime) -> dict:
    """Fee versus collected, per provider whose rate OLYWORK SET (fx.yaml `kind: olywork_shared_plan`).

    This is the monthly review's input (docs/SHARED-PLAN-PRICING-PLAN.md): a flat-fee subscription
    has no vendor per-call price, so olywork publishes its own rate with a stated break-even, and this
