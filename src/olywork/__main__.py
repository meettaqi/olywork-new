"""`python -m olywork` - run the server and its explicit maintenance commands."""
...
async def _prepare_serve() -> None:
    ...
    # Both imports are deliberately lazy so `python -m olywork keygen` stays on the light path.
...
def main() -> None:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "upgrade":
        ...
        print("olywork upgrade complete")
        return
    ...
    uvicorn.run("olywork.api:app", host="0.0.0.0", port=port, reload="--reload" in sys.argv)
